...
# --------------------------------------------------------------------
# File:        plugins/proxmox-ha/__init__.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: proxmox-ha — ProxmoxVEx Plugin
# Docs:        https://proxmoxvex.local/docs
# Generated:   2026-09-04
# --------------------------------------------------------------------
# --- ProxmoxVEx auto-header end ---
"""
proxmox-ha — ProxmoxVEx Plugin
Exposes Proxmox native HA resource management (add / update / remove VMs from HA)
through the ProxmoxVEx plugin API.

Proxmox does not expose HA management in its own public API surface that ProxmoxVEx
...
API (all require Bearer token auth, handled by ProxmoxVEx catch-all route):
...
def _validate_sid(sid):
    ...
    """
    if not sid:
        return None, ({"error": "'sid' is required (format: vm:<vmid> or ct:<vmid>)"}, 400)
    parts = sid.split(":", 1)
    if len(parts) != 2 or parts[0] not in ("vm", "ct") or not parts[1].isdigit():
        return None, ({"error": f"Invalid sid '{sid}'. Expected 'vm:<vmid>' or 'ct:<vmid>'"}, 400)
    return sid, None
...
def _get_clusters():
    ...
    try:
        return jsonify({"clusters": [{"id": c, "name": c} for c in (cluster_managers or {})]})
    except Exception:
        return jsonify({"clusters": []})


def _get_status():
    return jsonify({"plugin": PLUGIN_ID, "status": "running"})


def _get_ui():
    """Serve the Proxmox HA Resources HTML interface"""
...
def register(app):
    ...
    log.info("[%s] Registered route: /api/plugins/%s/api/ha", PLUGIN_ID, PLUGIN_ID)
