# ProxmoxVEx — Encryption Architecture (Operator Guide)
...
>
> **Important (v1.2.x):** ProxmoxVEx now uses **PostgreSQL** for persistence.
> The SQLCipher/SQLite material below describes the legacy single-file backend.
> Field-level Fernet encryption still applies to sensitive columns; PostgreSQL
> volume encryption is the operator's responsibility.

ProxmoxVEx persists secrets in two places:

1. **Application database** (PostgreSQL) — users, sessions, audit
...
## 1. Master-key loader (`ProxmoxVEx/core/keystore.py`)
...
| ---- | ------------------------------------- | ---------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1    | `ProxmoxVEx_DB_KEY` env var           | Docker secrets, k8s, CI            | Accepts urlsafe-base64 (44 chars) **or** hex (64 chars)                                                                                                             |
| 2    | `${CREDENTIALS_DIRECTORY}/db-key`     | systemd `LoadCredentialEncrypted=` | TPM2- or host-key-bound. Strongest at-rest option                                                                                                                   |
| 3    | `ProxmoxVEx_KEY_FILE` env var → file  | Custom path, e.g. NFS mount        | chmod 0600 (or 0640 root:ProxmoxVEx)                                                                                                                                |
| 4    | `/etc/ProxmoxVEx/secret.key`          | **System-service install default** | chmod **0640** root:ProxmoxVEx — group-read required so the systemd unit (running as `ProxmoxVEx`) can load it. 0600 root:ProxmoxVEx is unreadable for the service. |
| 5    | `~/.config/ProxmoxVEx/secret.key`     | Single-user / dev install          | chmod 0600 (owner = user running ProxmoxVEx)                                                                                                                        |
| 6    | `config/.ProxmoxVEx.key` (CONFIG_DIR) | **Legacy** — pre-0.9.9.3           | Triggers deprecation warning on every boot                                                                                                                          |

The loader **rejects** any file-based tier whose permissions have group-write,
group-exec, or _any_ other-user bits set — an accidentally chmod-755'd key file
never becomes the active key silently. The rejection is logged. Group-read is
**allowed** so the system-service install pattern (`root:ProxmoxVEx 0640`) works.

### Why the order matters

The 0.9.9.2 audit flagged the legacy default (Tier 6) for storing the key
_next to_ the encrypted data. With backups typically including the whole
...
```bash
sudo -u ProxmoxVEx python3 ProxmoxVEx_multi_cluster.py --keystore-status
```
...
```json
{
  "keystore": {
    "ok": true,
    "source": "/etc/ProxmoxVEx/secret.key",
    "source_path": "/etc/ProxmoxVEx/secret.key",
    "is_legacy": false,
    "tier": 4,
...
A green pill in the admin UI's _Security_ card consumes the same data via
`GET /api/security/keystore-status`.

---

## 2. SQLCipher full-DB encryption (`ProxmoxVEx/core/dbcrypto.py`)
...
The fallback is _not_ unencrypted — sensitive fields (cluster passwords,
...
ProxmoxVEx 0.9.9.3+ **auto-encrypts on first boot** when:
...
✓ DB auto-encrypted (8121 rows, 8.7s)
  backup: config/ProxmoxVEx.db.plain.bak.1778613855
```
...
Set `ProxmoxVEx_DISABLE_AUTO_ENCRYPT=1` in the service environment to skip
...
1. Stop the service: `sudo systemctl stop ProxmoxVEx`
...
```bash
sudo -u ProxmoxVEx python3 ProxmoxVEx_multi_cluster.py --migrate-db --dry-run
```
...
```bash
sudo -u ProxmoxVEx python3 ProxmoxVEx_multi_cluster.py --migrate-db --yes
```

The four-step process:

1. **Backup** — `config/ProxmoxVEx.db` → `config/ProxmoxVEx.db.plain.bak.<timestamp>`
2. **Encrypt** — new file `config/ProxmoxVEx.db.enc` built via SQLite's
...
3. **Verify** — every table's row count is compared between the plain
   ...
   _before_ the swap.
4. **Atomic swap** — `os.replace(ProxmoxVEx.db.enc, ProxmoxVEx.db)`. POSIX
...
| 5    | Row-count verification failed — encrypted DB _not_ swapped in               |

---

## 4. Recovery / disaster scenarios

> **The master key is non-recoverable.** ProxmoxVEx does not implement key
...
_to a different location and with different access controls_. Examples:
...
- Master key → printed once, sealed in a tamper-evident envelope, kept in
  ...
  vault under "Infra / ProxmoxVEx / master-key".
...
`ProxmoxVEx_multi_cluster.py --keystore-status` will report
...
If you can still get an old backup readable: stop ProxmoxVEx, swap the
...
1. Generate the key (one-time):
   ...
   print(base64.urlsafe_b64encode(secrets.token_bytes(32)).decode())' \
     | sudo tee /tmp/ProxmoxVEx-key.txt
   ```
...
2. Wrap it with systemd-creds:
   ...
   sudo systemd-creds encrypt --name=db-key /tmp/ProxmoxVEx-key.txt \
        /etc/ProxmoxVEx/secret.key.cred

   # TPM2-bound (stronger, requires TPM2 chip):
   sudo systemd-creds encrypt --name=db-key --with-key=tpm2 \
        /tmp/ProxmoxVEx-key.txt /etc/ProxmoxVEx/secret.key.cred
   ```

3. **Shred the plaintext copy:**

   ```bash
   sudo shred -u /tmp/ProxmoxVEx-key.txt
   ```

4. Edit `/etc/systemd/system/ProxmoxVEx.service` and add to `[Service]`:

   ```ini
   LoadCredentialEncrypted=db-key:/etc/ProxmoxVEx/secret.key.cred
   ```
...
5. Reload + restart:
   ...
   sudo systemctl restart ProxmoxVEx
   ```

6. Verify:

   ```bash
   journalctl -u ProxmoxVEx -n 50 | grep '\[KEYSTORE\]'
   # → [KEYSTORE] master key source: systemd-credential (/run/credentials/ProxmoxVEx.service/db-key)
   ```

7. Once verified, **delete the legacy file** if you migrated from Tier 6:

   ```bash
   sudo shred -u /etc/ProxmoxVEx/secret.key   # or config/.ProxmoxVEx.key
   ```
...
3. **No second-factor at the DB layer.** A compromised host running
   ProxmoxVEx can read the DB. Compartmentalization (running clusters in
...
_Maintainers: / Last revised on the 0.9.9.3 SQLCipher rollout._
