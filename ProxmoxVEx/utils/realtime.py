...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/utils/realtime.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: ProxmoxVEx Realtime Updates - Layer 4
# Docs:        https://proxmoxvex.local/docs
# Generated:   2026-09-04
# --------------------------------------------------------------------
# --- ProxmoxVEx auto-header end ---
"""
ProxmoxVEx Realtime Updates - Layer 4
...
# 2026-06-05 (#528 scaling): max SSE/WS broadcast message size. The old hard
...
_MAX_BROADCAST_BYTES = int(os.environ.get("PROXMOXVEX_MAX_BROADCAST_BYTES", str(5_000_000)))
...
def watched_clusters():
    ...
    for clusters nobody is viewing.  2026-06-05 (scale audit H4 / #528)."""
    watched = set()
...
def push_immediate_update(cluster_id: str, delay: float = 0.3):
    """Push immediate SSE update after VM actions for faster UI feedback"""
...
def broadcast_action(
    action: str, resource_type: str, resource_id: str, details: dict = None, cluster_id: str = None, user: str = None
):
...
def create_sse_token(username: str, allowed_clusters: list) -> str:
    ...
    with sse_tokens_lock:
        ...
        sse_tokens[token] = {"user": username, "expires": expires, "allowed_clusters": allowed_clusters}
...
# WS tokens for VNC/SSH, avoids putting session_id in WebSocket URLs
...
def broadcast_sse(update_type: str, data: dict, cluster_id: str = None):
    ...
    """
    try:
        # Skip the work of serialising and queuing if nobody is listening.
        # The broadcast loop calls this every tick; an empty client list is
        # common during idle periods or restarts.
        with sse_clients_lock:
            if not sse_clients:
                return

        # 2026-05-31 - `default=str` so a datetime / set / bytes / custom
        ...
        try:
            message = json.dumps(
                {"type": update_type, "data": data, "cluster_id": cluster_id, "timestamp": datetime.now().isoformat()},
                default=str,
            )
            ...
            logging.warning(
                f"[SSE] broadcast '{update_type}' (cluster={cluster_id}) unserialisable, skipped: {_ser_err}"
            )
            return
        ...
        # Added 'tasks' and 'resources' - broadcast loop sends these types
        cluster_specific_events = [
            "node_status",
            "vm_update",
            "task_update",
            "tasks",
            ...
            for client_id, client_info in list(sse_clients.items()):
                try:
                    ...
                    if not is_cluster_specific:
                        ...
                        # Subscribed=None means admin/all-access -> send everything
