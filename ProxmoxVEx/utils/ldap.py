...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/utils/ldap.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: ProxmoxVEx LDAP Authentication - Layer 4
# Docs:        https://proxmoxvex.local/docs
# Generated:   2026-09-04
# --------------------------------------------------------------------
# --- ProxmoxVEx auto-header end ---
"""
ProxmoxVEx LDAP Authentication - Layer 4
"""
...
def get_ldap_settings() -> dict:
    ...
    config = {
        ...
        "bind_password": get_db()._decrypt(
            settings.get("ldap_bind_password", "")
        ),  # Decrypt - stored encrypted since 0.6.5
        ...
        "verify_tls": settings.get(
            "ldap_verify_tls", False
        ),  # Default off, most AD envs use internal CAs not in system trust store
        # Custom group→role mappings for custom roles & tenants
        # Format: [{"group_dn": "CN=...", "role": "custom_role_name", "tenant": "tenant_id", "permissions": [...]}]
        "group_mappings": settings.get("ldap_group_mappings", []),
    }
    # Debug log when LDAP is enabled but looks misconfigured
    if config["enabled"] and (not config["server"] or not config["base_dn"]):
        ...
        try:
            ...
            logging.warning(
                f"[LDAP] Settings loaded but incomplete: enabled={config['enabled']}, "
...
def ldap_authenticate(username: str, password: str) -> dict:
    ...
    We do a two-step bind: first with service account to find the user DN,
    ...
    if not ldap_config["server"] or not ldap_config["base_dn"]:
        # Better diagnostic: tell them WHAT is missing
        missing = []
        ...
        logging.warning(
            f"[LDAP] LDAP not configured - missing: {', '.join(missing)}. "
    ...
    # SECURITY - Reject empty passwords (LDAP servers allow unauthenticated bind with empty password!)
    if not password or not password.strip():
        logging.warning(f"[LDAP] Rejected empty password for user '{username}'")
        return {"error": "Invalid LDAP credentials"}

    # SECURITY - Sanitize username against LDAP injection
    ...
    try:
        ...
        from ldap3.utils.conv import escape_filter_chars  # Proper LDAP escaping
    except ImportError:
    ...
    try:
        ...
        # SECURITY: configurable TLS cert verification (default CERT_NONE for backwards compat)
        tls_config = None
        ...
        server = Server(
            server_url, port=port, use_ssl=ldap_config["use_ssl"], tls=tls_config, get_info=ALL, connect_timeout=10
        )
        ...
        # STARTTLS has to happen BEFORE bind! auto_bind was sending creds in plaintext
        # Found this during the Feb audit... pretty bad tbh
        ...
        # SECURITY - Use ldap3's escape_filter_chars to prevent LDAP injection
        ...
        attributes = [
            ...
            "memberOf",  # AD stores group membership directly on user
        ...
        display_name = (
            str(user_entry[ldap_config["display_name_attribute"]])
            if ldap_config["display_name_attribute"] in user_entry
            else username
        )
        ...
        # (#353) - AD's `memberOf` only returns DIRECT group memberships.
        ...
        try:
            ...
            if base_for_groups and user_dn:
                ...
                conn.search(
                    search_base=base_for_groups, search_filter=chain_filter, search_scope=SUBTREE, attributes=["cn"]
                )
        ...
        if not member_of and ldap_config["group_base_dn"]:
            try:
                ...
                group_conn.search(
                    ldap_config["group_base_dn"], group_filter, search_scope=SUBTREE, attributes=["cn"]
                )  # Issue #70: removed 'dn' -- entry_dn is implicit
                member_of = [str(entry.entry_dn) for entry in group_conn.entries]
                group_conn.unbind()
            except Exception as e:
                logging.warning(f"[LDAP] Group search failed: {e}")

        # Step 5: Map LDAP groups to ProxmoxVEx roles
        role = ldap_config["default_role"]
        tenant = None
        extra_permissions = []
        tenant_permissions = {}

        # Case-insensitive group DN comparison (AD is case-insensitive)
        member_of_lower = [g.lower() for g in member_of]

        # Check built-in group mappings first (admin > user > viewer priority)
        ...
        # Custom group mappings (override built-in if matched)
        ...
        for mapping in custom_mappings:
            ...
            if map_group and map_group.lower() in member_of_lower:
                ...
                # Support per-tenant role assignment
                ...
                logging.info(
                    f"[LDAP] Custom group mapping matched: {map_group} → role={mapping.get('role')}, tenant={mapping.get('tenant')}"
                )
        ...
        # Catch TLS errors specifically so the user gets a useful hint
        try:
        ...
        if isinstance(e, _ssl.SSLError) or (LDAPSocketOpenError and isinstance(e, LDAPSocketOpenError)):
            ...
            return {
                "error": 'LDAP connection failed - TLS/certificate error. Disable "Verify TLS Certificate" for self-signed/internal CA certificates.'
            }
        logging.error(f"[LDAP] Authentication error: {e}")
        return {"error": "LDAP authentication failed"}  # Don't leak internal error details


def ldap_provision_user(ldap_result: dict) -> dict | None:
    from ProxmoxVEx.utils.auth import load_users, save_users

    """Create or update a local user from LDAP authentication result

    JIT (Just-In-Time) provisioning - user account is created on first login
    LDAP users have auth_source='ldap' and no local password
    Also syncs tenant, permissions, and tenant_permissions from group mappings
    """
    username = ldap_result["username"].lower()
    users = load_users()

    if username in users:
        # SECURITY - Don't overwrite local-only accounts with LDAP
        ...
        # Sync tenant assignment from LDAP group mapping
        if ldap_result.get("tenant"):
            user["tenant_id"] = ldap_result["tenant"]  # Must be tenant_id (not tenant) for code compatibility

        # Merge extra permissions from LDAP group mappings
        ...
        # Sync tenant-specific roles/permissions
        ...
        logging.info(
            f"[LDAP] Updated existing user '{username}' from LDAP (role={user['role']}, tenant={user.get('tenant')})"
        )
    else:
        # Create new user
        users[username] = {
            ...
            "tenant_id": ldap_result.get("tenant", ""),  # Must be tenant_id
        ...
        logging.info(
            f"[LDAP] Provisioned new user '{username}' from LDAP (role={ldap_result.get('role', ROLE_VIEWER)}, tenant={ldap_result.get('tenant')})"
        )
