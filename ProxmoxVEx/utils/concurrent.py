...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/utils/concurrent.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: ProxmoxVEx Concurrency Helpers - Layer 2
# Docs:        https://proxmoxvex.local/docs
# Generated:   2026-09-04
# --------------------------------------------------------------------
# --- ProxmoxVEx auto-header end ---
"""
ProxmoxVEx Concurrency Helpers - Layer 2
"""
...
# 2026-06-05 - env-tunable (was hard 50). Default raised to 100 now that
...
_NODE_POOL_SIZE = int(os.environ.get("PROXMOXVEX_NODE_POOL_SIZE", "100"))
try:
...
def get_paramiko():
    ...
    # Paramiko takes forever to import so we only do it when needed
    try:
        import paramiko
...
def run_concurrent(tasks: list, timeout: float = 30.0) -> list:
    ...
    # 2026-05-31 - CRITICAL FIX. The original check `if GEVENT_POOL and
...
# Exponential backoff helper for retryable SSH/API ops
...
def retry_with_backoff(fn, max_retries=3, base_delay=0.5, jitter=True):
    ...
    for attempt in range(max_retries):
        try:
            ...
            # Don't log first attempt failure, its noisy
            if attempt > 0:
...
# SSH-aware multi-node fanout for big clusters (15+ nodes).
...
def run_per_node(node_callables, max_concurrent=8, timeout=120):
    ...
    # Path 1: gevent pool — preferred since ProxmoxVEx is gevent-monkey-patched
    if GEVENT_AVAILABLE:
        try:
