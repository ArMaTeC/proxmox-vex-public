...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/utils/vnc_tunnel.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: SSH-tunneled local port forwarder for the ProxmoxVEx ↔...
# Docs:        https://proxmoxvex.local/docs
# Generated:   2026-09-04
# --------------------------------------------------------------------
# --- ProxmoxVEx auto-header end ---
"""SSH-tunneled local port forwarder for the ProxmoxVEx ↔ PVE-node VNC leg.

When TLS-inspection middleboxes (CrowdStrike Falcon NF, Palo Alto SSL Decryption,
Zscaler, Forcepoint, …) sit between ProxmoxVEx and the PVE node, they re-encrypt
...
anchor). The ProxmoxVEx daemon spawns a single persistent SSH client per PVE
...
Driven by customer report where Stable VNC Mode (browser-side
encryption) wasn't enough because their inspection-engine sat on the
ProxmoxVEx↔PVE side too.
"""
...
class TunnelEndpoint:
    ...
    def __init__(
        self, listener: socket.socket, transport, target_host: str, target_port: int, cluster_id: str, pve_host: str
    ):
        ...
        self._accept_thread = threading.Thread(
            target=self._accept_loop, name=f"VncTun-Accept-{self.local_port}", daemon=True
        )
    ...
    def run(self):
        ...
        try:
            ...
            while not self._stopped:
                ...
                if self._channel in ready and self._channel.recv_ready():
...
class SshVncTunnelPool:
    ...
    def __init__(self) -> None:
        self._clients: dict[str, Any] = {}  # cluster_id → paramiko.SSHClient
        self._lock = threading.Lock()
...
# ──────────────────────────────────────────────────────────────────────────
# Self-test (run directly: python -m ProxmoxVEx.utils.vnc_tunnel)
#
...
if __name__ == "__main__":
    ...
    class _SshServer(paramiko.ServerInterface):
        def check_auth_password(self, u, p):
            return paramiko.AUTH_SUCCESSFUL if (u == "tester" and p == "tt") else paramiko.AUTH_FAILED

        def check_channel_request(self, kind, chanid):
            return (
                paramiko.OPEN_SUCCEEDED
                if kind in ("direct-tcpip", "session")
                        ...
                        def _pump_bidir(chan, ts):
                            ...
                            try:
                                while True:
                                    ...
                                    if chan in ready and chan.recv_ready():
                                        d = chan.recv(8192)
                                        if not d:
                                            break
                                        ts.sendall(d)
