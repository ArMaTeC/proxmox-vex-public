...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/utils/oidc.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: ProxmoxVEx OIDC/OAuth2 Authentication - Layer 4
# Docs:        https://proxmoxvex.local/docs
# Generated:   2026-09-04
# --------------------------------------------------------------------
# --- ProxmoxVEx auto-header end ---
"""
ProxmoxVEx OIDC/OAuth2 Authentication - Layer 4
"""
...
# SSRF guard for admin-supplied OIDC URLs (discovery / token / userinfo).
...
# (CodeAnt #500) - pre-validate the authority URL before composing the
...
# PyJWT for proper signature verification
try:
    import jwt as pyjwt
...
# Microsoft cloud environment endpoint mapping
...
def get_oidc_settings() -> dict:
    ...
    Supports Microsoft Entra ID, Okta, Auth0, Keycloak, and any OIDC-compliant provider
    """
    from ProxmoxVEx.api.helpers import load_server_settings

    settings = (
        load_server_settings()
    )  # Must use load_server_settings() NOT get_server_settings() (that's the route handler!)
    provider = settings.get("oidc_provider", "entra")

    # Entra needs User.Read + GroupMember.Read.All for Graph API
    ...
    return {
        ...
        "provider": provider,
        "cloud_environment": settings.get("oidc_cloud_environment", "commercial"),  # GCC High/DoD support
        "client_id": settings.get("oidc_client_id", ""),
        "client_secret": get_db()._decrypt(settings.get("oidc_client_secret", "")),  # Encrypted
        ...
        "scopes": settings.get("oidc_scopes", "") or default_scopes,  # Use provider-specific default if not configured
...
def get_oidc_endpoints(config: dict) -> dict:
    ...
    Entra uses tenant-specific URLs, generic OIDC uses discovery
    ...
    if provider == "entra" and tenant_id:
        # Use cloud environment to determine base URLs
        ...
        # Try OpenID Connect Discovery (RFC 8414)
        # This works for Keycloak, Okta, Auth0, Google, Authentik, and any standard OIDC provider.
        # (#188) - bumped timeout 5→15s; loud-log discovery failures so admins notice
        ...
        try:
            try:
                # (#412): pass allow_private through so internal
                ...
                # (#188 follow-up): never return None - callers
                ...
                if not allow_private_ip and "private" in str(guard_err).lower():
                    hint = (
                        " If your IdP is on an internal network and this is "
            ...
            logging.warning(
                ...
                f"ProxmoxVEx will guess {authority}/authorize which is often a 404."
            )
...
def oidc_build_auth_url(config: dict, state: str) -> tuple:
    ...
    State parameter prevents CSRF - stored in session before redirect
...
def oidc_exchange_code(config: dict, code: str, code_verifier: str = None) -> dict:
    ...
    # Scope required by most providers (Authentik, Keycloak, Entra)
    ...
    try:
        try:
            # (#412 follow-up from @robertjakub): thread the
...
def oidc_decode_id_token(id_token: str, expected_nonce: str = None, config: dict = None) -> dict:
    """Decode and verify JWT ID token signature using JWKS

    Mar 2026: Now verifies signature via JWKS endpoint (PyJWT).
    Falls back to unsigned decode if PyJWT unavailable or JWKS fetch fails,
    so existing deployments don't break during upgrade.
    """
    # Try proper signature verification first
    ...
    if PYJWT_AVAILABLE and config and not skip_verify:
        try:
            ...
            if jwks_uri:
                # (CodeAnt SSRF) - gate the JWKS URI before fetching keys from it.
                ...
                # Audience list: client_id is always accepted,
                ...
                # (e.g. "ProxmoxVEx-prod") that maps to multiple deployments.
                ...
                # 2026-05-31 - `verify_iss=False` is INTENTIONAL, not a gap.
                ...
                if expected_nonce and claims.get("nonce") and claims["nonce"] != expected_nonce:
                    logging.warning("[OIDC] Nonce mismatch after sig verification")
                    return {"error": "OIDC nonce mismatch - possible replay attack"}

                return claims

        except Exception as e:
            # Don't break login if JWKS is temporarily unreachable
            logging.warning(
                f"[OIDC] JWKS verification failed ({type(e).__name__}: {e}), falling back to unverified decode"
            )
    ...
    try:
        ...
        # Validate expiry (5 min clock skew tolerance)
        exp = claims.get("exp")
        ...
        if expected_nonce and claims.get("nonce") and claims["nonce"] != expected_nonce:
            logging.warning(
                f"[OIDC] Nonce mismatch: expected={expected_nonce[:8]}..., got={str(claims.get('nonce', ''))[:8]}..."
            )
            return {"error": "OIDC nonce mismatch - possible replay attack"}
...
def oidc_get_user_info(config: dict, access_token: str) -> dict:
    ...
    For Entra, uses Microsoft Graph API for richer data
    """
    ...
    try:
        if config.get("provider") == "entra" and endpoints.get("graph_me"):
            # Microsoft Graph gives us more data than OIDC userinfo
...
def oidc_get_user_groups(config: dict, access_token: str) -> list:
    ...
    For Entra, uses Graph API /me/memberOf
    ...
    try:
        # Entra Graph API for group memberships
        ...
        while url:
            ...
            # Handle pagination (Entra paginates at 100 groups)
...
def oidc_map_groups_to_role(config: dict, groups: list, id_token_claims: dict = None) -> dict:
    """Map OIDC groups to ProxmoxVEx role, tenant, and permissions

    Works with Entra group IDs and generic OIDC group claims
    ...
    # Also check ID token 'groups' claim (Entra can embed group IDs in token)
    if id_token_claims:
        for gid in id_token_claims.get("groups", []):
            group_ids.add(str(gid).lower())

    # Built-in group mappings (admin > user > viewer priority)
    ...
    # Custom group mappings - pick highest role when user matches multiple groups
        ...
        if map_group and (map_group in group_ids or map_group in group_names):
            ...
            if mapping.get("tenant") and mapping.get("tenant_role"):
                result["tenant_permissions"][mapping["tenant"]] = {
                    ...
                    "extra": mapping.get("permissions", []),  # Must be 'extra' to match get_user_permissions()
                }
...
def oidc_provision_user(user_info: dict, role_mapping: dict, auth_source: str = "oidc") -> dict | None:
    from ProxmoxVEx.utils.auth import load_users, save_users

    """Create or update local user from OIDC authentication

    JIT provisioning - same pattern as LDAP but for OIDC providers
    Username derived from email or preferred_username
    """
    # Derive username from OIDC claims
    email = user_info.get("email") or user_info.get("preferred_username", "")
    raw_username = user_info.get("preferred_username") or email

    # Sanitize username - use part before @ for email-style usernames
    ...
    if username in users:
        # SECURITY - Don't allow OIDC to overwrite a local-only user
        ...
        if existing_source == "local":
            logging.warning(
                f"[OIDC] Rejected login for '{username}' - local account exists, cannot overwrite with OIDC"
            )
        ...
        if role_mapping.get("tenant"):
            user["tenant_id"] = role_mapping["tenant"]  # Must be tenant_id
        ...
        users[username] = {
            ...
            "tenant_id": role_mapping.get("tenant", ""),  # Must be tenant_id
        ...
        logging.info(
            f"[OIDC] Provisioned new user '{username}' (role={role_mapping.get('role', ROLE_VIEWER)}, source={auth_source})"
        )
