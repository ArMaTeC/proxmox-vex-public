...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/utils/vnc_grab.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: Vnc Grab PY source
...
def _vnc_auth_response(password, challenge):
    ...
    cipher = TripleDES(des3_key)  # nosec: B304  # lgtm[py/weak-cryptographic-algorithm]  # snyk:ignore:Use of a Broken or Risky Cryptographic Algorithm - required by the RFB/VNC auth challenge protocol
    enc = Cipher(cipher, modes.ECB()).encryptor()  # nosec: B305  # lgtm[py/weak-cryptographic-algorithm]  # snyk:ignore:Use of a Broken or Risky Cryptographic Algorithm - required by the RFB/VNC auth challenge protocol
    ...
    def read(self, n):
        while len(self.buf) < n:
            ...
            if chunk is None or chunk == "":
                raise OSError("vnc websocket closed mid-stream")
...
def grab_frame(ws, vnc_ticket, timeout=8.0):
    ...
    if not server_ver.startswith(b"RFB "):
        raise OSError(f"not an RFB stream: {server_ver[:12]!r}")
    try:
        major = int(server_ver[4:7])
        minor = int(server_ver[8:11])
    except Exception:
        major, minor = 3, 8
    ...
    if (major, minor) >= (3, 7):
        ...
        if n_types == 0:
            ...
            raise OSError("vnc security failed: {}".format(r.read(reason_len).decode("latin-1", "replace")))
        types = set(r.read(n_types))
        chosen = _SEC_VNC if _SEC_VNC in types else (_SEC_NONE if _SEC_NONE in types else None)
        if chosen is None:
            raise OSError(f"no supported vnc security type offered: {sorted(types)!r}")
    ...
    if chosen == _SEC_VNC:
        ...
        if sec_result != 0:
            ...
            try:
                ...
                why = "auth rejected"
            raise OSError(f"vnc auth failed: {why}")
    ...
    if fb_w <= 0 or fb_h <= 0 or fb_w > _MAX_DIM or fb_h > _MAX_DIM:
        raise OSError(f"implausible framebuffer size {fb_w}x{fb_h}")
    ...
    while True:
        ...
        if deadline_guard > 64:
            raise OSError("no framebuffer update after 64 server messages")
        msg_type = r.read(1)[0]
        if msg_type == _SRV_FB_UPDATE:
            ...
            for _ in range(n_rects):
                ...
                if enc != _ENC_RAW:
                    raise OSError(f"server used unexpected encoding {enc}")
                nbytes = w * h * 4
                if nbytes > _MAX_RAW_BYTES:
                    raise OSError(f"raw rect too large ({nbytes} bytes)")
                if nbytes == 0:
                    continue
                data = r.read(nbytes)
            ...
            raise OSError(f"unexpected server message type {msg_type}")
...
# Tiny, non-destructive mouse nudge sent via qm monitor to wake a sleeping
# virtual display (Windows lock screen / DPMS off) before we try to grab it.
# A relative mouse move is used instead of a key so we don't type into a
# password field if the guest is already at the login prompt.
def _wake_qemu_display(pve_mgr, node, vmid, timeout=5):
    from ProxmoxVEx.utils.ssh import _pve_node_exec

    vmid = int(vmid)
    # Two tiny relative moves: enough to turn the display back on, not enough
    # to accidentally click or drag anything (the cursor ends where it started).
    cmd = f"printf 'mouse_move 1 1\\nmouse_move -1 -1\\n' | qm monitor {vmid} >/dev/null 2>&1"
    _pve_node_exec(pve_mgr, node, cmd, timeout=timeout)


def _screendump_raw(pve_mgr, node, vmid, remote, timeout):
    """Run one qm monitor screendump and return a PIL Image (or raise)."""
    ...
    cmd = (
        f"echo screendump {remote} | qm monitor {vmid} >/dev/null 2>&1; "
    ...
    b64 = (out or "").strip()
    if not b64:
        raise OSError(f"screendump produced no data (rc={rc}, err={str(err)[:120]})")
    try:
        ppm = _gz.decompress(_b64.b64decode(b64))
    except Exception as e:
        raise OSError(f"screendump decode failed: {e}") from None
...
# Preferred grab for the console tile. QEMU 'screendump' over
...
def screendump_to_png(pve_mgr, node, vmid, max_width=480, timeout=20):
    ...
    remote = f"/tmp/pp_shot_{vmid}.ppm"  # nosec: B108 - VNC temp path on remote Proxmox node

    img = _screendump_raw(pve_mgr, node, vmid, remote, timeout=timeout)
    ex = img.getextrema()  # ((rmin,rmax),(gmin,gmax),(bmin,bmax))
    if max(hi for _lo, hi in ex) <= 10:
        # A powered-off / DPMS-asleep display dumps as pure black. Sending a
        # harmless input wakes the virtual monitor so the lock/login screen
        # renders; then we grab again before giving up.
        _wake_qemu_display(pve_mgr, node, vmid, timeout=5)
        time.sleep(1.5)  # give the guest time to repaint the framebuffer
        img = _screendump_raw(pve_mgr, node, vmid, remote, timeout=max(5, timeout - 7))
        ex = img.getextrema()

    # A genuinely off display (no GPU output, guest halted, etc.) still looks
    # like a black tile — keep the icon fallback instead of showing that.
    if max(hi for _lo, hi in ex) <= 10:
