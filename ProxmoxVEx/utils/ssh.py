...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/utils/ssh.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: ProxmoxVEx SSH Utilities - Layer 2
# Docs:        https://proxmoxvex.local/docs
# Generated:   2026-09-04
# --------------------------------------------------------------------
# --- ProxmoxVEx auto-header end ---
"""
ProxmoxVEx SSH Utilities - Layer 2
...
from ProxmoxVEx.constants import SSH_MAX_CONCURRENT
from ProxmoxVEx.globals import (
    _ssh_active_connections,
...
# Rate limiter for authenticated security actions
...
def check_auth_action_rate_limit(key: str, max_attempts: int = 5, window: int = 300) -> bool:
    ...
    5 attempts per 5 min by default, should be enough for typos but stops brute force
    """
    now = time.time()
...
# Track ProxmoxVEx user who initiated each task (UPID -> username)
...
def apply_ssh_host_key_policy(client, known_hosts_path=None):
    path = known_hosts_path or SSH_KNOWN_HOSTS_PATH
    if os.path.exists(path):
        with contextlib.suppress(Exception):
            client.load_host_keys(path)
    client.set_missing_host_key_policy(_PersistentTOFUPolicy(path))


def _parse_private_key(private_key):
    """Try each paramiko key type against a PEM-formatted key string."""
    import io
    ...
    for cls in key_classes:
        try:
            ...
            last_err = e
    raise Exception(f"Unable to parse private key: {last_err}")


def _ssh_exec_with_key(host, user, private_key, cmd, timeout, connect_timeout, port):
    """Minimal, isolated key-based auth path used only when a private key is
    supplied (see `_ssh_exec`'s `private_key` parameter). Deliberately does
    not share code with the password/keyboard-interactive methods above to
    avoid risking a regression in that well-exercised path."""
    import paramiko
    ...
    try:
        ...
        raise Exception(f"SSH key auth failed: {e}") from e

    with contextlib.suppress(Exception):
        client.save_host_keys(SSH_KNOWN_HOSTS_PATH)

    try:
        stdin, stdout, stderr = client.exec_command(cmd, timeout=timeout)  # nosec: B601 - cmd built by callers with shlex.quote
        ...
        with contextlib.suppress(BaseException):
            client.close()
        raise Exception(f"Paramiko exec failed: {e}") from e


def _ssh_exec(
    host, user, password, cmd, timeout=30, use_controlmaster=False, connect_timeout=8, port=22, private_key=None
):
    ...
    (dead-node UI hang fix) - split `timeout` into two phases:
    ...
    `private_key` (optional): PEM-formatted private key text. When provided,
    a dedicated key-based auth path is used instead of any of the
    password/keyboard-interactive methods below, and the sshpass subprocess
    fallback is skipped entirely (it has no key-auth equivalent). This keeps
    the existing, well-exercised password paths completely untouched when
    `private_key` is not supplied (the default), consistent with how `port`
    was added previously as an optional, backward-compatible parameter.
    """
    import socket as _socket

    if private_key:
        return _ssh_exec_with_key(host, user, private_key, cmd, timeout, connect_timeout, port)
    ...
    def _make_sock():
        ...
        try:
            return _socket.create_connection((host, port), timeout=connect_timeout)
        except Exception:
            return None
    ...
    def _configure_transport_algorithms(t):
        ...
        try:
            ...
            esxi_keys = ("ssh-rsa", "ecdsa-sha2-nistp256", "ssh-ed25519", "rsa-sha2-256", "rsa-sha2-512")
    ...
    try:
        ...
        client = paramiko.SSHClient()
        # TOFU: trust on first use, reject if key changes
        apply_ssh_host_key_policy(client)
        ...
        if not connected:
            try:
                ...
                apply_ssh_host_key_policy(client2)
        ...
        if not connected:
            try:
                ...
                apply_ssh_host_key_policy(client3)
                # `timeout` here is paramiko's CONNECT timeout
        ...
        # Persist host keys (TOFU model)
        with contextlib.suppress(Exception):
            client.save_host_keys(SSH_KNOWN_HOSTS_PATH)  # config dir might not be writable
    ...
    try:
        ...
        # ControlMaster opt-in. Adds connection-sharing args.
        ...
        result = subprocess.run(ssh_args, capture_output=True, text=True, timeout=timeout, env=env)
...
def _pve_node_exec(pve_mgr, node, cmd, timeout=600, use_controlmaster=True, ignore_node_backoff=False):
    """Execute a command on a Proxmox node via the Proxmox API.
    Uses POST /nodes/{node}/execute or falls back to SSH.

    Phase 2: use_controlmaster=True enables OpenSSH connection
    ...
    (dead-node UI hang) - short-circuit if the per-node circuit
    ...
    # Fail-fast if the breaker is open, unless the caller is already in a
    ...
    try:
        resp = pve_mgr._api_post(
            f"https://{pve_mgr.host}:{pve_mgr.api_port}/api2/json/nodes/{node}/execute", data={"commands": cmd}
        )
    ...
    if not node_host:
        # Use manager's _get_node_ip which does proper interface scoring
    ...
    if not node_host:
        # DO NOT silently fall back to pve_mgr.host when the
    ...
    try:
        rc, out, err = _ssh_exec(
            node_host, "root", pve_mgr.config.pass_, cmd, timeout=timeout, use_controlmaster=use_controlmaster
        )
        # SSH error patterns that indicate the node itself is dead, not the cmd
        looks_like_node_down = rc != 0 and any(
            s in str(err).lower()
            for s in (
                "tcp connect",
                "connection refused",
                ...
                "host is down",
                "auth failed",
                "paramiko exec failed",
                "all ssh methods failed",
            )
        )
        try:
