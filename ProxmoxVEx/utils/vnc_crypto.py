...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/utils/vnc_crypto.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: Per-session AES-256-GCM authenticated encryption for...
...
ProxmoxVEx and the inspection engine still re-encrypts that, but inside the
WebSocket frames we put a SECOND layer of authenticated encryption with a
session key shared only between browser and ProxmoxVEx server.
...
Split out from vms.py so it's importable + unit-testable.
"""
...
class VncCryptoSession:
    ...
    def decrypt(self, frame: bytes) -> bytes:
        ...
        with self._lock:
            ...
            if seq != expected:
                raise VncCryptoError(
                    f"out-of-order frame: got seq={seq}, expected {expected} (possible replay / middlebox reorder)"
                )
        ...
        try:
            ...
            raise VncCryptoError(
                "GCM auth tag mismatch — bytes were modified mid-stream (typical TLS-inspection / EDR symptom)"
            ) from exc
...
# ──────────────────────────────────────────────────────────
# Self-test (run directly: python -m ProxmoxVEx.utils.vnc_crypto)
# ──────────────────────────────────────────────────────────
