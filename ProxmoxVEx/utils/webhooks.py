...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/utils/webhooks.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: Webhook alert dispatcher for Slack / Discord / Teams /...
# Docs:        https://proxmoxvex.local/docs
# Generated:   2026-09-04
# --------------------------------------------------------------------
# --- ProxmoxVEx auto-header end ---
"""
Webhook alert dispatcher for Slack / Discord / Teams / ntfy /
...
# (audit fix M-11) - Slack/Discord/Teams/HEC webhook URLs embed
...
def _build_slack(alert):
    title = alert.get("alert_name") or "ProxmoxVEx alert"
        ...
        "attachments": [
            {
                ...
                "fields": [
                    ...
                    {
                        "title": "Target",
                        "value": f"{alert.get('target_type', '')}: {alert.get('target_name', '')}",
                        "short": True,
                    },
                    {
                        "title": "Metric",
                        "value": f"{alert.get('metric', '')} = {alert.get('current_value', '')}",
                        "short": True,
                    },
...
def _post_ntfy(channel, alert):
    ...
    if topic:
        url = f"{url}/{topic}"
    headers = {
        "Title": (alert.get("alert_name") or "ProxmoxVEx alert")[:200],
...
def send_to_channel(channel, alert):
    ...
    if ctype == "slack":
        ...
        body = {"alert": alert, "source": "ProxmoxVEx", "timestamp": datetime.now().isoformat()}
...
def send_to_channels(alert, channel_ids=None):
    ...
    for ch in channels:
        try:
            ...
            # (M-11) - detail comes from send_to_channel which already
