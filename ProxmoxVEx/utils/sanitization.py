...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/utils/sanitization.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: ProxmoxVEx Input Sanitization - Layer 2
# Docs:        https://proxmoxvex.local/docs
# Generated:   2026-09-04
# --------------------------------------------------------------------
# --- ProxmoxVEx auto-header end ---
"""
ProxmoxVEx Input Sanitization - Layer 2
"""

import html
import re

# Split from monolith - these were scattered all over the place before
...
def sanitize_string(value: str, max_length: int = 1000, allow_html: bool = False) -> str:
    ...
    # The regex looks scary but its just ASCII C0 control chars minus \t \n \r
...
def validate_storage_name(storage) -> bool:
    """Validate Proxmox storage identifier.

    PVE storage names are alphanumeric + dash + underscore + dot. Anything
...
# (pentest CRIT) - ISO/template filenames flow UNESCAPED into SSH
...
# 2026-06-05 (security audit C-2/M-2): ESXi datastore names and VM-directory
...
def sanitize_log_message(value) -> str:
    ...
    if value is None:
        return ""
    # Cheap str-replace, called on every audit log write.
...
# Control characters to strip in sanitize_console_message: C0 controls except
# \t (0x09) and \n (0x0a), plus DEL. \r is folded into \n rather than dropped
# so CRLF-terminated remote output still produces one line per row.
_CONSOLE_CONTROL_CHARS_RE = re.compile(r"[\x00-\x08\x0b\x0c\x0e-\x1f\x7f]")


def sanitize_console_message(value) -> str:
    """Sanitize a single structured console/job-log message.

    Unlike `sanitize_log_message` (which flattens a value onto one line for a
    plain-text audit log), this is used for messages stored as elements of a
    structured, per-line JSON log (`plugins/vm-update-manager` job output)
    that the UI renders with `white-space: pre-wrap`. Flattening `\\n` there
    would (and did) turn multi-line command output into an unreadable single
    line, so real newlines are intentionally preserved here.

    Still neutralizes other control characters and the unicode line
    separators U+2028/U+2029 (which some renderers/`json.dumps` treat as line
    breaks) to avoid log-injection-style rendering tricks.

    CWE-117 / OWASP Log Injection (newline-preserving variant).
    """
    if value is None:
        return ""
    s = str(value)
    s = s.replace("\r\n", "\n").replace("\r", "\n")
    s = s.replace("\u2028", "\n").replace("\u2029", "\n")
    s = _CONSOLE_CONTROL_CHARS_RE.sub("", s)
    return s
