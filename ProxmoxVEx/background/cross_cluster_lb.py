...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/background/cross_cluster_lb.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: ProxmoxVEx Cross-Cluster Load Balancer - Layer 7
# Docs:        https://proxmoxvex.local/docs
# Generated:   2026-09-04
# --------------------------------------------------------------------
# --- ProxmoxVEx auto-header end ---
"""
ProxmoxVEx Cross-Cluster Load Balancer - Layer 7
Background thread that balances VM load across clusters within a group.

Extends the per-cluster LB to work across cluster boundaries.
...
logger = logging.getLogger("ProxmoxVEx.xclb")
...
def _pick_node(manager, highest=True):
    """Find most or least loaded node on a cluster. DRY helper."""
    try:
        node_status = manager.get_node_status()
        if not node_status:
            return None
        ...
        for node, data in node_status.items():
            if data.get("status") != "online" or data.get("maintenance_mode", False) or node in config_excluded:
                continue
            s = data.get("score", 0)
...
def _token_cleanup_thread(target_mgr, source_mgr, token_name, task_upid, vmid, vm_type):
    ...
    Same approach as vms.py cross-cluster migration cleanup.
    """
    max_wait, poll_interval = 7200, 15
    min_wait = 120  # 2 min for automated LB (shorter than manual's 5 min)
    elapsed = 0
...
def run_cross_cluster_balance_check(group):
    ...
    Intentionally conservative - one VM per cycle, dry_run default on.
    """
    group_id = group["id"]
    ...
    if dry_run:
        ...
        log_audit(
            "system",
            "xclb.dry_run",
            f"Cross-cluster LB dry run: group '{group_name}' imbalance {diff:.1f} > {threshold} ({hi_cid} -> {lo_cid})",
        )
        return
    ...
    try:
        ...
        # Same endpoint format as manual cross-cluster migration
        endpoint = (
            f"apitoken=PVEAPIToken={token['token_id']}={token['token_value']},"
            f"host={fp['host']},fingerprint={fp['fingerprint']}"
        )

        logger.info(f"[XCLB] Migrating {vm_type}/{vmid} ({vm_name}): {hi_cid}/{source_node} -> {lo_cid}/{target_node}")
        ...
        if result.get("success"):
            ...
            log_audit(
                "system",
                "xclb.migrate",
                f"Cross-cluster LB: Migrated {vm_type}/{vmid} ({vm_name}) from {hi_cid} to {lo_cid} (group:{group_id})",
            )
            # spawn token cleanup thread
            threading.Thread(
                target=_token_cleanup_thread, args=(lo_mgr, hi_mgr, token_name, task_upid, vmid, vm_type), daemon=True
            ).start()
        else:
    ...
    try:
        db.execute(
            "UPDATE cluster_groups SET cross_cluster_last_run = ? WHERE id = ?", (datetime.now().isoformat(), group_id)
        )
    except Exception as e:
...
def cross_cluster_lb_loop():
    ...
    while _xclb_running:
        try:
            db = get_db()
            groups = db.query("SELECT * FROM cluster_groups WHERE cross_cluster_lb_enabled = 1")
            if groups:
                now = time.time()
                for row in groups:
                    group = dict(row)
