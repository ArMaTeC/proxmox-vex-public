...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/background/site_recovery.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: Site Recovery PY source
...
from ProxmoxVEx.utils.sanitization import (
    sanitize_log_message as _sl,  # #338693885 (Aikido CWE-117): plan name is request-body-controlled → strip CR/LF before any log/audit/SSE line
)

logger = logging.getLogger("ProxmoxVEx.site_recovery")
...
def _create_event(plan_id, event_type, triggered_by="system"):
    ...
    db = get_db()
    db.execute(
        """INSERT INTO site_recovery_events (id, plan_id, event_type, status, started_at, triggered_by)
        VALUES (?, ?, ?, 'running', ?, ?)""",
        (event_id, plan_id, event_type, now, triggered_by),
    )
    return event_id
...
def _complete_event(event_id, status, details=None):
    ...
    db = get_db()
    db.execute(
        "UPDATE site_recovery_events SET status = ?, completed_at = ?, details = ? WHERE id = ?",
...
def _fire_webhook(url):
    ...
    if not url:
        return
    try:
        # SSRF guard: failover webhooks are admin-set; refuse
        ...
        requests.post(
            url,
            json={"event": "site_recovery", "timestamp": datetime.utcnow().isoformat()},
            timeout=30,
...
# 2026-04-24 - pre-flight validation. Real-world cause of most "Site Recovery failed
    ...
    try:
        ...
        for src, tgt in (storage_map or {}).items():
            if not tgt:
                issues.append({
                    "severity": "error",
                    "msg": f"Storage mapping '{src}' → (empty) — fill in the target storage",
                })
    ...
    try:
        ...
        for src, tgt in (net_map or {}).items():
            if not tgt:
                issues.append({
                    "severity": "error",
                    "msg": f"Network mapping '{src}' → (empty) — fill in the target bridge/vnet",
                })
...
def _broadcast_progress(plan_id, message, progress=None):
    ...
    # (#413) - broadcast_sse signature is (update_type, data, cluster_id=None);
...
# The actual migration logic delegates to the existing cross-cluster-migrate infra
def _migrate_vm_cross_cluster(src_mgr, tgt_mgr, vmid, vm_type, storage_map, net_map):
    ...
    try:
        ...
        vm_node = None
        if node_status:
            for node_name, _ndata in node_status.items():
        ...
        # Check if VM's current storage has a mapping, else use first available
        # 2026-04-24: the old `except: pass` silently hid get_vm_config failures
        ...
        if storage_map:
            try:
                # (#413) - get_vm_config returns {'success','config'};
                ...
                config = (
                    ((_cfg_res.get("config") or {}).get("raw") or {})
                    if isinstance(_cfg_res, dict) and _cfg_res.get("success")
                    else {}
                )
                if not config:
                    ...
                    for k, v in (config or {}).items():
                        if (
                            k.startswith(("scsi", "virtio", "ide", "sata", "rootfs", "mp"))
                    ...
                    if mappings:
                        ...
                        unmapped = sorted(
                            ...
                            - set(storage_map.keys())
                            - {"none"}
                        )
                        fallback_reason = (
                            f"VM uses storage(s) {unmapped} but no mapping exists"
                            if unmapped
                            else "no disk entries in VM config"
                        )
            except Exception as _e:
        ...
        if not target_storage:
            ...
            fallback = sorted(storage_map.items())[0][1] if storage_map else "local-lvm"
            logger.warning(
                f"[SR] VM {vmid} falling back to storage '{fallback}'"
        ...
        token_result = tgt_mgr.create_api_token("ProxmoxVEx-sr")
        ...
        if not fp_result.get("success"):
            tgt_mgr.delete_api_token("ProxmoxVEx-sr")
        ...
        # (#413 layer 4) - remote_migrate_vm returns success the
        ...
        if not task_upid:
            ...
            return True, ""
        try:
            from ProxmoxVEx.api.vms import _wait_for_task
...
def _start_replicated_vm(tgt_mgr, vmid, vm_type="qemu"):
    ...
    Apr 2026: was calling non-existent start_vm() - use vm_action('start')
    ...
    try:
        ...
        try:
            res = tgt_mgr._api_get(
                f"https://{tgt_mgr.host}:{tgt_mgr.api_port}/api2/json/cluster/resources", params={"type": "vm"}
            )
...
def _target_vmid_exists(tgt_mgr, vmid):
    """is this VMID already present on the target cluster?
...
def _disconnect_test_nics(tgt_mgr, node, vmid, vm_type="qemu"):
    """bring a Test-Failover clone up with every NIC
    ...
    if not isinstance(res, dict) or not res.get("success"):
        logger.warning(
            f"[SR] link-down: get_vm_config failed for test VM {vmid}: "
...
def execute_failover(plan_id, failover_type="planned"):
    ...
    logger.info(
        f"[SR] Starting {failover_type} failover for plan '{_sl(plan['name'])}' ({len(vms)} VMs, {len(boot_groups)} boot groups)"
    )
    ...
    # 2026-04-24 - pre-flight: catch bad mappings BEFORE we start moving VMs.
        ...
        for vm in group_vms:
            ...
            if failover_type == "emergency":
                ...
                if not src_mgr or not src_mgr.is_connected:
                    ...
                    # (#413) - fast pre-flight: a target VMID already
                    ...
                    if tgt_mgr and _target_vmid_exists(tgt_mgr, _tgt_vmid):
                        ok, err = (
                            False,
                            (
                                f"Target VMID {_tgt_vmid} already exists on '{tgt_id}' "
    ...
    now = datetime.utcnow().isoformat()
    db.execute(
        "UPDATE site_recovery_plans SET status = ?, last_failover = ?, updated_at = ? WHERE id = ?",
    ...
    log_audit(
        "system",
        f"site_recovery.{failover_type}_complete",
        f"Plan '{_sl(plan['name'])}' {failover_type} {final_status}: {completed}/{total_vms} VMs",
    )

    logger.info(
        f"[SR] Failover {final_status} for '{_sl(plan['name'])}': {sum(1 for r in results.values() if r['success'])}/{total_vms} succeeded"
    )
...
def execute_test_failover(plan_id):
    ...
    # (#413) - guard against tgt_mgr=None so the loop body
    # surfaces a useful error instead of an AttributeError under `except Exception`.
    if tgt_mgr is None:
        for vm in vms:
            results[str(vm["vmid"])] = {
                "success": False,
                "error": f"Target cluster '{plan['target_cluster']}' not connected / not configured in ProxmoxVEx",
            }
        logger.error(
            f"[SR] Test failover: target cluster '{plan['target_cluster']}' unreachable; aborting plan '{_sl(plan['name'])}'"
        )
    ...
    for i, vm in enumerate(vms):
        ...
        vmid = vm["vmid"]
        vm_name = vm.get("vm_name", f"VM {vmid}")
        # (#413) - site_recovery_vms has a `target_vmid` column for
        ...
        try:
            ...
            logger.info(
                f"[SR] Test failover: searching for VM {target_vmid} (source vmid={vmid}) on "
                ...
                try:
                    ...
                    for v in tgt_vms:
                        if v.get("vmid") == target_vmid:
                            ...
                            clone_result = tgt_mgr.clone_vm(
                                node_name, target_vmid, vtype, newid=test_vmid, name=f"SR-TEST-{vm_name}"
                            )
                            # clone_vm returns dict {success, error, task?} OR truthy legacy value
                            clone_ok = (
                                clone_result.get("success") if isinstance(clone_result, dict) else bool(clone_result)
                            )
                            if clone_ok:
                                test_vmids.append({"vmid": test_vmid, "vm_type": vtype})
                                # (#413) - optionally disconnect every NIC
                                ...
                                # Apr 2026: was start_vm() which doesn't exist - use vm_action
                                ...
                                err = (
                                    clone_result.get("error", "Clone failed")
    ...
    # 2026-04-24 - track ok/failed/total so "6 out of 10 VMs cloned" is visible
    ...
    if event_status == "failed":
        db.execute(
            "UPDATE site_recovery_plans SET status = 'failed', last_test = ?, updated_at = ? WHERE id = ?",
            (now, now, plan_id),
        )
        ...
        if event_status == "partial":
            _broadcast_progress(
                plan_id,
                f"Test partial: {ok_count}/{total} cloned, {failed_count} failed. Review events + cleanup.",
                100,
            )
        else:
    ...
    log_audit(
        "system", "site_recovery.test_complete", f"Test failover for '{_sl(plan['name'])}': {ok}/{len(vms)} VMs cloned"
    )
...
def cleanup_test(plan_id):
    ...
    for entry in test_vmids:
        # Entry can be dict {vmid, vm_type} or int (legacy)
        if isinstance(entry, dict):
            ...
            vtype = "qemu"
        try:
            # Apr 2026: locate test VM via cluster resources (was iterating all nodes)
            try:
                res = tgt_mgr._api_get(
                    f"https://{tgt_mgr.host}:{tgt_mgr.api_port}/api2/json/cluster/resources", params={"type": "vm"}
                )
            ...
            if target_node:
                try:
                    # Was stop_vm/delete_vm - use vm_action; ignore "already stopped" errors
                    if current_status == "running":
    ...
    db.execute(
        "UPDATE site_recovery_plans SET status = 'ready', updated_at = ? WHERE id = ?",
        (datetime.utcnow().isoformat(), plan_id),
    )

    log_audit(
        "system",
        "site_recovery.test_cleanup_complete",
...
def _heartbeat_check():
    ...
    for row in plans:
        ...
        if plan_id not in _last_fail_times:
            ...
            logger.warning(
                f"[SR] Heartbeat: source '{plan['source_cluster']}' unreachable for plan '{_sl(plan['name'])}'"
            )
            continue
        ...
        if elapsed >= timeout:
            # Apr 2026: before auto-failover, verify every VM in plan has a healthy recent
            ...
            for vm_row in vms:
                ...
                if not repl_id:
                    ...
                    continue
                repl = db.query_one(
                    "SELECT enabled, last_status, last_run FROM cross_cluster_replications WHERE id = ?", (repl_id,)
                )
                if not repl:
            ...
            if blockers:
                logger.error(
                    f"[SR] BLOCKING auto-failover for '{_sl(plan['name'])}' — replication unhealthy: {'; '.join(blockers[:5])}"
                )
                log_audit(
                    "system",
                    "site_recovery.auto_failover_blocked",
            ...
            try:
                ...
                if cur.rowcount != 1:
                    ...
                    continue
                # Use crash-safe wrapper so a greenlet crash sets status to 'failed'
                from ProxmoxVEx.api.site_recovery import _safe_spawn_failover

                _safe_spawn_failover(execute_failover, plan_id, "emergency")
                log_audit(
                    "system",
                    "site_recovery.auto_failover",
...
def recover_orphan_runs():
    """One-shot cleanup of in-flight rows left over from a previous ProxmoxVEx
    ...
    try:
        ...
        for row in ev_rows:
            details = json.dumps({
                "reason": "ProxmoxVEx_restart",
                "note": "event was in flight when the service restarted; auto-aborted at boot",
            })
            db.execute(
                "UPDATE site_recovery_events SET status = 'aborted', completed_at = ?, details = ? WHERE id = ?",
                (now, details, row["id"]),
            )
            aborted_events += 1
        ...
        plan_rows = (
            db.query("SELECT id, name, status FROM site_recovery_plans WHERE status IN ('running', 'testing')") or []
        )
...
def start_heartbeat():
    ...
    # (#413): clean up any in-flight rows from the previous process
