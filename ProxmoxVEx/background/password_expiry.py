...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/background/password_expiry.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: ProxmoxVEx Password Expiry Check - Layer 7
# Docs:        https://proxmoxvex.local/docs
# Generated:   2026-09-04
# --------------------------------------------------------------------
# --- ProxmoxVEx auto-header end ---
"""
ProxmoxVEx Password Expiry Check - Layer 7
...
def check_password_expiry():
    ...
    This runs periodically to warn users about expiring passwords
    We track last notification to avoid spamming users
    Only sends once per day per user, even if check runs more often
    """
    ...
    for username, user in users_db.items():
        ...
        # This was a feature request from IT-Sec - they want admins to rotate too
        ...
        try:
            ...
            # Only send notifications at specific intervals, not every day
            # Users complained about too many emails, so we limit it to:
            # - First warning (configured warning_days, e.g. 14 days before)
            # - 7 days before
            # - 3 days before
            # - 1 day before
            # - When expired (then every 3 days as reminder)
            # We use a set to avoid duplicates if warning_days happens to be 7, 3, or 1
            ...
            # Some users might want english only but this works for everyone
            display_name = user.get("display_name", username)

            if days_until_expiry <= 0:
                subject = "[ProxmoxVEx] Password expired / Passwort abgelaufen"
                body = f"""Hello {display_name},

Your ProxmoxVEx password has expired. Please change it as soon as possible.
...
Best regards,
Your ProxmoxVEx System

---

Hallo {display_name},

Ihr ProxmoxVEx-Passwort ist abgelaufen. Bitte ändern Sie es so bald wie möglich.
...
Ihr ProxmoxVEx System"""
            else:
                subject = f"[ProxmoxVEx] Password expires in {days_until_expiry} days / Passwort läuft ab"
                body = f"""Hello {display_name},

Your ProxmoxVEx password will expire in {days_until_expiry} days.
...
Ihr ProxmoxVEx-Passwort läuft in {days_until_expiry} Tagen ab.
