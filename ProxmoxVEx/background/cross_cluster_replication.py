...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/background/cross_cluster_replication.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: ProxmoxVEx Cross-Cluster Replication Scheduler - Layer 7
# Docs:        https://proxmoxvex.local/docs
# Generated:   2026-09-04
# --------------------------------------------------------------------
# --- ProxmoxVEx auto-header end ---
"""
ProxmoxVEx Cross-Cluster Replication Scheduler - Layer 7
Background thread that runs snapshot-based replication jobs on schedule.

Cron-like scheduler for cross-cluster replication.
...
logger = logging.getLogger("ProxmoxVEx.xcrepl")

_xcrepl_thread = None
_xcrepl_running = False

# (#455 @DarmokNoob) - track jobs that are mid-flight so the
...
def _parse_interval_seconds(schedule):
    ...
    We only need the common patterns here since the UI only offers
...
def _xcrepl_loop():
    ...
    from ProxmoxVEx.api.vms import _execute_local_replication, _execute_replication
        ...
        try:
            ...
            if jobs:
                ...
                for job in jobs:
                    ...
                    if elapsed >= interval:
                        # (#455) - skip the tick if this job is still
                        ...
                        if not _claim_job(job["id"]):
                            logger.debug(
                                f"[XCREPL] Job {job['id']} (VM {job['vmid']}) still in-flight, skipping this tick"
                            )
                            continue

                        # Same-cluster uses local replication (no remote-migrate)
                        ...
                        logger.info(
                            f"[XCREPL] Scheduling {'local' if is_local else 'cross-cluster'} job {job['id']} (VM {job['vmid']})"
                        )
                        try:
                            # run in own thread so one slow job doesn't block others
                            threading.Thread(target=_tracked_run, args=(handler, job), daemon=True).start()
