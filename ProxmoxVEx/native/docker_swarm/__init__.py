...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/native/docker_swarm/__init__.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: Docker Manager — ProxmoxVEx Plugin
# Docs:        https://proxmoxvex.local/docs
# Generated:   2026-09-04
# --------------------------------------------------------------------
# --- ProxmoxVEx auto-header end ---
"""
Docker Manager — ProxmoxVEx Plugin
Monitor and manage Docker hosts (Swarm clusters or standalone engines) via SSH from ProxmoxVEx.

Connects to Swarm manager nodes over SSH (using paramiko, already in ProxmoxVEx),
executes docker CLI commands, and exposes results via ProxmoxVEx plugin API.
...
# ---------------------------------------------------------------------------
# Time-series of per-node CPU/RAM/tasks samples stored in PostgreSQL.
...
def _metrics_init_db():
    ...
    with _metrics_lock:
        ...
        try:
            ...
            """)
            cur.execute("CREATE INDEX IF NOT EXISTS idx_node_metrics_ts ON node_metrics(ts DESC)")
            conn.commit()
        finally:
            conn.close()
...
def _metrics_query_history(host, metric, duration_sec):
    ...
    with _metrics_lock:
        conn = PGConnection(_pg_dsn())
        try:
            query = f"SELECT ts, {metric} FROM node_metrics WHERE host = ? AND ts >= ? ORDER BY ts ASC"  # nosec: B608 - metric column is from a hardcoded allowlist
            cur = conn.execute(query, (host, cutoff))
...
class _PersistentTOFUPolicy:
    ...
    def missing_host_key(self, client, hostname, key):
        try:
            ...
            log.info(
                f"[{PLUGIN_ID}] new host key persisted for {hostname} "
...
def _ssh_exec(host_cfg_or_host, user=None, password=None, command="", timeout=15):
    ...
    for attempt in range(2):
        try:
            ...
            stdin, stdout, stderr = client.exec_command(command, timeout=timeout)  # nosec: B601 - command built by callers with shlex.quote
...
def _fetch_nodes():
    ...
    inspects_raw = _docker_cmd('docker node ls -q | xargs -r -I{} docker node inspect {} --format "{{json .}}"')
    inspects_by_id = {}
    if inspects_raw:
...
def _fetch_services():
    ...
    inspects_raw = _docker_cmd('docker service ls -q | xargs -r -I{} docker service inspect {} --format "{{json .}}"')
    inspects_by_id = {}
    inspects_by_name = {}
    if inspects_raw:
        for line in inspects_raw.strip().split("\n"):
...
def _fetch_stacks():
    ...
    for svc in services or []:
        ...
        if ns not in stack_map:
            stack_map[ns] = {"Name": ns, "Services": 0, "running": 0, "desired": 0, "svc_running": 0, "svc_total": 0}
...
def _fetch_containers():
    ...
    for h in hosts:
        out, err, code = _ssh_exec(h, command='docker ps -a --format "{{json .}}" --no-trunc')
...
def _fetch_tasks(service_id):
    ...
    tasks = _docker_json(f'docker service ps {shlex.quote(service_id)} --format "{{{{json .}}}}" --no-trunc') or []
    return tasks
...
def _fetch_images():
    ...
    for h in hosts:
        out, err, code = _ssh_exec(h, command='docker image ls --format "{{json .}}" --no-trunc')
        if code == 0 and out.strip():
            for line in out.strip().split("\n"):
                line = line.strip()
                if not line:
...
def _has_spread(svc):
    ...
    return any(isinstance(p, dict) and p.get("Spread") for p in prefs)
...
def _applier_update_rollback(svc):
    ...
    if not _valid(_RX_DOCKER_REF, name):
        raise ValueError(f"Invalid service name: {name!r}")
    return f"docker service update --update-failure-action rollback --update-monitor 30s {shlex.quote(name)}"
...
def _applier_update_parallelism(svc):
    ...
    if not _valid(_RX_DOCKER_REF, name):
        raise ValueError(f"Invalid service name: {name!r}")
    return f"docker service update --update-parallelism 1 --update-order start-first {shlex.quote(name)}"
...
def _is_check_applicable(svc, check_id):
    ...
    try:
        ...
        healthy = sum(
            1
            for n in nodes
            if (n.get("Availability", "") or "").lower() == "active" and (n.get("Status", "") or "").lower() == "ready"
        )
...
def _api_policy_apply():
    ...
    if not check_id or check_id not in POLICY_APPLIERS:
        return {
            "error": f"Unknown or non-fixable check_id. Auto-fixable: {sorted(POLICY_APPLIERS.keys())}",
        }, 400
    ...
    with contextlib.suppress(Exception):  # Audit should never block the response
...
def _api_policy_appliers():
    ...
    return {
        ...
        "manual_only": sorted({c["id"] for c in POLICY_CHECKS} - set(POLICY_APPLIERS.keys())),
    }
...
def _api_service_detail():
    ...
    if can_unmask:
        log_audit(_get_username(), "docker.service_envs_unmasked", f"Viewed unmasked envs for {service_id}")

    inspect = _docker_json(f'docker service inspect {shlex.quote(service_id)} --format "{{{{json .}}}}"')
    ...
    # Get tasks
    tasks = (
        _docker_json(f'docker service ps {shlex.quote(service_id)} --format "{{{{json .}}}}" --no-trunc 2>/dev/null')
        or []
    )
    ...
    detail = {
        ...
        "previous_image": prev_spec.get("TaskTemplate", {}).get("ContainerSpec", {}).get("Image", "")
        if prev_spec
        else "",
    }
...
def _api_service_scale():
    ...
    if result is not None:
        log_audit(_get_username(), "docker.service_scaled", f"Scaled {service_id} to {replicas} replicas")
        _invalidate("services")
...
def _api_service_restart():
    ...
    if result is not None:
        log_audit(_get_username(), "docker.service_restarted", f"Force-updated service {service_id}")
        _invalidate("services")
...
def _api_service_remove():
    ...
    if result is not None:
        log_audit(_get_username(), "docker.service_removed", f"Removed service {service_id}")
        _invalidate("all")
...
def _api_stack_deploy():
    ...
    cmd = (
        f"set -e; "
        f"tmpf=$(mktemp -t ProxmoxVEx_stack.XXXXXXXX) && "
...
def _api_stack_detail():
    ...
    can_unmask = want_unmask
    if can_unmask:
        log_audit(_get_username(), "docker.stack_envs_unmasked", f"Viewed unmasked envs for stack {name}")
    ...
    for svc in services:
        svc_name = svc.get("Name", "")
        tasks = (
            _docker_json(f'docker service ps {shlex.quote(svc_name)} --format "{{{{json .}}}}" --no-trunc 2>/dev/null')
            or []
        )
        svc["tasks"] = tasks
        # Get inspect for image, ports, etc.
        inspect = _docker_json(f'docker service inspect {shlex.quote(svc_name)} --format "{{{{json .}}}}"')
        if inspect and isinstance(inspect, list):
            inspect = inspect[0]
        if inspect:
            spec = inspect.get("Spec", {})
    ...
    networks = (
        _docker_json(
            f'docker network ls --filter label=com.docker.stack.namespace={shlex.quote(name)} --format "{{{{json .}}}}"'
        )
        or []
    )
...
def _api_stack_compose():
    ...
    raw = _docker_cmd(f"docker stack config {shlex.quote(name)} 2>/dev/null")
    ...
    for svc in services:
        ...
        user_labels = {k: v for k, v in spec.get("Labels", {}).items() if not k.startswith("com.docker.")}
        if user_labels:
...
def _api_stack_stop():
    ...
    return {
        "success": True,
        "message": f"Stack {stack_name} stopped ({scaled} services scaled to 0)",
        "saved_replicas": saved_replicas,
    }
...
def _api_stack_start():
    ...
    return {
        "success": True,
        "message": f"Stack {stack_name} started ({started} services restored)",
...
def _api_container_action():
    ...
    if host:
        ...
        result = _docker_cmd(cmd, host_cfg=target) if target else _docker_cmd(cmd)
    else:
        result = _docker_cmd(cmd)

    if result is not None:
        log_audit(_get_username(), f"docker.container_{action}", f"{action} container {container_id}")
...
def _api_node_action():
    ...
    if result is not None:
        log_audit(_get_username(), f"docker.node_{action}", f"Set node {node_id} to {action}")
        _invalidate("nodes")
        return {"success": True, "message": f"Node {node_id} set to {action}"}
    return {"error": "Node update failed"}, 500
...
def _api_topology():
    """GET — Return Swarm data formatted for ProxmoxVEx TopologyView multiCluster."""
    ...
    # Build nodes array matching ProxmoxVEx format: {name, status, cpu_percent, mem_percent, ...}
    topo_nodes = []
...
def _api_load_balance():
    ...
    def fetch_node_data(h):
        ...
        tasks_cmd = (
            (
                f'docker node ps {shlex.quote(node_id)} --filter "desired-state=running" '
    ...
    if total_tasks > 0:
        if len(healthy_nodes) <= 1:
            ...
            recommendation = (
                ...
                if len(healthy_nodes) == 1
                else "No hay nodos activos sirviendo tasks"
            )
...
def _api_rebalance_service():
    ...
    if result is not None:
        log_audit(_get_username(), "docker.service_rebalance", f"Force rebalanced service {service_name}")
        _invalidate("services")
...
def _service_is_pinned(svc):
    ...
    return any(isinstance(c, str) and _RX_PIN_CONSTRAINT.search(c) for c in svc.get("constraints") or [])
...
# keeps running in ProxmoxVEx. We move the loop to a daemon thread, return
        ...
        for idx, c in enumerate(candidates):
            ...
            if not _valid(_RX_DOCKER_REF, name):
                ...
                if out is None:
                    ...
                    state_cmd = f"docker service inspect {shlex.quote(name)} --format '{{{{.UpdateStatus.State}}}}'"
            ...
            with contextlib.suppress(Exception):
                log_audit(username, "docker.balance_rebalance_all", f"force-update {name} (success={success})")
...
#   - The endpoint requires the CALLER to be unauthenticated ProxmoxVEx-wise (CI),
#     so we MUST skip the global plugins.view auth — implemented by a bypass
#     marker. ProxmoxVEx's plugin route runner enforces auth by default; the
...
def _api_webhook_trigger():
    ...
    No ProxmoxVEx session required — auth is the secret in the URL. Designed for
    CI/CD systems (GitHub Actions, Drone, etc.). Compares secret with hmac.compare_digest
    to avoid timing attacks. Note: ProxmoxVEx may still gate this with `plugins.view`
    permission depending on global config; if so, configure the CI to send the
    ProxmoxVEx session token, OR disable the gate for this specific path in nginx.
    """
    ...
    log_audit("webhook", "docker.webhook_triggered", f"wid={wid} svc={service_name} tag={tag or '(force-only)'}")
    _invalidate("services")
    return {
        "success": True,
        "service_name": service_name,
...
_PRUNE_TARGETS = {
    ...
    "all-safe": (
        'docker builder prune -a -f && docker image prune -a -f --filter "until=24h" && docker container prune -f && docker network prune -f',
        "Todo excepto volúmenes (seguro)",
    ),
...
def _disk_auto_prune_tick():
    ...
    if actions:
        ...
        _save_config(cfg)
        log_audit("auto-prune", "docker.disk_auto_prune", f"freed={total_freed} actions={len(actions)}")
...
_SWARM_ONLY_ROUTES = frozenset({
    ...
    "rebalance-service",
    "balance/insights",
    "balance/rebalance-all",
    "balance/rebalance-status",
    "metrics/history",
    "metrics/trends",
    "policy/audit",
    "policy/checks",
...
def _api_clusters():
    ...
    out = []
    for c in _get_clusters():
        mode = _run_in_cluster(c["id"], lambda: _get_engine_mode_cached() or _detect_mode())
        out.append({
            "id": c["id"],
            "name": c["name"],
