...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/native/zabbix/__init__.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: Package initializer for zabbix
...
HOSTS_KEY = "zabbix_hosts"
SECRET_KEYS = ["api_token"]

if PLUGIN_DIR not in sys.path:
    sys.path.insert(0, PLUGIN_DIR)

from zabbix_src.client import ZabbixHost  # noqa: E402
...
def _host_from_dict(h: dict):
    """Convert a config.json host entry into a typed ZabbixHost."""
    return ZabbixHost(
        name=str(h.get("name", "zabbix")),
        url=str(h.get("url", "")),
        api_token=str(h.get("api_token", "")),
...
def _h_ui():
    return send_file(os.path.join(PLUGIN_DIR, "zabbix.html"), mimetype="text/html")
...
def _h_overview():
    from flask import jsonify
    from zabbix_src.routes.overview import build_overview_payload
...
def _h_hosts():
    from flask import jsonify
    from zabbix_src.routes.overview import build_hosts_payload
...
def _h_problems():
    from flask import jsonify
    from zabbix_src.routes.overview import build_problems_payload
    ...
    if not hosts:
        return _unconfigured_response()
    host = _host_from_dict(hosts[0])
    status, payload = build_problems_payload(host)
    return jsonify(payload), status
