...
(async function () {
  ...
  function t(key) {
    if (_parentI18n) return _parentI18n.t(key, { ns: 'zabbix' });
    return key;
  }
  function tf(key) {
    let s = t(key);
  ...
  if (_parentI18n) {
    await _parentI18n.loadPluginNamespaceFull('zabbix', '/api/native/zabbix/i18n');
  }
  applyI18n();
  ...
  // Load initial tab.
  loadData('overview', '/api/zabbix/overview');
})();
