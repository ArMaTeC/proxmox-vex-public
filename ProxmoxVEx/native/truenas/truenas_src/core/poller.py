...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/native/truenas/truenas_src/core/poller.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: F4a background poller: a single daemon thread, started...
...
log = logging.getLogger("native.truenas.poller")
...
def start(config_path, state_path, get_conn):
    ...
    with _lock:
        ...
        _thread = threading.Thread(
            target=_loop, args=(config_path, state_path, get_conn), name="truenas-poller", daemon=True
        )
        _thread.start()
...
def stop(timeout=5):
    """Used by tests (and a clean plugin shutdown, if ProxmoxVEx ever calls
...
def _summaries_to_readings(instances, summaries):
    ...
    for summary in summaries:
        ...
        pools = [{"name": row["pool"], "used_pct": row["pct"]} for row in summary.get("pool_usage", [])]
        readings.append({
            "instance_id": iid,
...
def run_one_cycle(config_path, state_path, get_conn):
    ...
    try:
        ...
        new_state, notifications = alerts.evaluate(prev_state, time.time(), readings, cfg["thresholds"])
        alerts.save_state(state_path, new_state)

        if notifications:
            notify_cfg = cfg.get("notify") or {}
            # Only attempt delivery for channels that are actually configured.
            # Unconfigured channels are intentional and should not warn.
            ...
            whatsapp_url = notify_cfg.get("whatsapp_gateway_url")
            whatsapp_instance = notify_cfg.get("whatsapp_instance")
            whatsapp_key = notify_cfg.get("whatsapp_api_key")
            whatsapp_target = notify_cfg.get("whatsapp_target")
            if whatsapp_url and whatsapp_instance and whatsapp_key and whatsapp_target:
                ok, err = notify.send_whatsapp(
                    whatsapp_url, whatsapp_instance, whatsapp_key, whatsapp_target, notifications
                )
                if not ok:
