...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/native/truenas/truenas_src/core/notify.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: Notification channels for the F4a alert engine...
...
log = logging.getLogger("native.truenas.notify")
...
def send_webhook(url, notifications, timeout=DEFAULT_TIMEOUT_S):
    ...
    if not url.lower().startswith(("http://", "https://")):
        return False, "invalid webhook URL (only http/https allowed)"
    body = json.dumps({"notifications": notifications}).encode("utf-8")
    req = urllib.request.Request(url, data=body, method="POST", headers={"Content-Type": "application/json"})
    try:
        with urllib.request.urlopen(req, timeout=timeout) as resp:  # nosec: B310 - URL validated above
            if resp.status >= 300:
...
def send_whatsapp(gateway_url, instance, api_key, target, notifications, timeout=DEFAULT_TIMEOUT_S):
    ...
    if not gateway_url.lower().startswith(("http://", "https://")):
        return False, "invalid whatsapp gateway URL (only http/https allowed)"
    ...
    req = urllib.request.Request(
        url, data=body, method="POST", headers={"Content-Type": "application/json", "apikey": api_key}
    )
    try:
        with urllib.request.urlopen(req, timeout=timeout) as resp:  # nosec: B310 - URL validated above
            if resp.status >= 300:
