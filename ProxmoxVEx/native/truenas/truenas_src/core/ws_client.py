...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/native/truenas/truenas_src/core/ws_client.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: Generic, reusable JSON-RPC 2.0 client over a persistent...
...
Design constraints (see ProxmoxVEx_PLUGIN_TRUENAS_BRIEF.md §2/§4/§9):
...
log = logging.getLogger("native.truenas.ws_client")
...
class TrueNASWSClient:
    ...
    def connect(self, _clear_closed=True):
        ...
        with self._connect_lock:
            ...
            if self._closed and not _clear_closed:
                raise TrueNASConnectionError("client was closed; refusing to reconnect without an explicit connect()")
            try:
                self._ws = self._transport_factory(self.url(), self.verify_tls, self.timeout, self.tls_server_name)
            except Exception as e:
                self._last_error = str(e)
                raise TrueNASConnectionError(
                    f"could not connect to {self.host}:{self.port}: {e}", retryable=not _is_permanent_tls_failure(e)
                ) from e
            ...
            self._reader_thread = threading.Thread(target=self._read_loop, name=f"truenas-ws-{self.host}", daemon=True)
            ...
            if self.keepalive_interval_s:
                ...
                self._keepalive_thread = threading.Thread(
                    target=self._keepalive_loop, name=f"truenas-keepalive-{self.host}", daemon=True
                )
        ...
        for attempt in range(attempts):
            try:
                ...
                if not e.retryable:
                    ...
                    log.warning(
                        f"[truenas] connect to {self.host}:{self.port} failed "
                ...
                if attempt < attempts - 1:
                    ...
                    log.warning(
                        f"[truenas] reconnect attempt {attempt + 1}/{attempts} failed, retrying in {delay:.1f}s: {e}"
                    )
                    self._sleep(delay)
    ...
    def _relogin_and_resubscribe(self):
        ...
        if self._api_key:
            try:
                ...
                log.error(f"[truenas] relogin after reconnect rejected by server (bad/revoked key?): {e}")
    ...
    def _handle_unexpected_disconnect(self, reason):
        ...
        with self._reconnect_guard_lock:
            if self._reconnecting:
                log.debug(
                    f"[truenas] reconnect already in progress for {self.host}:{self.port}, not spawning a duplicate"
                )
                return
            self._reconnecting = True
        threading.Thread(target=self._background_reconnect, name=f"truenas-reconnect-{self.host}", daemon=True).start()
            ...
            for cycle in range(self.max_reconnect_attempts):
                ...
                try:
                    ...
                    if cycle >= self.max_reconnect_attempts - 1:
                        log.error(
                            f"[truenas] gave up relogging in to {self.host}:{self.port} after {cycle + 1} cycles: {e}"
                        )
                        return
    ...
    def call(self, method, params=None, timeout=None):
        ...
        if response is None:
            raise TrueNASConnectionError(entry["error_reason"] or f"connection lost before '{method}' answered")
    ...
    def _read_loop(self):
        while not self._stop_reader.is_set():
            ...
            try:
                ...
                log.exception(
                    f"[truenas] reader loop crashed handling a frame from "
    ...
    def _handle_raw_frame(self, raw):
        ...
        if msg.get("id") is not None:
            ...
            log.warning(
                f"[truenas] server sent a protocol-level error (id=null) from "
    ...
    def _dispatch_response(self, msg):
        ...
        if entry is None:
            ...
            if "result" in msg or msg.get("error") is not None:
                log.warning(
                    f"[truenas] late response for expired id={req_id} carried "
    ...
    def login(self, api_key):
        ...
        one-shot "Test connection" check), where there's no shared session
    ...
    def _do_login(self, api_key):
        ...
        with self._connect_lock:
            if not self._connected:
                raise TrueNASConnectionError("socket dropped during login — session not established")
    ...
    def unsubscribe(self, name, callback=None):
        """Unsubscribe from a server-side collection.  Removes the local
        callback(s) and also sends ``core.unsubscribe`` on the wire so the
        server stops pushing ``collection_update`` events for ``name``."""
        ...
        if should_unsubscribe:
            try:
                self.call("core.unsubscribe", [name])
            except Exception as e:
                log.warning(f"[truenas] core.unsubscribe for {name!r} failed: {e}")
