...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/native/truenas/truenas_src/core/alerts.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: F4a alert engine: pure functions, no...
...
log = logging.getLogger("native.truenas.alerts")
...
def load_state(path):
    ...
    try:
        ...
        log.error(f"[truenas] alerts state at {path!r} is corrupt/unreadable, starting fresh: {e}", exc_info=True)
        return default_state()
...
def _step(prev_conditions, key, new_level, now, notif):
    ...
    return condition, dict(notif, level=new_level, ts=now, transition=f"{prev_level}->{new_level}")
...
def evaluate(prev_state, now, readings, global_thresholds):
    ...
    for reading in readings:
        ...
        for key, prev in prev_conditions.items():
            if key.startswith(f"{iid}|ta_alert|") and key not in seen_alert_keys and prev.get("level") == "active":
                _, notif = _step(
                    prev_conditions,
                    key,
                    "cleared",
    ...
    notifications, rate_limit = _apply_rate_limit(raw_notifications, prev_state.get("rate_limit", {}), now)
