...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/native/truenas/truenas_src/routes/config_store.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: ``config.json`` load/save/masking/validation for the...
...
verified in production by ``ProxmoxVEx-plugin-wake-on-lan``: GET always masks
...
belong to different clients in the same ProxmoxVEx panel. It is NOT sensitive
...
log = logging.getLogger("native.truenas.config_store")
...
def decrypt_value(stored, fernet, label):
    ...
    try:
        ...
        log.error(
            f"[truenas] {label}: stored value is encrypted but failed to decrypt "
...
def load_config(path):
    ...
    try:
        ...
        log.error(
            f"[truenas] config at {path!r} is corrupt/unreadable, falling back to "
    ...
    cfg["instances"] = [
        dict(
            inst,
            **{f: decrypt_value(inst.get(f), fernet, f"instance '{inst.get('id', '?')}'.{f}") for f in _KEY_FIELDS},
        )
...
def save_config(path, cfg):
    ...
    on_disk = dict(cfg)
    on_disk["instances"] = [
        dict(inst, **{f: encrypt_value(inst.get(f), fernet) for f in _KEY_FIELDS}) for inst in cfg.get("instances", [])
    ]
    tmp = path + ".tmp"
...
def validate_instances(raw_instances, old_instances, global_thresholds=None):
    ...
    for raw in raw_instances:
        ...
        for field in _KEY_FIELDS:
            ...
            if incoming == MASK:
                if not old.get(field):
                    return None, (
                        f"instance '{inst_id}': masked {field} received but no previous "
                        f"value stored (did you rename it? paste the key again)"
                    )
        ...
        if not use_tls and (keys["api_key_ro"] or keys["api_key_rw"]):
            return None, (
                f"instance '{inst_id}': use_tls must be true when an API key is "
                f"configured (TrueNAS automatically revokes the key over plain HTTP)"
            )
        ...
        clean.append({
            ...
            "tls_server_name": (str(raw.get("tls_server_name")).strip() if raw.get("tls_server_name") else None),
...
def validate_notify(raw, old_notify=None):
    ...
    if url == MASK:
        if not old_notify.get("webhook_url"):
            return None, (
                "notify.webhook_url: masked value received but no previous value stored (paste the URL again)"
            )
    ...
    if api_key == MASK:
        if not old_notify.get("whatsapp_api_key"):
            return None, (
                "notify.whatsapp_api_key: masked value received but no previous value stored (paste the key again)"
            )
