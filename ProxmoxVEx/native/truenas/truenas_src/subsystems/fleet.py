...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/native/truenas/truenas_src/subsystems/fleet.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: Fleet Overview (F3): fan out over every configured...
...
directly from TrueNAS's own admin UI, or ``svc-ProxmoxVEx-rw`` recording this
...
log = logging.getLogger("native.truenas.fleet")
...
def _instance_activity(conn):
    ...
    return (
        conn.call(
            "audit.query", [{"query-filters": _ACTIVITY_FILTERS, "query-options": options}], timeout=PER_CALL_TIMEOUT_S
        )
        or []
    )
...
def _fetch_instance(inst, get_conn):
    ...
    pools, pools_error = safe_call("pool.query", lambda: list_pools(conn), [])
    svcs, svcs_error = safe_call("service.query", lambda: list_services(conn), [])
    activity, activity_error = safe_call("audit.query", lambda: _instance_activity(conn), [])
...
def fetch_fleet(instances, get_conn):
    ...
    with ThreadPoolExecutor(max_workers=workers) as pool:
        future_map = {pool.submit(_fetch_instance, inst, get_conn): i for i, inst in enumerate(instances)}
        for future in as_completed(future_map):
            ...
            try:
                ...
                inst = instances[i]
                log.error(f"[truenas] fleet fetch crashed for instance '{inst.get('id')}': {e}", exc_info=True)
                results[i] = {
                    "id": inst.get("id"),
                    "name": inst.get("name", inst.get("id")),
                    "client_id": inst.get("client_id", "unassigned"),
                    "reachable": False,
                    "connect_error": f"unexpected error: {e}",
                }
    return results
...
def aggregate(instance_summaries):
    ...
    degraded = [
        s for s in reachable if s.get("unhealthy_pools") or s.get("critical_alert_count") or s.get("down_services")
    ]
