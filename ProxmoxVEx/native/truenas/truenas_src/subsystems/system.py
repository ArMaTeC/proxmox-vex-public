...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/native/truenas/truenas_src/subsystems/system.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: System subsystem: ``system.info``, ``alert.list``,...
...
class SystemSubsystem(Subsystem):
    ...
    def health(self, conn, active_alerts=None):
        ...
        healthy = not bad
        summary = "no active ERROR+ alerts" if healthy else f"{len(bad)} active alert(s) at ERROR or above"
        return HealthReport(
            healthy=healthy,
            summary=summary,
            details={
