...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/native/truenas/truenas_src/subsystems/services.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: Services subsystem: ``service.query`` (F4a, read-only) and
...
"ProxmoxVEx RW" on `.64`) after confirming no admin credential for widening
...
class ServicesSubsystem(Subsystem):
    ...
    def health(self, conn, services=None):
        ...
        summary = (
            ...
            if healthy
            else f"{len(down)} enabled service(s) not running: {', '.join(down)}"
        )
        return HealthReport(
            healthy=healthy,
            summary=summary,
