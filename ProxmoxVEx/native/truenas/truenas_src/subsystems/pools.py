...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/native/truenas/truenas_src/subsystems/pools.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: Pools & Disks subsystem: ``pool.query``...
...
class PoolsSubsystem(Subsystem):
    ...
    def health(self, conn, pools=None):
        ...
        summary = "all pools healthy" if healthy else f"{len(unhealthy)} pool(s) degraded: {', '.join(unhealthy)}"
        return HealthReport(
            healthy=healthy,
            summary=summary,
            details={
