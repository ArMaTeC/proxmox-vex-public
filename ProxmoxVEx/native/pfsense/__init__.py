...
# Version:     1.2.303
# Build:       2026.09.04
# Description: Package initializer for pfsense
...
def _host_from_dict(h: dict):
    """Convert a config.json host entry into a typed PFsenseHost."""
    return PFsenseHost(
...
def _h_overview():
    from flask import jsonify
    from pfsense_src.routes.overview import build_overview_payload
...
def _h_network():
    from flask import jsonify
    from pfsense_src.routes.overview import build_network_payload
...
def _h_rules():
    from flask import jsonify
    from pfsense_src.routes.overview import build_rules_payload
    ...
    if not hosts:
        return _unconfigured_response()
    host = _host_from_dict(hosts[0])
    status, payload = build_rules_payload(host)
    return jsonify(payload), status
