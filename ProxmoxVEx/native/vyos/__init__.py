...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/native/vyos/__init__.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: Package initializer for vyos
...
HOSTS_KEY = "vyos_hosts"
SECRET_KEYS = ["api_key"]

if PLUGIN_DIR not in sys.path:
    sys.path.insert(0, PLUGIN_DIR)

from vyos_src.client import VyOSHost  # noqa: E402
...
def _host_from_dict(h: dict):
    """Convert a config.json host entry into a typed VyOSHost."""
    return VyOSHost(
        name=str(h.get("name", "vyos")),
        url=str(h.get("url", "")),
...
def _h_ui():
    return send_file(os.path.join(PLUGIN_DIR, "vyos.html"), mimetype="text/html")
...
def _h_overview():
    from flask import jsonify
    from vyos_src.routes.overview import build_overview_payload
...
def _h_interfaces():
    from flask import jsonify
    from vyos_src.routes.overview import build_interfaces_payload
    ...
    if not hosts:
        return _unconfigured_response()
    host = _host_from_dict(hosts[0])
    status, payload = build_interfaces_payload(host)
    return jsonify(payload), status


def _h_routes():
    from flask import jsonify
    from vyos_src.routes.overview import build_routes_payload
    ...
    if not hosts:
        return _unconfigured_response()
    host = _host_from_dict(hosts[0])
    status, payload = build_routes_payload(host)
    return jsonify(payload), status
