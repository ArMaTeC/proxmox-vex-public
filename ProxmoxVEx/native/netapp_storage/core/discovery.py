...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/native/netapp_storage/core/discovery.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: Auto-Discovery: matches PVE storages to ONTAP...
...
def _pve_session(pve_host):
    ...
    try:
        ...
        raise RuntimeError(f"PVE-Login fehlgeschlagen ({pve_host['host']}): {exc}") from exc

    if r.status_code != 200:
        raise RuntimeError(f"PVE-Login {pve_host['host']} → HTTP {r.status_code}: {r.text[:200]}")
...
def run_discovery(endpoint_id=None):
    ...
    if not pve_rows:
        debug_info["no_match_reasons"].append("No PVE hosts configured. Please add a PVE host in the 'Endpoints' tab.")
    ...
    for row in pve_rows:
        ...
        if err:
            debug_info["no_match_reasons"].append(f"PVE '{pve['name']}' ({pve['host']}): {err}")
        elif not storages:
            ...
            try:
                ...
                if r2.status_code == 200:
                    all_types = [f"{s.get('storage')} (type={s.get('type')})" for s in r2.json().get("data", [])]
                debug_info["no_match_reasons"].append(
                    f"PVE '{pve['name']}': no NFS datastores found. Available storages: {all_types or '(none)'}"
                )
    ...
    for ep_row in ep_rows or []:
        ...
        for stor in all_pve_storages:
            ...
            try:
                ...
                row = db.query_one(
                    "SELECT * FROM netapp_volume_mapping WHERE pve_cluster_id=? AND pve_storage_id=?",
                ...
                msg = (
                    f"DB insert failed for '{stor['storage_id']}': {exc} "
    ...
    try:
        ...
        try:
            snap_out = ssh_run(
                ...
                u,
                p,
                "lvs --noheadings -o lv_name,vg_name 2>/dev/null | awk '$1==\"netapp_snapmanifest\"{print $2}'",
                capture=True,
                key_material=k,
            )
            ...
            if "nvme" in pv_dev:
                ...
                try:
                    ...
                    if not ns_uuid:
                        ...
                        uuid_out2 = ssh_run(
                            ...
                            u,
                            p,
                            f"nvme id-ns {shlex.quote(ns_dev)} 2>/dev/null | awk '/^uuid/{{print $NF; exit}}'",
                            capture=True,
                            key_material=k,
                        )
                        ns_uuid = uuid_out2.strip().lower()
                    stor["nvme_ns_uuid"] = ns_uuid
                    log.info(f"[netapp_storage] NVMe UUID {pv_dev} (NS={ns_dev}): {ns_uuid or '—'}")
                except Exception as exc:
                ...
                if pv_dev.startswith("/dev/mapper/"):
                    ...
                    if len(wwid) == 33 and wwid.upper().startswith("3600A098"):
                        ...
                        serial = wwid[9:].upper()
                        log.info(f"[netapp_storage] iSCSI multipath {pv_dev}: extracted serial {serial} from WWID")
                    if not serial:
                        ...
                        if not serial:
                            ...
                            try:
                                slaves_out = ssh_run(
                                    ...
                                    u,
                                    p,
                                    f"ls /sys/block/{shlex.quote(dev_base)}/slaves/ 2>/dev/null | head -1",
                                    capture=True,
                                    key_material=k,
                                )
...
def _run_san_discovery(db, ep_rows, pve_rows, found_mappings, debug_info, now):
    ...
    for row in pve_rows or []:
        ...
        if err:
            debug_info["no_match_reasons"].append(f"PVE '{pve['name']}' SAN-Storages: {err}")
        for s in storages:
    ...
    debug_info.setdefault("san_storages", []).extend([
        {
            ...
            "storage_id": s["storage_id"],
            "vg": s["vg_name"],
            "serial": s.get("pv_serial", ""),
            "nvme_uuid": s.get("nvme_ns_uuid", ""),
            "protocol": s["protocol"],
        }
    ...
    serial_to_stor = {s["pv_serial"].upper(): s for s in all_lvm_storages if s.get("pv_serial")}
    ...
    nvme_uuid_to_stor = {_norm_uuid(s["nvme_ns_uuid"]): s for s in all_lvm_storages if s.get("nvme_ns_uuid")}
    ...
    for ep_row in ep_rows or []:
        ...
        try:
            ...
            debug_info["no_match_reasons"].append(f"ONTAP '{ep['name']}': Verbindung fehlgeschlagen: {exc}")
            continue
        ...
        if serial_to_stor:
            try:
                ...
                debug_info["no_match_reasons"].append(f"ONTAP '{ep['name']}': LUN-Abfrage fehlgeschlagen: {exc}")
                luns = []
        ...
        if nvme_uuid_to_stor:
            ...
            if not namespaces:
                debug_info["no_match_reasons"].append(
                    f"ONTAP '{ep['name']}': NVMe-Namespace-Liste leer (suche UUIDs: {sorted(nvme_uuid_to_stor.keys())})"
                )
            else:
                ns_debug = [
                    {
                ...
                missing_vol_svms = {e["svm"] for e in ns_debug if not e["vol_uuid"] and e["svm"]}
...
def _san_upsert(db, ep, location, svm_obj, lun_uuid, lun_path, stor, found_mappings, debug_info, now):
    ...
    if not vol_uuid:
        debug_info["no_match_reasons"].append(f"LUN/Namespace {lun_path}: kein Volume-UUID in ONTAP-Antwort")
        return
    ...
    try:
        ...
        msg = (
            f"SAN DB-Insert fehlgeschlagen '{stor['storage_id']}': {exc} | lun_uuid={lun_uuid!r} vol_uuid={vol_uuid!r}"
        )
        log.warning(f"[netapp_storage] {msg}")
        debug_info["no_match_reasons"].append(msg)
