...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/native/netapp_storage/core/san_helpers.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: SAN helpers for iSCSI/NVMe-oF snapshots and restores.
...
def snapmanifest_initialize(ssh_host, ssh_user, ssh_pass, ssh_key, vg_name, lv_name="netapp_snapmanifest", size_mb=64):
    ...
    try:
        ...
        if free_mb < size_mb:
            raise RuntimeError(f"VG {vg_name}: only {free_mb:.0f} MB free, {size_mb} MB required.")
    except ValueError:
...
def snapmanifest_write_manifest(ssh_host, ssh_user, ssh_pass, ssh_key, vg_name, lv_name, manifest, jlog=None):
    ...
    mp = f"/tmp/.pgsi_{_uuid.uuid4().hex[:10]}"  # nosec: B108 - NetApp helper remote mount path
    mp_q = shlex.quote(mp)
    ...
    try:
        ...
        ssh_run(ssh_host, ssh_user, ssh_pass, f"lvchange -aey {vg_q}/{lv_q}", key_material=ssh_key)
        activated = True

        ssh_run(
            ssh_host, ssh_user, ssh_pass, f"mkdir -p {mp_q} && mount {shlex.quote(dev)} {mp_q}", key_material=ssh_key
        )
        mounted = True
        ...
        ssh_run(
            ssh_host, ssh_user, ssh_pass, f"cat > {mp_q}/manifest.json", stdin_data=manifest_bytes, key_material=ssh_key
        )
        ...
        if activated:
            try:
                ssh_run(ssh_host, ssh_user, ssh_pass, f"lvchange -an {vg_q}/{lv_q}", key_material=ssh_key)
            except Exception as exc:
...
def snapmanifest_read_manifest(ssh_host, ssh_user, ssh_pass, ssh_key, vg_name, lv_name="netapp_snapmanifest"):
    ...
    try:
        ...
        raw = ssh_run(ssh_host, ssh_user, ssh_pass, f"cat {mp_q}/manifest.json", capture=True, key_material=ssh_key)
        return json.loads(raw)

    except json.JSONDecodeError as exc:
        raise RuntimeError(f"snapmanifest manifest.json malformed: {exc}") from exc
    finally:
        if mounted:
            try:
                ssh_run(
...
def find_device_by_serial(ssh_host, ssh_user, ssh_pass, ssh_key, serial, timeout_s=90):
    ...
    while time.monotonic() < deadline:
        ...
        time.sleep(3)
    raise RuntimeError(
        f"Block device with serial {serial} not found after {timeout_s}s (tried {mapper_dev} and {byid_dev})"
    )
...
def flush_iscsi_clone_device(ssh_host, ssh_user, ssh_pass, ssh_key, serial):
    ...
    try:
        ssh_run(ssh_host, ssh_user, ssh_pass, flush_cmd, key_material=ssh_key, timeout=30)
...
def vg_import_clone(ssh_host, ssh_user, ssh_pass, ssh_key, device, base_vg_name):
    ...
    ssh_run(ssh_host, ssh_user, ssh_pass, f"pvscan --cache {dev_q} 2>/dev/null; true", key_material=ssh_key)
    ...
    ssh_run(ssh_host, ssh_user, ssh_pass, f"vgimportclone --basevgname {base_q} {dev_q}", key_material=ssh_key)
    ...
    ssh_run(ssh_host, ssh_user, ssh_pass, "udevadm settle --timeout=5 2>/dev/null; true", key_material=ssh_key)
    ssh_run(
        ssh_host,
        ssh_user,
        ssh_pass,
        f"vgchange -an {shlex.quote(actual)} 2>/dev/null; udevadm settle --timeout=3 2>/dev/null; true",
        key_material=ssh_key,
    )
...
def activate_lv_for_restore(ssh_host, ssh_user, ssh_pass, ssh_key, vg_name, lv_name, lvm_type, pool_name=""):
    ...
    if lvm_type == "thin" and pool_name:
        ...
        ssh_run(ssh_host, ssh_user, ssh_pass, f"lvchange -ay {vg_q}/{pool_q}", key_material=ssh_key)

    ssh_run(ssh_host, ssh_user, ssh_pass, f"lvchange -ay {vg_q}/{lv_q}", key_material=ssh_key)


def lv_copy(ssh_host, ssh_user, ssh_pass, ssh_key, src_vg, src_lv, dst_vg, dst_lv, jlog=None):
    ...
    tmp = f"/tmp/.dd_{_uuid.uuid4().hex[:8]}"  # nosec: B108 - NetApp helper remote dd temp path
...
def vg_deactivate(ssh_host, ssh_user, ssh_pass, ssh_key, vg_name):
    ...
    """
    vg_q = shlex.quote(vg_name)
    ssh_run(ssh_host, ssh_user, ssh_pass, f"vgchange -an {vg_q}", key_material=ssh_key)
...
def create_lv(ssh_host, ssh_user, ssh_pass, ssh_key, vg_name, lv_name, size_bytes, lvm_type, pool_name=""):
    ...
    if lvm_type == "thin" and pool_name:
        ...
        ssh_run(
            ...
            ssh_user,
            ssh_pass,
            f"lvcreate -V {size_bytes}B -T {vg_q}/{pool_q} -n {lv_q} --zero n --wipesignatures n",
            key_material=ssh_key,
        )
    else:
        ssh_run(
            ...
            ssh_user,
            ssh_pass,
            f"lvcreate -L {size_bytes}B -n {lv_q} {vg_q} --zero n --wipesignatures n",
            key_material=ssh_key,
        )
...
def nvme_list_devices(ssh_host, ssh_user, ssh_pass, ssh_key):
    ...
    try:
        out = ssh_run(
            ssh_host, ssh_user, ssh_pass, "ls /dev/nvme*n* 2>/dev/null || true", capture=True, key_material=ssh_key
        )
        return {
            line.strip()
            for line in out.splitlines()
...
def find_new_nvme_device(ssh_host, ssh_user, ssh_pass, ssh_key, devices_before, timeout_s=30):
...
def find_nvme_device_for_subsystem_nqn(
    ssh_host, ssh_user, ssh_pass, ssh_key, subsystem_nqn, timeout_s=60, devices_before=None
):
            ...
            for subsys in subsystems:
                ...
                for ctrl in subsys.get("Controllers") or subsys.get("controllers") or []:
                    for ns in ctrl.get("Namespaces") or ctrl.get("namespaces") or []:
                        ns_name = (ns.get("Name") or ns.get("name") or ns.get("NameSpace") or "").strip()
                        if ns_name:
    ...
    def _try_sysfs():
        ...
        try:
            ...
            out = ssh_run(ssh_host, ssh_user, ssh_pass, script, capture=True, key_material=ssh_key, timeout=15)
            dev = out.strip()
...
def nvme_connect_to_subsystem(ssh_host, ssh_user, ssh_pass, ssh_key, lif_ips, subsystem_nqn, timeout_s=15):
    ...
    for lif_ip in lif_ips:
        try:
            cmd = (
                f"nvme connect -t tcp -a {shlex.quote(lif_ip)} -s 4420 "
                f"-n {shlex.quote(subsystem_nqn)} 2>/dev/null; true"
            )
            ssh_run(ssh_host, ssh_user, ssh_pass, cmd, key_material=ssh_key, timeout=timeout_s)
        except Exception as exc:
...
def ensure_nvme_discovery_entries(ssh_host, ssh_user, ssh_pass, ssh_key, lif_ips):
    ...
    try:
        existing_raw = ssh_run(
            ssh_host, ssh_user, ssh_pass, f"cat {CONF} 2>/dev/null || true", capture=True, key_material=ssh_key
        )
    except Exception:
        existing_raw = ""

    lines = [lun_path.strip() for lun_path in existing_raw.splitlines() if lun_path.strip()]
    ...
    for lif_ip in lif_ips:
        ...
        if prefix not in iface_map:
            log.warning(
                f"[netapp_storage] ensure_nvme_discovery: no matching host interface "
                f"for LIF {lif_ip} on {ssh_host} (known prefixes: {list(iface_map)})"
            )
            continue
        iface, host_traddr = iface_map[prefix]
        entry = f"--transport=tcp --traddr={lif_ip} --host-iface={iface} --host-traddr={host_traddr}"
        new_entries.append(entry)

    if not new_entries:
        return 0

    append_cmd = " && ".join(f"echo {shlex.quote(e)} >> {CONF}" for e in new_entries)
    try:
        ssh_run(ssh_host, ssh_user, ssh_pass, append_cmd, key_material=ssh_key)
        log.info(
            f"[netapp_storage] Added {len(new_entries)} entry/entries to "
...
def nvme_disconnect_by_subsystem_name(ssh_host, ssh_user, ssh_pass, ssh_key, subsystem_name):
    ...
    try:
        ...
        out = ssh_run(
            ssh_host, ssh_user, ssh_pass, "nvme list-subsys 2>/dev/null", capture=True, key_material=ssh_key, timeout=15
        )
        for line in out.splitlines():
...
def vg_rescan_and_activate(ssh_host, ssh_user, ssh_pass, ssh_key, vg_name):
    ...
    """
    vg_q = shlex.quote(vg_name)
    ssh_run(ssh_host, ssh_user, ssh_pass, f"pvscan --cache 2>/dev/null; vgchange -ay {vg_q}", key_material=ssh_key)
