...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/native/netapp_storage/core/_helpers.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: Shared helpers for snapshot_engine, restore_engine, and...
...
def build_pve_client(db, pve_host_id):
    ...
    try:
        pve_host_id = int(pve_host_id)
    except (TypeError, ValueError):
        raise RuntimeError("pve_host_id must be a positive integer") from None
    if pve_host_id <= 0:
        raise RuntimeError("pve_host_id must be a positive integer")
...
class PluginPveSession:
    ...
    Implements the same interface as ProxmoxVEx ClusterManager so that
...
def ssh_run(host, user, password, cmd, capture=False, stdin_data=None, timeout=60, key_material=""):
    ...
    try:
        if key_material:
            ...
            fd, key_tmp = tempfile.mkstemp(prefix="ProxmoxVEx-ssh-key-")
            try:
                os.write(fd, key_material.encode() if isinstance(key_material, str) else key_material)
            finally:
                os.close(fd)
            os.chmod(key_tmp, 0o600)
            ssh_cmd = base_ssh + ["-i", key_tmp, "-o", "PasswordAuthentication=no", f"{user}@{host}", cmd]
            final_cmd = ssh_cmd
        elif password:
            # Prefer system key (PVE often disables password login for root)
            _system_key = _find_system_ssh_key()
            if _system_key:
                ssh_cmd = base_ssh + ["-i", _system_key, "-o", "PasswordAuthentication=no", f"{user}@{host}", cmd]
                final_cmd = ssh_cmd
            else:
        ...
        try:
            ...
            raise RuntimeError(f"SSH timeout after {timeout}s: {cmd[:80]}") from None
        except FileNotFoundError:
            raise RuntimeError("ssh binary not found") from None
