...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/native/netapp_storage/core/recovery_engine.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: NetApp ONTAP Plugin — DR Recovery Engine (v1.1)
...
def scan_volumes_for_recovery(client, svm_name):
    ...
    try:
        ...
        raise RuntimeError(f"Could not list volumes for SVM '{svm_name}': {exc}") from exc
...
def list_nfs_manifests(ssh_host, ssh_user, ssh_pass, ssh_key, mount_point):
    ...
    try:
        ...
        return [{"snap_name": n, "manifest_path": f"{base_path}/{n}/manifest.json"} for n in names]
    except Exception as exc:
...
def read_nfs_manifest(ssh_host, ssh_user, ssh_pass, ssh_key, mount_point, snap_name):
    ...
    try:
        ...
        raise RuntimeError(f"Manifest JSON malformed: {exc}") from exc
...
def _rename_vm_disks_nfs(old_vmid, new_vmid, storage_id, sh, su, sp, sk, jlog):
    ...
    for subdir in ("images", ""):
        candidate = f"{mount_base}/{subdir}/{old_vmid_s}" if subdir else f"{mount_base}/{old_vmid_s}"
        try:
            check = ssh_run(
                sh,
                su,
            ...
            if "EXISTS" in check:
                old_dir = candidate
                new_dir = f"{mount_base}/{subdir}/{new_vmid_s}" if subdir else f"{mount_base}/{new_vmid_s}"
                break
        except Exception:
            pass
    ...
    if not old_dir:
        jlog.log(
            f"  [rename] NFS disk dir for VM {old_vmid_s} not found on {sh} — "
    ...
    try:
        ...
        ssh_run(sh, su, sp, f"mv {shlex.quote(old_dir)} {shlex.quote(new_dir)}", key_material=sk, timeout=15)
    ...
    def _patch(content):
        ...
        content = content.replace(f":{old_vmid_s}/vm-{old_vmid_s}-", f":{new_vmid_s}/vm-{new_vmid_s}-")
        content = re.sub(rf"(?<=[:/])vm-{re.escape(old_vmid_s)}-disk-", f"vm-{new_vmid_s}-disk-", content)
        content = re.sub(rf"(?<=[:/])vm-{re.escape(old_vmid_s)}-cloudinit\b", f"vm-{new_vmid_s}-cloudinit", content)
        return content
...
def _rename_vm_disks_san(old_vmid, new_vmid, vg_name, sh, su, sp, sk, jlog):
    ...
    try:
        ...
        lvs = [lun_path.strip() for lun_path in out.splitlines() if lun_path.strip()]
    except Exception as exc:
    ...
    renamed = []
    for old_lv in lvs:
        new_lv = re.sub(rf"^vm-{re.escape(old_vmid_s)}-", f"vm-{new_vmid_s}-", old_lv)
        if old_lv == new_lv:
            continue
        try:
            ssh_run(
                ...
                su,
                sp,
                f"lvrename {shlex.quote(vg_name)} {shlex.quote(old_lv)} {shlex.quote(new_lv)}",
                key_material=sk,
                timeout=30,
            )
    ...
    def _patch(content):
        ...
        content = re.sub(rf"\bvm-{re.escape(old_vmid_s)}-disk-", f"vm-{new_vmid_s}-disk-", content)
        content = re.sub(rf"\bvm-{re.escape(old_vmid_s)}-cloudinit\b", f"vm-{new_vmid_s}-cloudinit", content)
        return content

    return _patch


def restore_vm_configs(
    manifest,
    pve_host_ids,
    vmid_offset,
    vmids_to_restore,
        ...
        for hid in pve_host_ids:
            try:
                ...
                if vmid_new != vmid_orig and sid_for_rename:
                    if protocol == "nfs":
                        patch_fn = _rename_vm_disks_nfs(vmid_orig, vmid_new, sid_for_rename, sh, su, sp, sk, jlog)
                    elif protocol in ("iscsi", "nvme") and vg_name:
                        patch_fn = _rename_vm_disks_san(vmid_orig, vmid_new, vg_name, sh, su, sp, sk, jlog)
                    ...
                    if patch_fn:
                        ...
                        jlog.log(
                            f"  [rename] VMID {vmid_orig} → {vmid_new}: "
                ...
                ssh_run(
                    ...
                    su,
                    sp,
                    f"mkdir -p {shlex.quote(conf_dir)} && cat > {shlex.quote(conf_path)}",
...
def run_bind(job_id, ds_id, params, username, db):
    ...
    from ..api.provisioning import _fail_job, _finish_job, _set_ds_status

    jlog = JobLogger(job_id, db)
    try:
        protocol = params.get("protocol", "")
        jlog.log(
            f"Recovery bind: {protocol} volume '{params.get('volume_name', '')}' "
...
def _bind_nfs(ds_id, params, db, jlog):
    ...
    if not junction_path:
        ...
        if current_path:
            jlog.log(f"Volume currently mounted at '{current_path}' — remounting at '{override_path}' …")
    ...
    db.execute(
        "UPDATE netapp_provisioned_datastores SET volume_name=?, nfs_junction_path=?, updated_at=? WHERE id=?",
...
def _bind_iscsi(ds_id, params, db, jlog):
    ...
    igroup_uuid, igroup_name = _adopt_or_create_igroup(client, svm_name, lun_uuid, volume_name, host_meta, params, jlog)
    ...
    for hid in ordered_hosts:
        ...
        if not vg_name:
            ...
            ssh_run(
                ...
                su,
                sp,
                f"pvscan --cache -aay {shlex.quote(mapper_dev)} 2>/dev/null; vgscan 2>/dev/null; true",
                key_material=sk,
                timeout=20,
            )
        ...
        if "EXISTS" in out:
            ...
            ssh_run(
                ...
                su,
                sp,
                f"pvscan --cache -aay {shlex.quote(mapper_dev)} 2>/dev/null; vgchange -ay {vg_q} 2>/dev/null; true",
                key_material=sk,
                timeout=30,
            )
...
def _bind_nvme(ds_id, params, db, jlog):
    ...
    for hid in ordered_hosts:
        ...
        if subsystem_nqn:
            try:
                device = find_nvme_device_for_subsystem_nqn(sh, su, sp, sk, subsystem_nqn, timeout_s=5)
        ...
        if not vg_name:
            try:
                ssh_run(sh, su, sp, "pvscan --cache 2>/dev/null; true", key_material=sk, timeout=15)
                detected = ssh_run(
                    sh,
                    su,
                    sp,
                    f"pvs --noheadings -o vg_name {shlex.quote(device)} 2>/dev/null | awk '{{print $1}}' | head -1",
                    capture=True,
                    key_material=sk,
                    timeout=10,
                ).strip()
                if detected:
                    ...
                    raise RuntimeError(
                        f"No LVM VG found on device {device}. Provide vg_name manually for a new/empty namespace."
                    )
            except RuntimeError:
                raise
            except Exception as exc:
                raise RuntimeError(f"VG name auto-detection failed: {exc}") from exc
        ...
        if "EXISTS" in out:
            ...
            ssh_run(
                ...
                su,
                sp,
                f"pvscan --cache -aay {dev_q} 2>/dev/null; vgchange -ay {vg_q} 2>/dev/null; true",
                key_material=sk,
                timeout=30,
            )
            jlog.log(f"[{sh}] VG active.")
...
def _build_host_meta(db, pve_host_ids, jlog, collect_iqn=False, collect_nqn=False):
...
def _adopt_or_create_igroup(client, svm_name, lun_uuid, volume_name, host_meta, params, jlog):
    ...
    try:
        existing_ig = next((g for g in client.list_igroups(svm_name=svm_name) if g.get("uuid") == igroup_uuid), None)
        existing_inits = {i.get("name", "") for i in ((existing_ig or {}).get("initiators") or [])}
...
def _ensure_nfs_export_rules(client, export_policy_id, pve_host_ids, db, jlog):
    ...
    try:
        ...
        existing_clients = {c.get("match", "") for r in existing_rules for c in (r.get("clients") or [])}
