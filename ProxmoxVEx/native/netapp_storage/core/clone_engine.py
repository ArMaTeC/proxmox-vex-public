...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/native/netapp_storage/core/clone_engine.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: NetApp ONTAP Clone Engine
...
from .restore_engine import (
    _fail_job as _re_fail_job,
)
from .restore_engine import (
    _find_vm_in_manifest,
    _load_manifest,
    _vm_start,
)
from .restore_engine import (
    _finish_job as _re_finish_job,
)
from .restore_engine import (
    _job_failed as _re_job_failed,
)
from .restore_engine import (
    _resolve_node_host as _re_resolve_node_host,
)
from .restore_engine import (
    _set_progress as _re_set_progress,
)
...
def _run_clone(job_id, params, username):
    ...
    try:
        ...
        conf_path_reserved = _reserve_vmid(pve_host, pve_user, pve_pass, pve_key, new_vmid, vm_type, jlog)

        # ── Create target directory ────────────────────────────────────
        target_dir = f"{mapping['nfs_mount_path']}/images/{new_vmid}"
        ssh_run(pve_host, pve_user, pve_pass, f"mkdir -p {shlex.quote(target_dir)}", key_material=pve_key)
        ...
        for i, disk in enumerate(disks, 1):
            ...
            jlog.log(f"  [{i}/{len(disks)}] {os.path.basename(src_path)} → {os.path.basename(dst_path)}")
        ...
        conf_lines = [lun_path for lun_path in conf_str.splitlines() if not lun_path.startswith(f"{name_key}:")]
        ...
        if conf_path_reserved and pve_host:
            with contextlib.suppress(Exception):
                ssh_run(pve_host, pve_user, pve_pass, f"rm -f {shlex.quote(conf_path_reserved)}", key_material=pve_key)
...
def _run_clone_san(job_id, params, username):
    ...
    try:
        ...
        # ── 3b. NVMe-oF: clone namespace ──────────────────────────────
            ...
            if not main_ns:
                raise RuntimeError(f"Cannot find NVMe namespace for volume {vol_uuid}. Re-run discovery or check SVM.")
        ...
        if manifest_deferred:
            ...
            try:
                ...
                disks, vm_type, raw_cfg = _disks_from_pve(mgr, src_vmid, mapping["pve_storage_id"], eff_node)
                vm_entry = {
                    "vmid": src_vmid,
                    "disks": disks,
                    "vm_type": vm_type,
        ...
        conf_path_reserved = _reserve_vmid(
            pve_host, pve_user, pve_pass, pve_key, new_vmid, vm_entry.get("vm_type", vm_type), jlog
        )
        ...
        for i, disk in enumerate(disks, 1):
            ...
            size_bytes = get_lv_size_bytes(pve_host, pve_user, pve_pass, pve_key, temp_vg_name, src_lv)
            if size_bytes == 0:
                raise RuntimeError(f"Cannot determine size of {temp_vg_name}/{src_lv}")

            # Create target LV in main VG
            create_lv(pve_host, pve_user, pve_pass, pve_key, vg_name, new_lv, size_bytes, lvm_type, pool_name)

            # Block copy
            lv_copy(pve_host, pve_user, pve_pass, pve_key, temp_vg_name, src_lv, vg_name, new_lv, jlog)
...
def _disks_from_pve(mgr, vmid, storage_id, node):
    ...
    try:
        ...
        raise RuntimeError(f"Cannot read VM {vmid} config from PVE: {exc}") from exc
    if not cfg:
...
def _build_clone_config(raw_conf, old_vmid, new_vmid, storage_id, disk_map, new_name, vm_type, network_isolated=False):
    ...
    if new_name and vm_type == "qemu" and not any(lun_path.startswith("name:") for lun_path in lines):
        lines.append(f"name: {new_name}")
    if new_name and vm_type == "lxc" and not any(lun_path.startswith("hostname:") for lun_path in lines):
    ...
    try:
        ...
        if not disks:
            ...
            try:
                ...
                found = [lun_path.strip() for lun_path in out.splitlines() if lun_path.strip()]
        ...
        if not disks:
            ...
            try:
                ...
                if avail:
                    ...
                    jlog.log(
                        f"Snapshot '{snap_name}' contains no VM images at all — "
        ...
        if vm_type == "qemu":
            ssh_run(
                pve_host, pve_user, pve_pass, f"qm set {new_vmid} --name {shlex.quote(safe_name)}", key_material=pve_key
            )
        else:
            ssh_run(
                pve_host,
