...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/native/netapp_storage/core/snapmirror.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: SnapMirror® business logic
...
def scan_relationships(db):
    ...
    """
    from ._helpers import build_ontap_client, get_endpoint
...
def match_endpoints(db):
    ...
    """
    from ._helpers import build_ontap_client
...
def _upsert_relationship(db, rel, mapping):
    ...
    existing = db.query_one("SELECT id FROM netapp_snapmirror_relationships WHERE relationship_uuid=?", (rel_uuid,))
...
def trigger_update_for_volume(db, volume_uuid, source_endpoint_id, jlog=None):
    ...
    rels = db.query(
        "SELECT * FROM netapp_snapmirror_relationships WHERE source_volume_uuid=? AND source_endpoint_id=?",
    ...
    for rel in rels:
        ...
        try:
            ...
            msg = (
                f"SnapMirror® update started: {rel['source_volume']} → {rel['dest_cluster_name']}:{rel['dest_volume']}"
            )
            if job_uuid:
...
def ensure_secondary_nfs_export(db, relationship_id):
    ...
    if not junction:
        ...
        log.info(f"[netapp_storage] DP volume '{rel['dest_volume']}' has no junction path — mounting at {junction}")
    ...
    if policy_id:
        ...
        has_open_rule = any(
            any(c.get("match") in ("0.0.0.0/0", "all", "::0/0") for c in (r.get("clients") or [])) for r in rules
        )
...
def check_secondary_connectivity(db, relationship_id):
    ...
    try:
        ...
        raise RuntimeError(f"Cannot reach secondary volume: {exc}") from exc

    if protocol == "nfs":
        nfs_ip, junction, created = ensure_secondary_nfs_export(db, relationship_id)
        msg = f"NFS OK — {nfs_ip}:{junction}" + (" (export rule created)" if created else "")
        return {
            "success": True,
            "protocol": "nfs",
            "details": msg,
        ...
        if not target_iqn:
            raise RuntimeError(f"iSCSI service has no target IQN on secondary SVM '{dest_svm}'.")
        msg = f"iSCSI OK — portal: {portal}, target: {target_iqn}, volume: {vol_name}"
        return {
            "success": True,
            "protocol": "iscsi",
            "details": msg,
        ...
        try:
            lifs = secondary_client._get_all_records(
                ...
                params={
                    ...
                    "svm.name": dest_svm,
                    "fields": "ip.address,state",
                    "max_records": 50,
                },
            )
        except Exception:
            lifs = []
        active_lifs = [(r.get("ip") or {}).get("address", "") for r in lifs if r.get("state", "up") == "up"]
        ...
        msg = f"NVMe/TCP OK — portal(s): {', '.join(active_lifs)}, volume: {vol_name}"
        return {
            "success": True,
            "protocol": "nvme",
            "details": msg,
