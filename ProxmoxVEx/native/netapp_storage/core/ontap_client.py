...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/native/netapp_storage/core/ontap_client.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: ONTAP REST API Client
...
class OntapClient:
    def __init__(self, host: str, username: str, password: str, ssl_verify: bool = True, timeout: int = 30) -> None:
    ...
    def _get(self, path, params=None):
        ...
        try:
            ...
            raise OntapError(f"GET {path} network error: {e}") from e
        if not r.ok:
    ...
    def _post(self, path, body=None, params=None, timeout=None):
        url = f"{self.base_url}/{path.lstrip('/')}"
        try:
            r = self._session.post(url, json=body or {}, params=params, timeout=timeout or self.timeout)
        except requests.RequestException as e:
            raise OntapError(f"POST {path} network error: {e}") from e
        if not r.ok:
    ...
    def _patch(self, path, body=None, params=None):
        ...
        try:
            ...
            raise OntapError(f"PATCH {path} network error: {e}") from e
        if not r.ok:
    ...
    def _delete(self, path, params=None):
        ...
        try:
            ...
            raise OntapError(f"DELETE {path} network error: {e}") from e
        if not r.ok:
    ...
    def get_volume(self, volume_uuid):
        return self._get(f"storage/volumes/{volume_uuid}", params={"fields": "uuid,name,svm,nas.path,state,space"})
    ...
    def create_snapshot(self, volume_uuid, snap_name, comment="", snapmirror_label=""):
        ...
        try:
            resp = self._post(f"storage/volumes/{volume_uuid}/snapshots", body=body, params={"return_timeout": 0})
            return (resp.get("job") or {}).get("uuid", "")
        except OntapError as exc:
            # ASA: "not supported on this platform" (code 1638644)
            if exc.status_code == 400 and ("1638644" in str(exc) or "not supported on this platform" in str(exc)):
                log.info("[netapp_storage] ASA platform: volume snapshot not supported, trying CG/CLI fallback")
    ...
    def _create_snapshot_asa(self, volume_uuid, snap_name, comment="", snapmirror_label=""):
        ...
        try:
            ...
            raise OntapError(f"ASA snapshot failed: CG method and CLI-Bridge both failed (CLI: {cli_exc})") from cli_exc
    ...
    def delete_snapshot(self, volume_uuid, snap_uuid, force=False, snap_name=""):
        ...
        if force:
            params["force"] = "true"
        try:
            resp = self._delete(f"storage/volumes/{volume_uuid}/snapshots/{snap_uuid}", params=params)
            return resp.get("job", {}).get("uuid", "")
        except OntapError as exc:
            if exc.status_code == 400 and "1638644" in str(exc):
                return self._delete_snapshot_asa(volume_uuid, snap_name or snap_uuid, force=force)
            raise
    ...
    def restore_volume_snapshot_san(self, volume_uuid, snap_name):
        ...
        resp = self._post("private/cli/volume/snapshot/restore", body=body, params={"return_timeout": 60}, timeout=90)
        return (resp.get("job") or {}).get("uuid", "")
    ...
    def get_volume_by_name(self, svm_name, volume_name):
        data = self._get(
            "storage/volumes", params={"svm.name": svm_name, "name": volume_name, "fields": "uuid,name,nas.path,state"}
        )
        records = data.get("records", [])
        if not records:
    ...
    def get_snapmirror_policy_labels(self, relationship_uuid):
        ...
        policies = self._get_all_records(
            "snapmirror/policies",
            params={
                "name": policy_name,
                "fields": "retention",
    ...
    def list_nfs_export_rules(self, export_policy_id):
        ...
        data = self._get(
            f"protocols/nfs/export-policies/{export_policy_id}/rules",
    ...
    def list_nfs_lifs(self, svm_name):
        ...
        records = self._get_all_records(
            "network/ip/interfaces",
            params={"services": "data_nfs", "svm.name": svm_name, "fields": "name,ip.address,state", "max_records": 50},
        )
        result = []
        for rec in records:
            if rec.get("state", "up") == "up":
    ...
    def unmount_volume(self, volume_uuid):
        ...
        try:
            r = self._session.patch(
                url, json={"nas": {"path": ""}}, params={"return_timeout": 30}, timeout=self.timeout
            )
        except requests.RequestException as e:
            raise OntapError(f"PATCH volume/{volume_uuid} network error: {e}") from e
        if not r.ok:
    ...
    def clone_lun_from_snapshot(
        self, source_vol_uuid, snap_name, svm_name, clone_vol_name, poll_interval=3, poll_timeout=300
    ):
    ...
    def clone_namespace_from_snapshot(
        self, source_vol_uuid, snap_name, svm_name, clone_vol_name, poll_interval=3, poll_timeout=300
    ):
        ...
        for ns in self.list_nvme_namespaces(svm_name=svm_name):
            ...
            if loc_vol.get("uuid") == clone_vol_uuid or loc_vol.get("name") == clone_vol_name:
                ...
                break
        if not ns_uuid:
            raise OntapError(f"No NVMe namespace found inside FlexClone volume {clone_vol_name!r}")
    ...
    def create_igroup(self, svm_name, igroup_name, protocol="iscsi", os_type="linux"):
        ...
        resp = self._post("protocols/san/igroups", body=body, params={"return_timeout": 15, "return_records": "true"})
    ...
    def remove_igroup_initiator(self, igroup_uuid, initiator_name):
        ...
        try:
            self._delete(f"protocols/san/igroups/{igroup_uuid}/initiators/{initiator_name}")
        except OntapError as exc:
    ...
    def get_iscsi_lif_for_svm(self, svm_name):
        ...
        records = self._get_all_records(
            "network/ip/interfaces",
            params={"services": "data_iscsi", "svm.name": svm_name, "fields": "ip.address,state", "max_records": 50},
        )
        for rec in records:
            if rec.get("state", "up") == "up":
                addr = (rec.get("ip") or {}).get("address", "")
    ...
    def list_aggregates(self):
        ...
        records = self._get_all_records(
            "storage/aggregates",
            params={"fields": "name,uuid,space.block_storage.available,node.name", "max_records": 200},
        )
        result = []
        for r in records:
            result.append({
    ...
    def disable_volume_snapshots_and_arp(self, vol_uuid):
        ...
        with contextlib.suppress(Exception):
            self._patch(f"storage/volumes/{vol_uuid}", body={"anti_ransomware": {"state": "disabled"}})
    ...
    def enable_inline_compression(self, vol_uuid):
        ...
        """
        self._patch(f"storage/volumes/{vol_uuid}", body={"efficiency": {"compression": "inline"}})

    def create_lun(self, svm_name, volume_name, lun_name, size_bytes, os_type="linux", auto_provision_as_flexvol=False):
    ...
    def list_nvme_namespaces(self, svm_name=None):
        ...
        """
        params = {
            "fields": "uuid,name,svm.name,location.volume.uuid,location.volume.name,space.size,status.state",
            "max_records": 500,
        }
        if svm_name:
            params["svm.name"] = svm_name
        ...
        with contextlib.suppress(OntapError):
    ...
    def _list_nvme_namespaces_storage(self, svm_name=None):
        ...
        """
        params = {
            "fields": "uuid,name,svm.name,space.size,status.state,location.volume.uuid,location.volume.name",
            "max_records": 500,
        }
        if svm_name:
            params["svm.name"] = svm_name
    ...
    def _list_nvme_namespaces_cli(self, svm_name=None):
        ...
        try:
            ...
            raise OntapError(f"NVMe CLI fallback failed: {exc}") from exc
    ...
    def _list_nvme_namespaces_via_subsystem_maps(self, svm_name=None):
        ...
        try:
            ...
            raise OntapError(f"NVMe subsystem-maps fallback failed: {exc}") from exc
        ...
        for m in maps:
            ...
            result.append({
                ...
                "name": ns_path,
                "svm": {"name": svm_found},
                "location": {"volume": {"name": vol_name, "uuid": vol_uuid}},
            })
        ...
        for item in result:
            ...
            if ns_bare and "/" not in ns_bare:
                ...
                if vol_uuid_by_name:
                    ...
                    log.info(f"[netapp_storage] Namespace {item['uuid']}: Volume via Name-Match → {svm_n}/{ns_bare}")
                    continue

            # Phase 3: if the SVM has exactly one non-root volume, use it.
            svm_n = item["svm"]["name"]
            non_root = sorted([
                (vn, vu) for (vs, vn), vu in vol_uuid_cache.items() if vs == svm_n and not vn.endswith("_root")
            ])
            if len(non_root) == 1:
                vol_name_fb, vol_uuid_fb = non_root[0]
                item["location"] = {"volume": {"name": vol_name_fb, "uuid": vol_uuid_fb}}
                log.info(f"[netapp_storage] Namespace {item['uuid']}: Volume via SVM-Fallback → {svm_n}/{vol_name_fb}")
            else:
                log.warning(
    ...
    def clone_namespace(self, source_ns_uuid, snap_name, dest_volume_name, dest_ns_name, svm_name):
        ...
        return self._clone_namespace_via_cli_volume_clone(
    ...
    def _clone_namespace_via_cli_volume_clone(
        self, snap_name, src_volume_name, dest_ns_name, svm_name, source_ns_uuid=""
    ):
        ...
        src_ns_path, src_subsystem_uuid, src_subsystem_name = self._get_ns_info_from_subsystem_maps(
            source_ns_uuid, svm_name
        )
        self._post(
        ...
        if not ns_uuid:
            try:
                data = self._get(
                    ...
                    params={"vserver": svm_name, "volume": clone_vol_name, "fields": "uuid,path,volume"},
                )
        ...
        if not ns_uuid:
            ...
            ns_summary = [
                (ns.get("uuid", ""), (ns.get("location") or {}).get("volume", {}).get("name", "")) for ns in all_ns
            ]
    ...
    def _delete_clone_volume(self, clone_vol_uuid, clone_vol_name="", svm_name=""):
        ...
        if clone_vol_name and svm_name:
            try:
                ...
                r = self._session.patch(cli_url, json={"state": "offline"}, params=cli_params, timeout=self.timeout)
                if not r.ok:
                    log.warning(
                        f"[netapp_storage] CLI volume offline {clone_vol_name}: {r.status_code}: {r.text[:200]}"
                    )
        ...
        log.warning(
            f"[netapp_storage] All delete attempts for clone volume "
    ...
    def delete_namespace(self, ns_uuid):
        ...
        if clone_info:
            ...
            return ""
        try:
            resp = self._delete(f"protocols/nvme/namespaces/{ns_uuid}", params={"return_timeout": 0})
            return (resp.get("job") or {}).get("uuid", "")
        except OntapError as exc:
            if exc.status_code != 404:
                raise
        # ASA R2: protocols/nvme/namespaces DELETE → 404; use storage/namespaces
        resp = self._delete(f"storage/namespaces/{ns_uuid}", params={"return_timeout": 0})
        return (resp.get("job") or {}).get("uuid", "")
    ...
    def remove_nvme_namespace_from_subsystem(self, subsystem_uuid, ns_uuid):
        ...
        """
        try:
            self._delete(f"protocols/nvme/subsystems/{subsystem_uuid}/namespaces/{ns_uuid}")
            return
        except OntapError as exc:
            if exc.status_code != 404:
    ...
    def create_namespace(self, svm_name, volume_name, ns_name, size_bytes, os_type="linux", aggregate_name=None):
        ...
        try:
            resp = self._post("protocols/nvme/namespaces", body=body, params={"return_timeout": 30})
            ns_uuid = resp.get("uuid", "")
            job_uuid = (resp.get("job") or {}).get("uuid", "")
            if job_uuid:
            ...
            if not ns_uuid:
                for ns in self.list_nvme_namespaces(svm_name=svm_name):
                    ...
                    if (loc.get("volume") or {}).get("name") == volume_name and loc.get("namespace") == ns_name:
                        ns_uuid = ns["uuid"]
                        break
            if not ns_uuid:
            ...
            if exc.status_code == 404:
                log.info("[netapp_storage] protocols/nvme/namespaces POST → 404, trying ASA fallback")
                return self._create_namespace_asa(svm_name, volume_name, ns_name, size_bytes, os_type, aggregate_name)
            raise

    def _create_namespace_asa(self, svm_name, volume_name, ns_name, size_bytes, os_type="linux", aggregate_name=None):
        ...
        try:
            ...
            resp = self._post("storage/namespaces", body=body, params={"return_timeout": 30, "return_records": "true"})
            # UUID may be top-level (ASA R2) or nested under "record" (AFF 9.16.1+)
            ns_uuid = resp.get("uuid", "") or (resp.get("record") or {}).get("uuid", "")
            job_uuid = (resp.get("job") or {}).get("uuid", "")
            if job_uuid:
                self.poll_job(job_uuid, interval_s=2, timeout_s=120)
            if ns_uuid:
            ...
            if exc.status_code not in (400, 404, 405):
                raise
            log.info(f"[netapp_storage] storage/namespaces → {exc.status_code}; trying private/cli/nvme/namespace")
        ...
        self._post("private/cli/nvme/namespace", body=cli_body, params={"return_timeout": 30}, timeout=60)

        # Locate the namespace UUID after CLI creation
        ns_uuid = self._find_namespace_uuid_after_asa_create(svm_name, volume_name, ns_name)
        if not ns_uuid:
            raise OntapError(f"NVMe namespace '{ns_name}' not found after ASA CLI creation")
        return ns_uuid
    ...
    def _find_namespace_uuid_after_asa_create(self, svm_name, volume_name, ns_name):
        ...
        for name_filter in (
            ...
            with contextlib.suppress(OntapError):
                records = self._get_all_records(
                    "storage/namespaces",
                    params={"name": name_filter, "svm.name": svm_name, "fields": "uuid,name", "max_records": 10},
                )
                if records:
        ...
        with contextlib.suppress(OntapError):
            ...
            for r in records:
                loc = r.get("location") or {}
                if loc.get("namespace") == ns_name or (loc.get("volume") or {}).get("name") == volume_name:
                    return r.get("uuid", "")
        # Strategy 2: CLI bridge by volume name (proven to work on ASA R2)
        with contextlib.suppress(OntapError):
            data = self._get(
                "private/cli/nvme/namespace",
                params={"vserver": svm_name, "volume": volume_name, "fields": "uuid,path,volume"},
            )
    ...
    def get_namespace(self, ns_uuid):
        ...
        try:
            return self._get(
                f"protocols/nvme/namespaces/{ns_uuid}", params={"fields": "uuid,name,location,space,os_type,svm"}
            )
        except OntapError as exc:
            if exc.status_code != 404:
                raise
        # ASA R2: try the new endpoint
        return self._get(f"storage/namespaces/{ns_uuid}", params={"fields": "uuid,name,location,space,os_type,svm"})
    ...
    def create_nvme_subsystem(self, svm_name, subsystem_name, os_type="linux"):
        ...
        resp = self._post("protocols/nvme/subsystems", body=body, params={"return_timeout": 30})
    ...
    def get_nvme_subsystem(self, subsystem_uuid):
        ...
        try:
            return self._get(
                f"protocols/nvme/subsystems/{subsystem_uuid}",
                params={"fields": "uuid,name,svm.name,os_type,hosts.nqn,target_nqn"},
            )
        except OntapError:
            return self._get(
                f"protocols/nvme/subsystems/{subsystem_uuid}", params={"fields": "uuid,name,svm.name,os_type,hosts.nqn"}
            )
    ...
    def get_nvme_lifs_for_svm(self, svm_name):
        ...
        records = self._get_all_records(
            "network/ip/interfaces",
            params={"services": "data_nvme_tcp", "svm.name": svm_name, "fields": "ip.address,state", "max_records": 50},
        )
        return [
            (rec.get("ip") or {}).get("address", "")
            for rec in records
            if rec.get("state", "up") == "up" and (rec.get("ip") or {}).get("address", "")
        ]
    ...
    def resize_volume(self, volume_uuid, new_size_bytes):
        ...
        try:
            r = self._session.patch(
                url, json={"size": new_size_bytes}, params={"return_timeout": 30}, timeout=self.timeout
            )
        except Exception as exc:
            raise OntapError(f"PATCH volume/{volume_uuid} network error: {exc}") from exc
        if not r.ok:
    ...
    def _resize_volume_asa(self, volume_uuid, new_size_bytes):
        ...
        self._patch(
            "private/cli/volume", body={"size": f"{size_gb}g"}, params={"vserver": svm_name, "volume": vol_name}
        )
    ...
    def resize_lun(self, lun_uuid, new_size_bytes):
        ...
        try:
            r = self._session.patch(
                url, json={"space": {"size": new_size_bytes}}, params={"return_timeout": 30}, timeout=self.timeout
            )
        except Exception as exc:
            raise OntapError(f"PATCH lun/{lun_uuid} network error: {exc}") from exc
        if not r.ok:
    ...
    def resize_namespace(self, ns_uuid, new_size_bytes):
        ...
        for path, body in candidates:
            ...
            try:
                r = self._session.patch(url, json=body, params={"return_timeout": 30}, timeout=self.timeout)
            except Exception as exc:
                raise OntapError(f"PATCH {path} network error: {exc}") from exc
            if r.ok:
        ...
        try:
            ...
            raise OntapError(f"{last_err or 'namespace resize failed'}; CLI bridge also failed: {cli_err}") from None

    def _resize_namespace_cli(self, ns_uuid, new_size_bytes):
        for src in (f"storage/namespaces/{ns_uuid}", f"protocols/nvme/namespaces/{ns_uuid}"):
            try:
        ...
        self._patch(
            "private/cli/vserver/nvme/namespace",
    ...
    def create_volume_nfs(
        self, svm_name, vol_name, size_bytes, junction_path, aggregate_name=None, export_policy="default"
    ):
    ...
    def create_export_policy(self, svm_name, policy_name):
        ...
        resp = self._post("protocols/nfs/export-policies", body=body, params={"return_timeout": 15})
        policy_id = resp.get("id", 0)
        if not policy_id:
            records = self._get_all_records(
                "protocols/nfs/export-policies",
                params={"name": policy_name, "svm.name": svm_name, "fields": "id", "max_records": 5},
            )
            if records:
    ...
    def ensure_export_policy_rw(self, svm_name, policy_name, client_match="0.0.0.0/0"):
        ...
        policy_id = records[0].get("id", 0) if records else self.create_export_policy(svm_name, policy_name)
        with contextlib.suppress(Exception):  # rule already exists
    ...
    def set_volume_export_policy(self, volume_uuid, policy_name):
        ...
        try:
            ...
            raise OntapError(f"PATCH volume/{volume_uuid} export_policy: {exc}") from exc
        if not r.ok:
            raise OntapError(f"PATCH volume/{volume_uuid} export_policy → {r.status_code}: {r.text[:300]}")
    ...
    def create_snapmirror_relationship(
        ...
        _cb("[INFO] Sending SnapMirror relationship request to ONTAP...")
        resp = self._post("snapmirror/relationships", body=body, params={"return_timeout": 0})
        ...
        if job_uuid:
            ...
            while True:
                ...
                if state == "success":
                    ...
                    break
                if state == "failure":
                    raise OntapError(f"ONTAP job {job_uuid[:8]} failed: {data.get('message', 'no message')}")
        ...
        if not rel_uuid:
            _cb("[INFO] Looking up relationship by destination path...")
            rels = self._get_all_records(
                "snapmirror/relationships",
                params={
                    "destination.path": dest_path,
                    "fields": "uuid",
