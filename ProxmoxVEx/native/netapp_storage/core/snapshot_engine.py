...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/native/netapp_storage/core/snapshot_engine.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: NetApp ONTAP Snapshot Engine
...
def _find_pve_for_vms(db, volume_uuid, vmids, fallback_cluster_id):
    ...
    rows = db.query("SELECT DISTINCT pve_cluster_id FROM netapp_volume_mapping WHERE volume_uuid=?", (volume_uuid,))
    for row in rows:
    ...
    try:
        ...
        for vmid in vmids:
            ...
            if not result.get("success"):
                jlog.log(
                    f"WARNING: {vm_type.upper()} {vmid} not found — skipping "
        ...
        if not vm_entries:
            jlog.log(
                "WARNING: No VMs could be processed — all VMs in this schedule are "
        ...
        manifest = {
            ...
            "vms": [
                {**{k: v for k, v in e.items() if k != "_raw_conf"}, "raw_config": e.get("_raw_conf", {})}
                for e in vm_entries
            ],
        }
        ...
        if is_san:
            ...
            try:
                ...
                ssh_run(pve_host, pve_user, pve_pass, f"mkdir -p {shlex.quote(snap_dir)}", key_material=pve_key)
                ssh_run(
                    pve_host,
                    pve_user,
                    pve_pass,
        ...
        if pre_script:
            ...
            _run_hook(pve_host, pve_user, pve_pass, pve_key, pre_script, vmids, snap_name, node, jlog, "Pre-Script")
        ...
        try:
            ...
            job_uuid = client.create_snapshot(
                mapping["volume_uuid"],
                snap_name,
                comment=f"ProxmoxVEx cluster={cluster_id} vms={','.join(str(v) for v in vmids)}",
            ...
            if post_script:
                ...
                _run_hook(
                    pve_host, pve_user, pve_pass, pve_key, post_script, vmids, snap_name, node, jlog, "Post-Script"
                )
        ...
        if schedule_id:
            try:
                ...
                if deleted:
                    jlog.log(f"Retention: removed {deleted} old snapshot(s) (keeping {retention_count}).")
        ...
        if params.get("notify_enabled") and params.get("notify_recipients"):
            try:
                ...
                vm_list = []
                try:
                    snap_row = db.query_one("SELECT manifest_json FROM netapp_snapshots WHERE id=?", (snapshot_id,))
...
def _apply_schedule_retention(db, schedule_id, retention_count, client, volume_uuid, snap_prefix, sched_name_safe):
    ...
    try:
        ...
        raise RuntimeError(f"Cannot list ONTAP snapshots for retention: {exc}") from exc

    # ── 2. Filter: snapshots that belong to this schedule ────────────────────
    sched_snaps = [
        s for s in all_snaps if s.get("name", "").startswith(snap_prefix) and s.get("name", "").endswith(suffix)
    ]
    ...
    for snap in to_delete:
        ...
        try:
            if snap_uuid:
                ...
                log.warning(f"[netapp_storage] Retention: snapshot '{snap_nm}' has no UUID — skipping ONTAP delete.")
                continue
        except Exception as exc:
            log.warning(f"[netapp_storage] Retention: ONTAP delete '{snap_nm}' failed: {exc} — keeping DB entry.")
        ...
        try:
            ...
            log.warning(f"[netapp_storage] Retention DB cleanup for '{snap_nm}': {db_exc}")
    ...
    try:
        ...
        for row in db_rows:
            nm = row["snap_name"] or ""
            if nm.startswith(snap_prefix) and nm.endswith(suffix) and nm not in ontap_names:
                db.execute("DELETE FROM netapp_snapshots WHERE id=?", (row["id"],))
                log.info(f"[netapp_storage] DB sync: removed orphaned entry for '{nm}' (no longer on ONTAP)")
...
def _detect_vm_type(mgr, node, vmid):
    ...
    try:
        host = mgr.host
        r = mgr._api_get(f"https://{host}:8006/api2/json/nodes/{node}/qemu/{vmid}/status/current")
...
def _extract_disk_files(config_raw, storage_id, vm_type="qemu"):
    ...
    disks = []
    # disk_keys: qemu uses scsi/virtio/ide/sata/efidisk/tpmstate; LXC uses rootfs/mp
    disk_keys = ("scsi", "virtio", "ide", "sata", "efidisk", "tpmstate") if vm_type == "qemu" else ("rootfs", "mp")
...
def _run_hook(pve_host, pve_user, pve_pass, pve_key, script, vmids, snap_name, node, jlog, label):
    """Runs a pre- or post-script via SSH on the PVE node.

    Environment variables for the script:
      ProxmoxVEx_VMIDS  – space-separated VMID list
      ProxmoxVEx_SNAP   – name of the ONTAP snapshot
      ProxmoxVEx_NODE   – PVE node name
    """
    env = (
        f"export ProxmoxVEx_VMIDS={shlex.quote(' '.join(str(v) for v in vmids))}; "
        f"export ProxmoxVEx_SNAP={shlex.quote(snap_name)}; "
        f"export ProxmoxVEx_NODE={shlex.quote(node)}; "
    )
    try:
        ssh_run(pve_host, pve_user, pve_pass, env + script, key_material=pve_key)
        jlog.log(f"{label}: OK.")
    except Exception as e:
