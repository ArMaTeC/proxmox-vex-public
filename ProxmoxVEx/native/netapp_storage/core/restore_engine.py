...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/native/netapp_storage/core/restore_engine.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: NetApp ONTAP Restore Engine
...
from ._job_registry import register as _reg_register
from ._job_registry import unregister as _reg_unregister
...
def _run_restore_sfsr(job_id, params, username):
    ...
    try:
        ...
        for i, disk in enumerate(disks, 1):
            ...
            ssh_run(pve_host, pve_user, pve_pass, f"mkdir -p {shlex.quote(nfs_dir)}", key_material=pve_key)
            client.restore_file(
        ...
        _restore_config(snap, mapping, vmid, vm_type, node, mgr, pve_host, pve_user, pve_pass, pve_key)
...
def _run_restore_flexclone(job_id, params, username):
    ...
    clone_mount_base = cfg.get("flexclone_mount_base", "/mnt/ProxmoxVEx-clone")
    ...
    try:
        ...
        ssh_run(pve_host, pve_user, pve_pass, f"mkdir -p {shlex.quote(clone_mount_point)}", key_material=pve_key)
        ssh_run(
            pve_host,
            pve_user,
            pve_pass,
        ...
        _flexclone_cleanup(job_id, clone_mount_point, clone_vol_uuid, snap, jlog, db)
        _reg_unregister(job_id)
...
def _run_restore_san(job_id, params, username):
    ...
    try:
        ...
        if not vg_name:
            raise RuntimeError("lvm_vg_name not set in mapping — SAN restore not possible. Re-run discovery.")
        ...
        for vid in all_vmids:
            ...
            try:
                ...
                if not vnode:
                    ...
                    continue
                r = mgr._api_get(f"https://{mgr.host}:{port}/api2/json/nodes/{vnode}/{vt}/{vid}/status/current")
                if not r.ok:
                    continue
        ...
        try:
            ds_row = db.query_one(
                "SELECT pve_host_ids FROM netapp_provisioned_datastores WHERE volume_uuid=?", (mapping["volume_uuid"],)
            )
            if ds_row:
        ...
        if not pve_host_ids:
            ...
            primary_node = stopped_vms[0][2] if stopped_vms else snap.get("node") or ""
        ...
        primary_host = _resolve_node_host(mgr, stopped_vms[0][2] if stopped_vms else snap.get("node") or "")
        for hid in pve_host_ids:
            try:
                h = build_pve_client(db, hid)
                hu, hp, hk = get_ssh_creds(h)
        ...
        for vid, vtype, vnode in stopped_vms:
            try:
                ...
                _restore_config(snap, mapping, vid, vtype, vnode, mgr, vhost, pve_user, pve_pass, pve_key)
...
def _cleanup_san_clone(
    client, protocol, temp_lun_uuid, igroup_uuid, temp_ns_uuid, subsystem_uuid, temp_iscsi_clone_vol_uuid="", jlog=None
):
...
def _run_restore_san_single(job_id, params, username):
    ...
    try:
        ...
        if protocol == "iscsi":
            ...
            from .san_helpers import find_device_by_serial, rescan_iscsi
            ...
            device = find_device_by_serial(pve_host, pve_user, pve_pass, pve_key, temp_serial, timeout_s=60)

        elif protocol == "nvme":
            jlog.log(f"Cloning NVMe namespace from snapshot '{snap_name}' …")
            from .san_helpers import find_new_nvme_device, nvme_list_devices, nvme_ns_rescan
            ...
            main_ns = next(
                (ns for ns in namespaces if (ns.get("location") or {}).get("volume", {}).get("uuid") == vol_uuid),
                None,
            )
            # Fallback: match by volume name when UUID is not returned by the API
            if not main_ns:
                main_ns = next(
                    (ns for ns in namespaces if (ns.get("location") or {}).get("volume", {}).get("name") == vol_name),
                    None,
                )
            if not main_ns:
            ...
            temp_ns_uuid, ns_job = client.clone_namespace(main_ns_uuid, snap_name, vol_name, temp_clone_name, svm_name)
            if ns_job:
                client.poll_job(
                    ns_job,
                    interval_s=poll_cfg.get("job_poll_interval_s", 3),
            ...
            clone_already_mapped = bool(client.get_nvme_subsystem_for_namespace(temp_ns_uuid, svm_name=svm_name))
            ...
            device = find_new_nvme_device(pve_host, pve_user, pve_pass, pve_key, devices_before, timeout_s=60)
        else:
        ...
        from .san_helpers import activate_lv_for_restore, cleanup_restore_vg, lv_copy, vg_import_clone

        temp_vg_name = vg_import_clone(pve_host, pve_user, pve_pass, pve_key, device, vg_name)
        ...
        for i, disk in enumerate(disks, 1):
            ...
            from .san_helpers import create_lv, get_lv_size_bytes

            # Activate source LV in temp VG
            activate_lv_for_restore(pve_host, pve_user, pve_pass, pve_key, temp_vg_name, src_lv, lvm_type, pool_name)

            # Create target LV if it doesn't exist (e.g. VM was deleted after snapshot)
            dst_size = get_lv_size_bytes(pve_host, pve_user, pve_pass, pve_key, vg_name, src_lv)
            if not dst_size:
                src_size = get_lv_size_bytes(pve_host, pve_user, pve_pass, pve_key, temp_vg_name, src_lv)
                if not src_size:
                    raise RuntimeError(f"Cannot determine size for {temp_vg_name}/{src_lv}")
                jlog.log(f"  Creating {vg_name}/{src_lv} ({src_size} B) …")
                create_lv(pve_host, pve_user, pve_pass, pve_key, vg_name, src_lv, src_size, lvm_type, pool_name)

            # Overwrite via dd — VM is stopped so no deactivation needed
            lv_copy(pve_host, pve_user, pve_pass, pve_key, temp_vg_name, src_lv, vg_name, src_lv, jlog)
        ...
        _cleanup_san_clone(
            client, protocol, temp_lun_uuid, igroup_uuid, temp_ns_uuid, subsystem_uuid, temp_iscsi_clone_vol_uuid, jlog
        )
...
def _run_restore_dr_iscsi(job_id, params, username, db, jlog, mapping):
    ...
    """
    from ._helpers import build_ontap_client, get_endpoint, get_ssh_creds
    ...
    try:
        rel = db.query_one("SELECT * FROM netapp_snapmirror_relationships WHERE id=?", (relationship_id,))
        if not rel:
        ...
        if not rel.get("dest_endpoint_id") or not rel.get("dest_volume_uuid"):
            raise RuntimeError("Secondary endpoint or volume UUID missing — run SnapMirror scan first.")
        ...
        snap_row = db.query_one(
            "SELECT * FROM netapp_snapshots WHERE mapping_id=? AND snap_name=? ORDER BY created_at DESC LIMIT 1",
        ...
        temp_igroup_uuid = secondary_client.create_igroup(dest_svm, temp_igroup_name, protocol="iscsi")
        ...
        if not sec_portal or not sec_target_iqn:
            raise RuntimeError(
                f"Cannot determine secondary iSCSI portal or target IQN (portal={sec_portal!r}, iqn={sec_target_iqn!r})"
            )

        jlog.log(f"Connecting host to secondary iSCSI {sec_portal} / {sec_target_iqn} …")
        connect_iscsi_target(pve_host, pve_user, pve_pass, pve_key, sec_portal, sec_target_iqn)
        ...
        device = find_device_by_serial(pve_host, pve_user, pve_pass, pve_key, temp_iscsi_serial, timeout_s=90)
        jlog.log(f"Clone device: {device}")
        ...
        for i, disk in enumerate(disks, 1):
            ...
            if not dst_size:
                ...
                activate_lv_for_restore(pve_host, pve_user, pve_pass, pve_key, vg_name, src_lv, lvm_type, pool_name)
        ...
        flush_iscsi_clone_device(pve_host, pve_user, pve_pass, pve_key, temp_iscsi_serial)
        temp_iscsi_serial = ""
        disconnect_iscsi_target(pve_host, pve_user, pve_pass, pve_key, sec_portal, sec_target_iqn)
...
def _dr_iscsi_cleanup(
    ...
    from .san_helpers import cleanup_restore_vg, disconnect_iscsi_target, flush_iscsi_clone_device
...
def _run_restore_dr_nvme(job_id, params, username, db, jlog, mapping):
    ...
    try:
        ...
        temp_subsystem_uuid = secondary_client.create_nvme_subsystem(dest_svm, temp_subsystem_name)
        secondary_client.add_nvme_host_to_subsystem(temp_subsystem_uuid, host_nqn)
        secondary_client.add_nvme_namespace_to_subsystem(temp_subsystem_uuid, temp_ns_uuid, svm_name=dest_svm)
        ...
        if not sec_lif_ips:
            raise RuntimeError(f"No NVMe/TCP data LIF found on secondary SVM '{dest_svm}'")
        ...
        if subsystem_nqn:
            ...
            nvme_connect_to_subsystem(pve_host, pve_user, pve_pass, pve_key, sec_lif_ips, subsystem_nqn)
        else:
            jlog.log(
                "WARNING: subsystem NQN unavailable from API — skipping direct connect; waiting for auto-discovery"
            )
        ...
        nvme_disconnect_by_subsystem_name(pve_host, pve_user, pve_pass, pve_key, temp_subsystem_name)
...
def _run_restore_dr(job_id, params, username):
    ...
    vmid = int(params["vmid"])
    cfg = load_plugin_config()
    dr_mount_base = cfg.get("flexclone_mount_base", "/mnt/ProxmoxVEx-clone")
    ...
    try:
        ...
        if mounted:
            jlog.log("NFS export rule created on secondary.")
        jlog.log(f"Secondary NFS: {nfs_ip}:{junction_path}")
        if not nfs_ip or not junction_path:
            raise RuntimeError("Secondary volume has no NFS IP or junction path. Check prerequisites in README.")
        ...
        ssh_run(pve_host, pve_user, pve_pass, f"mkdir -p {shlex.quote(dr_mount_point)}", key_material=pve_key)
        ssh_run(
            pve_host,
            pve_user,
            pve_pass,
            f"mount -t nfs -o ro,soft,timeo=30 {shlex.quote(nfs_src)} {shlex.quote(dr_mount_point)}",
            key_material=pve_key,
        )
        _set_progress(db, job_id, 20)
        ...
        try:
            out = ssh_run(
                pve_host, pve_user, pve_pass, f"cat {shlex.quote(manifest_path)}", capture=True, key_material=pve_key
            )
        ...
        if total == 0:
            ...
            try:
                find_cmd = (
                    f"find {shlex.quote(snap_base + '/images')} -name '*.qcow2' -o -name '*.raw' 2>/dev/null | head -20"
                )
                out = ssh_run(pve_host, pve_user, pve_pass, find_cmd, capture=True, key_material=pve_key)
                found_files = [lun_path.strip() for lun_path in out.splitlines() if lun_path.strip()]
        ...
        if total == 0:
            try:
                ...
                avail = [lun_path.strip() for lun_path in diag.splitlines() if lun_path.strip()]
                if avail:
                    jlog.log(f"VMIDs present in snapshot '{snap_name}': {', '.join(avail)}")
                else:
                    jlog.log(
                        f"Snapshot '{snap_name}' contains no VM images — "
        ...
        for i, disk in enumerate(disks, 1):
            ...
            dst = f"{target_base}/images/{file_path.lstrip('/')}"
            jlog.log(f"  [{i}/{total}] {file_path}")
            ssh_run(pve_host, pve_user, pve_pass, f"mkdir -p {shlex.quote(os.path.dirname(dst))}", key_material=pve_key)
            if vm_type == "qemu":
                ssh_run(
                    pve_host,
                    pve_user,
...
def _vm_stop(mgr, node, vmid, vm_type="qemu"):
    ...
    port = getattr(mgr, "port", 8006)
    try:
        r = mgr._api_get(f"https://{mgr.host}:{port}/api2/json/nodes/{node}/{vt}/{vmid}/status/current")
        if not r.ok:
...
def _load_manifest(snap, mapping, node, mgr, pve_host, pve_user, pve_pass, pve_key):
    ...
    if (
        manifest_path.startswith("snapmanifest:")
        or manifest_path.startswith("snapmeta:")
        or manifest_path.startswith("snapinfo:")
        or manifest_path.startswith("db:")
    ...
    # all ProxmoxVEx manifests present at that time.
    ...
    if "/.snapshot/" in snap_manifest_dir:
        try:
            out = ssh_run(
                ...
                pve_user,
                pve_pass,
                f"find {shlex.quote(snap_manifest_dir)} -name manifest.json 2>/dev/null | sort | tail -1",
                capture=True,
                key_material=pve_key,
            )
            found = out.strip()
            if found:
                out2 = ssh_run(
                    pve_host, pve_user, pve_pass, f"cat {shlex.quote(found)}", capture=True, key_material=pve_key
                )
...
def _restore_config(snap, mapping, vmid, vm_type, node, mgr, pve_host, pve_user, pve_pass, pve_key):
    """Writes the saved config back to /etc/pve/{qemu-server|lxc}/<vmid>.conf."""
    manifest_path = snap.get("manifest_path", "")

    if (
        manifest_path.startswith("db:")
        or manifest_path.startswith("snapmanifest:")
        or manifest_path.startswith("snapmeta:")
        or manifest_path.startswith("snapinfo:")
        or not manifest_path
    ):
...
def _rescan_and_start(mgr, node, vmid, vm_type, pve_host, pve_user, pve_pass, pve_key):
    ...
    try:
        if vm_type == "qemu":
            ssh_run(pve_host, pve_user, pve_pass, f"qm rescan {vmid} 2>/dev/null || true", key_material=pve_key)
        else:
            ssh_run(pve_host, pve_user, pve_pass, f"pct rescan {vmid} 2>/dev/null || true", key_material=pve_key)
    except Exception as e:
...
def _write_restore_audit(db, job_id, result):
    try:
        ...
        if job.get("snapshot_id"):
            snap = db.query_one(
                "SELECT snap_name, mapping_id, vmids_json FROM netapp_snapshots WHERE id=?", (job["snapshot_id"],)
            )
            if snap:
                snap_name = snap["snap_name"] or ""
                vmids = json.loads(snap["vmids_json"] or "[]")
                mapping = db.query_one(
                    "SELECT volume_name FROM netapp_volume_mapping WHERE id=?", (snap["mapping_id"],)
                )
                if mapping:
