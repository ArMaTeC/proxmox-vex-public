...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/native/netapp_storage/api/restore.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
...
def _start_restore():
    ...
    if data.get("native"):
        ...
        manifest_path = f"{mapping['nfs_mount_path']}/.snapshot/{snap_name}/{manifest_subdir}/{snap_name}/manifest.json"
...
def _list_restore_jobs():
    ...
    if vmid:
        rows = db.query(
            "SELECT * FROM netapp_jobs WHERE job_type LIKE 'restore%' AND vmid=? ORDER BY created_at DESC LIMIT 50",
            (int(vmid),),
        )
    elif snap_id:
        rows = db.query(
        ...
        rows = db.query("SELECT * FROM netapp_jobs WHERE job_type LIKE 'restore%' ORDER BY created_at DESC LIMIT 100")
...
def register_routes():
    register_native_route(PLUGIN_ID, "restore/start", _start_restore)
    register_native_route(PLUGIN_ID, "restore/dr-start", _start_dr_restore)
    register_native_route(PLUGIN_ID, "restore/status", _restore_status)
    register_native_route(PLUGIN_ID, "restore/jobs", _list_restore_jobs)
