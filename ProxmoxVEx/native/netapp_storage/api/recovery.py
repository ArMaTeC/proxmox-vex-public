...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/native/netapp_storage/api/recovery.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: NetApp ONTAP Plugin — Recovery API (v1.1)
...
def _recovery_manifests():
    ...
    if protocol == "nfs":
        ...
        mapping = db.query_one(
            "SELECT nfs_mount_path FROM netapp_volume_mapping WHERE pve_storage_id=? AND pve_cluster_id=? LIMIT 1",
            (pve_storage_id, first_host["id"]),
        )
        mount_point = dict(mapping or {}).get("nfs_mount_path") or f"/mnt/pve/{pve_storage_id}"
    ...
    # ── SAN (iSCSI / NVMe) ────────────────────────────────────────────────────
        ...
        try:
            ...
            return {
                ...
                "snapshots": [
                    {"snap_name": manifest.get("snap_name", "latest"), "manifest_path": f"snapmanifest:{vg_name}"}
                ],
                "protocol": protocol,
                "detected_storage_ids": _detect_storage_ids(manifest),
            }
...
def _detect_storage_ids(manifest: dict[str, Any]) -> list[str]:
...
def _recovery_bind():
    ...
    required = ["endpoint_id", "svm_name", "volume_uuid", "protocol", "pve_storage_id", "pve_host_ids", "name"]
    for f in required:
        if not body.get(f):
    ...
    db.execute(
        ...
        (
            ...
            body.get("junction_path_override", ""),
            body["pve_storage_id"],
...
def _recovery_restore_vms():
    ...
    vmids_to_restore = {int(v) for v in vmids_raw} if vmids_raw else None
...
def _run_bind_async(job_id, ds_id, params, username):
    ...
    def _target():
        ...
        run_bind(job_id, ds_id, params, username, db)  # snyk:ignore:Server-Side Request Forgery (SSRF)
...
def register_routes():
    ...
    register_native_route(PLUGIN_ID, "provisioning/recovery/scan-volumes", _recovery_scan_volumes)
    register_native_route(PLUGIN_ID, "provisioning/recovery/manifests", _recovery_manifests)
    register_native_route(PLUGIN_ID, "provisioning/recovery/bind", _recovery_bind)
    register_native_route(PLUGIN_ID, "provisioning/recovery/restore-vms", _recovery_restore_vms)
    register_native_route(PLUGIN_ID, "provisioning/recovery/used-vmids", _recovery_used_vmids)
