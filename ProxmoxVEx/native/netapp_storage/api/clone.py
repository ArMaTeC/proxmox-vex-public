...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/native/netapp_storage/api/clone.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: Clone API
...
def _start_clone():
    ...
    if data.get("native"):
        ...
        if is_san:
            ...
            manifest_path = (
                f"{mapping['nfs_mount_path']}/.snapshot/{snap_name}/{manifest_subdir}/{snap_name}/manifest.json"
            )
    ...
    db.execute(
        ...
        "VALUES (?,?,?,?,?,?,?)",
        (job_id, "clone", snapshot_id, int(data["new_vmid"]), "running", username, now),
    )
...
def _get_nextid():
    ...
    try:
        ...
        mgr = build_pve_client(db, pve_cluster_id)  # snyk:ignore:Server-Side Request Forgery (SSRF)
...
def register_routes():
    register_native_route(PLUGIN_ID, "clone/start", _start_clone)
    register_native_route(PLUGIN_ID, "clone/dr-start", _start_dr_clone)
    register_native_route(PLUGIN_ID, "clone/nextid", _get_nextid)
    register_native_route(PLUGIN_ID, "clone/nodes", _get_nodes)
