...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/native/netapp_storage/api/settings.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: Settings API — SMTP / email notification configuration...
...
from ProxmoxVEx.core.db import get_db  # noqa: E402
...
def _format_lag(s):
    ...
    if m.group(2):
        parts.append(f"{m.group(2)}h")
    if m.group(3):
        parts.append(f"{m.group(3)}m")
    if m.group(4):
        parts.append(f"{int(float(m.group(4)))}s")
    return " ".join(parts) if parts else "0s"


def _build_notification_email(
    subject, schedule_name, snap_name, job_status, log_lines=None, extra_rows=None, vm_list=None, datastore=None
):
    """
    ...
    _cfg = {
        ...
        "warn": {
            "banner": "#d97706",
            "icon": "⚠",
            "label": "Snapshot Completed with Warnings",
            "dot_color": "#d97706",
...
<div style="max-width:680px;margin:0 auto">
  ...
  <div style="background:{cfg["banner"]};border-radius:8px 8px 0 0;padding:22px 28px;color:#fff">
    ...
    <div style="font-size:13px;opacity:.85;margin-top:4px">NetApp ONTAP Storage Plugin · ProxmoxVEx</div>
  </div>
  ...
  <div style="text-align:center;font-size:11px;color:#9ca3af;margin-top:14px">
    ProxmoxVEx NetApp ONTAP Plugin
  </div>
...
def send_job_notification(
    ...
    try:
        ...
        subject = f"[ProxmoxVEx] Snapshot {status_str}: {schedule_name} — {snap_name}"
...
def _notify_test():
    ...
    subject = "[ProxmoxVEx] Test notification — NetApp ONTAP plugin"
    fake_log = [
...
def build_export_payload():
    ...
    for table in _EXPORT_TABLES:
        query = f"SELECT * FROM {table}"  # nosec: B608 - table name is from a hardcoded allowlist
        rows = db.query(query)
...
def _db_import():
    ...
    if request.content_type and "multipart" in request.content_type:
        ...
        try:
            ...
            log.warning("Invalid import JSON: %s", exc)
    ...
    try:
        ...
        log.warning("Import apply failed: %s", exc)
...
def register_routes():
    register_native_route(PLUGIN_ID, "settings/smtp", _smtp_get)
    register_native_route(PLUGIN_ID, "settings/smtp/save", _smtp_save)
    register_native_route(PLUGIN_ID, "settings/smtp/test", _smtp_test)
    register_native_route(PLUGIN_ID, "settings/notify-test", _notify_test)
    register_native_route(PLUGIN_ID, "settings/export", _db_export)
    register_native_route(PLUGIN_ID, "settings/import", _db_import)
