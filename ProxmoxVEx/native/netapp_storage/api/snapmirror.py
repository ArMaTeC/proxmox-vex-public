...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/native/netapp_storage/api/snapmirror.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: SnapMirror® API
...
def _dr_snap_vms():
    ...
    try:
        ...
        mapping = db.query_one("SELECT * FROM netapp_volume_mapping WHERE volume_uuid=?", (rel["source_volume_uuid"],))
        if not mapping:
        ...
        if vmid_set:
            vms = [{"vmid": vid, "name": f"vm-{vid}", "vm_type": vt} for vid, vt in sorted(vmid_set.items())]
            return {"vms": vms, "source": "history", "hint": "Snapshot not in DB — showing VMs from snapshot history"}

        return {"vms": [], "source": "none", "hint": "No VM information available — enter VMID manually"}
...
def _snapmirror_policy_labels():
    ...
    rel = db.query_one(
        "SELECT * FROM netapp_snapmirror_relationships WHERE source_volume_uuid=?", (mapping["volume_uuid"],)
    )
    if not rel:
...
def register_routes():
    register_native_route(PLUGIN_ID, "snapmirror/scan", _scan_relationships)
    register_native_route(PLUGIN_ID, "snapmirror/relationships", _list_relationships)
    register_native_route(PLUGIN_ID, "snapmirror/update", _update_relationship)
    register_native_route(PLUGIN_ID, "snapmirror/secondary-snapshots", _secondary_snapshots)
    register_native_route(PLUGIN_ID, "snapmirror/ensure-export", _ensure_export)
    register_native_route(PLUGIN_ID, "snapmirror/check-secondary", _check_secondary)
    register_native_route(PLUGIN_ID, "snapmirror/dr-snap-vms", _dr_snap_vms)
    register_native_route(PLUGIN_ID, "snapmirror/policy-labels", _snapmirror_policy_labels)
