...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/native/netapp_storage/api/snapshots.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
...
Routes under /api/netapp_storage/...
...
def _write_audit(db, action, user, result, target_name="", volume_name="", vmids=None, error_msg="", details=None):
    db.execute(
...
def _list_endpoints():
    db = get_db()
    rows = db.query(
        "SELECT id, name, host, username, ssl_verify, skip_nfs, san_optimized, created_at, updated_at "
...
def _add_endpoint():
    ...
    try:
        ...
        tmp = OntapClient(
            data["host"], data["username"], data["password"], ssl_verify=bool(data.get("ssl_verify", True))
        )
...
def _delete_endpoint():
    ...
    ds_rows = db.query("SELECT id, name FROM netapp_provisioned_datastores WHERE endpoint_id=?", (eid,))
...
def _test_endpoint():
    ...
    try:
        ...
        db.execute(
            "UPDATE netapp_endpoints SET san_optimized=?, updated_at=? WHERE id=?",
            (1 if san_optimized else 0, datetime.now(timezone.utc).isoformat(), eid),
        )
        return {"success": True, "cluster_name": cluster_name, "ontap_version": version, "san_optimized": san_optimized}
...
def _update_endpoint():
    ...
    if password:
        db.execute(
            ...
            (name, host, username, db._encrypt(password), 1 if ssl_verify else 0, 1 if skip_nfs else 0, now, eid),
        )
    else:
        db.execute(
            "UPDATE netapp_endpoints SET name=?, host=?, username=?, ssl_verify=?, skip_nfs=?, updated_at=? WHERE id=?",
...
def _list_pve_hosts():
    db = get_db()
    rows = db.query(
        "SELECT id, name, host, port, username, ssl_verify, nfs_ip, created_at FROM netapp_pve_hosts ORDER BY name"
    )
    return [dict(r) for r in rows]
...
def _test_pve_host():
    ...
    try:
        ...
        pve = build_pve_client(db, hid)  # snyk:ignore:Server-Side Request Forgery (SSRF)
        # query PVE version
...
def _delete_mapping():
    ...
    db = get_db()
    row = db.query_one("SELECT id, pve_storage_id FROM netapp_volume_mapping WHERE id=?", (mapping_id,))
    if not row:
...
def _create_snapshot():
    ...
    if not data.get("cluster_id"):
        row = db.query_one("SELECT pve_cluster_id FROM netapp_volume_mapping WHERE id=?", (data["mapping_id"],))
        if not row:
            return {"error": "Mapping not found"}, 404
    ...
    if not data.get("node"):
        try:
            ...
            pve = build_pve_client(db, data["cluster_id"])  # snyk:ignore:Server-Side Request Forgery (SSRF)
            # find VM node
...
def _delete_snapshot():
    ...
    if data.get("native"):
        ...
        vol_name = ""
        try:
            from ..core._helpers import get_endpoint, get_mapping

            mapping = get_mapping(db, mapping_id)
            vol_name = mapping.get("volume_name", "")
            endpoint = get_endpoint(db, mapping["endpoint_id"])
            client = build_ontap_client(endpoint)
            del_job = client.delete_snapshot(mapping["volume_uuid"], ontap_snap_uuid, force=force, snap_name=snap_name)
            if del_job:
                client.poll_job(del_job, timeout_s=120)
            _write_audit(
                db,
                "snapshot_delete_native",
                user,
                "success",
                target_name=snap_name,
    ...
    try:
        ...
        if ontap_snap_uuid:
            del_job = client.delete_snapshot(
                mapping["volume_uuid"], ontap_snap_uuid, snap_name=snap.get("snap_name", "")
            )
            if del_job:
                client.poll_job(del_job, timeout_s=120)
    except Exception as exc:
    ...
    _write_audit(
        ...
        user,
        "success",
        target_name=snap.get("snap_name", ""),
        volume_name=vol_name,
...
def _vms_for_mapping():
    ...
    db = get_db()
    try:
        from ..core._helpers import build_pve_client, get_mapping
        ...
        host_rows = db.query(
            "SELECT DISTINCT pve_cluster_id FROM netapp_volume_mapping WHERE volume_uuid=?", (volume_uuid,)
        )
        ...
        for hid in host_ids:
            try:
                ...
                for node_name in nodes:
                    ...
                    log.warning(
                        f"[netapp_storage] vms-for-mapping content {node_name}/{storage_id}: HTTP {rc.status_code} data={rc.json().get('data') if rc.ok else rc.text[:200]}"
                    )
                    if rc.ok:
        ...
        vms = [
            vm_info_map.get(vid, {"vmid": vid, "name": f"vm-{vid}", "node": "", "type": "qemu", "status": "unknown"})
...
def _snapshot_manifest():
    ...
    try:
        ...
        from ..core._helpers import build_pve_client, get_mapping, get_ssh_creds, load_plugin_config, ssh_run
        ...
        # 1. Exact path: snap_name as subdirectory (ProxmoxVEx snapshot)
        exact = f"{mapping['nfs_mount_path']}/.snapshot/{snap_name}/{manifest_subdir}/{snap_name}/manifest.json"
        manifest = None
        used_path = None
        try:
            out = ssh_run(pve_host, pve_user, pve_pass, f"cat {shlex.quote(exact)}", capture=True, key_material=pve_key)
        ...
        if manifest is None:
            search_dir = f"{mapping['nfs_mount_path']}/.snapshot/{snap_name}/{manifest_subdir}"
            try:
                out = ssh_run(
                    pve_host,
                    pve_user,
                    pve_pass,
                    f"find {shlex.quote(search_dir)} -name manifest.json 2>/dev/null | sort | tail -1",
                    capture=True,
...
def _cleanup_jobs():
    ...
    if err:
        return err
    db = get_db()
    result = db.query_one("SELECT COUNT(*) AS cnt FROM netapp_jobs WHERE status IN ('done','failed')")
...
def _list_audit():
    ...
    rows = db.query("SELECT * FROM netapp_audit_log ORDER BY timestamp DESC LIMIT ?", (limit,))
    result = []
    for r in rows:
        d = dict(r)
        d["vmids"] = json.loads(d.get("vmids_json") or "[]")
...
def _snapmanifest_init():
    ...
    db = get_db()
    from ..core._helpers import build_pve_client, get_mapping, get_ssh_creds
...
def _snapmanifest_check():
    ...
    db = get_db()
    from ..core._helpers import build_pve_client, get_mapping, get_ssh_creds, ssh_run
    ...
    try:
        out = ssh_run(
            ...
            ssh_user,
            ssh_pass,
            f"lvs {_shlex.quote(vg_name)}/{_shlex.quote(lv_name)} 2>/dev/null && echo EXISTS || echo MISSING",
            capture=True,
...
def register_routes():
    register_native_route(PLUGIN_ID, "endpoints", _list_endpoints)
    register_native_route(PLUGIN_ID, "endpoints/add", _add_endpoint)
    register_native_route(PLUGIN_ID, "endpoints/update", _update_endpoint)
    register_native_route(PLUGIN_ID, "endpoints/delete", _delete_endpoint)
    register_native_route(PLUGIN_ID, "endpoints/test", _test_endpoint)

    register_native_route(PLUGIN_ID, "pve-hosts", _list_pve_hosts)
    register_native_route(PLUGIN_ID, "pve-hosts/add", _add_pve_host)
    register_native_route(PLUGIN_ID, "pve-hosts/delete", _delete_pve_host)
    register_native_route(PLUGIN_ID, "pve-hosts/test", _test_pve_host)

    register_native_route(PLUGIN_ID, "volume-mappings", _list_mappings)
    register_native_route(PLUGIN_ID, "volume-mappings/delete", _delete_mapping)
    register_native_route(PLUGIN_ID, "discover", _run_discovery)
    register_native_route(PLUGIN_ID, "san/snapmanifest-init", _snapmanifest_init)
    register_native_route(PLUGIN_ID, "san/snapmanifest-check", _snapmanifest_check)

    register_native_route(PLUGIN_ID, "snapshots", _list_snapshots)
    register_native_route(PLUGIN_ID, "snapshots/create", _create_snapshot)
    register_native_route(PLUGIN_ID, "snapshots/delete", _delete_snapshot)
    register_native_route(PLUGIN_ID, "snapshots/volumes", _list_ontap_volumes)
    register_native_route(PLUGIN_ID, "snapshots/vms-for-mapping", _vms_for_mapping)
    register_native_route(PLUGIN_ID, "snapshots/manifest", _snapshot_manifest)

    register_native_route(PLUGIN_ID, "jobs/status", _job_status)
    register_native_route(PLUGIN_ID, "jobs/delete", _delete_job)
    register_native_route(PLUGIN_ID, "jobs/cancel", _cancel_job)
    register_native_route(PLUGIN_ID, "jobs/cleanup", _cleanup_jobs)
    register_native_route(PLUGIN_ID, "audit/list", _list_audit)
    register_native_route(PLUGIN_ID, "ui", _serve_ui)
