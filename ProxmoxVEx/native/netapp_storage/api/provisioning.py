...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/native/netapp_storage/api/provisioning.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: NetApp ONTAP Provisioning API
...
def _prov_datastores():
    ...
    if request.method == "GET":
        ...
        if err:
            return err
        db = get_db()
        rows = db.query("SELECT * FROM netapp_provisioned_datastores ORDER BY created_at DESC")
        result = []
        for r in rows:
            d = _ds_to_dict(r)
            ...
            if not d.get("size_bytes") and d.get("volume_uuid") and d.get("status") == "active":
                try:
                    from ..core._helpers import build_ontap_client, get_endpoint
    ...
    db.execute(
        ...
        (
            ...
            body["endpoint_id"],
            body.get("svm_name", ""),
            body.get("volume_uuid", ""),
            body.get("volume_name", ""),
            protocol,
            body.get("lun_uuid", ""),
            body.get("lun_path", ""),
            body.get("igroup_uuid", ""),
            body.get("igroup_name", ""),
            body.get("ns_uuid", ""),
            body.get("subsystem_uuid", ""),
            body.get("subsystem_name", ""),
            body.get("vg_name", ""),
            body.get("lvm_type", "linear"),
            body.get("lvm_pool_name", ""),
            body.get("nfs_junction_path", ""),
...
def _check_vms_on_storage(db, pve_host_id, vg_name):
    ...
    try:
        ...
        return [
            lun_path.strip()
            for lun_path in out.strip().splitlines()
            if lun_path.strip() and lun_path.strip() not in SYSTEM_LVS
        ]
    except Exception:
        return []
...
def _prov_ontap_resources():
    ...
    return {
        ...
        "luns": luns,
        "igroups": igroups,
        "nvme_subsystems": nvme_subsystems,
...
def _prov_import():
    ...
    existing = db.query_one("SELECT id FROM netapp_provisioned_datastores WHERE pve_storage_id=?", (pve_storage_id,))
    if existing:
    ...
    all_mappings = db.query("SELECT pve_cluster_id FROM netapp_volume_mapping WHERE volume_uuid=?", (volume_uuid,))
    ...
    size_bytes = 0
    try:
        vol_info = client._get(f"storage/volumes/{volume_uuid}", params={"fields": "space.size"})
    ...
    if protocol in ("iscsi", "nvme") and vg_name and pve_host_ids:
        try:
            pve_row = db.query_one("SELECT * FROM netapp_pve_hosts WHERE id=?", (pve_host_ids[0],))
            if pve_row:
                ph = dict(pve_row)
                ph_host = ph["host"]
                ...
                if "EXISTS" in chk:
                    ...
                    log.info(
                        f"[netapp_storage] import '{pve_storage_id}': snapmanifest LV detected → snapinfo_initialized=1"
                    )
        except Exception as exc:
    ...
    db.execute(
        ...
        (
            ...
            now,
            now,
            snapinfo_initialized,
        ),
    )
    return {"success": True, "id": ds_id, "name": ds_name}


def _storage_unified() -> dict[str, Any]:
    """
    GET  storage/unified
    ...
    for r in map_rows or []:
        ...
        host_rows = db.query(
            "SELECT DISTINCT pve_cluster_id FROM netapp_volume_mapping WHERE pve_storage_id=?", (storage_id,)
        )
...
def register_routes():
    ...
    register_native_route(PLUGIN_ID, "storage/unified", _storage_unified)
    register_native_route(PLUGIN_ID, "provisioning/datastores", _prov_datastores)
    register_native_route(PLUGIN_ID, "provisioning/datastores/import", _prov_import)
    register_native_route(PLUGIN_ID, "provisioning/datastores/remove", _prov_remove)
    register_native_route(PLUGIN_ID, "provisioning/datastores/resize", _prov_resize)
    register_native_route(PLUGIN_ID, "provisioning/datastores/add-host", _prov_add_host)
    register_native_route(PLUGIN_ID, "provisioning/datastores/remove-host", _prov_remove_host)
    register_native_route(PLUGIN_ID, "provisioning/ontap-resources", _prov_ontap_resources)
    register_native_route(PLUGIN_ID, "provisioning/pve-hosts", _prov_pve_hosts)
    register_native_route(PLUGIN_ID, "provisioning/svms", _prov_svms)
    register_native_route(PLUGIN_ID, "provisioning/nfs-lifs", _prov_nfs_lifs)
...
def _run_provision_job_async(job_id, ds_id, params, username):
    ...
    t = threading.Thread(target=_run_provision, args=(job_id, ds_id, params, username), daemon=True)
    t.start()
...
def _run_resize_job_async(job_id, ds_id, new_size_bytes, username):
    ...
    t = threading.Thread(target=_run_resize, args=(job_id, ds_id, new_size_bytes, username), daemon=True)
    t.start()
...
def _run_add_host_job_async(job_id, ds_id, host_id, username):
    ...
    t = threading.Thread(target=_run_add_host, args=(job_id, ds_id, host_id, username), daemon=True)
    t.start()
...
def _run_remove_host_job_async(job_id, ds_id, host_id, username):
    ...
    t = threading.Thread(target=_run_remove_host, args=(job_id, ds_id, host_id, username), daemon=True)
    t.start()
...
def _run_remove_job_async(job_id, ds_id, delete_ontap, username):
    ...
    t = threading.Thread(target=_run_remove, args=(job_id, ds_id, delete_ontap, username), daemon=True)
    t.start()
...
def _provision_iscsi(ds_id, params, db, jlog):
    ...
    if not volume_uuid:
        ...
        jlog.log(
            f"Creating ONTAP volume '{volume_name}' ({vol_size_bytes} bytes, {_vol_multiplier}× LUN size){ag_info} …"
        )
        try:
    ...
    if not lun_uuid:
        ...
        lun_uuid, serial = client.create_lun(
            svm_name, volume_name, lun_name, size_bytes, auto_provision_as_flexvol=asa_mode
        )
    ...
    if not igroup_uuid:
        ...
        jlog.log("Using existing iGroup, adding missing initiators …")
        for m in host_meta.values():
    ...
    db.execute(
        """UPDATE netapp_provisioned_datastores
           ...
           WHERE id=?""",
        (volume_uuid, volume_name, lun_uuid, lun_path, igroup_uuid, igroup_name, _now(), ds_id),
    )
    ...
    for i, hid in enumerate(ordered_hosts):
        ...
        ssh_run(
            ...
            su,
            sp,
            f"iscsiadm -m node -T {shlex.quote(target_iqn)} -p {shlex.quote(portal_ip)} --login 2>&1 || true",
            key_material=sk,
            timeout=30,
        )
        ...
        ssh_run(
            ...
            su,
            sp,
            "sleep 3; udevadm settle --timeout=15 2>/dev/null; multipath 2>/dev/null; sleep 2; true",
            key_material=sk,
            timeout=40,
        )
        ...
        if i == 0:
            ...
            out = ssh_run(
                sh, su, sp, f"vgs {vg_q} 2>/dev/null && echo EXISTS || echo MISSING", capture=True, key_material=sk
            )
            if "EXISTS" not in out:
                ...
                if lvm_type == "thin":
                    ...
                    ssh_run(sh, su, sp, f"lvcreate -l 95%VG --thin {vg_q}/{pool_q}", key_material=sk, timeout=30)
    ...
    lvm_pool_name_final = (
        params.get("lvm_pool_name")
        or (
            dict(db.query_one("SELECT lvm_pool_name FROM netapp_provisioned_datastores WHERE id=?", (ds_id,)) or {})
        ).get("lvm_pool_name", "")
        or "data"
    )
    if lvm_type == "thin":
        pvesm_cmd = (
            f"pvesm add lvmthin {sid_q} --vgname {vg_q}"
            f" --thinpool {shlex.quote(lvm_pool_name_final)}"
            f" --shared 1 --content images,rootdir"
        )
    else:
        pvesm_cmd = f"pvesm add lvm {sid_q} --vgname {vg_q} --shared 1 --content images,rootdir"
    ...
    for hid in ordered_hosts:
        ...
        try:
            check = ssh_run(
                ...
                su,
                sp,
                f"pvesm status {shlex.quote(pve_storage_id)} 2>/dev/null && echo EXISTS || echo MISSING",
                capture=True,
                key_material=sk,
            )
...
def _run_resize(job_id, ds_id, new_size_bytes, username):
    ...
    try:
        ...
        jlog.log(f"Resizing {protocol} datastore '{ds.get('name')}' from {old_size} to {new_size_bytes} bytes …")
            ...
            for hid in pve_host_ids:
                try:
                    ...
                    if vg_name:
                        ...
                        for pv in (lun_path.strip() for lun_path in out.splitlines() if lun_path.strip()):
                            jlog.log(f"[{sh}] Resizing PV {pv} …")
                            ssh_run(
                                sh,
                                su,
                                sp,
                                f"pvresize {shlex.quote(pv)} 2>/dev/null; pvscan --cache 2>/dev/null; true",
                                key_material=sk,
                                timeout=30,
                            )
                        jlog.log(f"[{sh}] PV resized.")
...
def _add_host_iscsi(ds_id, ds, host_id, db, jlog):
    from ..core.san_helpers import _iscsi_serial_to_mapper, find_device_by_serial, get_iscsi_initiator_iqn
    ...
    if lun_uuid:
        try:
            ...
            raise RuntimeError(f"Cannot fetch LUN serial: {exc}") from exc
    if not lun_serial:
    ...
    if igroup_uuid:
        jlog.log("Adding IQN to iGroup …")
        try:
            existing_ig = next(
                (g for g in client.list_igroups(svm_name=svm_name) if g.get("uuid") == igroup_uuid), None
            )
            existing_inits = (
                {i.get("name", "") for i in (existing_ig.get("initiators") or [])} if existing_ig else set()
            )
            if iqn not in existing_inits:
                client.add_igroup_initiator(igroup_uuid, iqn)
                jlog.log("IQN added to iGroup.")
            else:
                jlog.log("IQN already in iGroup.")
        except Exception as exc:
    ...
    try:
        ssh_run(
            sh, su, sp, f"pvscan --cache -aay {shlex.quote(mapper_dev)} 2>/dev/null; true", key_material=sk, timeout=30
        )
        jlog.log(f"[{sh}] pvscan done.")
    except Exception as exc:
...
def _remove_host_iscsi(ds_id, ds, host_id, db, jlog):
    ...
    try:
        ...
        if pve_storage_id:
            ...
            ssh_run(
                sh, su, sp, f"pvesm remove {shlex.quote(pve_storage_id)} 2>/dev/null; true", key_material=sk, timeout=30
            )
...
def _provision_nvme(ds_id, params, db, jlog):
    ...
    _saved = dict(
        db.query_one(
            ...
            "FROM netapp_provisioned_datastores WHERE id=?",
            (ds_id,),
        )
        or {}
    )
    ...
    if not volume_uuid:
        ag_info = f" on aggregate '{aggregate_name}'" if aggregate_name else " (auto-placement)"
        jlog.log(
            f"Creating ONTAP volume '{volume_name}' ({vol_size_bytes} bytes, "
    ...
    if not ns_uuid:
        ...
        ns_uuid = client.create_namespace(
            svm_name, volume_name, ns_name, size_bytes, aggregate_name=aggregate_name if asa_mode else None
        )
            ...
            try:
                ...
                try:
                    vols = client._get_all_records(
                        "storage/volumes",
                        params={"svm.name": svm_name, "name": volume_name, "fields": "uuid,name", "max_records": 5},
                    )
                    if vols:
    ...
    for hid in ordered_hosts:
        ...
        try:
            if dev:
                ssh_run(
                    sh, su, sp, f"pvscan --cache -aay {shlex.quote(dev)} 2>/dev/null; true", key_material=sk, timeout=30
                )
            else:
                # fallback: scan all (slower but safe)
                ssh_run(
                    sh, su, sp, f"vgchange -ay {shlex.quote(vg_name)} 2>/dev/null; true", key_material=sk, timeout=30
                )
            jlog.log(f"[{sh}] pvscan done.")
        except Exception as exc:
            jlog.log(f"[{sh}] WARNING: pvscan failed: {exc}")
...
def _remove_nvme(ds_id, ds, delete_ontap, db, jlog):
    ...
    for hid in pve_host_ids:
        try:
            ...
            if vg_name:
                vg_q = shlex.quote(vg_name)
                ssh_run(sh, su, sp, f"vgremove -f {vg_q} 2>/dev/null; true", key_material=sk, timeout=15)
...
def _add_host_nvme(ds_id, ds, host_id, db, jlog):
    ...
    if subsystem_uuid:
        try:
            ...
            if nqn not in existing_nqns:
                ...
                jlog.log("Host NQN added to subsystem.")
            else:
                jlog.log("Host NQN already in subsystem.")
        except Exception as exc:
    ...
    jlog.log(f"[{sh}] Activating VG via pvscan …")
    ssh_run(sh, su, sp, f"pvscan --cache -aay {shlex.quote(device)} 2>/dev/null; true", key_material=sk, timeout=30)
    jlog.log(f"[{sh}] pvscan done.")
    ...
    try:
        ns_info = client.get_namespace(ns_uuid)
        ns_name = (ns_info.get("location") or {}).get("namespace") or ns_info.get("name", "").split("/")[-1]
    except Exception:
        pass
    mid = str(_uuid.uuid4())
    try:
...
def _detect_nfs_client_ip(pve, nfs_lif_ip, jlog=None):
    ...
    try:
        out = ssh_run(
            ...
            pve.ssh_user,
            pve.ssh_password,
            f"ip route get {shlex.quote(nfs_lif_ip)} 2>/dev/null | grep -oP 'src \\K[0-9.]+'",
            capture=True,
            key_material=pve.ssh_key,
            timeout=10,
        )
...
def _provision_nfs(ds_id, params, db, jlog):
    ...
    if not volume_uuid:
        ag_info = f" on aggregate '{aggregate_name}'" if aggregate_name else " (auto-placement)"
        jlog.log(f"Creating NFS volume '{volume_name}' ({size_bytes} bytes) junction='{junction_path}'{ag_info} …")
        ...
        volume_uuid = client.create_volume_nfs(
            svm_name, volume_name, size_bytes, junction_path, aggregate_name=aggregate_name, export_policy=policy_name
        )
        jlog.log(f"Volume created: {volume_uuid}")
        try:
            client.enable_inline_compression(volume_uuid)
