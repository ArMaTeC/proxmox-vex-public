...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/native/netapp_storage/api/setup.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: Deploy Wizard API
...
from ProxmoxVEx.constants import RESERVED_ADMIN_USERNAME, SYSTEM_ACTOR
...
# ── Step 1: ProxmoxVEx system status ───────────────────────────────────────────
...
def _create_ontap_user():
    ...
    Body:
      ...
      admin_password
      new_username  — username to create (default: ProxmoxVEx)
    ...
    new_username = data.get("new_username", "ProxmoxVEx").strip()
    ...
    try:
        ...
        try:
            client._get(f"security/accounts/{new_username}")
            return jsonify({
                "ok": True,
                "created": False,
        ...
        return jsonify({
            ...
            "created": True,
            "message": (
                f"User '{new_username}' created with role '{role_name}'. HTTP/REST access only — no CLI or SSH access."
            ),
        })

    except OntapError as exc:
        log.warning("[setup] create_ontap_user failed: %s", exc)
        return jsonify({"error": "ONTAP user creation failed"}), 400
    except Exception as exc:
        log.error("[setup] create_ontap_user failed: %s", exc)
        return jsonify({"error": "ONTAP user creation failed"}), 500
...
def _get_ssh_pubkey():
    ...
    try:
        ...
        try:
            ...
            username = grp_name = RESERVED_ADMIN_USERNAME
        suggested_home = f"/home/{username}"
        hint = (
            f"Run as root on the ProxmoxVEx server:\n"
    ...
    try:
        ...
        return jsonify({
            ...
            "hint": "Make sure openssh-client is installed on the ProxmoxVEx host.",
        }), 500
    except Exception as exc:
        log.warning("[setup] ssh-pubkey generation failed: %s", exc)
        return jsonify({"error": "SSH key generation failed"}), 500
...
def _push_ssh_key():
    ...
    if not shutil.which("sshpass"):
        return jsonify({
            "error": "sshpass is not installed on ProxmoxVEx. Install it with: apt-get install sshpass",
        }), 400
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            log.error("[setup] ssh-copy-id failed for %s: %s", target_host, stderr)
            return jsonify({"error": "ssh-copy-id failed"}), 400
    except subprocess.TimeoutExpired:
        return jsonify({"error": "Connection timed out"}), 400
    except Exception as exc:
        log.error("[setup] ssh-copy-id failed: %s", exc)
        return jsonify({"error": "SSH key push failed"}), 500
...
def _test_ssh():
    ...
    for row in rows:
        ...
        try:
            out = ssh_run(host, user, passw, "echo __pgssh_ok__", capture=True, timeout=15)
...
def _check_packages():
    ...
    try:
        ...
        log.warning("[setup] package check over SSH failed for %s: %s", host, exc)
        return jsonify({"error": "SSH package check failed"}), 400
...
def _install_packages():
    ...
    username = request.session.get("user", SYSTEM_ACTOR) if hasattr(request, "session") else SYSTEM_ACTOR
...
def _run_pkg_install(job_id, host_row, packages):
    ...
    from ProxmoxVEx.core.db import get_db as _get_db
    ...
    try:
        db.execute("UPDATE netapp_jobs SET progress_pct=10 WHERE id=?", (job_id,))
...
# ── ProxmoxVEx cluster import ───────────────────────────────────────────────────


def _get_pve_clusters():
    """Returns ProxmoxVEx-managed Proxmox clusters with live node lists.

    Reads the ``clusters`` table (ProxmoxVEx core), decrypts credentials,
    then fetches ``GET /api2/json/nodes`` from each cluster so the wizard
    can show the exact nodes the user already has in ProxmoxVEx.
    """
        ...
        try:
            ...
            if lr.status_code != 200:
                ...
                if nr.status_code == 200:
                    entry["nodes"] = [
                        {"node": n["node"], "status": n.get("status", "?")} for n in nr.json().get("data", [])
                    ]
                else:
...
def _import_pve_nodes():
    """Imports selected PVE nodes from a ProxmoxVEx cluster into netapp_pve_hosts.
    ...
    db = get_db()
    row = db.query_one(
        "SELECT name, host, user, pass_encrypted, ssl_verification, api_port FROM clusters WHERE id=?", (cluster_id,)
    )
    if not row:
    ...
    if imported:
        ...
        if not pubkey:
            ssh_failed = [{"node": n, "error": "No SSH public key found on ProxmoxVEx"} for n in imported]
...
def _add_ontap_system():
    ...
    try:
        ...
        log.warning("[setup] ONTAP admin login failed: %s", exc)
        return jsonify({"error": "ONTAP admin login failed"}), 400
    except Exception as exc:
        log.warning("[setup] Cannot reach ONTAP cluster: %s", exc)
        return jsonify({"error": "Cannot reach ONTAP cluster"}), 400
    ...
    try:
        ...
        if exc.status_code != 404:
            log.warning("[setup] Cannot check ONTAP user %s: %s", new_username, exc)
            return jsonify({"error": "User check failed"}), 400
        ...
        try:
            ...
            if exc.status_code == 409:
                ...
                log.warning("[setup] Cannot create ONTAP user %s: %s", new_username, exc)
...
def register_routes():
    register_native_route(PLUGIN_ID, "setup/status", _setup_status)
    register_native_route(PLUGIN_ID, "setup/test-endpoints", _test_endpoints)
    register_native_route(PLUGIN_ID, "setup/test-ssh", _test_ssh)
    register_native_route(PLUGIN_ID, "setup/ssh-pubkey", _get_ssh_pubkey)
    register_native_route(PLUGIN_ID, "setup/push-ssh-key", _push_ssh_key)
    register_native_route(PLUGIN_ID, "setup/check-packages", _check_packages)
    register_native_route(PLUGIN_ID, "setup/install-packages", _install_packages)
    register_native_route(PLUGIN_ID, "setup/create-ontap-user", _create_ontap_user)
    register_native_route(PLUGIN_ID, "setup/pve-clusters", _get_pve_clusters)
    register_native_route(PLUGIN_ID, "setup/import-pve-nodes", _import_pve_nodes)
    register_native_route(PLUGIN_ID, "setup/add-ontap-system", _add_ontap_system)
