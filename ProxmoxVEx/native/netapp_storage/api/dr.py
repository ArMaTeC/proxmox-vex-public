...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/native/netapp_storage/api/dr.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: Disaster Recovery API (v3.0 — direct peer-to-peer sync)
...
def _set_role():
    ...
    db.execute(
        "UPDATE netapp_plugin_config SET role=?, role_forced=?, updated_at=? WHERE id='default'", (role, forced, _now())
    )
    # Notify peer — fire-and-forget in background
    threading.Thread(target=_peer_push_role_notify, args=(role,), daemon=True).start()
...
def _configure_peer():
    ...
    if existing:
        ...
        db.execute(
            ...
            "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)",
            ("default", name, url, ssl_verify, sync_token, "", "", "", "", "unconfigured", "", now, now),
        )
...
def _peer_call(method, path, payload=None, peer=None, timeout=15):
    ...
    url = f"{base}/api/netapp_storage/{ns}/{path}"
...
def _peer_push_heartbeat():
    ...
    cfg = _get_plugin_config()
    resp, err = _peer_call(
        "POST", "dr/peer/heartbeat", {"role": cfg.get("role", "PRIMARY"), "timestamp": _now()}, peer=peer, timeout=10
    )
    try:
        db = get_db()
        if err:
            db.execute("UPDATE netapp_dr_peer SET sync_status='offline', sync_error=? WHERE id='default'", (err[:500],))
        else:
            db.execute(
...
def _peer_push_sync(peer=None):
    ...
    try:
        db = get_db()
        if err:
            db.execute("UPDATE netapp_dr_peer SET sync_status='error', sync_error=? WHERE id='default'", (err[:500],))
            return False, err
        else:
            db.execute(
...
def _peer_role_notify_recv():
    ...
    try:
        ...
        if sender_role:
            db.execute("UPDATE netapp_dr_peer SET peer_role=?, last_seen=? WHERE id='default'", (sender_role, _now()))
        ...
        has_active = db.query_one("SELECT id FROM netapp_dr_plans WHERE state IN (?,?) LIMIT 1", active_states)
        if not has_active and suggested in ("PRIMARY", "SECONDARY", "STANDALONE") and not cfg.get("role_forced"):
            db.execute("UPDATE netapp_plugin_config SET role=?, updated_at=? WHERE id='default'", (suggested, _now()))
...
def _build_sync_payload():
    ...
    for e in db.query("SELECT * FROM netapp_dr_plan_entries") or []:
        ...
        ep = db.query_one("SELECT host FROM netapp_endpoints WHERE id=?", (entry.get("source_endpoint_id", ""),))
        entry["source_endpoint_host"] = ep["host"] if ep else ""
        ep2 = db.query_one("SELECT host FROM netapp_endpoints WHERE id=?", (entry.get("dr_endpoint_id", ""),))
...
def _apply_sync_payload(data, db):
        ...
        if existing:
            ...
            db.execute(
                ...
                (
                    ...
                    "standby",
                    p.get("notes", ""),
                    p.get("last_failover_at", ""),
                    p.get("last_test_at", ""),
                    p.get("created_by", "sync"),
    ...
    # Plan entries
    existing_entry_ids = {r["id"] for r in (db.query("SELECT id FROM netapp_dr_plan_entries") or [])}
        ...
        if e["id"] in existing_entry_ids:
            db.execute(
                ...
                (
                    ...
                    e.get("source_svm", ""),
                    e.get("source_volume", ""),
                    e.get("snapmirror_rel_uuid", ""),
                    dr_ep_id,
                    e.get("dr_svm", ""),
                    e.get("dr_volume", ""),
                    e.get("dr_pve_storage_id", ""),
                    e.get("dr_pve_host_ids", "[]"),
                    e.get("sort_order", 0),
                    e["id"],
                ),
            )
    ...
    # VM groups
    existing_group_ids = {r["id"] for r in (db.query("SELECT id FROM netapp_dr_vm_groups") or [])}
    ...
    for g in data.get("vm_groups") or []:
        ...
        if g["id"] in existing_group_ids:
            db.execute(
                ...
                (
                    g.get("name", ""),
                    g.get("group_type", "standard"),
                    g.get("sort_order", 0),
                    g.get("start_mode", "auto"),
                    g.get("startup_delay_sec", 30),
                    g.get("max_parallel", 1),
                    g["id"],
                ),
            )
        else:
    ...
    # VM assignments
    existing_vm_ids = {r["id"] for r in (db.query("SELECT id FROM netapp_dr_vm_assignments") or [])}
    received_vm_ids = set()
            ...
            if existing_rel:
                db.execute(
                    ...
                    (
                        ...
                        r.get("healthy", 1),
                        r.get("lag_time", ""),
                        r.get("last_transfer_time", ""),
                        r.get("last_scanned_at", now),
                        rel_uuid,
                    ),
                )
            else:
                ...
                db.execute(
                    ...
                    (
                        rid,
                        r.get("source_endpoint_id", ""),
                        r.get("source_volume_uuid", ""),
                        r.get("source_svm", ""),
                        r.get("source_volume", ""),
                        r.get("dest_endpoint_id", ""),
                        r.get("dest_cluster_name", ""),
                        r.get("dest_svm", ""),
                        r.get("dest_volume", ""),
                        r.get("dest_volume_uuid", ""),
                        r.get("dest_nfs_ip", ""),
                        r.get("dest_junction_path", ""),
                        rel_uuid,
                        r.get("policy_type", ""),
                        r.get("state", ""),
                        r.get("healthy", 1),
...
def _get_dr_plan_detail():
    ...
    for g in groups:
        grp = dict(g)
        vms = (
            db.query("SELECT * FROM netapp_dr_vm_assignments WHERE group_id=? ORDER BY start_order", (grp["id"],)) or []
        )
...
def _update_dr_plan():
    ...
    query = f"UPDATE netapp_dr_plans SET {', '.join(updates)} WHERE id=?"  # nosec: B608 - column names are from a hardcoded allowlist and values use ? placeholders
...
def _lookup_primary_storage_id(db, source_endpoint_id, source_svm, source_volume):
    row = db.query_one(
        "SELECT pve_storage_id FROM netapp_volume_mapping WHERE endpoint_id=? AND svm_name=? AND volume_name=? LIMIT 1",
        (source_endpoint_id, source_svm, source_volume),
    )
...
def _enrich_entry(entry, db):
    ...
    primary_storage_id = _lookup_primary_storage_id(
        db, entry.get("source_endpoint_id", ""), entry.get("source_svm", ""), entry.get("source_volume", "")
    )
    ...
    if not entry.get("dr_pve_storage_id") and primary_storage_id:
        ...
        if request.method in ("POST", "PUT", "PATCH"):
            with contextlib.suppress(Exception):
                db.execute(
                    "UPDATE netapp_dr_plan_entries SET dr_pve_storage_id=? WHERE id=?",
...
def _add_plan_entry():
    ...
    if not snapmirror_rel_uuid:
        ...
        if rel:
            ...
            if not dr_endpoint_id:
                dr_endpoint_id = rel["dest_endpoint_id"] or ""
            if not dr_svm:
                dr_svm = rel["dest_svm"] or ""
            if not dr_volume:
                dr_volume = rel["dest_volume"] or ""
...
def _update_plan_entry():
    ...
    query = f"UPDATE netapp_dr_plan_entries SET {', '.join(updates)} WHERE id=?"  # nosec: B608 - column names are from a hardcoded allowlist and values use ? placeholders
    db.execute(query, params)
    db.execute("UPDATE netapp_dr_plans SET updated_at=? WHERE id=?", (_now(), plan_id))
    ...
    for rel in [dict(r) for r in rels]:
        ...
        db.execute(
            ...
            (
                eid,
                plan_id,
                rel["source_endpoint_id"],
                rel["source_svm"],
                rel["source_volume"],
                rel["relationship_uuid"],
                rel.get("dest_endpoint_id", ""),
                rel["dest_svm"],
                rel["dest_volume"],
                primary_storage_id,
                "[]",
...
def _update_vm_group():
    ...
    query = f"UPDATE netapp_dr_vm_groups SET {', '.join(updates)} WHERE id=?"  # nosec: B608 - column names are from a hardcoded allowlist and values use ? placeholders
    db.execute(query, params)
    _spawn(_peer_push_sync)
...
def _add_vm_assignment():
    ...
    db.execute(
        "INSERT INTO netapp_dr_vm_assignments (id, group_id, vmid, vm_name, target_node, start_order, created_at) "
        "VALUES (?,?,?,?,?,?,?)",
        (aid, group_id, int(vmid), data.get("vm_name", ""), data.get("target_node", ""), start_order, _now()),
    )
    _spawn(_peer_push_sync)
...
def _update_vm_assignment():
    ...
    query = f"UPDATE netapp_dr_vm_assignments SET {', '.join(updates)} WHERE id=?"  # nosec: B608 - column names are from a hardcoded allowlist and values use ? placeholders
    db.execute(query, params)
    _spawn(_peer_push_sync)
...
def _failover_precheck():
    ...
    _chk(
        "Storage IDs",
        "error" if missing_storage else "ok",
    ...
    _chk(
        "DR PVE hosts",
        "error" if missing_hosts else "ok",
    ...
    unhealthy = [
        e["source_volume"]
        for e in entries
        if e.get("snapmirror_rel_uuid")
        and (lambda r: r and not r["healthy"])(
            db.query_one(
    ...
    vm_groups = (
        db.query(
            ...
            "FROM netapp_dr_vm_groups g WHERE g.plan_id=? ORDER BY g.sort_order",
            (plan_id,),
        )
        or []
    )
    ...
    _chk(
        "VM groups",
        "warn" if not vm_groups or core_vms == 0 else "ok",
        f"{len(vm_groups)} group(s), {total_vms} VM(s)"
        if vm_groups
...
def _start_failover():
    ...
    threading.Thread(
        target=_execute_failover, args=(job_id, plan_id, failover_type, entry_ids, snap_map), daemon=True
    ).start()
...
def _execute_failover(job_id, plan_id, failover_type, entry_ids=None, snap_map=None):
    ...
    from ..core._helpers import build_ontap_client, build_pve_client, get_endpoint, get_ssh_creds, ssh_run
    ...
    try:
        db = get_db()
        all_entries = (
            db.query("SELECT * FROM netapp_dr_plan_entries WHERE plan_id=? ORDER BY sort_order", (plan_id,)) or []
        )
        if not all_entries:
            _log("[ERR] No plan entries found")
            _finish("failed")
            return
        ...
        for entry in entries:
            ...
            if not rel_uuid:
                _log("[WARN] No SnapMirror relationship — skipping")
                continue
            if not storage_id:
                _log("[WARN] No DR storage ID — skipping")
                continue
            if not pve_host_ids:
                _log("[WARN] No DR PVE host — skipping")
                continue
            ...
            try:
                ...
                _log(f"[ERR] Cannot connect to DR ONTAP: {exc}")
                _finish("failed")
                return
            ...
            try:
                ...
                _log(f"[ERR] SnapMirror break failed: {exc}")
                _finish("failed")
                return
            ...
            try:
                ...
                _log(f"[ERR] Volume info/mount failed: {exc}")
                _finish("failed")
                return
            ...
            try:
                ...
                if not nfs_ip:
                    _log(f"[ERR] No NFS LIF on DR SVM '{dr_svm}'")
                    _finish("failed")
                    return
                _log(f"[INFO] NFS LIF: {nfs_ip}")
            except Exception as exc:
                _log(f"[ERR] NFS LIF lookup failed: {exc}")
                _finish("failed")
                return

            sid_q = shlex.quote(storage_id)
            pvesm_cmd = (
                f"pvesm add nfs {sid_q} --server {shlex.quote(nfs_ip)}"
                ...
                try:
                    ...
                    vms_in_plan = (
                        db.query(
                            ...
                            "JOIN netapp_dr_vm_groups g ON g.id=a.group_id WHERE g.plan_id=?",
                            (plan_id,),
                        )
                        or []
                    )
                    ...
                    for vm in vms_in_plan:
                        ...
                        if "MISSING" in check:
                            _log(f"[WARN] [{sh}] No config for VM {vmid} — skipping")
                            continue
                        existing = ssh_run(
                            sh,
                            su,
                        ...
                        if "EXISTS" in existing:
                            _log(f"[INFO] [{sh}] VM {vmid} already registered — keeping existing")
                            continue
        ...
        vm_groups = db.query("SELECT * FROM netapp_dr_vm_groups WHERE plan_id=? ORDER BY sort_order", (plan_id,)) or []
        if not vm_groups:
            ...
            for group in vm_groups:
                ...
                assignments = (
                    db.query(
                        "SELECT * FROM netapp_dr_vm_assignments WHERE group_id=? ORDER BY start_order", (group["id"],)
                    )
                    or []
                )
                mode_label = "AUTO" if group["start_mode"] == "auto" else "MANUAL"
                _log(
                    f"[INFO] Group '{group['name']}' [{group['group_type'].upper()} / {mode_label}] — {len(assignments)} VM(s)"
                )
                if group["start_mode"] == "manual":
                    _log("[INFO]   → Skipped (manual — start via UI after confirming primary is down)")
                    continue

                first_host = next((h for e in entries for h in _json_field(e.get("dr_pve_host_ids"))), None)
                if not first_host:
                    _log("[WARN]   → No DR PVE host found — skipping group")
                    continue
                ...
                for batch_start in range(0, len(assignments), max_par):
                    ...
                    for assignment in batch:
                        ...
                        try:
                            ...
                            if "MISSING" in check:
                                _log(f"[WARN]   VM {vmid} ({vm_name}): not registered — skipping")
                                continue
...
def _list_dr_snapshots():
    ...
    db = get_db()
    entry = db.query_one("SELECT * FROM netapp_dr_plan_entries WHERE id=? AND plan_id=?", (entry_id, plan_id))
    if not entry:
        return {"error": "Entry not found"}, 404
    entry = dict(entry)
    try:
        ...
        log.error("[dr] list_snapshots failed: %s", exc)
