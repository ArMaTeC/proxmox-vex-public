...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/native/netapp_storage/__init__.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: NetApp ONTAP Snapshot / Restore Plugin for ProxmoxVEx
# Docs:        https://proxmoxvex.local/docs
# Generated:   2026-09-04
# --------------------------------------------------------------------
# --- ProxmoxVEx auto-header end ---
"""
NetApp ONTAP Snapshot / Restore Plugin for ProxmoxVEx
...
# Must match the directory name — ProxmoxVEx uses the folder name as the plugin ID.
PLUGIN_ID = os.path.basename(PLUGIN_DIR)
CONFIG_PATH = os.path.join(PLUGIN_DIR, "config.json")

# Default contents for user customisation files, loaded after the standard
# ui.css/ui.js so users can override styles or behaviour without editing core files.
_CUSTOM_CSS = """/* --- ProxmoxVEx auto-header start ---
 * -------------------------------------------------------------------
 * File:        ProxmoxVEx/native/netapp_storage/custom.css
 * Project:     ProxmoxVEx
 * Version:     1.2.292
 * Build:       2026.09.03
 * Description: NetApp ONTAP user customisations.
...
/*
 * Put your theme overrides, colour tweaks and UI customisations here.
 * This file is loaded after ui.css and will override the default styles.
 *
 * Examples:
 *   :root { --accent: #00e5ff; }
...
_CUSTOM_JS = """/* --- ProxmoxVEx auto-header start ---
 * -------------------------------------------------------------------
 * File:        ProxmoxVEx/native/netapp_storage/custom.js
 * Project:     ProxmoxVEx
 * Version:     1.2.292
 * Build:       2026.09.03
 * Description: NetApp ONTAP user custom JS.
 * Docs:        https://proxmoxvex.local/releases/latest
 * Generated:   2026-09-03
 * -------------------------------------------------------------------
 * --- ProxmoxVEx auto-header end --- */
...
def _ensure_custom_files():
    """Create custom.css/custom.js if missing so the UI always loads cleanly."""
    files = {
        "custom.css": _CUSTOM_CSS,
        "custom.js": _CUSTOM_JS,
    }
        ...
        if not os.path.exists(path):
            try:
                with open(path, "w", encoding="utf-8") as f:
                    f.write(content)
                log.info(f"[{PLUGIN_ID}] Created {name}")
            except Exception as e:
                log.warning(f"[{PLUGIN_ID}] Could not create {name}: {e}")
...
def _migrate_dr_v3(db):
    ...
    try:
        ...
        """)
        db.execute(
            "INSERT INTO netapp_dr_plans_new SELECT id,name,COALESCE(dr_site_id,''),state,notes,last_failover_at,last_test_at,created_by,created_at,updated_at FROM netapp_dr_plans"
        )
    ...
    try:
        ...
        if "sync_host" in cols:
            ...
            for tbl in [
                ...
                with contextlib.suppress(Exception):
                    db.execute(f"DROP TABLE IF EXISTS {tbl}")  # nosec: B608 - table names are from a hardcoded migration allowlist
...
def _init_db():
    """Creates plugin tables in the central ProxmoxVEx.db (idempotent).
    ...
    try:
        ...
        # v1.1: recovery bind
        _add_column_if_missing(db, "netapp_provisioned_datastores", "imported_from", "TEXT NOT NULL DEFAULT ''")
        _add_column_if_missing(
            db, "netapp_provisioned_datastores", "snapinfo_initialized", "INTEGER NOT NULL DEFAULT 0"
        )
        _add_column_if_missing(
            db, "netapp_provisioned_datastores", "snapinfo_lv_name", "TEXT NOT NULL DEFAULT 'netapp_snapmanifest'"
        )
        ...
        _add_column_if_missing(
            db, "netapp_volume_mapping", "snapinfo_lv_name", "TEXT NOT NULL DEFAULT 'netapp_snapmanifest'"
        )
...
def _add_column_if_missing(db, table, column, col_def):
    try:
        rows = db.query(f"PRAGMA table_info({table})")  # nosec: B608 - table names are from hardcoded migration calls
        existing = {r["name"] for r in rows}
        if column not in existing:
            db.execute(f"ALTER TABLE {table} ADD COLUMN {column} {col_def}")  # nosec: B608 - table/column names and definitions are from hardcoded migration calls
...
def _h_config():
    """GET — Return the integration's raw config.json (or empty defaults)."""
    ...
    try:
        ...
        log.error(f"[{PLUGIN_ID}] config read failed: {e}")
        return jsonify({"error": "Failed to read config"}), 500


def _h_save_config():
    """POST — Persist the integration's raw config.json."""
    ...
    try:
        ...
        log.error(f"[{PLUGIN_ID}] config write failed: {e}")
        return jsonify({"error": "Failed to write config"}), 500


def register(app):
    """Called by ProxmoxVEx when the plugin is activated."""
    _ensure_custom_files()
    _init_db()

    register_native_route(PLUGIN_ID, "config", _h_config)
    register_native_route(PLUGIN_ID, "config/save", _h_save_config)
    ...
    from .api.schedules import register_routes as reg_schedules
    from .api.schedules import start_scheduler
    ...
    # Serve the management UI under /api/netapp_storage/ui
    def _h_ui():
        from flask import send_file

        return send_file(os.path.join(PLUGIN_DIR, "ui.html"))

    register_native_route(PLUGIN_ID, "ui", _h_ui)

    log.info(f"[PLUGINS] {PLUGIN_NAME} registriert (UI: /api/netapp_storage/ui)")
