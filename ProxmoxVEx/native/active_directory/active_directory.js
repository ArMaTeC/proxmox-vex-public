...
(async function () {
  ...
  // to mirror how plugin i18n is handled.
  let _parentI18n = null;
  try { _parentI18n = window.parent.ProxmoxVExI18n; } catch (e) { }
  function t(key) {
    if (_parentI18n) return _parentI18n.t(key, { ns: 'active_directory' });
    return key;
  }
  function tf(key) {
    let s = t(key);
  ...
  if (_parentI18n) {
    await _parentI18n.loadPluginNamespaceFull('active_directory', '/api/native/active_directory/i18n');
  }
  applyI18n();
  ...
  // Load initial tab.
  loadData('overview', '/api/active_directory/overview');
})();
