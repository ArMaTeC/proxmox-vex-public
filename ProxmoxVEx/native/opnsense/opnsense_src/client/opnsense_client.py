...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/native/opnsense/opnsense_src/client/opnsense_client.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: OPNsense HTTP client.
        ...
        *,
        ...
        for i in range(attempts):
            try:
                ...
                last_exc = e
                if i == attempts - 1:
                    raise OPNsenseTimeoutError(f"{method} {path} failed after {attempts} attempt(s): {e}") from e
            ...
            if not resp.ok:
                raise OPNsenseError(f"{method} {path} → HTTP {resp.status_code}: {resp.text[:200]}")

            try:
                return resp.json()
            except ValueError as e:
                raise OPNsenseError(f"{method} {path} → non-JSON response: {resp.text[:200]}") from e
