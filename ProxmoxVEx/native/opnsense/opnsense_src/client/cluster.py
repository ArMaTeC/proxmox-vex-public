...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/native/opnsense/opnsense_src/client/cluster.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: Cluster-aware wrapper over two `OPNsenseClient`s.
...
if TYPE_CHECKING:
    from opnsense_src.collectors.carp import CarpStatus
...
class OPNsenseClusterClient:
    def __init__(
        self, client_a: OPNsenseClient, client_b: OPNsenseClient, name_a: str = "NODOA", name_b: str = "NODOB"
    ) -> None:
        self.a = client_a
        self.b = client_b
        self.name_a = name_a
    ...
    def _ensure_probes(self) -> None:
        ...
        from opnsense_src.collectors.carp import collect_carp_status
    ...
    def master_side(self) -> Side:
        ...
        """
        self._ensure_probes()
        if not (self._probe_a is not None and self._probe_b is not None):
