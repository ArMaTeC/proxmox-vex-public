...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/native/opnsense/opnsense_src/collectors/certificates.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: Certificate collector — surfaces expiry-relevant fields.
...
def _parse_days_to(valid_to: str) -> int:
    ...
    if not valid_to:
        return 0
    try:
        dt = datetime.strptime(valid_to.strip(), "%b %d %H:%M:%S %Y %Z").replace(tzinfo=timezone.utc)
    except ValueError:
        # Tolerate inputs without explicit TZ
        try:
            dt = datetime.strptime(valid_to.strip()[:24], "%b %d %H:%M:%S %Y").replace(tzinfo=timezone.utc)
        except ValueError:
            return 0
