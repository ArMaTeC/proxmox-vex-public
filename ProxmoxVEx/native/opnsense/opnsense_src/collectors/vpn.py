...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/native/opnsense/opnsense_src/collectors/vpn.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: VPN collectors — WireGuard / IPsec / OpenVPN session...
...
def collect_wireguard(client: OPNsenseClient) -> tuple[bool, list[VPNPeer]]:
    ...
    enabled = _bool01(((general or {}).get("general", {}) or {}).get("enabled", 0))
    show = client.get("/api/wireguard/service/show")
    rows = show.get("rows", []) if isinstance(show, dict) else []
    peers = [_peer("wireguard", r, ("name", "instance"), ("endpoint", "endpoint_address")) for r in rows]
...
def collect_openvpn(client: OPNsenseClient) -> list[VPNPeer]:
    ...
    rows = payload.get("rows", []) if isinstance(payload, dict) else []
    return [_peer("openvpn", r, ("description", "common_name", "id"), ("real_address", "virtual_addr")) for r in rows]
