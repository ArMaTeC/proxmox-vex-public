...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/native/opnsense/opnsense_src/writers/dhcp.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: DHCPv4 (Kea) reservation CRUD writer.
    ...
    def create(self, payload: DhcpReservationInput) -> DhcpResult:
        ...
        with TimedAction() as t:
            ...
            try:
                ...
                self._record(
                    "dhcp_reservation.create", payload.hostname, "error", t, f"apply failed → rolled back: {e}"
                )
                return DhcpResult(ok=False, detail=f"apply failed → rolled back: {e}")
        sync = self._maybe_sync()
        entry = self._record(
            "dhcp_reservation.create",
            uuid,
            "ok",
            t,
            payload.description,
