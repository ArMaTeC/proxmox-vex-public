...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/native/opnsense/opnsense_src/writers/wireguard_peer.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: WireGuard peer (client) CRUD writer.
...
class WireguardPeerWriter:
    ...
    def create(self, payload: WireguardPeerInput) -> WireguardPeerResult:
        ...
        entry = self._record(
            "wgpeer.create", uuid, "ok", t, payload.name, payload_sha256=hash_payload(payload.to_payload())
        )
