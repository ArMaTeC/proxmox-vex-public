...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/native/opnsense/opnsense_src/writers/unbound.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: Unbound DNS host- and domain-override CRUD.
...
class UnboundWriter:
    ...
    def create(self, payload: UnboundHostInput) -> UnboundResult:
        ...
        entry = self._record(
            "unbound.create", uuid, "ok", t, payload.description, payload_sha256=hash_payload(payload.to_payload())
        )
...
class UnboundDotWriter:
    ...
    def create(self, payload: UnboundDotInput) -> UnboundResult:
        ...
        sync = self._maybe_sync()
        entry = self._record(
            "unbound_dot.create", uuid, "ok", t, payload.description, payload_sha256=hash_payload(payload.to_payload())
        )
        return UnboundResult(ok=True, uuid=uuid, sync=sync, audit=entry)
