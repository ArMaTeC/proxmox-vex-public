...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/native/opnsense/opnsense_src/writers/alias.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: Firewall alias CRUD writer.
...
class AliasWriter:
    ...
    def create(self, payload: AliasInput) -> AliasResult:
        ...
        entry = self._record(
            "alias.create", payload.name, "ok", t, f"uuid={uuid}", payload_sha256=hash_payload(payload.to_payload())
        )
    ...
    def update(self, uuid: str, payload: AliasInput) -> AliasResult:
        ...
        sync = self._maybe_sync()
        entry = self._record(
            "alias.update", uuid, "ok", t, payload.name, payload_sha256=hash_payload(payload.to_payload())
        )
        return AliasResult(ok=True, uuid=uuid, sync=sync, audit=entry)
