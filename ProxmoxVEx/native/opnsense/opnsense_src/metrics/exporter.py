...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/native/opnsense/opnsense_src/metrics/exporter.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: Prometheus text exposition for the OPNsense plugin.
...
`prometheus_client` library — the plugin runs inside ProxmoxVEx whose
...
def render_metrics(client: OPNsenseClient, host_label: str | None = None) -> str:
    ...
    for i in ifaces:
        out.append(
            _line("opnsense_iface_tx_bytes_total", i["sent_bytes"], {**common, "iface": i["name"], "label": i["label"]})
        )
    out.extend(_section("opnsense_iface_rx_errors_total", "counter", "Total receive errors per interface"))
    for i in ifaces:
        out.append(_line("opnsense_iface_rx_errors_total", i["received_errors"], {**common, "iface": i["name"]}))
    out.extend(_section("opnsense_iface_tx_errors_total", "counter", "Total transmit errors per interface"))
    for i in ifaces:
        out.append(_line("opnsense_iface_tx_errors_total", i["send_errors"], {**common, "iface": i["name"]}))
    out.extend(_section("opnsense_iface_drops_total", "counter", "Dropped packets per interface"))
    for i in ifaces:
        out.append(_line("opnsense_iface_drops_total", i["dropped_packets"], {**common, "iface": i["name"]}))
    out.extend(_section("opnsense_iface_up", "gauge", "1 if the interface is up+running"))
    for i in ifaces:
        out.append(_line("opnsense_iface_up", 1 if i["is_up"] else 0, {**common, "iface": i["name"]}))
    ...
    for g in gws:
        out.append(_line("opnsense_gateway_rtt_seconds", g["delay_ms"] / 1000.0, {**common, "gw": g["name"]}))
    out.extend(_section("opnsense_gateway_loss_ratio", "gauge", "Gateway monitor loss ratio (0..1)"))
    for g in gws:
        out.append(_line("opnsense_gateway_loss_ratio", g["loss_pct"] / 100.0, {**common, "gw": g["name"]}))
    out.extend(_section("opnsense_gateway_up", "gauge", "1 if the gateway monitor reports Online"))
    for g in gws:
        out.append(_line("opnsense_gateway_up", 1 if g["is_up"] else 0, {**common, "gw": g["name"]}))

    # ---- services ----
    svc = collect_services(client)
    out.extend(_section("opnsense_service_running", "gauge", "1 if the service is running"))
    for s in svc["items"]:
        out.append(_line("opnsense_service_running", 1 if s["running"] else 0, {**common, "service": s["id"]}))

    # ---- certs ----
    certs = collect_certificates(client)
    out.extend(
        _section("opnsense_cert_expiry_seconds", "gauge", "Seconds until the certificate expires (negative if past)")
    )
    for c in certs:
        out.append(
            _line(
                "opnsense_cert_expiry_seconds", c["days_to_expiry"] * 86400, {**common, "cert": c["name"] or c["uuid"]}
            )
        )
    ...
    out.append(_line("opnsense_vpn_peers_total", len(vpn["wireguard_peers"]), {**common, "type": "wireguard"}))
    out.append(_line("opnsense_vpn_peers_total", len(vpn["ipsec_phase1"]), {**common, "type": "ipsec"}))
    out.append(_line("opnsense_vpn_peers_total", len(vpn["openvpn_sessions"]), {**common, "type": "openvpn"}))
