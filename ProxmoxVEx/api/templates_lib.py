...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/api/templates_lib.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: Cloud-Init Template Library
...
Curated catalog of common cloud images. ProxmoxVEx automates the standard
...
def _current_user():
    """Return the logged-in username string. ProxmoxVEx stores the username
...
def _update_dep(dep_id, **fields):
    ...
    try:
        ...
        if not sets:
            return
        vals.append(dep_id)
        query = f"UPDATE cloud_init_deployments SET {', '.join(sets)} WHERE id = ?"  # nosec: B608 - fields from allowlist
...
def _run_deploy(dep_id, cluster_id, node, template_id, storage, vmid, vm_name):
    ...
    if not mgr or not tpl:
        _update_dep(dep_id, status="failed", error="cluster or template missing", finished_at=_now_iso())
        return

    img_basename = tpl["image_url"].rsplit("/", 1)[-1]
    img_path = f"{REMOTE_TMP_DIR}/ProxmoxVEx-ci-{template_id}-{uuid.uuid4().hex[:8]}-{img_basename}"
    ...
    try:
        ...
        if not ssh:
            _update_dep(dep_id, status="failed", error="SSH connect failed", finished_at=_now_iso())
            return
        ...
        # Every interpolated string goes through shlex.quote.
        ...
        # 1. download
        # Nc skip if exists, -q quiet output. show-progress would flood log
        ...
        run(
            f"qm set {vmid_i} --scsihw virtio-scsi-pci --scsi0 {scsi_arg} --ide2 {ide_arg} --boot c --bootdisk scsi0",
            "qm set",
        )
        ...
        _update_dep(dep_id, status="failed", error=str(e)[:500], log_append=f"FAILED: {e}", finished_at=_now_iso())
    finally:
        try:
            if ssh:
                ssh.close()
...
def add_custom_template():
    ...
    # Parse the URL properly and reject anything that's not a
    ...
    # (CodeAnt SSRF) - the char check above blocks shell metachars but NOT SSRF;
    ...
    try:
        ...
        logging.warning("[templates_lib] image_url rejected by SSRF guard: %s", _se)
        return jsonify({"error": "image_url rejected by SSRF guard"}), 400

    # Keep IDs predictable + URL-safe so frontend keys don't break
    import re
    import secrets
...
def delete_custom_template(tpl_id):
    ...
    try:
        ...
        logging.exception("handler error in templates_lib.py")
...
def deploy(cluster_id):
    ...
    # Defense-in-depth: shlex.quote in _run_deploy is the seatbelt,
    ...
    try:
        ...
        c.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, 'queued', 0, '', '', ?, ?)
        """,
            (dep_id, cluster_id, node, template_id, tpl["name"], vmid, storage, user, _now_iso()),
        )
        get_db().conn.commit()
    except Exception:
        logging.exception("db insert failed")
...
def deployment_status(dep_id):
    ...
    try:
        ...
        if not r:
            return jsonify({"error": "not found"}), 404
        d = dict(r)
        # May 2026: tenant ACL - dep_id is opaque to clients but a holder of
        ...
        ok, _ = check_cluster_access(d.get("cluster_id"))
        if not ok:
            return _access_denied()
        return jsonify(d)
    except Exception:
