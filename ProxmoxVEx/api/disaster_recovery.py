...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/api/disaster_recovery.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
...
summary stats are written. Frontend polls `/api/disaster-recoveries/<id>` until
finished and renders pass/warn/fail counts + per-check detail; PDF export
uses generateProxmoxVExPDF for the evidence document.
"""
...
bp = Blueprint("disaster_recovery", __name__)
...
def _record_check(drill_id, category, name, status, message, detail="", duration_ms=0, seq=0):
    try:
        ...
        c.execute(
            """INSERT INTO disaster_recovery_drill_checks
        ...
        logging.warning(f"[disaster-recovery] _record_check failed: {e}")
...
class _Runner:
    ...
    def check(self, category, name, fn):
        ...
        try:
            ...
            detail = ""
            logging.exception(f"[disaster-recovery] check {category}/{name} raised")
...
def _execute_drill(drill_id):
    ...
    on disaster_recovery_drills row so polling endpoint can return progress."""
    db = get_db()
    c = db.conn.cursor()
    c.execute("SELECT * FROM disaster_recovery_drills WHERE id = ?", (drill_id,))
    drow = c.fetchone()
    if not drow:
        return
    drill = dict(drow)
    ...
    if not prow:
        ...
        c.execute(
            "UPDATE disaster_recovery_drills SET status='failed', finished_at=?, summary=? WHERE id=?",
    ...
    runner.check(
        "plan",
        "has_vms",
        lambda: ("pass" if vms else "fail", f"{len(vms)} VM(s) in plan" if vms else "plan has no VMs", ""),
    )
    runner.check(
        "plan",
        "has_target_cluster",
        lambda: (
            "pass" if plan.get("target_cluster") else "fail",
    ...
    def _src():
        ...
        if not getattr(src_mgr, "is_connected", False):
            return (
                "warn",
                "source cluster not connected — DR scenario is plausible, but we cannot verify plan against live source",
    ...
    def _ram():
        ...
        try:
            ...
            free = sum(d.get("mem_total", 0) - d.get("mem_used", 0) for d in ns.values() if d.get("status") == "online")
        except Exception as e:
        ...
        if src_mgr and getattr(src_mgr, "is_connected", False):
            try:
                for r in src_mgr.get_vm_resources() or []:
                    if r.get("type") in ("qemu", "lxc") and any(
                        int(v.get("vmid", -1)) == int(r.get("vmid", -1)) for v in vms
                    ):
                        need += int(r.get("maxmem", 0) or 0)
            except Exception:
                pass
        if need == 0:
            return ("warn", "could not determine plan RAM footprint", f"target_free_gb={free / (1024**3):.1f}")
        margin_gb = (free - need) / (1024**3)
        if margin_gb < 0:
            return (
                "fail",
                f"target short by {-margin_gb:.1f} GB RAM",
                f"need_gb={need / (1024**3):.1f}, free_gb={free / (1024**3):.1f}",
            )
        if margin_gb < 4:
            return (
                "warn",
                f"tight: only {margin_gb:.1f} GB headroom after failover",
                f"need_gb={need / (1024**3):.1f}, free_gb={free / (1024**3):.1f}",
            )
        return (
            "pass",
            f"{margin_gb:.1f} GB RAM headroom on target",
            f"need_gb={need / (1024**3):.1f}, free_gb={free / (1024**3):.1f}",
        )
    ...
    def _net():
        ...
        if not net_maps:
            return (
                "warn",
                "no network mappings configured (VMs keep original bridge names — risky if names differ)",
                "",
            )
        if not (tgt_mgr and getattr(tgt_mgr, "is_connected", False)):
        ...
        if missing:
            return (
                "fail",
                f"{len(missing)} network mapping(s) point at non-existent target bridge: {', '.join(missing[:5])}",
                f"all_targets={sorted(tgt_bridges)}",
            )
        return ("pass", f"all {len(net_maps)} network mapping(s) resolve on target", f"mappings={net_maps}")
    ...
    def _stor():
        ...
        return ("pass", f"all {len(stor_maps)} storage mapping(s) valid", f"mappings={stor_maps}")
    ...
    def _repl():
        ...
        for vm in vms:
            ...
            rpo = 3600  # Default 1h RPO; could become per-VM in the schema later
            if age > rpo * 2:
        ...
        msg = (
            "all replication jobs fresh"
            if worst_status == "pass"
            else ("replication has warnings" if worst_status == "warn" else "replication has errors")
        )
    ...
    def _boot():
        ...
        if gaps:
            return (
                "warn",
                f"boot groups have gaps: {groups}",
    ...
    def _meta():
        plan_blob = json.dumps(
            {
                "id": plan["id"],
                "name": plan.get("name"),
                "src": plan["source_cluster"],
                "tgt": plan["target_cluster"],
                "net": plan.get("network_mappings"),
                "stor": plan.get("storage_mappings"),
        ...
        return ("pass", f"plan checksum recorded · executor: {executor}", f"sha256_prefix={sha}\nrun_at={run_at}")
    ...
    try:
        c.execute(
            """UPDATE disaster_recovery_drills SET finished_at=?, status=?, summary=?,
        ...
        c.execute("UPDATE site_recovery_plans SET last_test = ? WHERE id = ?", (finished_at, plan_id))
        db.conn.commit()
    except Exception:
        logging.exception("[disaster-recovery] finalize failed")
    ...
    try:
        ...
        log_audit(
            drill.get("started_by") or "system",
            "dr.drill_completed",
...
def _require_plan_access(plan_row):
    """DR-drill endpoints only had a role perm and never checked
...
@bp.route("/api/site-recovery/plans/<plan_id>/disaster-recovery", methods=["POST"])
...
def start_drill(plan_id):
    ...
    try:
        c.execute(
            """INSERT INTO disaster_recovery_drills (id, plan_id, plan_name, started_at, status, started_by)
                     VALUES (?, ?, ?, ?, 'running', ?)""",
    ...
    log_audit(user, "disaster_recovery.drill_started", f"plan_id={plan_id} drill_id={drill_id}")

    t = threading.Thread(target=_execute_drill, args=(drill_id,), daemon=True, name=f"disaster-recovery-{drill_id}")
    t.start()
    return jsonify({"drill_id": drill_id, "status": "running"})


@bp.route("/api/disaster-recoveries/<drill_id>", methods=["GET"])
...
def get_drill(drill_id):
    try:
        ...
        c.execute("SELECT * FROM disaster_recovery_drill_checks WHERE drill_id = ? ORDER BY sequence ASC", (drill_id,))
...
@bp.route("/api/site-recovery/plans/<plan_id>/disaster-recoveries", methods=["GET"])
@require_auth(perms=["site_recovery.view"])
    ...
    try:
        ...
        c.execute(
            """SELECT id, started_at, finished_at, status, summary,
                     ...
                     rpo_breach_seconds, estimated_rto_seconds
                     FROM disaster_recovery_drills WHERE plan_id = ? ORDER BY started_at DESC LIMIT 50""",
            (plan_id,),
        )
