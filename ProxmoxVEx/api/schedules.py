...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/api/schedules.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: scheduler + update schedule routes - split from...
# Docs:        https://proxmoxvex.local/docs
# Generated:   2026-09-04
# --------------------------------------------------------------------
# --- ProxmoxVEx auto-header end ---
"""scheduler + update schedule routes - split from monolith dec 2025"""
...
from ProxmoxVEx.api.nodes import cleanup_deleted_scripts, cleanup_orphaned_excluded_vms
...
def _perm_for_action(action):
    # Scheduling an action requires the same permission as running it by hand
    return {
        "start": "vm.start",
        "stop": "vm.stop",
        "shutdown": "vm.stop",
...
def load_schedules():
    ...
    try:
        ...
        actions = []
        last_id = 0
        # (#337): name + vm_type are persisted via columns added in the
...
def save_schedules(schedules):
    ...
    try:
        ...
        # (#337): include name + vm_type - previously dropped silently
        ...
        try:
            ...
            for action in schedules.get("actions", []):
                ...
                if schedule_time == current_time:
                    ...
                    elif schedule_type == "weekends" and current_day in ["saturday", "sunday"]:
                        should_run = True
            ...
            # Check for scheduled rolling updates
            try:
            ...
            if current_time == "03:00":
                try:
                    ...
                    # Cleanup orphaned excluded VMs (VMs that no longer exist)
...
def execute_scheduled_action(action):
    ...
    This is basically the same as the manual action endpoints
    but called from the scheduler

    Added rolling_update for automatic node updates
    """
    ...
    try:
        # Handle rolling_update action type separately
        ...
        # Log the action
        log_audit(
            "scheduler", f"scheduled.{action_type}", f"Scheduled {action_type} executed on VM {vmid} in {cluster_id}"
        )
...
def execute_scheduled_rolling_update(mgr, cluster_id: str, action: dict):
    ...
    This runs the same rolling update logic but triggered by scheduler
    """
    try:
        ...
        if hasattr(mgr, "_rolling_update") and mgr._rolling_update and mgr._rolling_update.get("status") == "running":
            logging.warning("[SCHEDULER] Rolling update already in progress, skipping")
            return
        ...
        if not nodes_to_update:
            logging.warning("[SCHEDULER] No nodes available for update")
            return
        ...
        mgr._rolling_update = {
            ...
            "pause_on_evacuation_error": False,
            "force_all": False,
        ...
        def run_scheduled_update():
            try:
                for idx, node_name in enumerate(nodes_to_update):
                        ...
                        try:
                            ...
                            if not mgr.get_node_apt_updates(node_name):
                                mgr._rolling_update["logs"].append(
                                    f"[{time.strftime('%H:%M:%S')}] {node_name} up-to-date, skipping"
                                )
                    ...
                    if not skip_evacuation:
                        ...
                        while waited < evacuation_timeout:
                            if node_name in mgr.nodes_in_maintenance:
                                ...
                                if task.status == "completed":
                                    ...
                                    mgr._rolling_update["logs"].append(
                                        f"[{time.strftime('%H:%M:%S')}] ⚠️ Evacuation: {getattr(task, 'migrated_vms', 0)}/{getattr(task, 'total_vms', 0)} migrated, {len(fv)} failed - continuing"
                                    )
                                    evacuation_ok = True
                                    break
                        ...
                        if not evacuation_ok:
                            mgr._rolling_update["logs"].append(
                                f"[{time.strftime('%H:%M:%S')}] ✗ Evacuation failed on {node_name}, skipping"
                            )
                    ...
                    if update_task:
                        ...
                        if update_task.status == "completed":
                            ...
                            if include_reboot:
                                ...
                                if wait_for_reboot:
                                    mgr._rolling_update["logs"].append(
                                        f"[{time.strftime('%H:%M:%S')}] Waiting for {node_name} to reboot..."
                                    )
                                    ow = 0
                                    while ow < 120:
                                        try:
                                    ...
                                    if not came_back:
                                        mgr._rolling_update["logs"].append(
                                            f"[{time.strftime('%H:%M:%S')}] ⚠ {node_name} did not come back online within 600s"
                                        )
                                else:
                                    mgr._rolling_update["logs"].append(
                                        f"[{time.strftime('%H:%M:%S')}] {node_name} rebooting (wait_for_reboot=False)"
                                    )
                        else:
                            mgr._rolling_update["logs"].append(
                                f"[{time.strftime('%H:%M:%S')}] ✗ {node_name} update failed"
                            )
                            mgr._rolling_update["failed_nodes"].append({"node": node_name, "error": "Update failed"})
                    # Exit_maintenance_mode now retries internally;
                    ...
                    if not mgr.exit_maintenance_mode(node_name):
                        mgr._rolling_update["logs"].append(
                            f"[{time.strftime('%H:%M:%S')}] ⚠ {node_name} maintenance exit failed — will retry at end"
                        )

                # Final sweep: any node still flagged as in
                ...
                if stuck_nodes:
                    mgr._rolling_update["logs"].append(
                        f"[{time.strftime('%H:%M:%S')}] Cleanup: {len(stuck_nodes)} node(s) still in maintenance, retrying..."
                    )
                    time.sleep(15)
                    for nn in stuck_nodes:
                        if mgr.exit_maintenance_mode(nn):
                            ...
                            mgr._rolling_update["logs"].append(
                                f"[{time.strftime('%H:%M:%S')}] ✗ {nn} STILL in maintenance — manual intervention needed"
                            )
                            mgr._rolling_update["failed_nodes"].append({
                                "node": nn,
...
def get_schedules():
    ...
    # (CodeAnt IDOR) - use the real access model. The old filter read the raw
...
def create_schedule():
    ...
    # Verify tenant has access to this cluster
    ok, _ = check_cluster_access(data["cluster_id"])
    if not ok:
...
def update_schedule(schedule_id):
    ...
    # Verify tenant has access to this schedule's cluster
    ok, _ = check_cluster_access(schedule.get("cluster_id", ""))
    if not ok:
        return jsonify({"error": "Access denied to this cluster"}), 403
    ...
    log_audit(request.session.get("user", "system"), "schedule.updated", f"Updated schedule ID {schedule_id}")
...
def delete_schedule(schedule_id):
    ...
    log_audit(request.session.get("user", "system"), "schedule.deleted", f"Deleted schedule ID {schedule_id}")
...
# Automatic rolling update scheduling (SQLite storage)
# ============================================
...
def save_update_schedule(cluster_id: str, schedule: dict, user: str = "system"):
    ...
    try:
        ...
        # Use INSERT OR REPLACE for older SQLite compatibility
        cursor.execute(
            """
...
def set_update_schedule(cluster_id):
    ...
    log_audit(
        usr,
        "update.schedule",
        f"Update schedule {'enabled' if schedule['enabled'] else 'disabled'} for {mgr.config.name}",
...
def delete_update_schedule(cluster_id):
    ...
    try:
        ...
        mgr = cluster_managers[cluster_id]
        log_audit(
            usr, "update.schedule.deleted", f"Update schedule deleted for {mgr.config.name}", cluster=mgr.config.name
        )

        return jsonify({"success": True})
    except Exception as e:
        _ = safe_error(e, "Schedule operation failed")
        return jsonify({"error": "Schedule operation failed"}), 500
    ...
    try:
        ...
        for cluster_id, schedule in schedules.items():
            ...
            if is_correct_day and is_correct_time:
                ...
                if (
                    hasattr(mgr, "_rolling_update")
                    and mgr._rolling_update
                    and mgr._rolling_update.get("status") == "running"
                ):
                    continue
