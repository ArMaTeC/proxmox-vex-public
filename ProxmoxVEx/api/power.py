...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/api/power.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: Power & Carbon Tracking
...
def _row_to_rates(r):
    return {
        "cluster_id": html.escape(r["cluster_id"]),
        ...
        "currency": html.escape(r["currency"] or "EUR"),
        "notes": html.escape(r["notes"] or ""),
        "updated_at": r["updated_at"],
        "updated_by": html.escape(r["updated_by"] or ""),
    }
...
def _compute_per_vm(snapshots, mgr, rates, hours_window):
    ...
    for _ts, cd in snapshots:
        ...
        {n: [] for n in nodes}
        for _, v in (cd.get("vms") or {}).items():
            if not v.get("r"):
...
def get_one(cluster_id):
    # (CodeAnt IDOR) - tenant gate (skip the __default__ pseudo-cluster, no owner)
...
def upsert(cluster_id):
    ...
    try:
        f = {
            k: float(body.get(k, _DEFAULT[k]))
            for k in ("node_idle_w", "node_max_w", "mem_w_per_gb", "pue", "kwh_price", "kg_co2_per_kwh")
        }
...
def cluster_summary(cluster_id):
    ...
    if not snaps:
        return jsonify({"enough_data": False, "cluster_id": html.escape(cluster_id), "rates": rates, "days": days})
    ...
    return jsonify({
        ...
        "by_node": {
            k: {
                "kwh": round(v["kwh"] * factor, 2),
                "cost": round(v["cost"] * factor, 2),
                "kg_co2": round(v["kg_co2"] * factor, 2),
            }
