...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/api/webauthn.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: WebAuthn / FIDO2 hardware token support.
...
Apr 2026: second-factor only, no passwordless flow (user explicitly asked for
...
from ProxmoxVEx.utils.auth import validate_session

# 2026-06-04 (CWE-117): username comes from a login attempt = attacker-
...
try:
    ...
    AttestedCredentialData = None  # type: ignore  # keeps name resolution happy when lib missing
...
# Challenges live in memory, keyed by (username, ceremony).
...
def _get_rp_or_error():
    ...
    if _is_ip_literal(host):
        return None, (
            jsonify({
                ...
                "Open ProxmoxVEx via its hostname (e.g. https://ProxmoxVEx.local:5000 or "
    ...
    return PublicKeyCredentialRpEntity(name="ProxmoxVEx", id=host), None
...
def register_begin():
    ...
    user_handle = existing_row["user_handle"] if existing_row else secrets.token_bytes(32)
    ...
    try:
        ...
        db.execute(
            ...
            (
                ...
                bytes(matched_cred.credential_id),
            ),
        )
    except Exception as e:
...
def is_available():
    ...
    show a helpful banner when the user is accessing ProxmoxVEx by IP."""
    host = _effective_host()
