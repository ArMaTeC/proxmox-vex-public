...
Settings JSON shape varies per type:
  ...
  http_json   : {headers: {...}, label_field: 'ProxmoxVEx_audit'}
  splunk_hec  : {token: '...', sourcetype: 'ProxmoxVEx', index: 'main'}
  elastic     : {username: '', password: '', index: 'ProxmoxVEx-audit'}
  generic     : {headers: {...}}
...
def _row_to_target(row):
    try:
        ...
        settings = {}
    # (CodeAnt data-exposure) - SIEM target settings can hold auth secrets
...
def _to_syslog_5424(event, app="ProxmoxVEx", facility="local0"):
...
def _verify_tls_for(target):
    """settings.verify_tls — default True (Audit fix M-4)."""
    return bool((target.get("settings") or {}).get("verify_tls", True))


# 2026-05-31 (CodeAnt siem.py:224/226): dedup set so the opt-out warning
...
def _warn_tls_downgrade(url):
    ...
    logging.warning(
        ...
        "SIEMs inside a trusted network.",
        host,
    )


def _http_post(url, body_bytes, headers, timeout=10, verify_tls=True):
    """Plain urllib HTTPS POST.  TLS verification on by default;
    admins opt out per target if they're shipping to a self-signed SIEM.

    SSRF guard: reject URLs aimed at internal/metadata IPs
    ...
    try:
        ...
        raise RuntimeError(f"SIEM endpoint rejected: {exc}") from None

    req = urllib.request.Request(url, data=body_bytes, method="POST", headers=headers)
    # 2026-05-31 (CodeAnt siem.py:224/226): ssl.create_default_context()
...
def _send_splunk_hec(target, event):
    ...
    payload = {
        ...
        "sourcetype": settings.get("sourcetype", "ProxmoxVEx:audit"),
        "source": "ProxmoxVEx",
    }
    if settings.get("index"):
        payload["index"] = settings["index"]
    _http_post(target["endpoint"], json.dumps(payload).encode("utf-8"), headers, verify_tls=_verify_tls_for(target))


def _send_elastic(target, event):
    settings = target.get("settings") or {}
    index = settings.get("index", "ProxmoxVEx-audit")
...
def _send_generic(target, event):
    ...
    _http_post(
        target["endpoint"],
        json.dumps(_to_json_line(event)).encode("utf-8"),
        headers,
...
def list_targets():
    try:
        ...
        logging.exception("handler error in siem.py")
...
def create_target():
    ...
    try:
        ...
        c.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
            (tid, name, typ, endpoint, fmt, enabled, json.dumps(settings), datetime.now().isoformat(), _current_user()),
        )
        get_db().conn.commit()
...
def update_target(tid):
    ...
    try:
        c = get_db().conn.cursor()
        query = f"UPDATE siem_targets SET {', '.join(fields)} WHERE id = ?"  # nosec: B608 - fields from allowlist
...
def test_target(tid):
    ...
    try:
        ...
        evt = {
            ...
            "user": _current_user() or "ProxmoxVEx",
            "action": "siem.test",
            "severity": "info",
            "cluster": "",
            "ip_address": request.remote_addr or "",
            "details": f"Test event from ProxmoxVEx for target {target['name']}",
        }
...
def types():
    ...
    return jsonify({
        "types": [
            ...
            {
                "id": "http_json",
                "label": "HTTP JSON (Loki / generic)",
                "endpoint_hint": "https://loki:3100/loki/api/v1/push",
                "settings_fields": ["headers", "verify_tls"],
            },
            {
                "id": "splunk_hec",
                "label": "Splunk HEC",
                "endpoint_hint": "https://splunk:8088/services/collector",
                "settings_fields": ["token", "sourcetype", "index", "verify_tls"],
            },
            {
                "id": "elastic",
                "label": "Elasticsearch",
                "endpoint_hint": "https://es:9200",
                "settings_fields": ["username", "password", "index", "verify_tls"],
            },
            {
                "id": "generic",
                "label": "Generic webhook",
                "endpoint_hint": "https://example.com/audit",
                "settings_fields": ["headers", "verify_tls"],
            },
        ]
    })
