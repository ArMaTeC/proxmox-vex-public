...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/api/nodes.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: node management, updates, SMBIOS & scripts routes -...
# Docs:        https://proxmoxvex.local/docs
# Generated:   2026-09-04
# --------------------------------------------------------------------
# --- ProxmoxVEx auto-header end ---
"""node management, updates, SMBIOS & scripts routes - split from monolith dec 2025"""
...
from ProxmoxVEx.utils.auth import load_users, require_auth, verify_password
from ProxmoxVEx.utils.sanitization import sanitize_log_message as _sl  # CWE-117

bp = Blueprint("nodes", __name__)

REMOTE_TMP_DIR = os.environ.get("PROXMOXVEX_REMOTE_TMP", "/tmp")  # nosec: B108 - default only, operator can override with env var
...
def get_node_ip_api(cluster_id, node):
    ...
    Tries multiple methods to get the node's IP:
    ...
    cluster_host, _cluster_port = mgr.host, mgr.api_port
    node_ip = None
    source = None

    # (PR #324): let manager._get_node_ip do the heavy lifting
    ...
    return jsonify({"ip": node_ip, "node": node, "source": source})
...
# Node-name regex (mirrors vms.py:2718 hardening). RFC-1035-ish
...
# Per-NIC error/drop counters (SSH /proc/net/dev).
...
# Cluster health: corosync ring + pvecm quorum + service state.
...
# Lm-sensors readings (CPU temp, fan rpm, voltages).
...
# Per-core CPU usage from /proc/stat via SSH.
# Returns instantaneous per-core percentages so the UI can overlay individual
# core lines on the Performance Metrics CPU chart.
@bp.route("/api/clusters/<cluster_id>/nodes/<node>/cpu-cores", methods=["GET"])
@require_auth(perms=["node.view"])
def get_node_cpu_cores_api(cluster_id, node):
    ok, _ = check_cluster_access(cluster_id)
    if not ok:
        return jsonify({"error": "Access denied to this cluster"}), 403
    bad, code = _reject_bad_node(node)
    ...
    if cluster_id not in cluster_managers:
        return jsonify({"error": "Cluster not found"}), 404
    result = cluster_managers[cluster_id].get_node_cpu_cores(node)
    if isinstance(result, dict) and result.get("error"):
        # 200 so the frontend can parse d.error and show the real SSH failure
        # (502 caused the browser to only show "Bad Gateway").
        return jsonify(result), 200
    return jsonify(result)


# Per-core CPU history for the Performance Metrics chart. Reads persisted 5-min
# metrics_history snapshots (off-hub, cached) so it matches the other historical
# charts without live SSH on every page load.
@bp.route("/api/clusters/<cluster_id>/nodes/<node>/cpu-cores-history", methods=["GET"])
@require_auth(perms=["node.view"])
def get_node_cpu_cores_history_api(cluster_id, node):
    ok, _ = check_cluster_access(cluster_id)
    if not ok:
        return jsonify({"error": "Access denied to this cluster"}), 403
    bad, code = _reject_bad_node(node)
    if bad is not None:
        return bad, code
    from ProxmoxVEx.background.metrics import load_metrics_history
    ...
    minutes = {"hour": 60, "day": 1440, "week": 10080, "month": 43200, "year": 525600}.get(timeframe, 60)
    try:
        ...
        for snap in hist.get("snapshots") or []:
            ...
            if ts_dt < cutoff:
                continue
            cluster = (snap.get("clusters") or {}).get(cluster_id) or {}
            cores = (cluster.get("nodes") or {}).get(node, {}).get("cpu_cores")
            if not cores:
                continue
            series.append({"ts": int(ts_dt.timestamp()), "cores": cores})
        series.sort(key=lambda x: x["ts"])
        # Build per-core time series arrays
        timestamps = [s["ts"] for s in series]
        core_keys = sorted(
            {k for s in series for k in s["cores"]},
            key=lambda k: int(k.replace("cpu", "")) if k.startswith("cpu") and k[3:].isdigit() else 9999,
        )
        metrics = {k: [s["cores"].get(k, 0) for s in series] for k in core_keys}
        return jsonify({"timestamps": timestamps, "metrics": metrics, "count": len(timestamps)})
    except Exception:
        logging.exception("cpu-cores history read failed")
        return jsonify({"error": "cpu-cores history read failed"}), 500
...
def get_node_temperature_history_api(cluster_id, node):
    ...
    try:
        ...
        return jsonify({"error": "history read failed"}), 500
    series.sort(key=lambda x: x["ts"])
...
def get_node_task_log_api(cluster_id, node, upid):
    ...
    Fixed Dec 2025 - frontend expects { log: "..." } format
    """
    ok, _ = check_cluster_access(cluster_id)
    if not ok:
        return jsonify({"error": "Access denied to this cluster"}), 403
...
# Automatically sets SMBIOS data on new VMs for Windows licensing etc.
# This was surprisingly tricky to get right, Proxmox smbios format is picky
# =============================================================================


# 2026-04-24 - sudo detection is shared across SMBIOS helpers. We query once per
...
def _ssh_sudo_prefix(ssh):
    ...
    cached = getattr(ssh, "_ProxmoxVEx_sudo_prefix", None)
    ...
    with contextlib.suppress(Exception):
        ssh._ProxmoxVEx_sudo_prefix = prefix
    return prefix


def _ssh_run_checked(ssh, cmd, timeout=30):
    """Run a command via SSH, raise on non-zero exit.  used to be
    ...
    2026-04-24 - auto-prefix with sudo when login isn't root (ProxmoxVEx@pam)."""
...
def _ssh_write_file(ssh, path, content, mode=None):
    """Write file via SSH. Works for both root and non-root (ProxmoxVEx@pam + sudo) logins.
    ...
    2026-04-24 - ProxmoxVEx@pam + NOPASSWD sudo deployments
    """
    prefix = _ssh_sudo_prefix(ssh)
    ...
    if not prefix:
        ...
        try:
            ...
            if rc != 0:
                err = stderr.read().decode("utf-8", errors="replace").strip()
                raise RuntimeError(f"write {path} failed (rc={rc}): {err or 'unknown'}") from None
        ...
        tmp_path = f"{REMOTE_TMP_DIR}/ProxmoxVEx-stage-{_uuid.uuid4().hex[:12]}"
        q_tmp = shlex.quote(tmp_path)
        try:
            sftp = ssh.open_sftp()
            with sftp.file(tmp_path, "w") as f:
                f.write(content)
            sftp.chmod(tmp_path, 0o750)  # nosec: B103 - 0o750 is restrictive
            sftp.close()
        except OSError as e:
            ...
            rc = stdout.channel.recv_exit_status()
            if rc != 0:
                raise RuntimeError(f"staging {tmp_path} failed (rc={rc})") from None
...
Deployed by ProxmoxVEx - automatically configures SMBIOS for new VMs
...
# Configuration - set by ProxmoxVEx when deployed
...
# Paths
LOG_FILE = "/var/log/ProxmoxVEx-smbios.log"
PROCESSED_VMS_FILE = "/var/lib/ProxmoxVEx-smbios-processed.txt"  # keeps track of what we already did
...
def main():
    log_message("=== ProxmoxVEx SMBIOS Auto-Configurator started ===")
...
Description=ProxmoxVEx SMBIOS Auto-Configurator
After=pve-cluster.service
Wants=pve-cluster.service

[Service]
Type=simple
ExecStart=/usr/bin/python3 /opt/ProxmoxVEx-smbios-autoconfig.py
Restart=always
RestartSec=10
...
def get_smbios_autoconfig(cluster_id):
    ...
    try:
        ...
        # defaults if nothing configured - Proxmox doesnt allow underscores so no spaces either
        mgr = cluster_managers[cluster_id]
        settings = getattr(mgr.config, "smbios_autoconfig", None) or {
            "enabled": False,
            "manufacturer": "Proxmox",
            "product": "ProxmoxVExManagment",
            "version": "v1",
            "family": "ProxmoxVE",
        }
        ...
        _ = safe_error(e, "Failed to get SMBIOS config")
        return jsonify({"error": "Failed to get SMBIOS config"}), 500
...
def update_smbios_autoconfig(cluster_id):
    ...
    # Feb 2026: Validate SMBIOS values to prevent template injection
    for key in ["manufacturer", "product", "version", "family"]:
        val = data.get(key, "")
        if val and not re.match(r"^[a-zA-Z0-9 ._-]{1,64}$", val):
            return jsonify({
                "error": f"Invalid {key}: only alphanumeric, spaces, dots, hyphens allowed (max 64 chars)"
            }), 400
    ...
    usr = getattr(request, "session", {}).get("user", "system")
    log_audit(usr, "smbios_autoconfig.updated", "SMBIOS auto-config updated", cluster=mgr.config.name)
...
def get_smbios_autoconfig_status(cluster_id, node):
    ...
    try:
        # (PR #324): one real resolver instead of 3 ad-hoc
        # cluster/status resolvers. cluster/status can point at the corosync IP.
        node_ip = mgr._get_node_ip(node)
        if not node_ip:
            return jsonify({
                "installed": False,
                "running": False,
                "error": f"Could not determine SSH-reachable IP for node {node}",
            })
        ...
        if not ssh:
            return jsonify({
                "installed": False,
                "running": False,
                "error": "SSH not available - check SSH key in cluster settings",
            })

        # Check if script exists
        stdin, stdout, stderr = ssh.exec_command("test -f /opt/ProxmoxVEx-smbios-autoconfig.py && echo exists")
        installed = "exists" in stdout.read().decode()

        # Check if service is running
        stdin, stdout, stderr = ssh.exec_command(
            "systemctl is-active ProxmoxVEx-smbios-autoconfig 2>/dev/null || echo inactive"
        )
        ...
        stdin, stdout, stderr = ssh.exec_command(
            'tail -5 /var/log/ProxmoxVEx-smbios.log 2>/dev/null || echo "No logs yet"'
        )
        logs = stdout.read().decode().strip()

        ssh.close()

        return jsonify({"installed": installed, "running": running, "status": status, "logs": logs})

    except Exception as e:
        logging.error(f"Error checking SMBIOS autoconfig status: {e}")
        _ = safe_error(e, "Failed to get node status")
        return jsonify({"installed": False, "running": False, "error": "Failed to get node status"})
...
def deploy_smbios_autoconfig(cluster_id, node):
    ...
    try:
        ...
        # Retry SSH 3x, transient failures shouldn't kill the whole deploy
        ...
        # Generate script with settings (defense-in-depth: strip quotes/backslashes - Feb 2026)
        ...
        script = SMBIOS_SCRIPT_TEMPLATE.format(
            ...
            product=_sanitize_smbios(settings.get("product", "ProxmoxVExManagment")),
        ...
        _ssh_write_file(ssh, "/opt/ProxmoxVEx-smbios-autoconfig.py", script, 0o750)
        _ssh_write_file(ssh, "/etc/systemd/system/ProxmoxVEx-smbios-autoconfig.service", SMBIOS_SERVICE_TEMPLATE)
        ...
        _ssh_run_checked(ssh, "rm -f /var/lib/ProxmoxVEx-smbios-processed.txt")
        _ssh_run_checked(ssh, "systemctl daemon-reload")
        _ssh_run_checked(ssh, "systemctl enable ProxmoxVEx-smbios-autoconfig")
        _ssh_run_checked(ssh, "systemctl restart ProxmoxVEx-smbios-autoconfig")

        # confirm it actually became active (systemctl start/restart can succeed even when unit fails)
        stdin, stdout, stderr = ssh.exec_command("systemctl is-active ProxmoxVEx-smbios-autoconfig")
        active = stdout.read().decode("utf-8", errors="replace").strip()
        stdout.channel.recv_exit_status()
        if active != "active":
            # grab last log lines for context
            stdin, stdout, stderr = ssh.exec_command(
                "journalctl -u ProxmoxVEx-smbios-autoconfig -n 10 --no-pager 2>/dev/null | tail -10"
            )
        ...
        _ = safe_error(e, "SMBIOS deploy failed")
        return jsonify({"error": "SMBIOS deploy failed"}), 500
    finally:
        if ssh:
...
def remove_smbios_autoconfig(cluster_id, node):
    ...
    try:
        ...
        # for ProxmoxVEx@pam deployments (2026-04-24).
        commands = [
            "systemctl stop ProxmoxVEx-smbios-autoconfig 2>/dev/null || true",
            "systemctl disable ProxmoxVEx-smbios-autoconfig 2>/dev/null || true",
            "rm -f /etc/systemd/system/ProxmoxVEx-smbios-autoconfig.service",
            "rm -f /opt/ProxmoxVEx-smbios-autoconfig.py",
            "rm -f /var/lib/ProxmoxVEx-smbios-processed.txt",
        ...
        _ = safe_error(e, "SMBIOS removal failed")
        return jsonify({"error": "SMBIOS removal failed"}), 500
...
def control_smbios_autoconfig(cluster_id, node):
    ...
    try:
        ...
        # Rescan = nuke the processed list and restart, forces re-check of all VMs
        if action == "rescan":
            cmd = "rm -f /var/lib/ProxmoxVEx-smbios-processed.txt && systemctl restart ProxmoxVEx-smbios-autoconfig"
        else:
            cmd = f"systemctl {action} ProxmoxVEx-smbios-autoconfig"
        ...
        _ = safe_error(e, "SMBIOS service control failed")
        return jsonify({"error": "SMBIOS service control failed"}), 500
...
def get_smbios_autoconfig_status_all(cluster_id):
    ...
    try:
        ...
        for node_name in node_names:
            ...
            try:
                ...
                try:
                    # Check if script exists
                    stdin, stdout, stderr = ssh.exec_command(
                        "test -f /opt/ProxmoxVEx-smbios-autoconfig.py && echo exists"
                    )
                    installed = "exists" in stdout.read().decode()
                    ...
                    results[node_name] = {"installed": installed, "running": running, "status": status}
                finally:
                    ssh.close()
        ...
        _ = safe_error(e, "Failed to get SMBIOS status")
        return jsonify({"error": "Failed to get SMBIOS status"}), 500
...
def deploy_smbios_autoconfig_all(cluster_id):
    ...
    try:
        ...
        return jsonify({"error": "Could not get cluster nodes"}), 500
    ...
    script = SMBIOS_SCRIPT_TEMPLATE.format(
        ...
        product=_sanitize_smbios_val(settings.get("product", "ProxmoxVExManagment")),
    ...
    for node in nodes:
        ...
        ssh = None
        try:
            # Staggered connections to prevent SSH server overload
            ...
            # Clear processed list + checked systemctl commands
            _ssh_run_checked(ssh, "rm -f /var/lib/ProxmoxVEx-smbios-processed.txt")
            _ssh_run_checked(ssh, "systemctl daemon-reload")
            _ssh_run_checked(ssh, "systemctl enable ProxmoxVEx-smbios-autoconfig")
            _ssh_run_checked(ssh, "systemctl restart ProxmoxVEx-smbios-autoconfig")
            ...
            _ = safe_error(e, "SMBIOS deploy failed")
            results.append({"node": node, "success": False, "error": "SMBIOS deploy failed"})
        finally:
            if ssh:
                with contextlib.suppress(BaseException):
                    ssh.close()
    ...
    log_audit(
        usr,
        "smbios_autoconfig.deployed_all",
        f"SMBIOS auto-config deployed to {success_count}/{len(nodes)} nodes",
...
# One-click install of the StarWind x Proxmox SAN Integration plugin (the
...
def _safe_repo_url(u, default):
    ...
    # (CodeAnt SSRF) - this URL is curl'd / apt-fetched inside a ROOT bash script on
...
# ProxmoxVEx — install the StarWind x Proxmox SAN plugin (starlvm) on this node.
set -uo pipefail
...
)  # Root apt-install on every node + caller-overridable repo/key → admin-only (not node.maintenance, which tenant_operator holds)
def install_starlvm_plugin(cluster_id):
    ...
    try:
        ...
        logging.exception("Invalid repository or key URL: %s", e)
        return jsonify({"error": "Invalid repository or key URL"}), 400
    force = "1" if body.get("force") else "0"
    only = set(body.get("nodes") or [])

    script = (
        STARLVM_INSTALL_SCRIPT
    ...
    def _install_one(node):
        ...
        try:
            ...
            script_path = f"{REMOTE_TMP_DIR}/ProxmoxVEx-starlvm-{uuid.uuid4().hex[:12]}.sh"
            _ssh_write_file(ssh, script_path, script, 0o750)
            out, _e = _ssh_run_checked(ssh, f"bash {shlex.quote(script_path)}", timeout=200)
            with contextlib.suppress(Exception):
                ssh.exec_command(f"rm -f {shlex.quote(script_path)}")
            ...
            logging.exception("StarWind install failed on %s", node)
            return {"node": node, "success": False, "error": "install failed"}
        finally:
            if ssh:
                with contextlib.suppress(Exception):
                    ssh.close()

    from ProxmoxVEx.utils.concurrent import run_per_node

    res_map = run_per_node(dict.fromkeys(nodes, _install_one), max_concurrent=8, timeout=240)
...
def starlvm_plugin_status(cluster_id):
    ...
    def _probe(node):
        ...
        try:
            ...
            _ = safe_error(e, "probe failed")
            return {"node": node, "reachable": False, "error": "probe failed"}
        finally:
            if ssh:
                with contextlib.suppress(Exception):
                    ssh.close()
    ...
    res_map = run_per_node(dict.fromkeys(nodes, _probe), max_concurrent=8, timeout=60)
...
# Run custom .sh/.py scripts on cluster nodes with permission control
# =============================================================================
...
def get_custom_scripts(cluster_id):
    ...
    try:
        ...
        scripts = db.query(
            "SELECT * FROM custom_scripts WHERE cluster_id = ? AND deleted_at IS NULL ORDER BY name", (cluster_id,)
        )
        return jsonify([dict(s) for s in scripts] if scripts else [])
    except Exception as e:
        logging.error(f"Error loading scripts: {e}")
        _ = safe_error(e, "Failed to load scripts")
    ...
    try:
        ...
        for script in deleted:
            ...
            log_audit(
                "system",
                "script.purged",
                f"Permanently deleted script '{script['name']}' after 20-day retention",
                cluster=script["cluster_id"],
            )
        if deleted:
...
def cleanup_orphaned_excluded_vms():
    ...
    This runs daily to clean up stale entries from balancing_excluded_vms
    ...
    try:
        ...
        for entry in excluded_entries:
            ...
            if cluster_id not in cluster_managers:
                ...
                cursor.execute(
                    "DELETE FROM balancing_excluded_vms WHERE cluster_id = ? AND vmid = ?", (cluster_id, vmid)
                )
                removed_count += 1
                continue
...
def create_custom_script(cluster_id):
    ...
    cluster_name = cluster_name.config.name if hasattr(cluster_name, "config") else cluster_id

    log_audit(
        usr,
        "script.created",
        f"Created script '{data.get('name')}' (ID: {script_id}, Type: {script_type})",
...
def update_custom_script(cluster_id, script_id):
    ...
    script = db.query_one(
        "SELECT * FROM custom_scripts WHERE id = ? AND cluster_id = ? AND deleted_at IS NULL", (script_id, cluster_id)
    )
    if not script:
    ...
    log_audit(
        usr,
        "script.updated",
        f"Updated script '{data.get('name', script['name'])}' (ID: {script_id})",
        cluster=cluster_name,
    )
...
def delete_custom_script(cluster_id, script_id):
    ...
    log_audit(
        usr,
        "script.deleted",
        f"Soft-deleted script '{script['name']}' (ID: {script_id}) - will be purged in 20 days",
        cluster=cluster_name,
    )
...
def run_custom_script(cluster_id, script_id):
    ...
    if not stored_salt or not stored_hash or not verify_password(password, stored_salt, stored_hash):
        ...
        log_audit(
            usr,
            "script.run_denied",
            f"Failed password verification for script execution (ID: {script_id})",
            cluster=cluster_name,
        )
    ...
    log_audit(
        usr,
        "script.execution_started",
        f"Starting execution of script '{script['name']}' (ID: {script_id}) on {len(nodes_to_run)} nodes: {', '.join(nodes_to_run)}",
        cluster=cluster_name,
    )
    ...
    # Was a serial `for node in nodes_to_run` loop. With 15-node
    ...
    def _exec_on_node(node):
        ...
        try:
            ...
            script_path = f"{REMOTE_TMP_DIR}/ProxmoxVEx-script-{script_id}-{uuid.uuid4().hex[:12]}{script_ext}"
            sftp = ssh.open_sftp()
            with sftp.file(script_path, "w") as f:
                f.write(script["content"])
            sftp.chmod(script_path, 0o750)  # nosec: B103 - 0o750 is restrictive
            sftp.close()

            stdin, stdout, stderr = ssh.exec_command(f"{interpreter} {shlex.quote(script_path)} 2>&1", timeout=300)
    ...
    raw = run_per_node(
        dict.fromkeys(nodes_to_run, _exec_on_node),
        max_concurrent=8,
    ...
    log_audit(
        usr,
        "script.executed",
        f"Script '{script['name']}' completed: {success_count}/{len(nodes_to_run)} nodes succeeded ({status})",
        cluster=cluster_name,
    )
...
def get_script_output(cluster_id, script_id):
    ...
    db = get_db()
    script = db.query_one(
        "SELECT name, last_run, last_status, last_output FROM custom_scripts WHERE id = ? AND cluster_id = ? AND deleted_at IS NULL",
...
def get_deleted_scripts(cluster_id):
    ...
    try:
        ...
        _ = safe_error(e, "Failed to load deleted scripts")
        return jsonify({"error": "Failed to load deleted scripts"}), 500
...
def restore_deleted_script(cluster_id, script_id):
    ...
    db = get_db()
    script = db.query_one(
        "SELECT * FROM custom_scripts WHERE id = ? AND cluster_id = ? AND deleted_at IS NOT NULL",
        (script_id, cluster_id),
    )
    ...
    log_audit(
        usr, "script.restored", f"Restored deleted script '{script['name']}' (ID: {script_id})", cluster=cluster_name
    )
...
def enable_pool_ha_api(cluster_id):
    ...
    result = mgr.enable_pool_ha(
        heartbeat_srs=data.get("heartbeat_srs"), host_failures_to_tolerate=int(data.get("host_failures_to_tolerate", 1))
    )
