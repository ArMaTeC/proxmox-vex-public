...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/api/insights.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: Right-sizing + Capacity Forecasting endpoints
# Docs:        https://proxmoxvex.local/docs
# Generated:   2026-09-04
# --------------------------------------------------------------------
# --- ProxmoxVEx auto-header end ---
"""
Right-sizing + Capacity Forecasting endpoints
...
from ProxmoxVEx.api.helpers import check_cluster_access, load_metrics_window, safe_error
...
def _load_history(cluster_id, days=30):
    ...
    2026-06-04: fetch+parse happen off-hub + cached/single-flighted in
...
def _linear_regression(xs, ys):
    ...
    May 2026 (#374) - added R² so callers can distinguish a real trend from
...
def right_sizing(cluster_id):
    ...
    for _ts, cd in history:
        ...
        for vmid, m in vms.items():
            entry = by_vm.setdefault(vmid, {"cpu": [], "mem": [], "running": 0, "meta": {}, "type": m.get("t", "qemu")})
            if m.get("r"):
                entry["running"] += 1
    ...
    for vmid, e in by_vm.items():
        ...
        if cpu_avg < 5 and cpu_p95 < 25 and maxcpu >= 2:
            ...
            flags.append({
                ...
                "detail": f"avg {cpu_avg}% / p95 {cpu_p95}% on {maxcpu} cores",
                "current": maxcpu,
                "recommended": maxcpu + max(1, maxcpu // 4),
        ...
        if slope is not None and slope_per_day is not None:
            if slope_per_day >= 0.05 and current < threshold:
                ...
                if seconds_to_threshold and seconds_to_threshold > 0:
                    ...
                    # May 2026 (#374) - gate "warning"/"critical" on actual
                    ...
                    if eta_days < 7:
                        status = "critical" if is_real_trend else "trending_up"
                    elif eta_days < 30:
                        status = "warning" if is_real_trend else "trending_up"
                    else:
                        status = "trending_up"
...
# Top-N noisy neighbors. Pure aggregator over /cluster/resources,
...
def rollups(cluster_id):
    # Per-tag / per-pool aggregation over /cluster/resources.
    ...
    # Defense-in-depth - bound the rollup payload. Fleets with many
...
def long_term_history(cluster_id):
    ...
    2026-06-03 (#456): existing right-sizing / forecast / rollups
    ...
    Query params:
      days     int  default 7,  max retention_days (= ProxmoxVEx_METRICS_RETENTION_DAYS)
...
def force_snapshot():
    ...
    try:
        from ProxmoxVEx.background.metrics import collect_metrics_snapshot, save_metrics_snapshot
        ...
        _ = safe_error(e, "Failed to force metrics snapshot")
        return jsonify({"ok": False, "error": "Failed to force metrics snapshot"}), 500
