...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/api/push.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: Web Push (browser notifications)
...
import time
import urllib.parse
import urllib.request  # Module-level `class _NoRedirect(urllib.request...)` needs
...
_VAPID_SUBJECT = "mailto:admin@ProxmoxVEx.local"
...
def _current_user():
    """Return the logged-in username string. ProxmoxVEx stores user in
...
def _load_vapid():
    ...
    (audit fix M-1) - private key is now stored encrypted at rest
...
def _vapid_jwt(audience: str) -> str:
    ...
    if not isinstance(priv, ec.EllipticCurvePrivateKey):
        raise AssertionError("VAPID private key must be an EC key")
        ...
        with _opener.open(req, timeout=10) as resp:
            ...
            if 200 <= code < 300:
                ...
                try:
                    c = get_db().conn.cursor()
                    c.execute(
                        "UPDATE push_subscriptions SET failures = 0, last_used_at = ? WHERE id = ?",
...
def _alert_handler(alert_data: dict):
    ...
    *all* admin users (since alerts aren't user-scoped today) and sends
    ...
    try:
        title = alert_data.get("alert_name") or "ProxmoxVEx Alert"
        ...
        # (CodeAnt exploitation - cross-tenant alert leak) - a cluster-scoped alert
        ...
        for u in users:
            if cid:
                # (CodeAnt re-scan - fail-open fix) - a subscriber whose user record
...
def _is_internal_or_metadata_host(host):
    ...
    (audit fix M-12) - even though wake-up pushes carry no body,
...
def subscribe():
    ...
    # (audit fix M-12) - endpoint URL validation. Must be HTTPS,
    ...
    try:
        ...
        logging.exception("Push operation failed: %s", e)
        return jsonify({"error": "Push operation failed"}), 500
...
def list_subs():
    ...
    try:
        c = get_db().conn.cursor()
        c.execute(
            """SELECT id, endpoint, user_agent, created_at, last_used_at, failures
                     FROM push_subscriptions WHERE username = ? ORDER BY created_at DESC""",
            (user,),
        )
        out = []
        for r in c.fetchall():
...
def send_test():
    ...
    if not user:
        return jsonify({"error": "session missing"}), 401
    _push_to_inbox(
        user, "ProxmoxVEx — Test Push", "If you see this, browser notifications are working.", "info", "/", "test-push"
    )
    _wake_user(user)
    return jsonify({"ok": True})
...
def inbox_clear():
    ...
    try:
        c = get_db().conn.cursor()
        c.execute(
            "UPDATE push_inbox SET read_at = ? WHERE username = ? AND read_at IS NULL",
