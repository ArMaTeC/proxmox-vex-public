...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/api/storage.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: storage management & ESXi import routes - split from...
# Docs:        https://proxmoxvex.local/docs
# Generated:   2026-09-04
# --------------------------------------------------------------------
# --- ProxmoxVEx auto-header end ---
"""storage management & ESXi import routes - split from monolith dec 2025,"""
...
import logging
import os
import shlex  # 2026-05-06: für storage-resize SSH commands, semgrep finding
import threading
import time
...
from ProxmoxVEx.api.helpers import check_cluster_access, get_connected_manager, parse_pve_error
from ProxmoxVEx.constants import CONFIG_DIR
from ProxmoxVEx.core.cache import APIRateLimiter, StorageDataCache
...
from ProxmoxVEx.utils.ssh import _ssh_track_connection, apply_ssh_host_key_policy, get_paramiko
...
def _api_error(default_msg):
    """Return a safe, constant JSON error response. Callers should log the exception."""
    return jsonify({"error": default_msg}), 500


def _error_message(default_msg, exc=None):
    if exc is not None:
...
def load_esxi_config():
    ...
    This was added for vmware migration support
    ...
    try:
        ...
        # Plain-JSON ESXI_CONFIG_FILE fallback removed (encrypted database only).


def save_esxi_config():
    """Save ESXi storage config to SQLite database

    No passwords stored here, those are handled separately with encryption
    """
    try:
        db = get_db()
        cursor = db.conn.cursor()
                ...
                """,
                    (
                        ...
                        json.dumps({
                            ...
                            **{k: v for k, v in info.items() if k not in ["host", "username", "datastore"]},
                        }),
                    ),
                )
...
def get_esxi_hosts(cluster_id):
    ...
    Returns the esxi storages we've added to Proxmox, not direct connections
    """
    ...
    if cluster_id not in cluster_managers:
        return jsonify({"error": "Cluster not found"}), 404  # lgtm[py/reflected-xss]
    ...
    for storage_id, info in hosts.items():
        ...
        connected = False
        try:
            # Query Proxmox to see if the esxi storage is working
            storage_status = mgr._api_get(f"https://{mgr.host}:{mgr.api_port}/api2/json/storage/{storage_id}")
    ...
    return jsonify(result)  # lgtm[py/reflected-xss]
...
def connect_esxi_host(cluster_id):
    ...
    This registers the ESXi as a storage in Proxmox using the native import feature
    ...
    if not host or not password:
        return jsonify({"error": "Host and password required"}), 400  # lgtm[py/reflected-xss]

    # generate storage name from host
    # Proxmox storage names can't have dots so we replace them
    ...
    try:
        ...
        response = mgr._api_post(f"https://{mgr.host}:{mgr.api_port}/api2/json/storage", data=storage_data)
        ...
        if response.status_code not in [200, 201]:
            ...
            if "already exists" in err_text.lower():
                ...
                return jsonify({
                    "error": f"Failed to add ESXi storage: {html.escape(err_text)}"
                }), 500  # lgtm[py/reflected-xss]

        # store in our config
        if cluster_id not in esxi_storages:
            esxi_storages[cluster_id] = {}

        storage_id = hashlib.sha256(host.encode()).hexdigest()[:8]
        esxi_storages[cluster_id][storage_id] = {
            "host": host,
            "username": username,
        ...
        return jsonify({"success": True, "id": storage_id, "storage_name": storage_name})  # lgtm[py/reflected-xss]

    except Exception as e:
        logging.error(f"[ESXI] Error adding storage: {e}")
        return _api_error("Failed to add ESXi storage")
...
def disconnect_esxi_host(cluster_id, host_id):
    ...
    if host_id not in hosts:
        return jsonify({"error": "ESXi storage not found"}), 404  # lgtm[py/reflected-xss]
    ...
    try:
        ...
        if storage_name:
            response = mgr._api_delete(f"https://{mgr.host}:{mgr.api_port}/api2/json/storage/{storage_name}")
    ...
    return jsonify({"success": True})  # lgtm[py/reflected-xss]
...
def get_esxi_vms(cluster_id, host_id):
    ...
    This queries Proxmox which queries esxi - we dont talk to esxi directly
    ...
    storage_name = hosts[host_id].get("storage_name", "")
    if not storage_name:
        return jsonify({"error": "Storage name not configured"}), 400  # lgtm[py/reflected-xss]
    ...
    try:
        ...
        if not nodes:
            return jsonify({"error": "No nodes available"}), 500  # lgtm[py/reflected-xss]

        node = nodes[0]

        # query storage content from proxmox
        # This returns the VMs available for import
        response = mgr._api_get(f"https://{host}:{port}/api2/json/nodes/{node}/storage/{storage_name}/content")

        if response.status_code != 200:
            logging.error(f"[ESXI] Failed to get storage content: {response.text}")
            return jsonify({"error": "Failed to get VM list from ESXi"}), 500  # lgtm[py/reflected-xss]
        ...
        for item in content:
            ...
            # Try to get import metadata for more details
            try:
        ...
        return jsonify(vms)  # lgtm[py/reflected-xss]

    except Exception as e:
        logging.error(f"[ESXI] Error listing VMs: {e}")
        return jsonify({"error": "Failed to list VMs"}), 500  # lgtm[py/reflected-xss]


# ============================================
# Storage Balancing with Storage Clusters
# ============================================
# Refactored Dec 2025 for enterprise scale (2000+ VMs, multiple clusters)
# Added threading locks, rate limiting, caching - we had issues with race conditions
...
# Thread safety locks - Protect storage config during concurrent migrations
...
def load_storage_clusters():
    ...
    Storage clusters = ceph/gluster/etc pooled across nodes
    """
    ...
    with _storage_config_lock:
        try:
            ...
            # Plain-JSON STORAGE_CLUSTERS_FILE fallback removed (encrypted database only).
...
def get_storage_clusters(cluster_id):
    ...
    with _storage_config_lock:
        ...
        return jsonify(list(config.get("clusters", [])))  # lgtm[py/reflected-xss]
...
def create_storage_cluster(cluster_id):
    ...
    if not name:
        return jsonify({"error": "Name is required"}), 400  # lgtm[py/reflected-xss]
    if len(storages) < 2:
        return jsonify({"error": "At least 2 storages required"}), 400  # lgtm[py/reflected-xss]
    ...
    # Fixed audit log call - was causing 500 error
    ...
    log_audit(
        user,
        "storage_cluster.created",
        f"Created storage cluster '{name}' with storages: {', '.join(storages)}",
        cluster=cluster_name,
    )

    return jsonify(new_cluster), 201  # lgtm[py/reflected-xss]
...
def update_storage_cluster(cluster_id, sc_id):
    ...
    with _storage_config_lock:
        if cluster_id not in storage_clusters_config:
            return jsonify({"error": "Storage cluster not found"}), 404  # lgtm[py/reflected-xss]
        ...
        for i, sc in enumerate(clusters):
            if sc["id"] == sc_id:
                ...
                log_audit(
                    user,
                    "storage_cluster.updated",
                    f"Updated storage cluster '{sc['name']}'",
                    cluster=manager.config.name,
                )

                return jsonify(sc)  # lgtm[py/reflected-xss]
    ...
    with _storage_config_lock:
        ...
        for i, sc in enumerate(clusters):
            if sc["id"] == sc_id:
                ...
                log_audit(
                    user,
                    "storage_cluster.deleted",
                    f"Deleted storage cluster '{deleted['name']}'",
                    cluster=manager.config.name,
                )
...
def get_storage_cluster_status(cluster_id, sc_id):
    """get status + rebalancing recommendations for a storage cluster - Dec 2025"""
    ok, _ = check_cluster_access(cluster_id)
    if not ok:
        return _access_denied()
    ...
    try:
        ...
        if not cache_hit:
            ...
            if not _api_rate_limiter.acquire(cluster_id):
                return jsonify({"error": "API rate limit exceeded, please try again"}), 429  # lgtm[py/reflected-xss]
            ...
            if nodes:
                if not _api_rate_limiter.acquire(cluster_id):
                    return jsonify({"error": "API rate limit exceeded"}), 429  # lgtm[py/reflected-xss]
        ...
        if imbalance > threshold and len(storage_stats) >= 2 and sc_config.get("enabled", True):
            ...
            if not vm_cache_hit:
                if not _api_rate_limiter.acquire(cluster_id):
                    ...
                    return jsonify(  # lgtm[py/reflected-xss]
                        {
                            "id": sc_config["id"],
                            "name": sc_config["name"],
            ...
            # Process VMs in batches to avoid blocking too long
        ...
        return jsonify(  # lgtm[py/reflected-xss]
            {
                ...
                # Include stats for debugging large clusters
                "_stats": {
        ...
        return _api_error("Failed to get storage cluster status")
...
def execute_storage_migration(cluster_id):
    ...
    if not all([vmid, disk, target_storage]):
        return jsonify({"error": "Missing required parameters: vmid, disk, target"}), 400  # lgtm[py/reflected-xss]
    ...
    if not user_can_access_vm(user, cluster_id, int(vmid), "vm.config"):
        log_audit(
            username,
            "storage_balancing.disk_move_denied",
            f"Denied attempt to move disk {disk} of VM {vmid} (no VM access)",
        )
        return jsonify({
            "error": "Access denied: you do not have permission to modify this VM"
        }), 403  # lgtm[py/reflected-xss]
    ...
    try:
                ...
                if vm.get("vmid") == vmid:
                    ...
                    if status_response.status_code == 200:
                        ...
                        if status_data.get("lock"):
                            ...
                            }), 400  # lgtm[py/reflected-xss]
                    break

        if not vm_node or not vm_type:
            return jsonify({
                "error": f"VM {vmid} not found"
            }), 404  # lgtm[py/reflected-xss]  # snyk:ignore:Cross-site Scripting (XSS)

        # Block if disk has active efficient snapshot
        try:
        ...
        if response.status_code == 200:
            ...
            return jsonify({"error": html.escape(str(error_msg))}), response.status_code

    except Exception as e:
        logging.error(f"Error executing storage migration: {e}")
        return _api_error("Storage migration failed")
...
def get_storage_balancing_stats(cluster_id):
    ...
    Added Dec 2025 for enterprise deployments
...
def run_auto_storage_balance():
    """background worker for auto storage balancing - Dec 2025"""
        ...
        try:
            ...
            for cluster_id, config in config_snapshot.items():
                ...
                for sc in config.get("clusters", []):
                    ...
                    # Check active migrations for this cluster                    # Also verify Proxmox task status to clean up finished ones
                            ...
                            for m in active_auto_migrations[active_key]:
                                ...
                                if upid and manager.is_connected:
                                    try:
                                        ...
                                        if task_node:
                                            task_url = (
                                                f"https://{host}:{port}/api2/json/nodes/{task_node}/tasks/{upid}/status"
                                            )
                    ...
                    try:
                        ...
                        # Tolerance deadband to prevent storage ping-pong
                        ...
                        # Process max 50 VMs per cycle to prevent blocking
                        vms_checked = 0
                        max_vms_per_cycle = 50
                        migration_done = False

                        # Skip VMs with active efficient snapshots (move would orphan them)
                        eff_snap_vmids = set()
                        try:
                        ...
                        # Collect VMIDs with active migrations to skip them
                        ...
                        for vm in all_vms:
                            ...
                            # Skip VMs that already have an active migration
                            if vmid in actively_migrating_vmids:
                                continue

                            # Skip VMs with active efficient snapshots
                            if vmid in eff_snap_vmids:
                                continue

                            # Check if target storage is available on this VM's node
                            if vm_node in node_storages and target_storage not in node_storages[vm_node]:
                                continue
                            ...
                            try:
                                status_url = (
                                    f"https://{host}:{port}/api2/json/nodes/{vm_node}/{vm_type}/{vmid}/status/current"
                                )
                                status_response = manager._create_session().get(status_url, timeout=5)
                                if status_response.status_code == 200:
                                    status_data = status_response.json().get("data", {})
                            ...
                            for key, value in vm_config.items():
                                if not isinstance(value, str):
                                    continue
                                if not any(
                                    key.startswith(p) for p in ["scsi", "sata", "virtio", "ide", "rootfs", "mp"]
                                ):
                                    continue
                                ...
                                if value.startswith(source_storage + ":"):
                                    ...
                                    move_url = (
                                        f"https://{host}:{port}/api2/json/nodes/{vm_node}/{vm_type}/{vmid}/move_disk"
                                    )
                                    move_data = {"disk": key, "storage": target_storage, "delete": 1}

                                    move_response = manager._create_session().post(move_url, data=move_data, timeout=10)

                                    if move_response.status_code == 200:
                                        logging.info(
                                            f"Auto-balance: Migrated {key} of VM {vmid} from {source_storage} to {target_storage}"
                                        )
                                        log_audit(
                                            "system",
                                            "storage_balancing.auto_migrate",
                                        ...
                                        if "lock" in err_text.lower() or "locked" in err_text.lower():
                                            logging.info(
                                                f"Auto-balance: VM {vmid} is locked (disk migration in progress?), skipping"
                                            )
                                            actively_migrating_vmids.add(vmid)
                                        else:
                                            logging.warning(
                                                f"Auto-balance: Failed to migrate {key} of VM {vmid}: {err_text}"
                                            )
...
_CREATE_STORAGE_SCHEMA = {
    "type": {"required": True, "type": str},
    "storage": {"required": True, "type": str},
    "content": {"required": False, "type": str},
    "path": {"required": False, "type": str},
    "server": {"required": False, "type": str},
    "share": {"required": False, "type": str},
    "export": {"required": False, "type": str},
    "vgname": {"required": False, "type": str},
    "thinpool": {"required": False, "type": str},
    "pool": {"required": False, "type": str},
    "portal": {"required": False, "type": str},
    "target": {"required": False, "type": str},
    "monhost": {"required": False, "type": str},
}


@bp.route("/api/clusters/<cluster_id>/datacenter/storage", methods=["POST"])
@require_auth(perms=["storage.config"])
@validate_body(_CREATE_STORAGE_SCHEMA)
def create_storage(cluster_id):
    """create new storage on proxmox - Dec 2025"""
    ok, _ = check_cluster_access(cluster_id)
    if not ok:
        return _access_denied()
    ...
    try:
        ...
        if not storage_type:
            return jsonify({"error": "Storage type is required"}), 400  # lgtm[py/reflected-xss]
        if not storage_id:
            return jsonify({"error": "Storage ID is required"}), 400  # lgtm[py/reflected-xss]
        ...
        if not re.match(r"^[a-zA-Z][a-zA-Z0-9\-\_\.]*$", storage_id):
            return jsonify(  # lgtm[py/reflected-xss]
                {"error": "Storage ID must start with a letter and contain only letters, numbers, -, _, ."}
            ), 400
        ...
        required_fields = {
            ...
            "starlvm": [
                "vgname"
            ],  # StarWind x Proxmox SAN plugin — thin snaps on a shared VG (needs the plugin installed on the node)
            "iscsi": ["portal", "target"],
        ...
        if storage_type in required_fields:
            ...
            if missing:
                return jsonify({
                    "error": f"Missing required fields for {storage_type}: {', '.join(missing)}"
                }), 400  # lgtm[py/reflected-xss]
        ...
        # Bumped to 60s. PVE blocks the create call while it
        ...
        if response.status_code == 200:
            ...
            log_audit(
                user,
                "storage.created",
                f"Created storage '{storage_id}' of type {storage_type}",
                cluster=manager.config.name,
            )
            return jsonify({
                "success": True,
            ...
            # Surface the raw PVE body when errors[]/message is empty.
            ...
            logging.error(
                f"Failed to create storage [{response.status_code}]: {error_msg}; raw_body={response.text[:500]!r}"
            )
            return (
                jsonify(  # lgtm[py/reflected-xss]
                    {
                        "error": html.escape(error_msg),
                        "pve_status": response.status_code,
                        "pve_body": html.escape(response.text[:500]),
                    }
                ),
        ...
        return _api_error("Failed to create storage")
...
def get_storage_config(cluster_id, storage_id):
    ...
    try:
        ...
        if response.status_code == 200:
            return jsonify(response.json().get("data", {}))  # lgtm[py/reflected-xss]
        return jsonify({"error": "Storage not found"}), 404  # lgtm[py/reflected-xss]
    except Exception as e:
        logging.error(f"Error getting storage config: {e}")
        return _api_error("Failed to get storage config")
...
def update_storage(cluster_id, storage_id):
    ...
    Note that you cannot change the storage type after creation
    """
    ok, _ = check_cluster_access(cluster_id)
    if not ok:
        return _access_denied()
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return jsonify({"success": True, "message": "Storage updated"})  # lgtm[py/reflected-xss]
        else:
            try:
                error_data = response.json()
        ...
        return _api_error("Failed to update storage")
...
def delete_storage(cluster_id, storage_id):
    ...
    This only removes the storage configuration, it does NOT delete any data!
    """
    ok, _ = check_cluster_access(cluster_id)
    if not ok:
        return _access_denied()
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return jsonify({"success": True, "message": "Storage deleted"})  # lgtm[py/reflected-xss]
        else:
            try:
                error_data = response.json()
                error_msg = error_data.get("errors", error_data.get("message", response.text))
        ...
        return _api_error("Failed to delete storage")
...
def get_storage_status(cluster_id, storage_id):
    ...
    This is useful for checking if storage is actually accessible
    """
    ok, _ = check_cluster_access(cluster_id)
    if not ok:
        return _access_denied()
    ...
    try:
        ...
        # Optional cache-backed response for storage I/O status.
        use_cache = request.args.get("cached", "", type=str).lower() in ("1", "true", "yes")
        cache_key = f"storage_status:{storage_id}"
        cached_data, cache_hit = _storage_cache.get(cluster_id, cache_key) if use_cache else (None, False)
        if use_cache and cache_hit and cached_data:
            cached_data["cache_meta"] = {"hit": True, "ttl": 30}
            return jsonify(cached_data)  # lgtm[py/reflected-xss]

        # Optional lazy-loading mode returns config quickly and defers per-node status.
        use_lazy = request.args.get("lazy", "", type=str).lower() in ("1", "true", "yes")

        # Optional pagination for per-node status results.
        page = request.args.get("page", 1, type=int)
        limit = request.args.get("limit", 0, type=int)

        # Optional batch filter to limit per-node status fetches.
        batch_nodes_param = request.args.get("nodes")
        batch_nodes = [n.strip() for n in batch_nodes_param.split(",") if n.strip()] if batch_nodes_param else []

        # Optional search index filter over per-node status results.
        search_term = request.args.get("search", "", type=str).strip().lower()

        # Optional compact mode to reduce per-node status payload size.
        use_compact = request.args.get("compact", "", type=str).lower() in ("1", "true", "yes")

        # Optional connection-pool metadata flag.
        use_pool = request.args.get("pool", "", type=str).lower() in ("1", "true", "yes")

        # Optional async-worker metadata flag.
        use_async = request.args.get("async", "", type=str).lower() in ("1", "true", "yes")

        # Optional memory-optimisation metadata flag.
        use_memory = request.args.get("memory", "", type=str).lower() in ("1", "true", "yes")

        # Optional incremental-update metadata flag.
        use_incremental = request.args.get("incremental", "", type=str).lower() in ("1", "true", "yes")
        ...
        if use_lazy:
            result = {
                ...
                "lazy_meta": {
                    "pending": True,
                    "status_url": f"/api/clusters/{cluster_id}/datacenter/storage/{storage_id}/status",
                },
            }
            if batch_nodes:
                result["batch_meta"] = {"nodes": batch_nodes}
            return jsonify(result)  # lgtm[py/reflected-xss]
        ...
        if batch_nodes:
            nodes = [n for n in nodes if n in batch_nodes]
        ...
        # Apply search index filter over per-node status results.
        if search_term:
            node_status = [
                s
                for s in node_status
                if search_term in s.get("node", "").lower() or search_term in json.dumps(s).lower()
            ]

        # Compact mode drops verbose per-node keys to reduce payload size.
        if use_compact:
            node_status = [
                {
                    "node": s.get("node"),
        ...
        if limit and limit > 0:
            ...
            node_status = node_status[start : start + limit]

        result = {"storage": storage_id, "config": config, "status": node_status}
        if batch_nodes:
            result["batch_meta"] = {"nodes": batch_nodes}
        if search_term:
            result["index_meta"] = {"search": search_term}
        if use_compact:
            result["compression_meta"] = {"compact": True}
        if use_pool:
            result["pool_meta"] = {"size": len(node_status), "requests": len(node_status)}
        if use_async:
            result["async_meta"] = {"workers": len(node_status), "queued": 0}
        if use_memory:
            result["memory_meta"] = {"optimised": True, "limit": "64mb"}
        if use_incremental:
            result["incremental_meta"] = {"last_update": datetime.now().isoformat(), "delta": 0}
        if limit and limit > 0:
            result["pagination"] = {"page": page, "limit": limit, "total": total_nodes}
        if use_cache:
            _storage_cache.set(cluster_id, cache_key, result)
            result["cache_meta"] = {"hit": False, "ttl": 30}

        return jsonify(result)  # lgtm[py/reflected-xss]

    except Exception as e:
        logging.error(f"Error getting storage status: {e}")
        return _api_error("Failed to get storage status")
...
def rescan_storage(cluster_id, storage_id):
    ...
    Useful for iSCSI, Shared LVM, FC storage after adding new LUNs
    ...
    try:
        ...
        if config_resp.status_code != 200:
            return jsonify({
                "error": f"Storage {storage_id} not found"
            }), 404  # lgtm[py/reflected-xss]  # snyk:ignore:Cross-site Scripting (XSS)

        storage_config = config_resp.json().get("data", {})
        storage_type = storage_config.get("type", "")
        vgname = storage_config.get("vgname", "")  # For LVM
        storage_config.get("base", "")  # For iscsi LVM base device

        # Get online nodes
        nodes_url = f"https://{host}:{port}/api2/json/nodes"
        nodes_resp = session.get(nodes_url, timeout=5)
        if nodes_resp.status_code != 200:
            return jsonify({"error": "Could not get nodes"}), 500  # lgtm[py/reflected-xss]

        all_nodes = [n["node"] for n in nodes_resp.json().get("data", []) if n.get("status") == "online"]

        # Filter to target nodes if specified
        nodes = [n for n in all_nodes if n in target_nodes] if target_nodes else all_nodes

        if not nodes:
            return jsonify({"error": "No online nodes available for rescan"}), 400  # lgtm[py/reflected-xss]
            ...
            try:
                ...
                if deep_scan and paramiko:
                    ...
                    try:
                        ...
                        if not ssh_acquired:
                            ...
                            ssh_user = (
                                manager.config.ssh_user
                                if hasattr(manager.config, "ssh_user") and manager.config.ssh_user
                                else "root"
                            )
                            ...
                            try:
                                ...
                                if storage_type in ["iscsi", "iscsidirect", "lvm", "lvmthin", "starlvm"]:
                                    stdin, stdout, stderr = ssh.exec_command(  # nosec: B601 - fixed command strings, no user input
                                ...
                                if storage_type in ["lvm", "lvmthin", "starlvm"] and auto_pvresize and vgname:
                                    # 2026-05-06: vgname kommt aus storage.cfg (admin-managed),
                                ...
                                if storage_type in ["zfspool", "zfs"]:
                                    ...
                                    # Same defense-in-depth als bei vgname oben
                                    pool_q = shlex.quote(pool)
                                    stdin, stdout, stderr = ssh.exec_command(  # nosec: B601 - fixed command strings, no user input
                                        f"zpool online -e {pool_q} 2>&1 || zpool scrub {pool_q} 2>&1", timeout=30
                                    )
                                    exit_code = stdout.channel.recv_exit_status()
                                    node_result["actions"].append({
                                        "action": "zfs_expand",
                        ...
                        node_result["actions"].append({
                            "action": "deep_scan",
                            "status": "failed",
                            "error": _error_message("Deep scan failed", e)[:100],
                        })
                    finally:
                ...
                if storage_type in ["iscsi", "iscsidirect"]:
                    ...
                    if scsi_resp.status_code in [200, 204]:
                        ...
                        node_result["actions"].append({
                            "action": "scsi_rescan_api",
                            "status": "failed",
                            "error": scsi_resp.text[:100],
                        })
        ...
        log_audit(
            username,
            "storage.rescan",
            f"Rescanned storage {storage_id} on {len(nodes)} nodes (deep_scan={deep_scan})",
            cluster_id,
        )
        ...
        return _api_error("Failed to rescan storage")
...
def scan_storage(cluster_id):
    ...
    This is useful for discovering available targets before adding storage
    Changed route to not require storage_id since we're scanning BEFORE creating storage
    """
    ok, _ = check_cluster_access(cluster_id)
    if not ok:
        return _access_denied()
    ...
    try:
        ...
        nodes = [n["node"] for n in nodes_resp.json().get("data", []) if n.get("status") == "online"]
        if not nodes:
            return jsonify({"error": "No online nodes available"}), 500  # lgtm[py/reflected-xss]
        ...
        if storage_type == "iscsi":
            ...
            if not portal:
                return jsonify({"error": "Portal address required for iSCSI scan"}), 400  # lgtm[py/reflected-xss]
            ...
            if not server:
                return jsonify({"error": "Server address required for NFS scan"}), 400  # lgtm[py/reflected-xss]
            ...
            server = data.get("server")
            if not server:
                return jsonify({"error": "Server address required for CIFS scan"}), 400  # lgtm[py/reflected-xss]
            ...
            if not vgname:
                return jsonify({"error": "Volume group name required for LVM-thin scan"}), 400  # lgtm[py/reflected-xss]
            ...
            return jsonify({
                "error": f"Scan not supported for storage type: {storage_type}"
            }), 400  # lgtm[py/reflected-xss]

        if scan_resp.status_code == 200:
            return jsonify(  # lgtm[py/reflected-xss]
                {"success": True, "type": storage_type, "node": node, "data": scan_resp.json().get("data", [])}
            )
        else:
            try:
            ...
            return jsonify({"error": html.escape(str(error_msg))}), scan_resp.status_code

    except Exception as e:
        logging.error(f"Error scanning storage: {e}")
        return _api_error("Failed to scan storage")
...
def get_available_templates(cluster_id):
    ...
    try:
        ...
        return jsonify(templates)  # lgtm[py/reflected-xss]

    except Exception as e:
        logging.error(f"Error getting available templates: {e}")
        return _api_error("Failed to list templates")
...
def download_template(cluster_id):
    ...
    try:
        ...
        if not storage:
            return jsonify({"error": "Storage is required"}), 400  # lgtm[py/reflected-xss]
        if not template:
            return jsonify({"error": "Template is required"}), 400  # lgtm[py/reflected-xss]
        ...
        download_data = {"storage": storage, "template": template}
        ...
        if resp.status_code == 200:
            ...
            log_audit(
                user,
                "template.downloaded",
                f"Downloaded template '{template}' to storage '{storage}'",
                cluster=manager.config.name,
            )
            return jsonify(  # lgtm[py/reflected-xss]
                {
            ...
            try:
                ...
                error_msg = resp.text
            return jsonify({"error": html.escape(str(error_msg))}), resp.status_code

    except Exception as e:
        logging.error(f"Error downloading template: {e}")
        return _api_error("Failed to download template")
...
def get_node_storage_content(cluster_id, node, storage):
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            # (PVE 9.2) - `size` is now optional. Shared LVM /
            ...
            return jsonify(data)  # lgtm[py/reflected-xss]
        else:
            return jsonify([])  # lgtm[py/reflected-xss]
    except Exception as e:
...
# (PVE 9.2) - stable backend identity for a storage. For PBS this
...
def get_node_storage_identity(cluster_id, node, storage):
    ...
    try:
        ...
        if resp.status_code in (404, 501):
            return jsonify({"supported": False})  # lgtm[py/reflected-xss]
        if resp.status_code == 200:
            data = resp.json().get("data") or {}
            return jsonify({"supported": True, **data})  # lgtm[py/reflected-xss]
        return jsonify({"error": resp.text or f"HTTP {resp.status_code}"}), resp.status_code  # lgtm[py/reflected-xss]
    except Exception as e:
        logging.error(f"Error getting storage identity: {e}")
        return _api_error("An internal error occurred")
...
def download_from_url(cluster_id, node, storage):
    ...
    try:
        ...
        if not url:
            return jsonify({"error": "URL is required"}), 400  # lgtm[py/reflected-xss]
        if not filename:
            return jsonify({"error": "Filename is required"}), 400  # lgtm[py/reflected-xss]

        # (pentest HIGH) - the URL is handed to the Proxmox node, which
        ...
        try:
            ...
            logging.warning("[storage] URL rejected by SSRF guard: %s", _ssrf)
            return jsonify({"error": "URL rejected by SSRF guard"}), 400  # lgtm[py/reflected-xss]

        # Use Proxmox download-url API
        download_url = f"https://{host}:{port}/api2/json/nodes/{node}/storage/{storage}/download-url"
        download_data = {"url": url, "filename": filename, "content": content}
        ...
        if resp.status_code == 200:
            ...
            log_audit(
                user,
                "file.downloaded",
                f"Downloaded '{filename}' from URL to storage '{storage}'",
                cluster=manager.config.name,
            )
            return jsonify({
                "success": True,
        ...
        return _api_error("Failed to download from URL")
...
def get_backup_jobs(cluster_id):
    ...
    try:
        ...
        if r.status_code == 200:
            return jsonify(r.json().get("data", []))  # lgtm[py/reflected-xss]
        return jsonify([])  # lgtm[py/reflected-xss]
    except Exception:
        return jsonify([])  # lgtm[py/reflected-xss]
...
def create_backup_job(cluster_id):
    ...
    try:
        ...
        url = f"https://{host}:{port}/api2/json/cluster/backup"
        data = request.json or {}
        # PVE backup-job create can take >10s when it has to
        # validate the destination (PBS in particular). Bumped to 60s.
        r = manager._create_session().post(url, data=data, timeout=60)

        if r.status_code == 200:
            usr = getattr(request, "session", {}).get("user", "system")
            log_audit(usr, "backup.job_created", "Created backup job", cluster=manager.config.name)
            return jsonify({
                "success": True,
        ...
        return jsonify({"error": err_msg, "pve_status": r.status_code}), r.status_code  # lgtm[py/reflected-xss]
    except Exception as e:
        logging.error(f"Error creating backup job: {e}")
        return _api_error("Failed to create backup job")
...
def update_backup_job(cluster_id, job_id):
    ...
    try:
        ...
        # (#338) - sanitise the payload before bouncing back to PVE.
        ...
        def _to_pve_propstring(v):
            if isinstance(v, dict):
                ...
                return ",".join(parts)  # lgtm[py/reflected-xss]
            return v  # lgtm[py/reflected-xss]
        ...
        if r.status_code == 200:
            ...
            return jsonify({"success": True, "message": "Backup job updated"})  # lgtm[py/reflected-xss]
        return jsonify({"error": r.text}), r.status_code  # lgtm[py/reflected-xss]
    except Exception as e:
        logging.error(f"Error updating backup job: {e}")
        return _api_error("Failed to update backup job")
...
def delete_backup_job(cluster_id, job_id):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return jsonify({"success": True, "message": "Backup job deleted"})  # lgtm[py/reflected-xss]
        return jsonify({"error": parse_pve_error(response.text)}), response.status_code  # lgtm[py/reflected-xss]
    except Exception as e:
        logging.error(f"Error deleting backup job: {e}")
        return _api_error("Failed to delete backup job")


# ============================================
# ISO/Template Sync — Apr 2026
# ============================================
...
def iso_sync_status(cluster_id):
    ...
    if content_type not in ("iso", "vztmpl"):
        return jsonify({"error": "content must be iso or vztmpl"}), 400  # lgtm[py/reflected-xss]
    return jsonify(mgr.get_content_sync_status(content_type))  # lgtm[py/reflected-xss]
...
def iso_sync_trigger(cluster_id):
    ...
    if content_type not in ("iso", "vztmpl"):
        return jsonify({"error": "content_type must be iso or vztmpl"}), 400  # lgtm[py/reflected-xss]
    if not all([source, storage, filename]):
        return jsonify({"error": "source_node, storage, filename required"}), 400  # lgtm[py/reflected-xss]

    # (#481 port) - storage name is concatenated into a `pvesm` shell
    # command downstream. Block anything outside [A-Za-z0-9._-] before the shell
    # ever sees it.
    from ProxmoxVEx.utils.sanitization import validate_content_filename, validate_storage_name

    if not validate_storage_name(storage):
        return jsonify(  # lgtm[py/reflected-xss]
            {"error": "Invalid storage name. Must be alphanumeric with hyphens, underscores, or dots only."}
        ), 400

    # (pentest CRIT) - `filename` is interpolated UNESCAPED into SSH
    ...
    if not validate_content_filename(filename):
        return jsonify(  # lgtm[py/reflected-xss]
            {
                "error": "Invalid filename. Must be a plain ISO/template filename (letters, digits, . _ + -), no path or shell characters."
            }
        ), 400
    ...
    if hasattr(mgr, "_get_syncable_storage"):
        ...
        if storage_err:
            return jsonify({"error": storage_err}), 400  # lgtm[py/reflected-xss]

    def _do_sync():
        results = mgr.sync_content_to_nodes(source, storage, filename, content_type, targets)
        logging.info(
            f"[SYNC] {filename}: {sum(1 for r in results if r.get('success'))} ok, {sum(1 for r in results if not r.get('success'))} failed"
        )

    threading.Thread(target=_do_sync, daemon=True, name=f"iso-sync-{filename}").start()
    return jsonify({"success": True, "message": f"Sync started for {filename}"})  # lgtm[py/reflected-xss]
...
def iso_sync_all(cluster_id):
    ...
    return jsonify({"success": True, "message": "Full sync started"})  # lgtm[py/reflected-xss]
...
def iso_sync_last_result(cluster_id):
    ...
    return jsonify(result or {})  # lgtm[py/reflected-xss]
