...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/api/datacenter.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: datacenter routes (SDN, firewall, multipath, disks) -...
# Docs:        https://proxmoxvex.local/docs
# Generated:   2026-09-04
# --------------------------------------------------------------------
# --- ProxmoxVEx auto-header end ---
"""datacenter routes (SDN, firewall, multipath, disks) - split from monolith dec 2025,"""
...
def _error_message(default_msg, exc=None):
    """Log an exception and return a safe string for an error field."""
    if exc is not None:
        logging.exception(default_msg)
    return default_msg
...
def _mask_subscription_key(key: str) -> str:
    ...
    2026-05-30 - the cluster-wide endpoint is gated by `cluster.view`, which
...
def get_datacenter_subscriptions(cluster_id):
    ...
    try:
        ...
        for node in _list_cluster_node_names(manager):
            ...
            # 2026-05-30 - see _mask_subscription_key docstring. Aggregator
        ...
        return _api_error("Failed to get subscriptions", exc=e)
...
def _get_node_multipath_data(manager, node):
    ...
    Uses paramiko via manager._ssh_connect() for reliable SSH auth.
    ...
    try:
        ...
        if not ssh:
            result["error"] = f"SSH connection failed to {html.escape(node)} ({node_ip}). Check credentials."
            return result  # lgtm[py/reflected-xss]  # snyk:ignore:python/XSS

        # Helper: run command on existing SSH connection
        def ssh_run(command, timeout=15):
            try:
                stdin, stdout, stderr = ssh.exec_command(command, timeout=timeout)
                return stdout.read().decode("utf-8", errors="replace")  # lgtm[py/reflected-xss]
            except Exception as e:
                logging.debug(f"[Multipath] exec failed on {node}: {e}")
                return None  # lgtm[py/reflected-xss]
        ...
        if check_output is None:
            result["error"] = f"SSH command failed on {html.escape(node)} ({node_ip})."
            return result  # lgtm[py/reflected-xss]  # snyk:ignore:python/XSS
        ...
        if topo_output:
            for line in topo_output.strip().split("\n"):
                ...
                if len(parts) >= 4:
                    ...
                    paths_output = ssh_run(
                        f'multipathd show paths raw format "%m %d %t %T %s" 2>/dev/null | grep "^{dev_name}"'
                    )
        ...
        result["error"] = _error_message("Failed to get multipath status", e)
    finally:
        if ssh:
...
def get_cluster_multipath_status(cluster_id):
    ...
    try:
        ...
        # Call internal helper directly instead of Flask route
        ...
        for node in nodes:
            try:
                ...
                cluster_status["nodes"][node] = {
                    "error": _error_message("Failed to get node multipath data", e),
                    "installed": False,
                    "running": False,
                    "devices": [],
                }
        ...
        # snyk:ignore:python/ServerInformationExposure
        return jsonify({
            "error": _error_message("Failed to get cluster multipath status", e)
...
def setup_multipath(cluster_id):
    ...
    try:
        ...
        for node in target_nodes:
            ...
            try:
                ...
                if not ssh:
                    ...
                    node_result["error"] = (
                        f"SSH connection failed to {html.escape(node)} ({node_ip}). Check credentials."
                    )
                    results.append(node_result)
                    continue

                def _exec(cmd, timeout=30, _ssh=ssh):
                    """Run command, return (rc, stdout, stderr)"""
                    stdin, stdout, stderr = _ssh.exec_command(cmd, timeout=timeout)
                    out = stdout.read().decode("utf-8", errors="replace")
                    err = stderr.read().decode("utf-8", errors="replace")
                    rc = stdout.channel.recv_exit_status()
                    return rc, out, err  # lgtm[py/reflected-xss]
                ...
                if not already_installed:
                    rc, out, err = _exec(
                        "DEBIAN_FRONTEND=noninteractive apt-get update -qq && apt-get install -y multipath-tools 2>&1",
                        timeout=120,
                    )
                    ...
                    node_result["steps"].append({"action": "install", "success": True, "output": "Already installed"})
                ...
                if config_exists and skip_existing_config:
                    ...
                    # Base64 output is [A-Za-z0-9+/=] only, so injection-safe.
                ...
                rc, out, err = _exec(
                    "systemctl enable multipathd && systemctl restart multipathd && sleep 2 && systemctl is-active multipathd"
                )
                node_result["steps"].append({"action": "service", "success": "active" in out, "status": out.strip()})

                # Step 6: Scan for devices
                rc, out, _ = _exec(
                    'multipathd reconfigure && sleep 1 && multipath -ll 2>/dev/null | head -20 || echo "No multipath devices found"'
                )
                node_result["steps"].append({"action": "scan", "success": rc == 0, "devices": out[:1000]})
                ...
                node_result["success"] = False
                node_result["error"] = _error_message("Multipath setup failed on node", e)
            finally:
                if ssh:
                    with contextlib.suppress(Exception):
                        ssh.close()
        ...
        log_audit(
            user,
            "multipath.setup",
            f"Multipath Easy Setup on {success_count}/{len(results)} nodes (vendor={vendor}, policy={policy}, configs_skipped={skipped_count})",
        ...
        return _api_error("Multipath setup failed", exc=e)
...
""",
    ...
    return f"""# Multipath configuration - Generated by ProxmoxVEx  # lgtm[py/reflected-xss]
# Vendor: {vendor}
# Policy: {policy}
...
def reconfigure_multipath(cluster_id, node):
    ...
    try:
        ...
        if not ssh:
            # snyk:ignore:python/XSS
            return jsonify({
                "error": f"SSH connection failed to {html.escape(node)} ({node_ip}). Check credentials."
            }), 500
        ...
        # lgtm[py/reflected-xss]
        # snyk:ignore:python/XSS
        return jsonify({"success": rc == 0, "output": out, "error": err if rc != 0 else None})

    except Exception as e:
        return _api_error("Failed to reconfigure multipath", exc=e)
    finally:
        if ssh:
            with contextlib.suppress(Exception):
                ssh.close()
...
def discover_iscsi_targets(cluster_id, node):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # lgtm[py/reflected-xss]
            # snyk:ignore:python/XSS
            return jsonify({"portal": portal, "targets": targets})
        else:
            # lgtm[py/reflected-xss]
            # snyk:ignore:python/XSS
        ...
        return _api_error("Failed to discover iSCSI targets", exc=e)


# (F2 fix) - iSCSI parameters used to be interpolated into shell
...
_IQN_RE = re.compile(
    r"^(iqn\.\d{4}-\d{2}\.[a-z0-9.\-]+(?::[\x21-\x7e]{1,200})?|eui\.[0-9a-fA-F]{16}|naa\.[0-9a-fA-F]{16,32})$"
)
...
def _validate_iscsi_inputs(portal, target, username="", password=""):
    ...
    if not _PORTAL_RE.match(portal or ""):
        return False, "Invalid iSCSI portal — expected host[:port], no shell metacharacters"  # lgtm[py/reflected-xss]
    if not _IQN_RE.match(target or ""):
        return (
            False,
            "Invalid iSCSI target — expected IQN/EUI/NAA format (e.g. iqn.YYYY-MM.org.example:storage.lun01)",
        )  # lgtm[py/reflected-xss]
    if username and not _CHAP_USER_RE.match(username):
        return False, "Invalid CHAP username — alphanumeric plus . _ - only, max 256 chars"  # lgtm[py/reflected-xss]
    if password and not _CHAP_PASS_RE.match(password):
        return (
            False,
            "Invalid CHAP password — printable ASCII without shell metacharacters; max 256 chars",
        )  # lgtm[py/reflected-xss]
    return True, ""  # lgtm[py/reflected-xss]
...
def login_iscsi_target(cluster_id, node):
    ...
    try:
        ...
        return _api_error("Failed to login to iSCSI target", exc=e)
    finally:
        if ssh:
            with contextlib.suppress(Exception):
                ssh.close()
...
# Proxmox SDN API is a bit inconsistent, some endpoints return
    ...
    try:
        ...
        try:
            ...
            if sdn_resp.status_code == 200:
                ...
                # (#413) - PVE 9.x returns /cluster/sdn as a *list* of
        ...
        return _api_error("Failed to get SDN overview", exc=e)
...
def get_sdn_zones(cluster_id):
    ...
    # 501 means SDN not enabled on this cluster - return empty list instead of error
    manager, _ = get_connected_manager(cluster_id)
    if not manager:
        if cluster_id not in cluster_managers:
            return _cluster_not_found()
    ...
    try:
        ...
        return _api_error("Failed to get SDN zones", exc=e)
...
def create_sdn_zone(cluster_id):
    # Zone types: simple, vlan, qinq, vxlan, evpn - each has different required params
    ...
    try:
        ...
        if response.status_code in [200, 201]:
            ...
            log_audit(
                user,
                "sdn.zone_created",
                f"Created SDN zone: {data.get('zone', 'unknown')}",
                cluster=manager.config.name,
            )
            # lgtm[py/reflected-xss]
            # snyk:ignore:python/XSS
        ...
        return _api_error("Failed to create SDN zone", exc=e)
...
def update_sdn_zone(cluster_id, zone_id):
    ...
    try:
        ...
        return _api_error("Failed to update SDN zone", exc=e)
...
def delete_sdn_zone(cluster_id, zone_id):
    ...
    try:
        ...
        return _api_error("Failed to delete SDN zone", exc=e)
...
def get_sdn_vnets(cluster_id):
    ...
    # VNets are the main abstraction layer - each vnet belongs to exactly one zone
    manager, _ = get_connected_manager(cluster_id)
    if not manager:
        if cluster_id not in cluster_managers:
            return _cluster_not_found()
    ...
    try:
        ...
        return _api_error("Failed to get SDN vnets", exc=e)
    ...
    try:
        ...
        if response.status_code in [200, 201]:
            ...
            log_audit(
                user,
                "sdn.vnet_created",
                f"Created SDN VNet: {data.get('vnet', 'unknown')}",
                cluster=manager.config.name,
            )
            # lgtm[py/reflected-xss]
            # snyk:ignore:python/XSS
        ...
        return _api_error("Failed to create SDN vnet", exc=e)
...
def update_sdn_vnet(cluster_id, vnet_id):
    ...
    try:
        ...
        return _api_error("Failed to update SDN vnet", exc=e)
...
def delete_sdn_vnet(cluster_id, vnet_id):
    ...
    try:
        ...
        return _api_error("Failed to delete SDN vnet", exc=e)


@bp.route("/api/clusters/<cluster_id>/datacenter/sdn/vnets/<vnet_id>/subnets", methods=["GET"])
@require_auth(perms=["node.view"])
def get_sdn_subnets(cluster_id, vnet_id):
    # Subnets are nested under vnets in the API but stored flat in PVE config
    ...
    try:
        ...
        return _api_error("Failed to get SDN subnets", exc=e)
    ...
    try:
        ...
        if response.status_code in [200, 201]:
            ...
            log_audit(
                user,
                "sdn.subnet_created",
                f"Created subnet in VNet {vnet_id}: {data.get('subnet', 'unknown')}",
                cluster=manager.config.name,
            )
            # lgtm[py/reflected-xss]
            # snyk:ignore:python/XSS
        ...
        return _api_error("Failed to create SDN subnet", exc=e)
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            log_audit(
                user,
                "sdn.subnet_deleted",
                f"Deleted subnet {subnet_id} from VNet {vnet_id}",
                cluster=manager.config.name,
            )
            # lgtm[py/reflected-xss]
            # snyk:ignore:python/XSS
        ...
        return _api_error("Failed to delete SDN subnet", exc=e)
...
def apply_sdn_config(cluster_id):
    ...
    try:
        ...
        return _api_error("Failed to apply SDN config", exc=e)


# ============================================
# SDN Fabrics (PVE 9.2+ first-class object)
# ============================================
# Fabrics are a new top-level SDN object family in 9.2. Full
...
# Actual fabric records live at /cluster/sdn/fabrics/fabric on
...
def get_sdn_fabrics(cluster_id):
    ...
    try:
        ...
        return _api_error("Failed to list SDN fabrics", exc=e)
...
def create_sdn_fabric(cluster_id):
    ...
    try:
        ...
        return _api_error("Failed to create SDN fabric", exc=e)
...
def update_sdn_fabric(cluster_id, fabric_id):
    ...
    try:
        ...
        return _api_error("Failed to update SDN fabric", exc=e)
...
def delete_sdn_fabric(cluster_id, fabric_id):
    ...
    try:
        ...
        return _api_error("Failed to delete SDN fabric", exc=e)


# ============================================
# SDN Route Maps + Prefix Lists (PVE 9.2+)
# ============================================
# Two new SDN object families for BGP/EVPN policy. We expose
...
def _sdn_crud_resource(cluster_id, family, item_id=None):
    ...
    try:
        ...
        return _api_error("SDN operation failed", e)
...
def list_sdn_routemaps(cluster_id):
    ...
    if not ok:
        return _access_denied()
    return _sdn_crud_resource(cluster_id, "route-maps")  # lgtm[py/reflected-xss]  # snyk:ignore:python/XSS
...
def create_sdn_routemap(cluster_id):
    ...
    if result[1] == 200:
        ...
        log_audit(
            user,
            "sdn.routemap_created",
            f"Created SDN routemap: {name}",
...
def update_sdn_routemap(cluster_id, routemap_id):
    ...
    if not ok:
        return _access_denied()
    # _sdn_crud_resource escapes PVE error text and only reflects request body to upstream
    return _sdn_crud_resource(cluster_id, "route-maps", routemap_id)  # lgtm[py/reflected-xss]  # snyk:ignore:python/XSS
...
def list_sdn_prefixlists(cluster_id):
    ...
    if not ok:
        return _access_denied()
    return _sdn_crud_resource(cluster_id, "prefix-lists")  # lgtm[py/reflected-xss]  # snyk:ignore:python/XSS
...
def create_sdn_prefixlist(cluster_id):
    ...
    if result[1] == 200:
        ...
        log_audit(
            user,
            "sdn.prefixlist_created",
            f"Created SDN prefixlist: {name}",
            cluster=manager.config.name if manager else cluster_id,
        )
    return result  # lgtm[py/reflected-xss]  # snyk:ignore:python/XSS
...
def update_sdn_prefixlist(cluster_id, prefixlist_id):
    ...
    if not ok:
        return _access_denied()
    return _sdn_crud_resource(
        cluster_id, "prefix-lists", prefixlist_id
...
def get_sdn_controllers(cluster_id):
    # Controllers are optional - only needed for EVPN/BGP setups
    """Get SDN controllers"""
    ok, _ = check_cluster_access(cluster_id)
    if not ok:
        return _access_denied()
    ...
    try:
        ...
        return _api_error("Failed to get SDN controllers", exc=e)
...
def create_sdn_controller(cluster_id):
    ...
    try:
        ...
        if response.status_code in [200, 201]:
            ...
            log_audit(
                user,
                "sdn.controller_created",
                f"Created SDN controller: {data.get('controller', 'unknown')} ({data.get('type', '')})",
                cluster=manager.config.name,
            )
            # lgtm[py/reflected-xss]
            # snyk:ignore:python/XSS
        ...
        return _api_error("Failed to create SDN controller", exc=e)
...
def update_sdn_controller(cluster_id, controller_id):
    ...
    try:
        ...
        if response.status_code == 200:
            user = getattr(request, "session", {}).get("user", "system")
            log_audit(
                user, "sdn.controller_updated", f"Updated SDN controller: {controller_id}", cluster=manager.config.name
            )
            # lgtm[py/reflected-xss]
            # snyk:ignore:python/XSS
        ...
        return _api_error("Failed to update SDN controller", exc=e)
...
def delete_sdn_controller(cluster_id, controller_id):
    ...
    try:
        ...
        if response.status_code == 200:
            user = getattr(request, "session", {}).get("user", "system")
            log_audit(
                user, "sdn.controller_deleted", f"Deleted SDN controller: {controller_id}", cluster=manager.config.name
            )
            # lgtm[py/reflected-xss]
            # snyk:ignore:python/XSS
        ...
        return _api_error("Failed to delete SDN controller", exc=e)
...
def get_sdn_ipams(cluster_id):
    # IPAM = IP Address Management, default is PVE-internal but can use phpIPAM or Netbox
    ...
    try:
        ...
        return _api_error("Failed to get SDN IPAMs", exc=e)
    ...
    try:
        ...
        if response.status_code in [200, 201]:
            ...
            log_audit(
                user,
                "sdn.ipam_created",
                f"Created IPAM: {data.get('ipam', 'unknown')} ({data.get('type', '')})",
                cluster=manager.config.name,
            )
            # lgtm[py/reflected-xss]
            # snyk:ignore:python/XSS
        ...
        return _api_error("Failed to create IPAM", exc=e)
...
def update_sdn_ipam(cluster_id, ipam_id):
    ...
    try:
        ...
        return _api_error("Failed to update IPAM", exc=e)
...
def delete_sdn_ipam(cluster_id, ipam_id):
    ...
    try:
        ...
        return _api_error("Failed to delete IPAM", exc=e)
...
def get_sdn_dns(cluster_id):
    # DNS integration for auto-registration of VMs in zones
    ...
    try:
        ...
        return _api_error("Failed to get SDN DNS configs", exc=e)
...
def create_sdn_dns(cluster_id):
    ...
    try:
        ...
        if response.status_code in [200, 201]:
            user = getattr(request, "session", {}).get("user", "system")
            log_audit(
                user, "sdn.dns_created", f"Created DNS: {data.get('dns', 'unknown')}", cluster=manager.config.name
            )
            # lgtm[py/reflected-xss]
            # snyk:ignore:python/XSS
        ...
        return _api_error("Failed to create DNS config", exc=e)
...
def update_sdn_dns(cluster_id, dns_id):
    ...
    try:
        ...
        return _api_error("Failed to update DNS config", exc=e)
...
def delete_sdn_dns(cluster_id, dns_id):
    ...
    try:
        ...
        return _api_error("Failed to delete DNS config", exc=e)
...
def get_sdn_zone_details(cluster_id, zone_id):
    ...
    try:
        ...
        return _api_error("Failed to get SDN zone details", exc=e)
...
def get_sdn_vnet_details(cluster_id, vnet_id):
    ...
    try:
        ...
        return _api_error("Failed to get SDN vnet details", exc=e)
...
def update_sdn_subnet(cluster_id, vnet_id, subnet_id):
    ...
    try:
        ...
        if response.status_code == 200:
            user = getattr(request, "session", {}).get("user", "system")
            log_audit(
                user, "sdn.subnet_updated", f"Updated subnet {subnet_id} in VNet {vnet_id}", cluster=manager.config.name
            )
            # lgtm[py/reflected-xss]
            # snyk:ignore:python/XSS
        ...
        return _api_error("Failed to update SDN subnet", exc=e)
...
def wipe_node_disk_api(cluster_id, node):
    ...
    # SECURITY: require confirmation for destructive disk wipe
    if data.get("confirm_name") != disk:
        # snyk:ignore:python/XSS
        return jsonify({
            "error": "Confirmation required: send confirm_name matching the disk name"
