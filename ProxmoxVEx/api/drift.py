...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/api/drift.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: Config Drift Detection
...
# PVE returns these comma-separated lists in non-deterministic order (e.g.
...
def _severity_for(kind, diffs):
    ...
    sensitive_network = {"address", "netmask", "gateway", "bridge_ports", "type", "bond_slaves", "vlan-raw-device"}
...
def _load_baselines(cluster_id):
    try:
        ...
        c.execute(
            "SELECT id, kind, scope, snapshot, created_at, created_by FROM drift_baselines WHERE cluster_id = ?",
            (cluster_id,),
        )
        out = {}
        for r in c.fetchall():
...
def _set_baseline(cluster_id, kind, scope, snapshot, user):
    try:
        ...
        c.execute("DELETE FROM drift_baselines WHERE cluster_id=? AND kind=? AND scope=?", (cluster_id, kind, scope))
        c.execute(
            """
            INSERT INTO drift_baselines (id, cluster_id, kind, scope, snapshot, created_at, created_by)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """,
            (uuid.uuid4().hex[:12], cluster_id, kind, scope, json.dumps(snapshot), datetime.now().isoformat(), user),
        )
        get_db().conn.commit()
    except Exception as e:
...
def _record_event(cluster_id, kind, scope, diffs, severity, summary):
    try:
        ...
        c.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, 'open')
        """,
            (cluster_id, kind, scope, severity, summary, json.dumps(diffs), datetime.now().isoformat()),
        )
        eid = c.lastrowid
        get_db().conn.commit()
        return eid
...
def _scan_cluster(cluster_id, autobaseline=False):
    ...
    for kind, scope, snap in state:
        ...
        new_events.append({"id": eid, "kind": kind, "scope": scope, "severity": sev, "summary": summary})
    ...
    if new_events:
        try:
            from ProxmoxVEx.background import alerts as alerts_mod

            count = len(new_events)
            top_sev = (
                "critical"
                if any(e["severity"] == "critical" for e in new_events)
                else "warning"
                if any(e["severity"] == "warning" for e in new_events)
                else "info"
            )
            payload = {
...
def drift_status(cluster_id):
    ...
    try:
        ...
        logging.exception("handler error in drift.py")
...
def list_events(cluster_id):
    ...
    try:
        ...
        if status == "all":
            c.execute(
                """SELECT * FROM drift_events WHERE cluster_id=?
                         ORDER BY detected_at DESC LIMIT ?""",
                (cluster_id, limit),
            )
        else:
            c.execute(
                """SELECT * FROM drift_events WHERE cluster_id=? AND status=?
                         ORDER BY detected_at DESC LIMIT ?""",
...
def acknowledge_event(eid):
    ...
    try:
        ...
        # (CodeAnt IDOR) - tenant gate on the event's cluster before ack/promote
        # (was ack-ing / rebaselining another tenant's drift events).
        from ProxmoxVEx.api.helpers import check_cluster_access

        ok, _ = check_cluster_access(ev["cluster_id"])
        if not ok:
            return jsonify({"error": "Access denied to this cluster"}), 403
        c.execute(
            """UPDATE drift_events SET status='acknowledged',
...
def reset_baseline(cluster_id):
    ...
    try:
        ...
        c.execute("UPDATE drift_events SET status='superseded' WHERE cluster_id=? AND status='open'", (cluster_id,))
        for kind, scope, snap in state:
            c.execute(
                """
                INSERT INTO drift_baselines (id, cluster_id, kind, scope, snapshot, created_at, created_by)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """,
                (uuid.uuid4().hex[:12], cluster_id, kind, scope, json.dumps(snap), datetime.now().isoformat(), user),
            )
        get_db().conn.commit()
