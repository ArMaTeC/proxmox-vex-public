...
def add_mirror_peer(cluster_id, pool):
    ...
    # Validate client/site to prevent injection
    ...
    if mon_host:
        # Mon_host can have commas/colons for multiple monitors
...
def add_mirror_schedule(cluster_id, pool):
    ...
    data, cmd_err = _rbd_cmd(
        manager, node_ip, f"mirror snapshot schedule add --pool {pool} {interval}", expect_json=False
    )
    if cmd_err:
        return cmd_err  # lgtm[py/reflected-xss]
...
def remove_mirror_schedule(cluster_id, pool):
    ...
    data, cmd_err = _rbd_cmd(
        manager, node_ip, f"mirror snapshot schedule remove --pool {pool} {interval}", expect_json=False
    )
    if cmd_err:
        return cmd_err  # lgtm[py/reflected-xss]
