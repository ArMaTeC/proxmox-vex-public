...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/api/groups.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: cluster groups & rename routes
# Docs:        https://proxmoxvex.local/docs
# Generated:   2026-09-04
# --------------------------------------------------------------------
# --- ProxmoxVEx auto-header end ---
"""cluster groups & rename routes"""
...
from ProxmoxVEx.utils.rbac import DEFAULT_TENANT_ID, get_user_clusters

# 2026-06-04 (CWE-117): group_id from URL path goes into the logger below.
from ProxmoxVEx.utils.sanitization import sanitize_log_message as _sl

bp = Blueprint("groups", __name__)

# =============================================================================
# CLUSTER GROUPS & RENAME
...
def _user_tenant(user: dict):
    ...
    2026-06-05 (audit M-3): the whole file used to read user.get('tenant'),
...
def create_cluster_group():
    ...
    log_audit(
        usr,
        "cluster_group.created",
        f"Created cluster group '{data.get('name')}' (ID: {group_id}){tenant_info}",
        ip_address=ip,
    )
...
def update_cluster_group(group_id):
    ...
    if user.get("role") != ROLE_ADMIN:
        ...
        if group["tenant_id"] and group["tenant_id"] != user_tenant:
            log_audit(
                usr,
                "cluster_group.update_denied",
                f"Access denied to group '{group['name']}' (ID: {group_id}) - tenant mismatch",
                ip_address=ip,
            )
    ...
    # Added cross-cluster LB fields to group update
    db.execute(
        """
    ...
    log_audit(
        usr,
        "cluster_group.updated",
        f"Updated cluster group '{data.get('name', group['name'])}' (ID: {group_id})",
        ip_address=ip,
    )
...
def delete_cluster_group(group_id):
    ...
    if user.get("role") != ROLE_ADMIN:
        ...
        if group["tenant_id"] and group["tenant_id"] != user_tenant:
            log_audit(
                usr,
                "cluster_group.delete_denied",
                f"Access denied to delete group '{group['name']}' (ID: {group_id}) - tenant mismatch",
                ip_address=ip,
            )
            return jsonify({"error": "Access denied - group belongs to different tenant"}), 403
    ...
    log_audit(
        usr,
        "cluster_group.deleted",
        f"Deleted cluster group '{group['name']}' (ID: {group_id}) - {affected_count} clusters ungrouped",
        ip_address=ip,
    )
...
def rename_cluster(cluster_id):
    ...
    # (CodeAnt exploitation) - defense-in-depth: same source-cluster ownership gate
    ...
    if _owned is not None and cluster_id not in _owned:
        log_audit(
            usr,
            "cluster.rename_denied",
            f"Access denied to rename cluster {cluster_id} — not tenant-owned",
            ip_address=ip,
        )
    ...
    msg = (
        f"Renamed cluster from '{old_name}' to '{new_name}'"
        if new_name
        else f"Reset cluster name to original (was '{old_name}')"
    )
...
def assign_cluster_to_group(cluster_id):
    ...
    # (CodeAnt exploitation - cross-tenant cluster hijack) - check_cluster_access
    ...
    if _owned is not None and cluster_id not in _owned:
        log_audit(
            usr,
            "cluster.group_assign_denied",
            f"Access denied to re-group cluster {cluster_id} — not tenant-owned",
            ip_address=ip,
        )
        return jsonify({"error": "Access denied - you do not own this cluster"}), 403
        ...
        if user.get("role") != ROLE_ADMIN:
            ...
            if group["tenant_id"] and group["tenant_id"] != user_tenant:
                log_audit(
                    usr,
                    "cluster.group_assign_denied",
                    f"Access denied to assign cluster {cluster_id} to group '{group['name']}' - tenant mismatch",
                    ip_address=ip,
                )
                return jsonify({"error": "Access denied - group belongs to different tenant"}), 403
    ...
    log_audit(
        usr,
        "cluster.group_changed",
        f"Moved cluster '{cluster_name}' from '{old_group_name}' to '{group_name}'",
        ip_address=ip,
    )
...
# Cross-cluster LB status endpoint
# Aggregates across all clusters in the group
...
def get_cluster_group_status(group_id):
    ...
    try:
        ...
        user = users.get(usr, {})
        tenant_id = _user_tenant(user)
        if tenant_id and group["tenant_id"] and group["tenant_id"] != tenant_id:
        ...
        for cid in cluster_ids:
            ...
            try:
                ...
                for _nname, ndata in nodes.items():
...
def get_cluster_group_lb_history(group_id):
    ...
    # Grab anything tagged with xclb.* that mentions this group
    events = db.query(
...
def trigger_xclb_balance_now(group_id):
    ...
    from ProxmoxVEx.background.cross_cluster_lb import run_cross_cluster_balance_check
