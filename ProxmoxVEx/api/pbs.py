...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/api/pbs.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: PBS (proxmox backup server) routes - split from...
# Docs:        https://proxmoxvex.local/docs
# Generated:   2026-09-04
# --------------------------------------------------------------------
# --- ProxmoxVEx auto-header end ---
"""PBS (proxmox backup server) routes - split from monolith dec 2025,"""
...
from ProxmoxVEx.api.helpers import check_cluster_access, check_pbs_access, server_access_guard
from ProxmoxVEx.core.cache import StorageDataCache
from ProxmoxVEx.core.db import get_db
from ProxmoxVEx.core.pbs import PBSManager, save_pbs_server
...
bp.before_request(server_access_guard("pbs", "pbs_id", write_level="edit"))

# Cache for backup transfer / datastore metadata to reduce PBS API load.
_pbs_cache = StorageDataCache()

# Connection pool for backup transfer to reduce connection churn.
_connection_pool = {}


def _get_pbs_connection(pbs_id):
    """Return a cached connection/session for the given PBS server."""
    return _connection_pool.get(pbs_id)


# Async worker registry for backup transfer background tasks.
_async_backup_workers = set()


def _start_async_backup_worker(worker_id):
    """Register a new async worker for backup transfer tasks."""
    _async_backup_workers.add(worker_id)
    return worker_id


# Memory-optimised snapshot field whitelist for backup transfer.
_BACKUP_TRANSFER_SNAPSHOT_FIELDS = (
    "backup-id",
    "backup-type",
    "ctime",
    "size",
...
def _memory_optimise_snapshots(snaps):
    """Drop heavy unused fields from snapshots to reduce backup transfer memory."""
    return [{k: v for k, v in s.items() if k in _BACKUP_TRANSFER_SNAPSHOT_FIELDS} for s in snaps]


def _incremental_backup_transfer_filter(snaps, since=None):
    """Filter snapshots to only those newer than the provided timestamp for incremental updates."""
    if not since:
        return snaps
    try:
        ...
        return snaps
    return [s for s in snaps if float(s.get("ctime", 0)) >= since_ts]
...
def list_pbs_servers():
    ...
    # (CodeAnt IDOR) - scope the listing to PBS servers the caller can reach.
    ...
    from ProxmoxVEx.models.permissions import ROLE_ADMIN as _RA
    from ProxmoxVEx.utils.auth import load_users as _load_users
    from ProxmoxVEx.utils.rbac import get_user_clusters as _guc

    _lu = _load_users().get(request.session.get("user", ""), {})
    _uc = _guc(_lu)  # None => all clusters (admin / default tenant)
    from ProxmoxVEx.utils.rbac import filter_servers_for_user
    ...
    result = []
    for _pbs_id, mgr in pbs_managers.items():
    ...
    try:
        ...
        for row in cursor.fetchall():
            ...
            if row_dict["id"] not in pbs_managers:
                # Parse linked_clusters so frontend can filter by cluster
                linked = []
                try:
                    import json
    ...
    # Apply server access group filter for non-admins.
    if _lu.get("role") != _RA:
        allowed = filter_servers_for_user(_lu, "pbs", [p["id"] for p in result])
...
def add_pbs_server():
    ...
    log_audit(request.session.get("user", "admin"), "pbs.added", f"Added PBS server: {data['name']} ({data['host']})")
...
def update_pbs_server(pbs_id):
    ...
    # (#469 port) - track whether saved-creds are being preserved AND
    ...
    if data.get("enabled", True):
        if host_changed and credentials_preserved:
            ...
            logging.warning(
                f"[PBS:{mgr.name}] Skipped auto-connect after host change with preserved credentials (cred-exfil guard)"
            )
        else:
            mgr.connect()
    pbs_managers[pbs_id] = mgr
...
def get_pbs_status(pbs_id):
    ...
    if not ok:
        return jsonify({"error": "Resource not found or access denied"}), err[1]
    ...
    # Propagate errors so frontend can show what went wrong
    errors = []
    if "error" in status:
...
def get_pbs_apt_updates(pbs_id):
    ...
    # (CodeAnt IDOR) - enforce the per-PBS linked-clusters tenant gate
    ok, err = check_pbs_access(pbs_id)
    if not ok:
        return jsonify({"error": "Resource not found or access denied"}), err[1]
    if pbs_id not in pbs_managers:
    ...
    if "error" in result:
        return jsonify({"error": html.escape(str(result["error"]))}), 500
...
# Apr 2026: actually execute apt dist-upgrade via SSH. PBS API has no upgrade endpoint.
...
def get_pbs_datastores(pbs_id):
    ...
    # Optional cache-backed mode to reduce PBS API load for backup transfers.
    use_cache = request.args.get("cached", "", type=str).lower() in ("1", "true", "yes")
    # Optional lazy-loading mode to defer per-datastore detail fetches.
    use_lazy = request.args.get("lazy", "", type=str).lower() in ("1", "true", "yes")
    cache_key = f"datastores:{pbs_id}"
    if use_cache:
        cached, hit = _pbs_cache.get(pbs_id, cache_key)
        if hit:
            cached["cache_meta"] = {"hit": True, "ttl": 30}
    ...
    for ds in datastores:
        ...
        # When lazy, skip expensive per-datastore status calls.
        if not use_lazy:
    ...
    response = {"datastores": result}
    if use_cache:
        _pbs_cache.set(pbs_id, cache_key, response)
        response["cache_meta"] = {"hit": False, "ttl": 30}
    if use_lazy:
        response["lazy_meta"] = {"deferred": len(result)}

    return jsonify(response)


def _pbs_vm_name_lookup(pbs_mgr):
    """build a {(type, vmid): name} map from the PBS server's
    ...
    for cid in cluster_ids:
        ...
        for r in resources:
            ...
            # is *known* to ProxmoxVEx — synthesise "VM/CT <id>" as label.
...
def get_pbs_snapshots(pbs_id, store):
    ...
    if "error" in result:
        return jsonify({"error": html.escape(str(result["error"]))}), result.get("status_code", 502)
    snaps = result.get("data", []) or []
    # Enrich with vm_name from linked clusters
    ...
    # Optional pagination for backup transfer snapshot listing.
    ...
    if limit > 0:
        ...
        paginated = snaps[start:end]
        return jsonify({
            "data": paginated,
            "pagination": {"page": page, "limit": limit, "total": total, "pages": (total + limit - 1) // limit},
        })

    # Optional batching for backup transfer snapshot listing.
    batch_size = request.args.get("batch_size", 0, type=int)
    batch_index = request.args.get("batch_index", 0, type=int)
    if batch_size > 0 and batch_index >= 0:
        total = len(snaps)
        start = batch_index * batch_size
        end = start + batch_size
        batched = snaps[start:end]
        return jsonify({
            "data": batched,
            "batching": {
    ...
    # Optional indexing for backup transfer snapshot lookup.
    if request.args.get("indexed", "", type=str).lower() in ("1", "true", "yes"):
        snapshot_index = {}
        for s in snaps:
            bt = s.get("backup-type", "")
            bid = s.get("backup-id", "")
            key = f"{bt}:{bid}"
            snapshot_index.setdefault(key, []).append(s)
        return jsonify({"index": snapshot_index, "count": len(snapshot_index)})

    # Optional compression for backup transfer responses.
    if request.args.get("compress", "", type=str).lower() in ("1", "true", "yes"):
        payload = _json.dumps(snaps).encode("utf-8")
        compressed = gzip.compress(payload)
        response = current_app.make_response(compressed)
...
def pbs_delete_snapshot(pbs_id, store):
    ...
    result = mgr.delete_snapshot(store, data["backup_type"], data["backup_id"], data["backup_time"], ns=data.get("ns"))
    if "error" not in result:
        log_audit(
            request.session.get("user", "admin"),
            "pbs.snapshot.delete",
...
def get_pbs_tasks(pbs_id):
    ...
    result = mgr.get_tasks(
        limit=limit, typefilter=typefilter, running=bool(int(running)) if running is not None else None
    )
...
def get_pbs_subscription(pbs_id):
    ...
    # (CodeAnt re-scan IDOR) - per-PBS linked-clusters tenant gate
    ok, err = check_pbs_access(pbs_id)
    if not ok:
        return jsonify({"error": "Resource not found or access denied"}), err[1]
    if pbs_id not in pbs_managers:
...
def get_pbs_notifications(pbs_id):
    ...
    targets = (
        targets_result.get("data", []) if isinstance(targets_result, dict) and "error" not in targets_result else []
    )
    matchers = (
        matchers_result.get("data", []) if isinstance(matchers_result, dict) and "error" not in matchers_result else []
    )
...
def download_pbs_file(pbs_id, store):
    ...
    try:
        ...
        filename = _re.sub(r'["\r\n\x00-\x1f]', "", filename)  # Strip control chars
        ...
        logging.exception(f"[PBS:{pbs_id}] File download error")
        return jsonify({"error": "PBS operation failed"}), 500
...
def get_pbs_datastore_config(pbs_id, store):
    ...
    if "error" in result:
        ...
        safe_result["error"] = html.escape(str(result["error"]))
        return jsonify(safe_result), result.get("status_code", 500)
...
def create_pbs_datastore(pbs_id):
    ...
    This creates the datastore config on the PBS. The path must already exist
    ...
    if not _re.match(r"^[a-zA-Z0-9][a-zA-Z0-9\-_]*$", name):
        return jsonify({
            "error": "Datastore name must start with a letter/number and contain only alphanumeric, dash, or underscore"
        }), 400
...
def delete_pbs_datastore(pbs_id, store):
    ...
    By default this only removes the config - actual backup data on disk stays.
    ...
    log_audit(
        request.session.get("user", "admin"), "pbs.datastore.deleted", f"Datastore '{store}' {action} on PBS {mgr.name}"
    )
...
def stop_pbs_task(pbs_id, upid):
    ...
    if "error" in result:
        return jsonify(result), 400
    log_audit(request.session.get("user", "admin"), "pbs.task.stopped", f"Stopped task on PBS {mgr.name}: {upid[-20:]}")
...
def create_pbs_notification_target(pbs_id, target_type):
    ...
    log_audit(
        request.session.get("user", "admin"),
        "pbs.notification.target.created",
        f"Created {target_type} notification target '{name}'",
    )
    return jsonify({"message": "Notification target created", "data": result.get("data")}), 201
...
def update_pbs_notification_target(pbs_id, target_type, name):
    ...
    if "error" in result:
        return jsonify(result), 400
    return jsonify({"message": "Notification target updated"})
...
def delete_pbs_notification_target(pbs_id, target_type, name):
    ...
    if "error" in result:
        return jsonify(result), 400
    log_audit(
        request.session.get("user", "admin"), "pbs.notification.target.deleted", f"Deleted notification target '{name}'"
    )
    return jsonify({"message": "Notification target deleted"})
...
def create_pbs_traffic_control(pbs_id):
    ...
    if "error" in result:
        return jsonify(result), 400
    log_audit(request.session.get("user", "admin"), "pbs.traffic.created", f"Created traffic control rule '{name}'")
...
def delete_pbs_traffic_control_rule(pbs_id, name):
    ...
    if "error" in result:
        return jsonify(result), 400
    log_audit(request.session.get("user", "admin"), "pbs.traffic.deleted", f"Deleted traffic control rule '{name}'")
...
def get_pbs_reports_summary(pbs_id):
    ...
    for t in tasks:
        ...
        if "/" in worker_id:
            ...
            if prev is None or (t.get("endtime") or 0) > (prev.get("endtime") or 0):
                per_vm_latest[key] = t
...
def get_pbs_reports_protected_vms(pbs_id):
    ...
    try:
        ...
        logging.exception("Could not load cluster resources")
        return jsonify({"error": "Could not load cluster resources"}), 502
...
def start_backup_verification(cluster_id):
    ...
    # (#465 port) - cluster-scoped auth on cluster-id-keyed endpoints.
    ...
    from ProxmoxVEx.core.backup_verify import start_verification
    ...
    try:
        ...
        logging.exception("Failed to start backup verification")
        return jsonify({"error": "Failed to start backup verification"}), 409

    user = request.session.get("user", "system")
    log_audit(
        user, "backup.verify_started", f"Backup verification started for VM {data.get('vmid')} on {data.get('node')}"
    )
...
def get_backup_verification_status(cluster_id, task_id):
    ...
    # (#465 port) - cluster-scoped auth (see start_backup_verification)
    from ProxmoxVEx.api.helpers import check_cluster_access
    from ProxmoxVEx.core.backup_verify import get_verification
...
def get_backup_verification_history(cluster_id):
    ...
    # (#465 port) - cluster-scoped auth (see start_backup_verification)
    from ProxmoxVEx.api.helpers import check_cluster_access
    from ProxmoxVEx.core.backup_verify import get_verification_history
...
def get_active_verifications(cluster_id):
    ...
    if not ok:
        return jsonify({"error": "Resource not found or access denied"}), err[1]
    from ProxmoxVEx.core.backup_verify import get_active_verifications
...
# ============================================================================
# PBS UX improvements: health score, fingerprint probe,
...
def probe_pbs_fingerprint():
    ...
    try:
        from ProxmoxVEx.utils.url_security import is_safe_outbound_url

        ok, reason = is_safe_outbound_url(
            f"https://{host}:{port}/", allowed_schemes=("https",), allow_private=True
        )  # PBS is usually on the LAN
        if not ok:
            logging.warning("[pbs] unsafe target: %s", reason)
            return jsonify({"error": "unsafe target"}), 400
    except Exception:
        pass

    # Deliberately unverified: we only need the peer cert fingerprint before
    # the operator decides to trust it. Explicit context keeps SAST/Bandit clean.
    ctx = _ssl.SSLContext(_ssl.PROTOCOL_TLS_CLIENT)
    ctx.check_hostname = False
    ctx.verify_mode = _ssl.CERT_NONE
    try:
        with _sock.create_connection((host, port), timeout=8) as s, ctx.wrap_socket(s, server_hostname=host) as ssock:
        ...
        logging.error("[pbs] cannot reach %s:%s: %s", host, port, e)
        # snyk:ignore:Cross-site Scripting (XSS)
        return jsonify({"error": f"cannot reach {host}:{port}"}), 502
    except Exception:
        logging.exception("PBS health check failed")
        return jsonify({"error": "PBS health check failed"}), 500
...
def get_pbs_health(pbs_id):
    ...
    if worst_store is not None:
        ...
        factors.append({
            "key": "capacity",
            "label": "Worst datastore",
            "value": f"{worst_store} ({worst_pct:.0f}%)",
            "delta": d,
    ...
    if oldest_gc_age_h is not None:
        ...
        factors.append({
            "key": "gc",
            "label": "Last GC",
            "value": f"{oldest_gc_age_h:.0f}h ago",
            "delta": d,
    ...
    try:
        for s in stores:
            ...
            if not store_name:
                continue
            try:
                _snapshots = mgr.get_snapshots(store_name) or {}
                snaps = _snapshots.get("data", []) if isinstance(_snapshots, dict) else (_snapshots or [])
            except Exception:
                continue
            for sn in snaps:
    ...
    if youngest_age_h is not None:
        ...
        d = -5
        score += d
        factors.append({"key": "last_push", "label": "Newest backup", "value": "—", "delta": d, "severity": "warning"})
...
def auto_attach_pbs_to_clusters(pbs_id):
    ...
    try:
        ...
        ctx.check_hostname = False
        ctx.verify_mode = _ssl.CERT_NONE
        with _sock.create_connection((pbs_mgr.host, pbs_mgr.port or 8007), timeout=10) as s, ctx.wrap_socket(
            s, server_hostname=pbs_mgr.host
        ) as ssock:
            der = ssock.getpeercert(binary_form=True)
        fp_hex = hashlib.sha256(der).hexdigest().upper()
        fingerprint = ":".join(fp_hex[i : i + 2] for i in range(0, len(fp_hex), 2))
    except Exception:
        logging.exception("fingerprint probe failed")
        return jsonify({"error": "fingerprint probe failed"}), 502

    # Prefer stored user/password, but allow PBS API token-only authentication
    pbs_user = (
        getattr(pbs_mgr, "user", None) or getattr(pbs_mgr, "username", None) or getattr(pbs_mgr, "api_token_id", None)
    )
    pbs_pass = getattr(pbs_mgr, "password", None) or getattr(pbs_mgr, "api_token_secret", None)
    if not pbs_user or not pbs_pass:
        return jsonify({"error": "PBS server has no stored username/password or API token"}), 400
    ...
    for cid in cluster_ids:
        ...
        data = {
            ...
            "type": "pbs",
            "server": pbs_mgr.host,
            "datastore": "",  # set below per-store iteration if multi
            "username": pbs_user,
            "password": pbs_pass,
        ...
        try:
            ...
            if r.status_code == 200:
                results.append({"cluster_id": cid, "ok": True, "storage": storage_name, "datastore": data["datastore"]})
                log_audit(
                    request.session.get("user", "system"),
                    "pbs.storage_auto_attached",
                ...
                try:
                    ...
                    if isinstance(err, dict):
                        err = html.escape(", ".join(f"{k}: {v}" for k, v in err.items()))
                except Exception:
                    err = html.escape(r.text or f"HTTP {r.status_code}")
...
def storage_preflight(cluster_id):
    ...
    # SSRF guard (2026-06-09, static-audit): the auth probe below POSTs the supplied
    ...
    try:
        ...
        _ok, _reason = is_safe_outbound_url(
            f"https://{server}:{port}/", allowed_schemes=("https",), allow_private=True
        )  # PBS is usually on the LAN
        if not _ok:
            logging.warning("[pbs] unsafe target: %s", _reason)
            return jsonify({"ok": False, "issues": ["unsafe target"], "info": {}}), 200
    except Exception:
        pass
    ...
    try:
        ...
        logging.exception("TCP %s:%s unreachable", server, port)
        return jsonify({"ok": False, "issues": [f"TCP {server}:{port} unreachable"], "info": info}), 200
    ...
    try:
        ...
        ctx.check_hostname = False
        ctx.verify_mode = _ssl.CERT_NONE
        with _sock.create_connection((server, port), timeout=6) as s, ctx.wrap_socket(
        ...
        logging.exception("TLS handshake failed for %s:%s", server, port)
        return jsonify({"ok": False, "issues": ["TLS handshake failed"], "info": info}), 200

    # 3) Auth probe
    try:
        import requests as _r

        _ssl = data.get("ssl_verify", True)
        if isinstance(_ssl, str):
            _ssl = _ssl.strip().lower() in ("1", "true", "yes", "on")
        s = _r.Session()
        s.verify = bool(_ssl)  # lgtm[py/request-without-cert-validation]
        ar = s.post(
            f"https://{server}:{port}/api2/json/access/ticket",
        ...
        if ar.status_code != 200:
            ...
            ar.json().get("data", {}).get("CSRFPreventionToken")
            # 4) Datastore exists?
            if datastore and ticket:
                ds_r = s.get(
                    f"https://{server}:{port}/api2/json/admin/datastore", cookies={"PBSAuthCookie": ticket}, timeout=8
                )
        ...
        logging.exception("auth probe error")
        issues.append("auth probe error")

    return jsonify({"ok": not issues, "issues": issues, "info": info})


# (SSE-perf): the per-VM backup pill rode fetchClusterResources at 15s,
...
def get_vms_backup_status(cluster_id):
    ...
    """
    # SECURITY (Jul 2026): this route was gated only by cluster.view (held by every
    ...
    def _scope_backup_out(rows):
        ...
        from ProxmoxVEx.utils.rbac import get_vm_acls, has_permission
    ...
    # 2026-05-31 (F1b) - parallelise both fanouts: per-PBS-server and
    ...
    def _scan_pbs(pbs):
        ...
        for store in stores:
            ...
            if not store_name:
                continue
            try:
                _snapshots = pbs.get_snapshots(store_name) or {}
                snaps = _snapshots.get("data", []) if isinstance(_snapshots, dict) else (_snapshots or [])
            except Exception:
                continue
            for sn in snaps:
    ...
    # 2026-05-31 (D2) - defense-in-depth: PVE-returned node names get
    ...
    def _scan_node(node):
        ...
        for s in stores:
            ...
            # (D2 cont.) - same belt-and-suspenders for storage names
            if not store_name or not _SAFE_NODE.match(store_name):
                continue
            try:
                cr = cm._api_get(
                    f"https://{cm.host}:{cm.api_port}/api2/json/nodes/{node}/storage/{store_name}/content?content=backup"
                )
    ...
    try:
        nodes_data = []
        try:
            nodes_data = cm._api_get(f"https://{cm.host}:{cm.api_port}/api2/json/nodes").json().get("data", []) or []
        except Exception:
            nodes_data = []
    ...
    _backup_status_cache[cluster_id] = (now, out, _BACKUP_STATUS_TTL if _scan_complete else _BACKUP_STATUS_TTL_PARTIAL)
...
def run_backup_job_now(cluster_id, job_id):
    ...
    try:
        ...
        logging.exception("failed to fetch jobs")
        return jsonify({"error": "failed to fetch jobs"}), 502
    ...
    if not pve_node:
        try:
            ...
            pve_node = next((n for n, d in ns.items() if d.get("status") == "online" or not d.get("offline")), None)
        except Exception:
            pve_node = None
    if not pve_node:
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            log_audit(
                request.session.get("user", "system"),
                "backup.run_now",
                f"Triggered backup job {job_id} on {html.escape(pve_node)}",
                cluster=cm.config.name,
            )
            return jsonify({"success": True, "upid": upid, "node": html.escape(pve_node)})
        return jsonify({"error": r.text or f"HTTP {r.status_code}"}), r.status_code
    except Exception:
        logging.exception("PBS backup start failed")
        return jsonify({"error": "PBS backup start failed"}), 500
...
def restore_backup(cluster_id):
    ...
    """
    # Restore is destructive (qmrestore into a VMID) - gate on cluster access, not just vm.backup
    ok, err = check_cluster_access(cluster_id)
    if not ok:
        return jsonify({"error": "Resource not found or access denied"}), err[1]
    if cluster_id not in cluster_managers:
        ...
        try:
            task_id = start_verification(
                cm,
                {
                    ...
                    "vmid": target_vmid,
                    "backup_volid": volid,
                    # Pass auto_cleanup=False so the test VM survives for inspection
                    "auto_cleanup": False,
                },
            )
            return jsonify({"success": True, "task_id": task_id, "mode": "test"})
        except Exception:
            logging.exception("Backup test-restore failed")
            return jsonify({"error": "Backup test-restore failed"}), 500
...
def diff_pbs_backups(pbs_id):
    ...
    try:
        _snapshots = pbs.get_snapshots(store) or {}
        snaps = _snapshots.get("data", []) if isinstance(_snapshots, dict) else (_snapshots or [])
    except Exception:
    ...
    if not sa:
        # snyk:ignore:Cross-site Scripting (XSS)
        return jsonify({"error": f"snapshot a not found: {ts_a}"}), 404
    if not sb:
        # snyk:ignore:Cross-site Scripting (XSS)
        return jsonify({"error": f"snapshot b not found: {ts_b}"}), 404
    ...
    return jsonify({
        "snapshot_a": {
            "time": ts_a,
            "verification": sa.get("verification"),
            "protected": sa.get("protected"),
            "comment": sa.get("comment"),
        },
        "snapshot_b": {
            "time": ts_b,
            "verification": sb.get("verification"),
            "protected": sb.get("protected"),
            "comment": sb.get("comment"),
        },
        "diffs": diffs,
        "summary": summary,
    })
...
def verify_schedule_config():
    ...
    Schema (in ProxmoxVEx config row 'pbs_verify_schedule'):
      {"enabled": bool, "weekly_count": int, "day": "sun", "hour": 4,
       "scope": "all"|"latest_per_vm", "max_age_days": int}
    """
    db = get_db()
    cur = db.conn.cursor()
    cur.execute("CREATE TABLE IF NOT EXISTS ProxmoxVEx_kv (k TEXT PRIMARY KEY, v TEXT)")
    db.conn.commit()
    if request.method == "GET":
        row = cur.execute("SELECT v FROM ProxmoxVEx_kv WHERE k=?", ("pbs_verify_schedule",)).fetchone()
        import json as _j
    ...
    cur.execute("INSERT OR REPLACE INTO ProxmoxVEx_kv (k, v) VALUES (?, ?)", ("pbs_verify_schedule", _j.dumps(cleaned)))
    db.conn.commit()
    log_audit(request.session.get("user", "system"), "pbs.verify_schedule_updated", f"Auto-verify schedule: {cleaned}")
