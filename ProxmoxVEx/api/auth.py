...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/api/auth.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: auth routes (login, logout, 2FA, OIDC, API tokens) -...
# Docs:        https://proxmoxvex.local/docs
# Generated:   2026-09-04
# --------------------------------------------------------------------
# --- ProxmoxVEx auto-header end ---
"""auth routes (login, logout, 2FA, OIDC, API tokens) - split from monolith dec 2025,"""
...
# Removed add_allowed_origin import (no longer auto-adding on login)
import requests
from flask import Blueprint, jsonify, make_response, request

from ProxmoxVEx.api.helpers import (
    effective_reverse_proxy,
    get_login_settings,
    get_session_timeout,
    load_server_settings,
)
from ProxmoxVEx.constants import RESERVED_ADMIN_USERNAME, ProxmoxVEx_VERSION
...
from ProxmoxVEx.utils.ldap import get_ldap_settings, ldap_authenticate, ldap_provision_user
from ProxmoxVEx.utils.oidc import (
    get_oidc_endpoints,
    get_oidc_settings,
    oidc_build_auth_url,
    oidc_decode_id_token,
    oidc_exchange_code,
    oidc_get_user_groups,
    oidc_get_user_info,
    oidc_map_groups_to_role,
    oidc_provision_user,
)
from ProxmoxVEx.utils.rbac import DEFAULT_TENANT_ID, get_user_permissions
from ProxmoxVEx.utils.sanitization import sanitize_identifier, sanitize_username
...
def oidc_authorize():
    ...
    Generates CSRF state, stores in session, redirects to IdP
    """
    ...
    # Auto-detect redirect URI if not configured
    # never use Origin header - attacker can spoof it to steal OAuth tokens
    if not config.get("redirect_uri"):
        base_url = request.host_url.rstrip("/")
        # Behind reverse proxy, trust forwarded headers for correct scheme/host
    ...
    # Append redirect_after for portal OIDC flow
    # 2026-05-31 - defense-in-depth: `startswith('/')` allows `//attacker.com`
...
def oidc_callback_redirect():
    """Safety net - if IdP sends GET to API callback URL, redirect to frontend SPA route"""
...
def oidc_callback():
    ...
    Called by frontend after redirect back from IdP
    Frontend sends: {code, state} from URL query params
    """
    # Use centralized IP resolution (respects trusted_proxies)
    ...
    # Auto-detect redirect URI if not configured (must match what was sent in authorize!)
    if not config.get("redirect_uri"):
        base_url = request.host_url.rstrip("/")
        if request.headers.get("X-Forwarded-Proto") == "https":
            fwd_host = request.headers.get("X-Forwarded-Host") or request.headers.get("Host") or request.host
    ...
    # Verify CSRF state + extract nonce + PKCE verifier
    ...
    if not stored_cookie:
        logging.warning(
            "[OIDC] oidc_state cookie MISSING — browser didn't send it back. Check proxy cookie forwarding."
        )
    else:
    ...
    # Redirect_after for portal OIDC flow
    stored_redirect_after = cookie_parts[3] if len(cookie_parts) >= 4 else ""
    if not stored_state or stored_state != state:
        logging.warning(
            f"[OIDC] State mismatch — stored={stored_state[:8] if stored_state else 'EMPTY'}..., received={state[:8] if state else 'EMPTY'}..."
        )
        return jsonify({
            "error": "Invalid state parameter (CSRF protection). Your browser may not be sending cookies — check reverse proxy configuration."
        }), 400
    ...
    # Step 2: Decode ID token for claims (with nonce + exp validation - Feb 2026)
    id_claims = {}
    if id_token_raw:
    ...
    if not config["auto_create_users"]:
        ...
        # This has to match the logic in oidc_provision_user or we get mismatches
    ...
    # SECURITY - oidc_provision_user returns None if local account would be overwritten
    if not user:
    ...
    # Create_session() handles max 3 sessions per user, session rotation, save_sessions()
    ...
    # Include portal_only so client portal can validate OIDC users
    ...
    response.set_cookie(
        "session_id", session_token, httponly=True, samesite="Strict", secure=is_secure, max_age=get_session_timeout()
    )
...
def oidc_get_public_config():
    ...
    Frontend needs to know if OIDC is enabled and button text
...
def oidc_test_connection():
    ...
    Tests connectivity to OIDC discovery and token endpoints
    """
    data = request.get_json() or {}
    config = get_oidc_settings()
    ...
    # (#188 follow-up): accept `oidc_issuer` as an alias for
    ...
    env_label = {"commercial": "Commercial", "gcc": "GCC", "gcc_high": "GCC High", "dod": "DoD"}.get(
        cloud_env, cloud_env
    )
    results.append({
        "step": "Configuration",
    ...
    # (#188 follow-up): if endpoint resolution failed outright
    ...
    # (#188) - surface whether discovery succeeded so admins immediately see when
    # ProxmoxVEx is using the (often wrong) issuer-relative fallback.
    if config.get("provider") != "entra":
        if endpoints.get("_discovery_used"):
            results.append({
                "step": "Discovery",
                "status": "ok",
                "detail": "OIDC discovery succeeded — using endpoints from .well-known",
            })
        else:
            results.append({
                "step": "Discovery",
    ...
    try:
        ...
        # SSRF guard. Same pattern as utils/oidc.py (token/discovery/jwks).
        ...
        try:
            # (CodeAnt #504) - use normalised URL returned by the guard
            ...
            logging.exception("SSRF guard rejected OIDC authorization URL")
            hint = ""
            ...
            results.append({
                ...
                "status": "error",
                "detail": f"URL rejected by SSRF guard; check the authority/issuer URL{hint}",
            })
            # lgtm[py/server-information-exposure]
        ...
        logging.exception("Authorization Endpoint test failed")
        results.append({
            "step": "Authorization Endpoint",
            "status": "error",
            "detail": "Authorization Endpoint test failed",
        })

    # Step 3: Test JWKS endpoint
    try:
        # Same SSRF guard, same allow-private toggle.
        allow_private_ip = bool(config.get("oidc_allow_private_ip", False))
        try:
            # (CodeAnt #504) - same normalisation as Step 2 above.
            validated_jwks_url = sanitize_outbound_url(endpoints["jwks"], allow_private=allow_private_ip)
        except SsrfError:
            logging.exception("SSRF guard rejected OIDC JWKS URL")
            hint = ""
            if not allow_private_ip:
                hint = ' (set "Allow private IP" in OIDC settings if this is intentional)'
            results.append({
        ...
        logging.exception("JWKS Endpoint test failed")
        results.append({
            "step": "JWKS Endpoint",
            "status": "error",
            "detail": "JWKS Endpoint test failed",
        })
...
# First-run setup endpoint. Reachable only while the install
# is uninitialised; closes itself once the first admin is created. Replaces
# the old auto-bootstrapped `ProxmoxVEx/admin` default that exposed every
...
def auth_setup():
    if is_initialized():
        ...
        return jsonify({
            "error": "ProxmoxVEx is already initialised",
    ...
    if username == RESERVED_ADMIN_USERNAME.lower():
        # snyk:ignore:Cross-site Scripting (XSS)
        return jsonify({"error": f"Choose a different username ('{RESERVED_ADMIN_USERNAME}' is reserved)"}), 400
    if not password:
    ...
    log_audit(username, "admin.initial_setup", f"First admin '{username}' created via setup wizard from {client_ip}")
...
def auth_login():
    ...
    # First-run safety gate. Before the setup wizard runs there
    # is no admin to authenticate against; refusing /login here closes the old
    # hardcoded-creds path (`ProxmoxVEx/admin` was bootstrapped automatically
    # which let any network-reachable fresh install be taken over).
    if not is_initialized():
        return jsonify({
            "error": "ProxmoxVEx is not initialised — run the setup wizard first",
    ...
    # Centralized IP resolution, handles trusted proxies + IPv6 normalization
    client_ip = get_client_ip()
    ...
    if client_ip in login_attempts_by_ip:
        ...
        if attempt_info.get("locked_until", 0) > current_time:
            ...
            # 2026-04-24 - uniform 401 (audit #5 applied here too). Previously
            # returned 429 with a distinctive body, which let an observer distinguish
            # IP-lockout vs. user-lockout vs. bad credentials.
            resp = jsonify({"error": "Invalid credentials", "locked": True, "retry_after": remaining})
            resp.headers["Retry-After"] = str(remaining)
            return resp, 401
        # cleanup old attempts
        attempt_info["attempts"] = [t for t in attempt_info.get("attempts", []) if current_time - t < attempt_window]
    ...
    # WebAuthn proof token from a prior /api/webauthn/auth/finish
    ...
    # 2026-04-24 - security audit: uniform 401 response here prevents username
    ...
    if username in login_attempts_by_user:
        ...
        user_attempt_info["attempts"] = [
            t for t in user_attempt_info.get("attempts", []) if current_time - t < attempt_window
        ]

    # Helper function to record failed attempt (both IP and username)
    # Also logs to audit trail now (was missing, security audit finding)
    def record_failed_attempt(target_username=None):
        ...
        recent_ip = [t for t in login_attempts_by_ip[client_ip]["attempts"] if current_time - t < attempt_window]
        ...
        if target_username:
            ...
            recent_user = [
                t for t in login_attempts_by_user[target_username]["attempts"] if current_time - t < attempt_window
            ]
    ...
    # Had a bug where user changes werent reflected until restart
    users_db = load_users()

    # =================================================================
    # LDAP Authentication (tried before local auth)
    ...
    if ldap_config["enabled"]:
        ...
        if ldap_result.get("success"):
            # LDAP auth succeeded - provision/update local user
            if ldap_config["auto_create_users"] or username in users_db:
                user = ldap_provision_user(ldap_result)
                if user is None:
                    # Local account exists - fall through to local auth
                ...
                return jsonify({"error": "User not authorized in ProxmoxVEx. Contact admin."}), 401
        elif ldap_result.get("error") == "User not found in LDAP":
            # User not in LDAP - fall through to local auth
            ...
            if username in users_db and users_db[username].get("auth_source") == "ldap":
                ...
                if locked:
                    # Uniform 401 - retry timing via header, not status code (audit
            ...
            # TLS errors should be surfaced for LDAP-only users
            # but still fall through for local users so they aren't locked out
            if (
                ("TLS" in ldap_err or "certificate" in ldap_err.lower())
                and username in users_db
                and users_db[username].get("auth_source") == "ldap"
            ):
    ...
    if not ldap_authenticated:
        ...
        if username not in users_db:
            # 2026-04-24 - equalize login timing. Without this the unknown-user branch
            ...
            if locked:
                # Uniform 401 - retry via header (audit #5, username enum prevention)
                resp = jsonify({"error": "Invalid credentials", "locked": True, "retry_after": lockout_time})
                resp.headers["Retry-After"] = str(lockout_time)
                return resp, 401
            return jsonify({"error": "Invalid credentials"}), 401
        ...
        # LDAP-only users cannot login with local password
    ...
    # 2FA: TOTP and/or WebAuthn. If a user has EITHER registered,
    ...
    try:
        _db_probe = get_db()
        _cnt_row = _db_probe.query_one("SELECT COUNT(*) AS n FROM webauthn_credentials WHERE username = ?", (username,))
    ...
    # Auto-migrate password to Argon2id if using old PBKDF2 format
    ...
    log_audit(username, "user.login", "User logged in" + (" (with 2FA)" if user.get("totp_enabled") else ""))

    # Removed auto-allow CORS origin on login (security audit)
    # Was allowing any authenticated origin permanently. Use PROXMOXVEX_ALLOWED_ORIGINS env var instead.

    # Get default theme for response
    settings = load_server_settings()
    default_theme = settings.get("default_theme", "proxmoxDark")

    # Check if user needs to set up 2FA (force_2fa setting)
    ...
    # Debug log for theme sync issues
    ...
    response = jsonify({
        ...
        "user": {
            ...
            "auth_source": user.get("auth_source", "local"),  # For LDAP/Entra/OIDC badge
            "permissions": get_user_permissions(user),  # Frontend can hide/show buttons
            "tenant_id": user.get("tenant_id", DEFAULT_TENANT_ID),
            "totp_enabled": user.get("totp_enabled", False),
            "theme": user_theme,  # Use default if empty
            "language": user.get("language", ""),
            "ui_layout": user.get("ui_layout", "modern"),
            "taskbar_auto_expand": user.get("taskbar_auto_expand", True),  # Feb 2026
            "taskbar_visible": user.get("taskbar_visible", True),
            "taskbar_expanded": user.get("taskbar_expanded", False),
            "sidebar_show_vmid": user.get("sidebar_show_vmid", False),  # Corporate sidebar VMIDs
            "layout_chosen": user.get("layout_chosen", False),
            "portal_only": user.get("portal_only", False),
        },
        "session_id": session_id,
        "portal_only": user.get("portal_only", False),
        "default_theme": default_theme,  # Include for frontend fallback
        "reverse_proxy_enabled": effective_reverse_proxy(settings),
        "air_gap_mode": settings.get("air_gap_mode", False),
        "requires_2fa_setup": requires_2fa_setup,  # Force 2FA
    ...
    # Secure flag only when using HTTPS (important for production!)
    from ProxmoxVEx.utils.audit import _is_trusted_proxy
...
# Session self-service. Admins could see all sessions via the users API
...
def _revocation_token_for(sess):
    ...
    return the same token.  replaces returning
...
def list_own_sessions():
    ...
    from ProxmoxVEx.utils.auth import active_sessions, sessions_lock
...
def revoke_own_session(token):
    ...
    Was previously by-full-sid; now by token (M-5 fix)."""
    session_id = request.headers.get("X-Session-ID") or request.cookies.get("session_id")
    current = validate_session(session_id)
    if not current:
        return jsonify({"error": "not authenticated"}), 401
...
@bp.route("/api/auth/sessions/kill-all", methods=["POST"])
@require_auth(roles=[ROLE_ADMIN])
def kill_all_sessions():
    """Admin panic button: invalidate every active session for every user,
    including the admin's own current one. Sessions already persist across
    restarts, so this is the counterpart for "log everyone out right now"
    during a suspected credential compromise, without needing a restart.
    """
    actor = request.session["user"]
    count = invalidate_all_sessions()
    log_audit(actor, "auth.sessions_kill_all", f"Killed all {count} active session(s) system-wide")
    response = jsonify({"success": True, "sessions_killed": count})
    # The caller's own session just died too - clear their cookie so the
    # frontend doesn't keep sending a dead session_id.
...
def auth_logout():
    ...
    if session_id:
        ...
        if session:
            ...
            log_audit(session["user"], "user.logout", "User logged out")
...
def health_check():
    ...
    return jsonify({"status": "ok", "version": ProxmoxVEx_VERSION})
...
def auth_check():
    ...
    session = validate_session(session_id)
    if not session:
        # Include LDAP/OIDC status so login page can show indicators
        settings = load_server_settings()
        ...
        # First-run signal so the React shell can show the setup
        # wizard instead of the login form on an uninitialised install.
        # Return 200 with authenticated=False so the login page does not spam
        # the browser console with "Failed to load resource" 401 messages.
        return jsonify({
            "authenticated": False,
    ...
    # If user was disabled while session is active, force logout
    if not user or not user.get("enabled", True):
        logging.info(f"[AUTH_CHECK] User '{session['user']}' is disabled or deleted, invalidating session")
        invalidate_session(session_id)
        return jsonify({"authenticated": False, "reason": "account_disabled"}), 200
    ...
    # Admins are exempt by default, but can opt-in via settings
    # LDAP/OIDC users are ALWAYS exempt - they don't have local passwords
    ...
    should_check_expiry = (
        settings.get("password_expiry_enabled") and not is_external_auth and (not is_admin or include_admins)
    )
    ...
    user_theme = user.get("theme", "") or default_theme
    logging.debug(
        f"[AUTH_CHECK] User {session['user']} theme from DB: '{user.get('theme', '')}', using: '{user_theme}'"
    )

    # Always check database for latest role, session might be stale if admin changed it
    ...
    if fresh_role != session["role"]:
        # Update session to match database (avoids stale role in session)
    ...
    # Get effective permissions for UI visibility
    ...
    return jsonify({
        ...
        "user": {
            ...
            "avatar_url": user.get("avatar_url", ""),
            "auth_source": user.get("auth_source", "local"),  # For LDAP/Entra/OIDC badge
            "tenant_id": user.get("tenant_id", DEFAULT_TENANT_ID),  # For multi-tenant UI
            "permissions": user_permissions,  # So frontend knows what buttons to show
            "theme": user_theme,
            "language": user.get("language", ""),
            "ui_layout": user.get("ui_layout", "modern"),
            "taskbar_auto_expand": user.get("taskbar_auto_expand", True),  # Feb 2026
...
def auth_validate():
    ...
    Shell/VNC WebSocket needs a simple endpoint to validate session
    Returns 200 if valid, 401 if not
    """
    # Check both cookie (sent via requests.get with cookies=) and header
    session_id = (
        request.cookies.get("session") or request.cookies.get("session_id") or request.headers.get("X-Session-ID")
    )
    ...
    return jsonify({"valid": True, "user": session["user"], "role": session["role"]})
...
def get_cluster_creds_internal(cluster_id):
    ...
    Returns node IPs for SSH connections
    ...
    # Check_cluster_access reads request.session['user'], which
    ...
    # Mar 2026: use standard cluster access check (validates user's cluster assignments + tenant)
    ...
    # Check permissions - Feb 2026
    users_db = load_users()
    user_data = users_db.get(session["user"], {})
    # 2026-06-10 (RBAC): gate on node.shell only - admin holds it via all-perms, so the
    ...
    # (PR #324): ?node=<name> resolves only the requested node.
    ...
    # Feb 2026: Never expose Proxmox password via API - shell proxy handles auth server-side
    # Mar 2026: removed user field from response, only SSH proxy needs it internally
    # Apr 2026: expose ssh_port so the WS server can honor non-22 configs
    # 2026-06-04: expose verify_pve_tls so the SSH-WS subprocess can decide
    ...
    # 2026-06-05 (C-1): the termproxy WS proxy gets the PVE session cookie
...
def verify_password_api():
    ...
    # Security audit - rate limit to prevent brute-force after session compromise
    ...
    # (#587): OIDC/Entra admins have no local password - they re-auth via the
    ...
    if auth_source == "local":
        if not user.get("password_hash") or not verify_password(
            password, user.get("password_salt", ""), user["password_hash"]
        ):
        ...
        # May 2026: stolen-cookie replay risk - the original "session valid → accept"
        ...
        # (v0.9.9.1 follow-up) - TOTP step-up was wrong as a hard
        # requirement: ProxmoxVEx blocks OIDC users from enrolling TOTP at /2fa/setup
...
def auth_change_password():
    ...
    Security feature - invalidates all other sessions after password change
    ...
    # Rate limit this - someone with a stolen session could brute-force the current password
    ...
    # LDAP/OIDC users cannot change their password here - they must change it in their identity provider
    if user.get("auth_source", "local") in ("ldap", "oidc", "entra"):
        provider_name = {
            "ldap": "LDAP/Active Directory",
            "oidc": "your OIDC provider",
            "entra": "Microsoft Entra ID",
        }.get(user["auth_source"], "your identity provider")
        # snyk:ignore:Cross-site Scripting (XSS)
    ...
    user["password_changed_at"] = datetime.now().isoformat()  # Reset expiry timer
    ...
    # 2026-04-24 - security audit finding #1: invalidate ALL sessions including the
    ...
    request.cookies.get("session_id") or request.headers.get("X-Session-ID")
    ...
    log_audit(
        ...
        f"Password changed, {sessions_removed} session(s) invalidated (incl. current)",
    )
...
def setup_2fa():
    ...
    if not TOTP_AVAILABLE:
        return jsonify({
            "error": "2FA not available. Please install pyotp and qrcode: pip install pyotp qrcode[pil]"
        }), 500

    username = request.session["user"]
    logging.info(f"2FA setup requested for user: {username}")  # Debug
    users_db = load_users()
    ...
    # OIDC/Entra users should use their IdP's MFA, not ProxmoxVEx 2FA
    # ProxmoxVEx 2FA only works for login form (LDAP + local), not OIDC redirect flow
    ...
    save_users(users_db)
    logging.info(f"2FA setup: saved pending secret for user {username}")  # Debug

    # Generate provisioning URI
    totp = pyotp.TOTP(secret)
    uri = totp.provisioning_uri(name=username, issuer_name="ProxmoxVEx")
    ...
    return jsonify({"secret": secret, "qr_code": f"data:image/png;base64,{qr_base64}", "uri": uri})
...
def verify_2fa_setup():
    ...
    if not code:
        logging.warning("2FA verify: no code provided")  # Debug
        return jsonify({"error": "TOTP code required"}), 400

    username = request.session["user"]

    # Only 1M possible 6-digit codes, easy to brute force without this
    # Tightened from 5/300s to 3/120s - reduces brute-force success probability
    ...
    logging.info(f"2FA verify requested for user: {username}, code length: {len(code)}")  # Debug
    users_db = load_users()
    ...
    if not pending_secret:
        logging.warning(f"2FA verify: no pending secret for user {username}. User keys: {list(user.keys())}")  # Debug
...
def disable_2fa():
    ...
    # Same issue as pwd change - stolen session + unlimited guesses = bad
    ...
    # OIDC/Entra users manage MFA through their IdP - shouldn't have ProxmoxVEx 2FA
    if user.get("auth_source", "local") in ("oidc", "entra"):
        return jsonify({"error": "2FA is managed by your identity provider"}), 400

    # LDAP users verify against LDAP (they have no local password hash)
...
def get_2fa_status():
    ...
    return jsonify({"enabled": user.get("totp_enabled", False), "available": TOTP_AVAILABLE})


# =====================================================
# USER PREFERENCES - # Per-user settings (theme, language)
# =====================================================

# =============================================================================
# API Token Management Endpoints
# Users can create tokens for CI/CD, scripts, monitoring integrations
# Admins can see all tokens, users can only manage their own
# =============================================================================
...
def list_api_tokens():
    ...
    ensure_api_tokens_table()
    username = request.session["user"]
    request.session.get("role", ROLE_VIEWER)

    # 2026-06-10 (RBAC): the admin.api perm (not the admin role) sees all tokens - admin holds it via all-perms.
    from ProxmoxVEx.utils.rbac import has_permission as _has_perm
    ...
    if _has_perm(load_users().get(username, {}), "admin.api") and request.args.get("all") == "true":
        try:
            ...
            logging.exception("Failed to list tokens")
...
def revoke_api_token_endpoint(token_id):
    ...
    # 2026-06-10 (RBAC): the admin.api perm can revoke any token (admin holds it via all-perms).
    from ProxmoxVEx.utils.rbac import has_permission as _has_perm
