...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/api/site_recovery.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: site recovery plans & failover orchestration - Mar 2026
# Docs:        https://proxmoxvex.local/docs
# Generated:   2026-09-04
# --------------------------------------------------------------------
# --- ProxmoxVEx auto-header end ---
# site recovery plans & failover orchestration - Mar 2026
...
# 2026-06-04 (CWE-117): plan-name + status are admin-controlled but the
...
def _plan_with_vms(plan):
    ...
    for vm in plan["vms"]:
        if vm.get("replication_job_id"):
            repl = db.query_one(
                "SELECT last_run, last_status FROM cross_cluster_replications WHERE id = ?", (vm["replication_job_id"],)
            )
            if repl:
...
def list_plans():
    db = get_db()
    # (pentest MEDIUM) - a site_recovery.view holder must not enumerate
...
def create_plan():
    ...
    # Validate clusters actually exist
    ...
    db.execute(
        """INSERT INTO site_recovery_plans
    ...
    log_audit(
        usr,
        "site_recovery.plan_created",
        f"Created recovery plan '{data['name']}': {data['source_cluster']} → {data['target_cluster']}",
    )
...
def get_plan_detail(plan_id):
    ...
    ok, _ = check_cluster_access(plan["source_cluster"])
    if not ok:
        return jsonify({"error": "Access denied to this cluster"}), 403
    ok, _ = check_cluster_access(plan["target_cluster"])
    if not ok:
        return jsonify({"error": "Access denied to this cluster"}), 403
...
def update_plan(plan_id):
    ...
    db = get_db()
    query = f"UPDATE site_recovery_plans SET {', '.join(updates)} WHERE id = ?"  # nosec: B608 - fields from allowlist
...
def add_plan_vm(plan_id):
    ...
    # (#477 port) - VM-level ACL gate on top of cluster-level above.
    ...
    # check for duplicate
    exists = db.query_one("SELECT id FROM site_recovery_vms WHERE plan_id = ? AND vmid = ?", (plan_id, data["vmid"]))
    if exists:
        # snyk:ignore:Cross-site Scripting (XSS)
        return jsonify({"error": f"VM {data['vmid']} already in this plan"}), 409

    db.execute(
        """INSERT INTO site_recovery_vms
        ...
        (
            ...
            data.get("boot_group", 0),
            data.get("boot_delay", 30),
            data.get("replication_job_id", ""),
            data.get("notes", ""),
        ),
    )
...
def update_plan_vm(plan_id, vm_id):
    ...
    if updates:
        params.append(vm_id)
        query = f"UPDATE site_recovery_vms SET {', '.join(updates)} WHERE id = ?"  # nosec: B608 - fields from allowlist
        db.execute(query, params)
...
def check_readiness(plan_id):
    ...
    for vm in vms:
        if vm.get("replication_job_id"):
            repl = db.query_one(
                "SELECT last_run, last_status, enabled FROM cross_cluster_replications WHERE id = ?",
    ...
    # (#350) - get_node_status() returns mem_total/mem_used, not the
    ...
    if not net_maps:
        issues.append({
            "severity": "info",
            "msg": "No network mappings configured (VMs will keep original bridge names)",
        })

    # 2026-04-24 - actually verify mapping targets exist on target cluster.
    ...
    if tgt_mgr and tgt_mgr.is_connected:
        try:
            from ProxmoxVEx.background.site_recovery import validate_mappings

            issues.extend(validate_mappings(tgt_mgr, stor_maps, net_maps))
        except Exception:
            logging.exception("Mapping validation failed to run")
            issues.append({"severity": "warning", "msg": "Mapping validation failed to run"})

    # update last readiness check timestamp
    now = datetime.utcnow().isoformat()
    db.execute(
        "UPDATE site_recovery_plans SET last_readiness_check = ?, updated_at = ? WHERE id = ?", (now, now, plan_id)
    )
    ...
    return jsonify({"status": status, "issues": issues, "vm_count": len(vms), "checked_at": now})
...
def execute_planned_failover(plan_id):
    ...
    # (#413 layer 5) - was `WHERE status = 'ready'` which broke the moment
    ...
    if cur.rowcount != 1:
        ...
        return jsonify({
            "error": f"Cannot start failover — plan is in state '{actual}' (need ready/completed/failed)"
        }), 409

    from ProxmoxVEx.background.site_recovery import execute_failover
...
def execute_emergency_failover(plan_id):
    ...
    # (#413 layer 5) - mirror the planned-failover relaxation: a successful
    ...
    if cur.rowcount != 1:
        ...
        actual = (fresh or {}).get("status", "unknown")
        return jsonify({
            "error": f"Cannot start emergency failover — plan is in state '{actual}' (need ready/completed/failed)"
        }), 409
...
def execute_test_failover(plan_id):
    ...
    from ProxmoxVEx.background.site_recovery import execute_test_failover
...
def cleanup_test_failover(plan_id):
    ...
    from ProxmoxVEx.background.site_recovery import cleanup_test
...
def get_plan_events(plan_id):
    ...
    db = get_db()
    rows = db.query(
        "SELECT * FROM site_recovery_events WHERE plan_id = ? ORDER BY started_at DESC LIMIT 50", (plan_id,)
    )
    events = []
    for r in rows or []:
        ev = dict(r)
