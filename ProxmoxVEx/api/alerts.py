...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/api/alerts.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: alerts & cluster affinity rules routes - split from...
# Docs:        https://proxmoxvex.local/docs
# Generated:   2026-09-04
# --------------------------------------------------------------------
# --- ProxmoxVEx auto-header end ---
"""alerts & cluster affinity rules routes - split from monolith dec 2025"""
...
from ProxmoxVEx.background.alerts import load_alerts_config, save_alerts_config
...
def load_cluster_alerts():
    ...
    Keeps falling back to JSON if database fails which is kinda nice for debugging
    ...
    try:
        ...
        for row in cursor.fetchall():
            ...
            try:
                ...
                alerts[cluster_id].append({"id": row["alert_type"], "name": row["alert_type"], "config": row["config"]})

        return alerts
    except Exception as e:
        logging.error(f"Error loading cluster alerts from DB: {e}", exc_info=True)
...
def save_cluster_alerts(alerts):
    ...
    Stores each alert as a row with config containing full alert object
            ...
            if isinstance(alert_list, list):
                for alert in alert_list:
                    ...
                    cursor.execute(
                        ...
                        VALUES (?, ?, ?, ?, ?)
                    """,
                        (cluster_id, alert_id, json.dumps(alert), 1 if alert.get("enabled", True) else 0, now),
                    )
            # Handle dict format (legacy)
                ...
                for alert_type, config in alert_list.items():
                    cursor.execute(
                        ...
                        VALUES (?, ?, ?, 1, ?)
                    """,
                        (cluster_id, alert_type, json.dumps(config) if isinstance(config, dict) else str(config), now),
                    )

        db.conn.commit()
    except Exception as e:
        logging.error(f"Error saving cluster alerts to DB: {e}", exc_info=True)
...
def get_cluster_alerts(cluster_id):
    ...
    try:
        ...
        logging.error(f"Error getting cluster alerts: {e}", exc_info=True)
        return jsonify({"alerts": [], "error": "Alert operation failed"})


# #501 hardening (F1): bound the escalation chain + channel fan-out so a runaway
...
def create_cluster_alert(cluster_id):
    ...
    # Used to silently drop `channels` and `cluster_id`, so the
    ...
    alert = {
        ...
        "channels": channels,
        "severity": data.get("severity", "auto"),  # #501: 'auto' | critical | warning | info
        "escalation": _sanitize_escalation(data.get("escalation")),  # #501 (F1: bounded chain)
    ...
    for alert in cluster_alerts:
        if alert["id"] == alert_id:
            ...
            if "escalation" in data:
                alert["escalation"] = _sanitize_escalation(data["escalation"])  # #501 (F1: bounded)
...
def delete_cluster_alert(cluster_id, alert_id):
    ...
    # Delete directly from database for efficiency
    try:
        db = get_db()
        cursor = db.conn.cursor()
        cursor.execute("DELETE FROM cluster_alerts WHERE cluster_id = ? AND alert_type = ?", (cluster_id, alert_id))
        db.conn.commit()
        deleted = cursor.rowcount > 0
        return jsonify({"success": True, "deleted": deleted})
    except Exception as e:
        logging.error(f"Error deleting cluster alert: {e}", exc_info=True)
...
def get_active_alerts(cluster_id):
    ...
    try:
        ...
        query = (
            "SELECT current_value, threshold, operator, triggered_at, "
            "last_fired_at, acked_at, acked_by, escalation_step "
            "FROM active_alerts WHERE cluster_id = ? AND resolved_at IS NULL "
            "ORDER BY triggered_at DESC"
        )
        rows = cur.execute(query, (cluster_id,)).fetchall()
        return jsonify({"active_alerts": [dict(zip(cols, r)) for r in rows]})
    except Exception as e:
        logging.error(f"Error listing active alerts: {e}", exc_info=True)
        return jsonify({"active_alerts": [], "error": "Alert operation failed"})
...
def ack_active_alert(cluster_id, fired_id):
    ...
    if not ok:
        return jsonify({"error": "Access denied to this cluster"}), 403
    user = getattr(request, "username", None) or (
        request.session.get("user", "unknown") if hasattr(request, "session") else "unknown"
    )
    try:
        db = get_db()
        cur = db.conn.cursor()
...
def load_cluster_affinity_rules():
    ...
    Affinity = keep VMs together, anti-affinity = keep them apart
    useful for HA where you want replicas on different hosts
    Reuses the affinity_rules table we already had
    """
    try:
        db = get_db()
        cursor = db.conn.cursor()
        ...
        for row in cursor.fetchall():
            ...
            rules[cluster_id].append({
                ...
                "type": row["type"],
                "vms": vms_list,
                "vm_ids": vms_list,  # Alias for frontend compatibility
        ...
        logging.error(f"Error loading affinity rules from DB: {e}", exc_info=True)
        # Fallback to JSON for backwards compat
        try:
...
def save_cluster_affinity_rules(rules):
    ...
    Uses upsert pattern, handles both 'vms' and 'vm_ids' field names
    """
    try:
        db = get_db()
        cursor = db.conn.cursor()
        ...
        logging.error(f"Error saving affinity rules to DB: {e}", exc_info=True)
...
def get_cluster_affinity_rules(cluster_id):
    ...
    try:
        ...
        logging.error(f"Error getting affinity rules: {e}", exc_info=True)
        return jsonify({"rules": [], "error": "Alert operation failed"})
...
def create_cluster_affinity_rule(cluster_id):
    ...
    # Get vms from either 'vm_ids' or 'vms' field
...
def delete_cluster_affinity_rule(cluster_id, rule_id):
    ...
    # Delete directly from database instead of load/filter/save
    try:
        db = get_db()
        cursor = db.conn.cursor()
        cursor.execute("DELETE FROM affinity_rules WHERE id = ? AND cluster_id = ?", (rule_id, cluster_id))
        db.conn.commit()
        deleted = cursor.rowcount > 0
        return jsonify({"success": True, "deleted": deleted})
    except Exception as e:
        logging.error(f"Error deleting affinity rule: {e}", exc_info=True)
        return jsonify({"error": "Alert operation failed"}), 500
...
def get_alerts():
    ...
    # (CodeAnt IDOR) - scope alert configs to the caller's reachable clusters
...
def list_alert_channels():
    ...
    # Scrub url secrets on read - ?full=1 bypasses for edit flows
...
def create_alert_channel():
    from ProxmoxVEx.api.helpers import load_server_settings, save_server_settings
    from ProxmoxVEx.utils.webhooks import new_channel
    ...
    log_audit(
        ...
        "alerts.channel_create",
        f"added webhook '{ch.get('name')}' ({ch.get('type')})",
    )
...
def update_alert_channel(cid):
    ...
    for i, ch in enumerate(channels):
        ...
        save_server_settings(settings)
        log_audit(
            request.session.get("user", "admin"), "alerts.channel_update", f"updated webhook '{updated.get('name')}'"
        )
...
def test_alert_channel(cid):
    from ProxmoxVEx.api.helpers import load_server_settings
    from ProxmoxVEx.utils.webhooks import _guard_url, send_to_channel
    ...
    if not ok_url:
        return jsonify({"error": "blocked: unsafe url"}), 400
    alert = {
        "alert_name": "ProxmoxVEx test alert",
        "metric": "test",
        "current_value": 42,
        "threshold": 0,
        "operator": ">",
        ...
        "severity": "info",
        "message": "This is a test alert triggered from ProxmoxVEx settings.",
        "timestamp": datetime.now().isoformat(),
    }
    ok, _detail = send_to_channel(ch, alert)  # nosec CWE-918
...
# When customers report "alerts don't fire", the previous
...
def alerts_diagnostics():
    from ProxmoxVEx.api.helpers import load_server_settings
    from ProxmoxVEx.background import alerts as A
    ...
    return jsonify({
        ...
        "webhook_channels": [
            {"id": c.get("id"), "name": c.get("name"), "type": c.get("type"), "enabled": c.get("enabled", True)}
            for c in (settings.get("alert_webhooks") or [])
        ],
        "clusters_loaded": sorted(
            [{"id": cid, "connected": bool(getattr(m, "is_connected", False))} for cid, m in cluster_managers.items()],
            key=lambda r: r["id"],
        ),
        "alerts": [
            {
...
def alerts_force_check():
    ...
    try:
        ...
        logging.error(f"Error forcing alert check: {e}", exc_info=True)
