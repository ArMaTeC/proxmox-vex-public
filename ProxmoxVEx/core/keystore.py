...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/core/keystore.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: Master-key loader for ProxmoxVEx.
# Docs:        https://proxmoxvex.local/docs
# Generated:   2026-09-04
# --------------------------------------------------------------------
# --- ProxmoxVEx auto-header end ---
"""
Master-key loader for ProxmoxVEx.

Resolves the AES-256 master key from the most-secure available source.
Order of precedence (highest first):

  1. PROXMOXVEX_DB_KEY            — env var (Docker secrets / k8s / explicit)
  ...
  3. PROXMOXVEX_KEY_FILE          — env var pointing to a custom location
  4. /etc/ProxmoxVEx/secret.key   — system-service install default
  5. ~/.config/ProxmoxVEx/secret.key — user install default
...
Addresses the "decryption key sits next to the encrypted database"
    ...
    with _CACHE_LOCK:
        ...
        if _CACHED.is_legacy:
            _LOG.warning(
                ...
                "recommended: move to /etc/ProxmoxVEx/secret.key or use systemd "
                "LoadCredentialEncrypted. See docs/SECURITY.md.",
                _CACHED.source_path,
            )
        else:
            _LOG.info(
                "[KEYSTORE] master key source: %s%s",
                _CACHED.source,
...
def _safe_is_file(p: Path) -> bool:
    ...
    (#417 follow-up from tfoks): if a tier path lives under a
    parent the service user can't traverse (e.g. systemd `ProtectHome=true`,
    or `/home/ProxmoxVEx` exists but is mode 700 owned by someone else, or
    ...
    valid `config/.ProxmoxVEx.key` in place failed at boot with
    `[Errno 13] Permission denied: '/home/ProxmoxVEx/.config/...'`. Each
...
def _resolve() -> MasterKey:
    # Tier 1: PROXMOXVEX_DB_KEY env (Docker / k8s / explicit)
    if env_val := os.environ.get("PROXMOXVEX_DB_KEY"):
        return _from_env_value(env_val, source="env:PROXMOXVEX_DB_KEY")
    ...
    if path_val := os.environ.get("PROXMOXVEX_KEY_FILE"):
        p = Path(path_val).expanduser()
        if _safe_is_file(p):
            return _from_file(p, source="env:PROXMOXVEX_KEY_FILE")

    # Tier 4: /etc/ProxmoxVEx/secret.key (system-service install default)
    p = Path("/etc/ProxmoxVEx/secret.key")
    if _safe_is_file(p):
        return _from_file(p, source="/etc/ProxmoxVEx/secret.key")

    # Tier 5: ~/.config/ProxmoxVEx/secret.key (user install default)
    p = Path("~/.config/ProxmoxVEx/secret.key").expanduser()
    if _safe_is_file(p):
    ...
    # 0.9.9.x installs.  A subsequent operator-initiated `ProxmoxVEx secure-key
...
def _from_env_value(val: str, source: str) -> MasterKey:
    ...
    try:
        ...
        raise RuntimeError(f"[KEYSTORE] {source} key must decode to 32 bytes ({e})") from None

    if len(raw) != 32:
        raise RuntimeError(f"[KEYSTORE] {source} key must decode to 32 bytes (got {len(raw)})")
    return MasterKey(
...
def _from_file(path: Path, source: str, strict_perms: bool = True) -> MasterKey:
    ...
    try:
        raw = _decode_key_string(
            raw_or_b64.decode("ascii", errors="replace") if all(c < 128 for c in raw_or_b64) else raw_or_b64
        )
    except ValueError as e:
        raise RuntimeError(f"[KEYSTORE] key at {path} must decode to 32 bytes ({e})") from None

    if len(raw) != 32:
        raise RuntimeError(f"[KEYSTORE] key at {path} must decode to 32 bytes (got {len(raw)})")
    return MasterKey(
        key_b64=base64.urlsafe_b64encode(raw),
        key_raw=raw,
        source=source,
...
def _generate_at(path: Path, source: str, is_legacy: bool = False) -> MasterKey:
    ...
    try:
        ...
        raise RuntimeError(f"[KEYSTORE] cannot create key dir {path.parent}: {e}") from None
...
def _legacy_key_path() -> Path:
    ...
    Imported lazily to avoid circular imports with ProxmoxVEx.constants."""
    try:
        from ProxmoxVEx.constants import KEY_FILE
...
def _enforce_perms(path: Path) -> None:
    ...
    Accepted modes:
      ...
      - 0640 / 0440 — key owned by root with the service group granted read
                       ...
                       systemd service running as ProxmoxVEx)
    Rejected: anything with group-write, group-exec, or any other-perm bit set.
    """
    try:
        st = os.stat(path)
    except Exception as e:
        raise RuntimeError(f"[KEYSTORE] cannot stat {path}: {e}") from None

    forbidden = stat.S_IWGRP | stat.S_IXGRP | stat.S_IRWXO
    if st.st_mode & forbidden:
        raise RuntimeError(
            f"[KEYSTORE] key at {path} has loose permissions ({oct(st.st_mode)[-3:]}) — must be 0600 or 0640 only."
        )
...
def _tier_for_source(source: str) -> int:
    ...
    if source.startswith("env:PROXMOXVEX_DB_KEY"):
        return 1
    if source.startswith("systemd-credential"):
        return 2
    if source.startswith("env:PROXMOXVEX_KEY_FILE"):
        return 3
    if source.startswith("/etc/ProxmoxVEx"):
        return 4
    if source.startswith("user-config"):
        return 5
    return 6


def _recommendation_for(mk: MasterKey) -> str | None:
    if mk.is_legacy:
        return (
            "Master key in CONFIG_DIR is the legacy default. "
            "Run `ProxmoxVEx secure-key migrate` to move it to "
            "/etc/ProxmoxVEx/secret.key, or set up systemd LoadCredentialEncrypted "
