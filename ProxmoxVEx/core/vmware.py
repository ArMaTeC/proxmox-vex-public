...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/core/vmware.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: ProxmoxVEx VMware/vCenter Integration - Layer 5
# Docs:        https://proxmoxvex.local/docs
# Generated:   2026-09-04
# --------------------------------------------------------------------
# --- ProxmoxVEx auto-header end ---
"""
ProxmoxVEx VMware/vCenter Integration - Layer 5
...
class VMwareManager:
    ...
    Uses vSphere REST API (available since vSphere 6.5+).
    ...
    def __init__(self, vmware_id: str, config: dict):
        ...
        self._si: Any = None  # pyvmomi ServiceInstance
        self._soap_content: Any = None  # pyvmomi ServiceContent
        ...
        # Session-keepalive plumbing. Customers reported ProxmoxVEx
    ...
    def connect(self) -> bool:
        ...
        The /API/session endpoint was introduced in vSphere 6.5.
                    ...
                    try:
                        ...
                        if legacy_path == "/api/session":
                            ...
                            headers["Authorization"] = (
                                "Basic "
                                + __import__("base64").b64encode(f"{self.username}:{self.password}".encode()).decode()
                            )
                            resp = requests.post(
                                f"{self._base_url}{legacy_path}", headers=headers, verify=self.ssl_verify, timeout=15
                            )
                        else:
                            resp = requests.post(
                ...
                for test_path in ["/api/vcenter/vm", "/api/vcenter/host", "/rest/vcenter/vm"]:
                    try:
                        test_resp = requests.get(
                            f"{self._base_url}{test_path}", headers=self._headers(), verify=self.ssl_verify, timeout=10
                        )
                ...
                if not validation_ok:
                    ...
                    try:
                        ...
                        if "error" not in info:
                            ...
                            about = self.api_get(
                                "/api/vcenter/system/config/version"
                                if self._api_style == "modern"
                                else "/rest/vcenter/system/config/version"
                            )
                    ...
                    logging.info(
                        f"[VMware:{self.id}] Connected to {self.host} ({self.server_type}, API: {self.api_version}, style: {self._api_style})"
                    )
                    return True
        ...
        try:
            ...
            try:
                _probe = _sock.create_connection(
                    ...
                    timeout=float(os.environ.get("PROXMOXVEX_VMWARE_CONNECT_TIMEOUT", "8")),
                )
                _probe.close()
            ...
            logging.info(
                f"[VMware:{self.id}] Connected via SOAP/pyvmomi to {self.host} ({self.server_type}, v{self.api_version})"
            )
            return True
            ...
            if "InvalidLogin" in err_str or "incorrect user name or password" in err_str:
                ...
                if fail_count < 2:
                    logging.warning(
                        f"[VMware:{self.id}] SOAP login failed - wrong username/password for {self.username}@{self.host}"
                    )
            elif (
                "No route" in err_str
    ...
    def _soap_get_container(self, obj_type, recursive=True):
        ...
        # Self-heal stale SOAP sessions. Long-running V2P jobs
        ...
        try:
            ...
            view_type = [obj_type]
            container_view = self._soap_content.viewManager.CreateContainerView(container, view_type, recursive)
    ...
    def _build_validated_url(self, path: str) -> str:
        ...
        (Aikido #489 manual port) - every caller passes a hardcoded
    ...
    def api_get(self, path: str, params: dict = None) -> dict:
        ...
        try:
            ...
            resp = requests.get(url, headers=self._headers(), params=params, verify=self.ssl_verify, timeout=30)
            if resp.status_code == 401 and self.connect():
    ...
    def api_delete(self, path: str) -> dict:
        ...
        try:
            ...
            try:
                ...
                return {"error": f"invalid path: {ve}"}
            resp = requests.delete(url, headers=self._headers(), verify=self.ssl_verify, timeout=30)
            ...
            for vm in vms:
                try:
                    ...
                    result.append({
                        ...
                        "power_state": str(runtime.powerState).replace("powered", "POWERED_").upper()
                        if runtime
                        else "UNKNOWN",
        ...
        try:
                ...
                for dev in cfg.hardware.device:
                    ...
                    if isinstance(dev, vim.vm.device.VirtualSCSIController):
                        ...
                        if "ParaVirtualSCSI" in ctrl_type or "PVSCSI" in ctrl_type.upper():
                            pve_type = "pvscsi"
                        elif "LsiLogicSAS" in ctrl_type or "LsiLogic" in ctrl_type or "BusLogic" in ctrl_type:
                            pve_type = "lsi"
            ...
            if cfg:
                ...
                if "efi" in str(fw).lower():
                    firmware = "efi"
                # Detect Secure Boot for OVMF pre-enrolled-keys
                try:
            ...
            # Extended detection for migration wizard
            ...
            with contextlib.suppress(BaseException):
                notes = getattr(cfg, "annotation", "") or ""
            ...
            if scsi_controllers:
                ...
                primary_scsi = str(primary.get("pve_type", "virtio-scsi-single"))
                primary_scsi_vmware = str(primary.get("type", "unknown"))
            ...
            if disks:
                ...
                primary_disk_bus = str(first_disk.get("bus", "scsi"))
    ...
    def _soap_get_snapshots(self, vm_id: str) -> dict:
        # (#393) - ESXi 8 sometimes returns a stale .snapshot tree
        ...
        try:
            ...
            for attempt in range(3):
                try:
                    ...
                    def _walk_snaps(snap_list, _result=result):
                        for snap in snap_list:
                            _result.append({
                                ...
                                "power_state": str(snap.state).replace("powered", "POWERED_").upper()
                                if snap.state
                                else "",
                            })
    ...
    def create_snapshot(
        self, vm_id: str, name: str, description: str = "", memory: bool = False, quiesce: bool = True
    ) -> dict:
        """Create a snapshot.  added SOAP path; REST returned 200 but did
        ...
        return self.api_post(
            ...
            data={"spec": {"name": name, "description": description, "memory": memory, "quiesce": quiesce}},
        )

    def _soap_create_snapshot(
        self, vm_id: str, name: str, description: str = "", memory: bool = False, quiesce: bool = True
    ) -> dict:
        """fire CreateSnapshot_Task and verify by polling get_snapshots
        ...
        try:
            ...
            if not vm:
                return {"error": "VM not found"}
            try:
                vm.CreateSnapshot_Task(name=name, description=description, memory=memory, quiesce=quiesce)
            except Exception as e:
    ...
    def _soap_delete_snapshot(self, vm_id: str, snapshot_id: str) -> dict:
        """SOAP fallback. snapshot_id can be snapshot _moId or name."""
        try:
            from pyVmomi import vim
    ...
    def get_vm_console_ticket(self, vm_id: str) -> dict:
        ...
        if self._connection_type != "soap":
            for ticket_type in ["WEBMKS", "MKS"]:
                result = self.api_post(f"/api/vcenter/vm/{vm_id}/console/tickets", data={"spec": {"type": ticket_type}})
        ...
        web_url = (
            f"https://{self.host}/ui/#/host/vms/{vm_id}/console"
            if self.server_type == "esxi"
            else f"https://{self.host}/ui/app/vm;nav=v/urn:vmomi:VirtualMachine:{vm_id}/console"
        )
        return {
            "data": {
                "ticket": "",
        ...
        try:
            ...
            if vm.config and vm.config.hardware and vm.config.hardware.device:
                for dev in vm.config.hardware.device:
                    if isinstance(dev, vim.vm.device.VirtualEthernetCard) and (dev.key == nic_key or nic_key == 0):
                        nic_device = dev
                        break
            if not nic_device:
            ...
            if isinstance(target_net, vim.dvs.DistributedVirtualPortgroup):
                ...
                dvs = target_net.config.distributedVirtualSwitch
                if dvs is None:
                    return {"error": "No distributed virtual switch for portgroup"}
                dvs_port.switchUuid = dvs.uuid
    ...
    def _soap_update_boot_order(self, vm_id: str, boot_order: list) -> dict:
        try:
            ...
            order: list[vim.vm.BootOptions.BootableDevice] = []
    ...
    def get_tags(self, category_id: str = None) -> dict:
        ...
        if category_id:
            return self.api_post("/api/cis/tagging/tag/id:{category_id}?~action=list-tags-for-category")
    ...
    def get_vm_tags(self, vm_id: str) -> dict:
        ...
        return self.api_post(
            ...
            data={"object_id": {"type": "VirtualMachine", "id": vm_id}},
        )
    ...
    def clone_vm(
        self, vm_id: str, name: str, folder: str = None, resource_pool: str = None, datastore: str = None
    ) -> dict:
    ...
    # Rewritten to use SSHFS + qm importdisk (real approach)
    ...
    def create_migration_snapshot(self, vm_id: str) -> dict:
        ...
        return self.create_snapshot(
            vm_id, "_ProxmoxVEx_migration_snap", "ProxmoxVEx live migration - do not delete manually", False, True
        )
    ...
    def delete_migration_snapshot(self, vm_id: str) -> dict:
        ...
        if "data" in snaps:
            ...
            for s in snap_list:
                if s.get("name") == "_ProxmoxVEx_migration_snap":
    ...
    def get_summary(self) -> dict:
        ...
        if "data" in vms and isinstance(vms["data"], list):
            ...
            for vm in vms["data"]:
                ...
                if ps == "POWERED_ON":
                    summary["vms"]["powered_on"] += 1
                elif ps == "POWERED_OFF":
                    summary["vms"]["powered_off"] += 1
                elif ps == "SUSPENDED":
                    summary["vms"]["suspended"] += 1
    ...
    # Keep idle ESXi sessions alive. ESXi default idle timeout
    ...
    def _ping_session(self) -> bool:
        ...
        try:
            ...
            if self._last_ping_ok and (now - self._last_ping_ok) > (self.PING_INTERVAL * 2):
                logging.warning(
                    f"[VMware:{self.id}] No successful ping in {int(now - self._last_ping_ok)}s - marking stale"
                )
                self.connected = False
                self.session_id = None
                self._si = None
    ...
    def ensure_connected(self) -> bool:
        ...
        try:
            ...
            if fail_count == 0 or fail_count % 5 == 0:
                logging.info(
                    f"[VMware:{self.id}] Session lost, reconnecting...{f' (attempt #{fail_count + 1})' if fail_count > 0 else ''}"
                )
            ...
            if not result and self.last_error:
                ...
                if "Login failed" in err or "InvalidLogin" in err or "incorrect" in err:
                    ...
                    "No route" in err or "Cannot connect" in err or "Cannot reach" in err or "Connection refused" in err
                ):
                    self._last_connect_error_type = "network"
                else:
    ...
    def set_cluster_drs(self, cluster_id: str, enabled: bool, automation: str = None) -> dict:
        ...
        spec: dict[str, Any] = {"drs_enabled": enabled}
        if automation:
    ...
    def _soap_set_cluster_drs(self, cluster_id: str, enabled: bool, automation: str = None) -> dict:
        try:
            ...
            if automation:
                bmap = {
                    "FULLY_AUTOMATED": vim.cluster.DrsConfigInfo.DrsBehavior.fullyAutomated,
                ...
                spec.drsConfig.defaultVmBehavior = bmap.get(
                    automation.upper(), vim.cluster.DrsConfigInfo.DrsBehavior.manual
                )
            ...
            for cl in clusters:
                try:
                    ...
                    result.append({
                        ...
                        "drs_enabled": drs.enabled if drs else False,
                        "drs_automation": str(drs.defaultVmBehavior).upper()
                        if drs and drs.defaultVmBehavior
                        else "MANUAL",
                        "ha_enabled": ha.enabled if ha else False,
                        "ha_admission_control": ha.admissionControlEnabled if ha else False,
                    })
            ...
            for vm in ds.vm or []:
                try:
                    vm_list.append({
                        "vm": vm._moId,
                        "name": vm.name,
                        "power_state": str(vm.runtime.powerState).replace("powered", "POWERED_").upper()
                        if vm.runtime
                        else "UNKNOWN",
                    })
                except Exception:
        ...
        for row in rows:
            ...
            if row_dict.get("pass_encrypted"):
                try:
                    ...
                    # SECURITY: only log safe metadata, never password chars
...
def save_vmware_server(vmware_id: str, config: dict):
    ...
    """,
        (
            vmware_id,
            config.get("name", "vCenter"),
            config.get("host", ""),
            int(config.get("port", 443)),
            config.get("username", "administrator@vsphere.local"),
            pass_encrypted
            or (
                cursor.execute("SELECT pass_encrypted FROM vmware_servers WHERE id = ?", (vmware_id,)).fetchone()
                or [""]
            )[0],
            config.get("server_type", "vcenter"),
            int(config.get("ssl_verify", False)),
            int(config.get("enabled", True)),
            json.dumps(config.get("linked_clusters", [])),
            config.get("notes", ""),
            vmware_id,
