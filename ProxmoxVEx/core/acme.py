...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/core/acme.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: Acme PY source
...
def _signed_request(url, payload, account_key, nonce, kid=None):
    ...
    _guard_acme_url(url)
    resp = requests.post(
        url, json=body, headers={"Content-Type": "application/jose+json"}, timeout=30, allow_redirects=False
    )
    return resp
...
def request_certificate(
    ...
    try:
        ...
        logging.info("[ACME] Challenge token set, waiting for validation...")
...
def prepare_dns01_challenge(
    domain, email, ssl_dir, staging=False, directory_url=None, dns_provider="manual", dns_config=None
):
