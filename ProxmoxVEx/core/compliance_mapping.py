...
# --------------------------------------------------------------------
# File:        ProxmoxVEx/core/compliance_mapping.py
# Project:     ProxmoxVEx
# Version:     1.2.303
# Build:       2026.09.04
# Description: Compliance framework control mappings for ProxmoxVEx...
# Docs:        https://proxmoxvex.local/docs
# Generated:   2026-09-04
# --------------------------------------------------------------------
# --- ProxmoxVEx auto-header end ---
"""Compliance framework control mappings for ProxmoxVEx hardening checks.
...
- VS-NfD                         (largely a profile over BSI Grundschutz +
                                  ProxmoxVEx-specific informational checks)
...
CMMC_L2_NIST_171 = {
    ...
    "pw_quality": [
        {
            "ref": "IA.L2-3.5.7",
            "family": "IA",
            "title": "Enforce minimum password complexity and change of characters when new passwords are created",
        },
    ],
    "pw_history": [
        {
    ...
    "apparmor": [
        {"ref": "AC.L2-3.1.5", "family": "AC", "title": "Employ the principle of least privilege"},
        {
            "ref": "CM.L2-3.4.7",
            "family": "CM",
            "title": "Restrict, disable, prevent the use of nonessential programs, functions, ports, protocols and services",
        },
    ],
    "fs_modules": [
        {
    ...
    "pkg_cleanup": [
        ...
        },
        {
            "ref": "CM.L2-3.4.8",
            "family": "CM",
            "title": "Apply deny-by-exception policy to prevent unauthorized software",
        },
    ],
    "remove_legacy_svcs": [
        {
    ...
    "pve_fail2ban": [
        {
            "ref": "SI.L2-3.14.6",
            "family": "SI",
            "title": "Monitor systems including network traffic to detect attacks and indicators",
        },
        {"ref": "AC.L2-3.1.8", "family": "AC", "title": "Limit unsuccessful logon attempts"},
    ],
    "apt_show_versions": [
...
CMMC_L1 = {
    ...
    # Audit/accountability is NOT a CMMC Level 1 domain - L1 covers only
...
DISA_STIG = {
    ...
    "ssh_crypto": [
        ...
        },
        {
            "ref": "RHEL-09-255075",
            "family": "IA",
            "title": "The RHEL 9 SSH server must be configured to use only Message Authentication Codes (MACs) employing FIPS 140-3 validated cryptographic hash algorithms.",
        },
    ],
    "ssh_perms": [
        {
    ...
    # Rebuild 2026-06-15 (RHEL 9 STIG V2R8, web-verified). Intentionally unmapped:
...
BSI_GRUNDSCHUTZ = {
    ...
    "file_perms": [
        {
            "ref": "SYS.1.3.A14",
            "family": "SYS",
            "title": "Verhinderung des Ausspähens von Informationen über das System und über Benutzende",
        },
    ...
    "apparmor": [
        ...
        },
        {
            "ref": "SYS.1.3.A16",
            "family": "SYS",
            "title": "Zusätzliche Verhinderung der Ausbreitung bei der Ausnutzung von Schwachstellen",
        },
    ],
    "inactive_accounts": [
        {"ref": "ORP.4.A7", "family": "ORP", "title": "Vergabe von Zugriffsrechten"},
    ...
    "audit_rules": [
        {
            "ref": "OPS.1.1.5.A3",
            "family": "OPS",
            "title": "Konfiguration der Protokollierung auf System- und Netzebene",
        },
    ...
    "auditd_service": [
        {
            "ref": "OPS.1.1.5.A1",
            "family": "OPS",
            "title": "Erstellung einer Sicherheitsrichtlinie für die Protokollierung",
        },
        {
            "ref": "OPS.1.1.5.A3",
            "family": "OPS",
    ...
    # Rebuild 2026-06-15 (BSI IT-Grundschutz, web-verified). Dropped the fabricated
...
# R-numbers + French titles verified against the official ANSSI PDF
...
ANSSI_BP028 = {
    ...
    "file_perms": [
        {
            "ref": "R50",
            "family": "fichiers",
            "title": "Restreindre les droits d'accès aux fichiers et aux répertoires sensibles",
        },
    ...
    "apparmor": [
        ...
        {
            "ref": "R37",
            "family": "controle_acces",
            "title": "Utiliser des fonctionnalités de contrôle d'accès obligatoire MAC",
        },
    ],
    "inactive_accounts": [
    ...
    "journald": [
        {"ref": "R71", "family": "journalisation", "title": "Mettre en place un système de journalisation"},
        {
            ...
            "title": "Mettre en place des journaux d'activité de service dédiés",
        },
    ],
    "file_integrity": [
...
REMEDIATION = {
    "pam_faillock": {
        ...
        "how_to_fix": 'ProxmoxVEx > Settings > Hardening > Apply "PAM faillock". Manually: edit /etc/security/faillock.conf with deny=5, unlock_time=900, fail_interval=900, then enable faillock in /etc/pam.d/common-auth and common-account.',
    },
    "session_limit": {
        "summary": "Limit concurrent sessions per user (TMOUT + maxlogins).",
        "how_to_fix": "Set TMOUT=900 in /etc/profile.d/tmout.sh and maxlogins in /etc/security/limits.conf. Apply via ProxmoxVEx hardening or manual.",
    },
    "shell_timeout": {
        "summary": "Auto-logout idle interactive shells after a defined idle time.",
        "how_to_fix": "Set TMOUT=900 in /etc/profile.d/tmout.sh and ensure it is exported and read-only. ProxmoxVEx hardening apply handles this.",
    },
    "login_banners": {
        "summary": "Display a legal/security warning banner before login.",
        "how_to_fix": "Populate /etc/issue, /etc/issue.net, /etc/motd with the appropriate notice (DoD CnC banner for federal use). Apply via ProxmoxVEx.",
    },
    "inactive_accounts": {
        "summary": "Disable user accounts after a defined period of inactivity (35 days for STIG).",
        "how_to_fix": "Set INACTIVE=35 in /etc/default/useradd and run useradd -D -f 35. ProxmoxVEx hardening enforces 35 days by default.",
    },
    "pw_quality": {
    ...
    "pw_hash_rounds": {
        ...
        "how_to_fix": "In /etc/pam.d/common-password: pam_unix.so ... yescrypt rounds=5 OR sha512 rounds=10000. ProxmoxVEx enforces yescrypt by default.",
    },
    "ssh_crypto": {
    ...
    "file_perms": {
        ...
        "how_to_fix": "ProxmoxVEx hardening apply enforces 644/640/600 on /etc/passwd, /etc/shadow, /etc/gshadow, /etc/group and similar.",
    },
    "default_umask": {
    ...
    "mount_options": {
        "summary": "/dev/shm, /tmp, /var/tmp mounted with nodev,nosuid,noexec.",  # nosec: B108 - compliance documentation text, not a temp path
    ...
    "audit_rules": {
        ...
        "how_to_fix": "Drop a CIS-aligned ruleset into /etc/audit/rules.d/cis.rules and run augenrules --load. ProxmoxVEx hardening ships a complete ruleset.",
    },
    "audit_boot": {
    ...
    "sysctl_hardening": {
        ...
        "how_to_fix": "ProxmoxVEx ships /etc/sysctl.d/99-ProxmoxVEx-hardening.conf with the full set. Apply via the hardening UI or sysctl --system.",
    },
    "pkg_cleanup": {
    ...
    "pve_fail2ban": {
        ...
        "how_to_fix": 'ProxmoxVEx > Settings > Hardening > Apply "fail2ban (PVE)". Auto-detects Debian version and picks iptables or nftables banaction.',
    },
    "apt_show_versions": {
    ...
    "pam_password_repair": {
        ...
        "how_to_fix": 'ProxmoxVEx > Settings > Hardening > Apply "Repair PAM password stack (recovery)". Either inserts pam_pwquality.so before pam_pwhistory.so (preferred) or strips use_authtok from pwhistory if libpam-pwquality is not installed. Backup of the original file is saved as common-password.bak.repair-<timestamp>.',
    },
    "vsnfd_disk_encryption": {
        "summary": "Disk encryption (LUKS or ZFS native) for VS-NfD-classified data at rest.",
        "how_to_fix": "INFORMATIONAL — must be planned at install time. ProxmoxVEx cannot retroactively encrypt; document operator decision per BSI guidance.",
    },
    "vsnfd_audit_retention": {
    ...
    "vsnfd_secure_boot": {
        ...
        "how_to_fix": "INFORMATIONAL — enable Secure Boot in UEFI; ensure shim/grub are signed. ProxmoxVEx detects but cannot enable Secure Boot.",
    },
    "vsnfd_kernel_lockdown": {
    ...
    "vsnfd_password_min_12": {
        ...
        "how_to_fix": "Set minlen=12 in /etc/security/pwquality.conf (ProxmoxVEx CIS profile uses minlen=14 by default which already satisfies this).",
    },
}
...
SEVERITY = {
    ...
    "pam_faillock": "high",
    "sshd_hardening": "high",  # Root SSH login policy + MaxAuthTries
    "pam_nullok_removal": "high",  # Blocks empty-password PAM auth
    "ssh_crypto": "high",
    "ssh_perms": "high",
    "file_perms": "high",
    "pw_quality": "high",
...
FRAMEWORK_META = {
    ...
    "nist53": {
        ...
        "control_count": 287,
        "note": "Federal civilian baseline. Mod baseline = ~287 controls. ProxmoxVEx maps the technical-control subset relevant to a Linux hypervisor.",
    },
    "stig": {
    ...
    "iso": {
        ...
        "control_count": 93,
        "note": "93 controls in 4 themes: Organizational, People, Physical, Technological. ProxmoxVEx covers the Technological controls applicable to Linux node hardening.",
    },
    "bsi": {
        ...
        "control_count": None,
        "note": "Bundesamt für Sicherheit in der Informationstechnik. Modules SYS / OPS / NET / DER / CON / APP. ProxmoxVEx focuses on SYS.1.3 (general Linux server) and adjacent modules.",
    },
    "vs-nfd": {
        "full_name": "VS-NfD (Verschlusssache - Nur für den Dienstgebrauch)",
        "revision": "VS-Anweisung (VSA), based on BSI IT-Grundschutz; ProxmoxVEx adds 6 informational checks",
        ...
        "control_count": None,
        "note": "German national restricted-use classification. ProxmoxVEx provides a Proxmox-safe BSI Grundschutz subset + 6 VS-NfD-specific informational checks.",
    },
    "dr": {
        ...
        "control_count": None,
        "note": "French national restricted classification. II 901 sets the protection regime for DR-level information systems; ProxmoxVEx maps the technical Linux-hardening measures to ANSSI-BP-028 (the recognised French baseline). Audit-prep evidence — not a homologation.",
    },
    "rgs": {
        ...
        "control_count": None,
        "note": "French general security framework for administrations. RGS points to ANSSI technical guides for implementation; ProxmoxVEx maps the Linux node-hardening subset to ANSSI-BP-028. Technical-config evidence only.",
    },
}
...
GLOSSARY = {
    ...
    "PVE": "Proxmox Virtual Environment — the open-source hypervisor management platform ProxmoxVEx orchestrates.",
    "ProxmoxVEx": "The multi-cluster Proxmox VE management platform that produced this report.",
...
METHODOLOGY = {
    "overview": (
        "ProxmoxVEx executes deterministic checks over SSH against each in-scope "
    ...
    "procedures": [
        (
            "Inquiry",
            'ProxmoxVEx reads the configured hardening profile (e.g. "CMMC L2") and resolves it to a list of internal control checks via FRAMEWORK_MAPPING.',
        ),
        (
            "Inspection",
            "ProxmoxVEx connects to each node via SSH and inspects configuration files, runtime kernel parameters, service status and file permissions.",
        ),
        (
            "Re-performance",
            "ProxmoxVEx executes the documented check command on each node and verifies the actual output matches the expected value.",
        ),
        (
            "Evidence capture",
            "When verbose mode is enabled, ProxmoxVEx records the exact check command and the actual node output for each control, included in Appendix C.",
        ),
    ],
    ...
    "criteria": "Mappings between internal checks and framework control IDs are maintained in ProxmoxVEx/core/compliance_mapping.py. Mappings should be re-verified against the current revision of the applicable framework before the report is used as input to a formal audit.",
    "limitations": (
        "ProxmoxVEx assesses technical configuration only. The following are out of scope and must be evaluated separately by a qualified auditor: "
...
def remediation_for(internal_id):
    ...
    return REMEDIATION.get(
        internal_id,
        {
            ...
            "how_to_fix": "Consult ProxmoxVEx Hardening tool for the apply command, or the relevant CIS / STIG benchmark.",
        },
    )
