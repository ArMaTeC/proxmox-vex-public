 ...
 * -------------------------------------------------------------------
 * File:        static/js/novnc/core/ra2.js
 * Project:     ProxmoxVEx
 * Version:     1.2.303
 * Build:       2026.09.04
 * Description: Ra2 JS source
...
export class AESEAXCipher {
    constructor() {
        ...
        this._ctrKey = null;
        this._cbcKey = null;
        this._zeroBlock = new Uint8Array(16); // snyk:ignore:javascript/HardcodedSecret // lgtm[js/hardcoded-credentials]
    ...
    async _encryptBlock(block) {
        const encrypted = await window.crypto.subtle.encrypt({
            name: "AES-CBC", // snyk:ignore:javascript/InsecureCipherNoIntegrity // lgtm[js/weak-cryptographic-algorithm]
            iv: this._zeroBlock,
        }, this._cbcKey, block);
...
export default class RSAAESAuthenticationState extends EventTargetMixin {
    ...
    async negotiateRA2neAuthAsync() {
        ...
        clientSessionKey = await window.crypto.subtle.digest("SHA-1", clientSessionKey); // snyk:ignore:javascript/InsecureHash // lgtm[js/weak-cryptographic-algorithm]
        clientSessionKey = new Uint8Array(clientSessionKey).slice(0, 16);
        serverSessionKey = await window.crypto.subtle.digest("SHA-1", serverSessionKey); // snyk:ignore:javascript/InsecureHash // lgtm[js/weak-cryptographic-algorithm]
        ...
        serverHash = await window.crypto.subtle.digest("SHA-1", serverHash); // snyk:ignore:javascript/InsecureHash // lgtm[js/weak-cryptographic-algorithm]
        clientHash = await window.crypto.subtle.digest("SHA-1", clientHash); // snyk:ignore:javascript/InsecureHash // lgtm[js/weak-cryptographic-algorithm]
