 ...
 * -------------------------------------------------------------------
 * File:        web/src/worldmap.js
 * Project:     ProxmoxVEx
 * Version:     1.2.303
 * Build:       2026.09.04
 * Description: Worldmap JS source
...
function WorldMap({ clusters, compact = false, onClusterClick }) {
    ...
    // Zoom/pan state. scale=1 = full world; viewBox shrinks +
    ...
    // Greedy label-collision avoidance. Capitals are pre-sorted
            ...
            {svgText && (
                <>
                    ...
                    {(() => {
                        ...
                        if (!window.DOMPurify) {
                            console.warn('worldmap: DOMPurify not available, skipping SVG render');
                            return (
                                <div
                        ...
                        const clean = window.DOMPurify.sanitize(html, { USE_PROFILES: { svg: true } });
                        return (
                            <div
                                className="absolute inset-0 w-full h-full pointer-events-none"
                    ...
                    {!compact && (
                        <div className="absolute top-3 right-3 flex flex-col gap-1.5 z-20">
                            <button onClick={zoomIn} title={t('zoomIn')}
                            ...
                            <button onClick={zoomOut} title={t('zoomOut')} disabled={view.scale === 1}
                                ...
                                style={{ background: isLight ? '#ffffffd0' : '#0f172ad0', color: isLight ? '#0f172a' : '#f1f5f9', backdropFilter: 'blur(4px)' }}>−</button>
                            <button onClick={resetView} title={t('resetZoom')} disabled={view.scale === 1 && view.panX === 0 && view.panY === 0}
                            ...
                            <button onClick={() => setShowCapitals(s => !s)}
                                title={showCapitals ? (t('hideCapitals')) : (t('showCapitals'))}
                    ...
                    {!compact && (
                        ...
                        >
                            {plottable.length} / {(clusters || []).length} {t('plottedOfTotal')}
                    ...
                    {!compact && plottable.length === 0 && view.scale === 1 && (
                        <div className="absolute inset-0 flex items-center justify-center px-4 pointer-events-none">
                            ...
                            >
                                <div className="font-semibold mb-1">{t('worldMapNoneSet')}</div>
                                <div className="text-xs opacity-70">{t('worldMapSetHint')}</div>
                            </div>
                        </div>
                    )}
                </>
    ...
    return (
        <div className="space-y-3">
            <div className="grid grid-cols-2 gap-2">
                <div>
                    <label className={isCorporate ? 'corp-label' : 'block text-[11px] uppercase tracking-wide text-gray-400 mb-1'}>
                        {t('latitude')}
                    </label>
                    <input
                        type="text" value={lat} onChange={(e) => setLat(e.target.value)}
                        className={isCorporate ? 'corp-input' : 'w-full px-2.5 py-1.5 bg-proxmox-dark border border-proxmox-border rounded-md text-white text-sm font-mono focus:outline-none focus:border-proxmox-orange'}
                        placeholder="50.1109"
                    />
                </div>
                <div>
            ...
            <div>
                <label className={isCorporate ? 'corp-label' : 'block text-[11px] uppercase tracking-wide text-gray-400 mb-1'}>
                    {t('locationLabelField')} <span className="opacity-60 normal-case tracking-normal">({t('locationLabelOptional')})</span>
                </label>
                <input
                    type="text" value={label} onChange={(e) => setLabel(e.target.value.slice(0, 120))}
                    className={isCorporate ? 'corp-input' : 'w-full px-2.5 py-1.5 bg-proxmox-dark border border-proxmox-border rounded-md text-white text-sm focus:outline-none focus:border-proxmox-orange'}
            ...
            <div className="flex gap-2">
                <button
                    ...
                    {busy ? (t('savingLocation2')) : (t('saveLocation2'))}
                </button>
    ...
    return (
        <div className="flex flex-col xl:flex-row gap-4 p-4">
            ...
            <div className="w-full xl:w-80 flex-shrink-0">
                <div className={isCorporate ? 'corp-settings-card' : 'bg-proxmox-card border border-proxmox-border rounded-2xl overflow-hidden'}>
                    <div className="px-4 py-3 border-b border-proxmox-border flex items-center justify-between bg-proxmox-dark/50">
                        <div>
                            <h3 className={isCorporate ? 'corp-card-header' : 'text-white font-semibold text-sm'}>{t('clusterLocations')}</h3>
                            <p className="text-xs text-gray-500">
                                {plottedCount} / {(clusters || []).length} {t('plottedOfTotal')}
                            </p>
                        </div>
                        <Icons.Globe />
                    </div>
                    <div className="max-h-[40rem] overflow-y-auto">
                        ...
                        {(clusters || []).map(c => {
                            ...
                            return (
                                <div key={c.id} className={`px-4 py-3 border-b border-proxmox-border last:border-b-0 ${isEditing ? 'bg-proxmox-dark/50' : ''}`}>
                                    <div className="flex items-center justify-between gap-2">
                                        ...
                                        <button
                                            ...
                                            {isEditing
                                                ? (t('close'))
                                                : (placed ? (t('editLocation')) : (t('setLocation')))}
                                        </button>
                                    </div>
                                    {isEditing && (
