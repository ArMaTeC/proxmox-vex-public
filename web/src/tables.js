 ...
 * -------------------------------------------------------------------
 * File:        web/src/tables.js
 * Project:     ProxmoxVEx
 * Version:     1.2.303
 * Build:       2026.09.04
 * Description: Tables JS source
...
function NodeCard({ name, metrics, index, clusterId, onMaintenanceToggle, onStartUpdate, onOpenNodeConfig, onNodeAction, onRemoveNode, onMoveNode }) {
    ...
    // 20 data points = last ~40s of sparkline at 2s polling interval
    ...
    return (
        <div
            className={`card-hover bg-proxmox-card border rounded-xl p-5 animate-slide-up ${isUpdating ? 'border-blue-500/50 bg-blue-500/5' :
        ...
        >
            ...
            {isUpdating && updateTask && (
                <div className="mb-4 -mt-1 -mx-1">
                    <div className={`${updateTask.status === 'failed' ? 'bg-red-500/10 border-red-500/30' :
                        ...
                        <div className="flex items-center justify-between mb-2">
                            <div className="flex items-center gap-2">
                                ...
                                <span className={`${updateTask.status === 'failed' ? 'text-red-400' :
                            ...
                            <div className="flex items-center gap-2">
                                <span className={`text-xs px-2 py-0.5 rounded ${updateTask.status === 'completed' ? 'bg-green-500/20 text-green-400' :
                ...
                <div className="mb-4 -mt-1 -mx-1">
                    <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-lg p-3">
                        <div className="flex items-center justify-between mb-2">
                            ...
                            <span className={`text-xs px-2 py-0.5 rounded ${maintenanceTask?.status === 'completed' ? 'bg-green-500/20 text-green-400' :
                        ...
                        {maintenanceTask?.status === 'completed_with_errors' && !metrics.maintenance_acknowledged && (
                            <div className="space-y-3 mt-2">
                                ...
                                <div className="flex gap-2">
                                    <button
                                        onClick={async () => {
                                            if (confirm(t('forceMaintenanceWarning2'))) {
                        ...
                        {(maintenanceTask?.status === 'completed' || (maintenanceTask?.status === 'completed_with_errors' && metrics.maintenance_acknowledged)) && (
                            <div className="space-y-2 mt-2">
                                ...
                                {/* Remove / Move Node - Only available after maintenance */}
            ...
            <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                    <div className={`p-2 rounded-lg ${isUpdating ? 'bg-blue-500/10' :
                        isInMaintenance ? 'bg-yellow-500/10' : 'bg-proxmox-orange/10'
                        }`}>
                        {isUpdating ? <Icons.RotateCw className="w-4 h-4 animate-spin" /> : isInMaintenance ? <Icons.Wrench /> : <Icons.Server />}
                    </div>
                    <div>
                        <h3 className="font-semibold text-white">{name}</h3>
                        <div className="flex items-center gap-2 mt-0.5">
                            <span className={`w-2 h-2 rounded-full ${isUpdating ? 'bg-blue-500 animate-pulse' :
                ...
                <div className="flex items-center gap-2">
                    ...
                    <div className="text-right">
                        ...
                        <div className={`font-mono font-bold text-lg ${metrics.score < 100 ? 'text-green-400' : metrics.score < 150 ? 'text-yellow-400' : 'text-red-400'
                            }`}>
                        ...
                        {metrics.disk_percent != null && <><div className="flex items-center justify-between text-xs">
                            ...
                            <div className="flex items-center gap-2">
                                <div className="w-16 h-1.5 bg-proxmox-dark rounded-full overflow-hidden">
                                    <div
                                        className={`h-full rounded-full ${metrics.disk_percent > 90 ? 'bg-red-500' :
            ...
            {showMaintenanceConfirm && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60" onClick={() => setShowMaintenanceConfirm(false)}>
                    <div
                        className={isCorporate ? 'w-full max-w-md overflow-hidden corp-settings-card' : "w-full max-w-md bg-proxmox-card border border-proxmox-border rounded-2xl shadow-2xl overflow-hidden"}
            ...
            {showUpdateLog && updateTask && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 modal-backdrop bg-black/60" onClick={() => setShowUpdateLog(false)}>
                    <div
                        className={isCorporate ? 'w-full max-w-2xl overflow-hidden corp-settings-card' : "w-full max-w-2xl bg-proxmox-card border border-proxmox-border rounded-2xl shadow-2xl animate-scale-in overflow-hidden"}
                        onClick={e => e.stopPropagation()}
                    >
                        <div className={isCorporate ? 'corp-card-header flex items-center justify-between' : "p-4 border-b border-proxmox-border flex items-center justify-between"}>
                            <div className="flex items-center gap-3">
                                <Icons.Terminal />
            ...
            {showRebootConfirm && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 modal-backdrop bg-black/60" onClick={() => setShowRebootConfirm(false)}>
                    <div
                        className={isCorporate ? 'w-full max-w-md overflow-hidden corp-settings-card' : "w-full max-w-md bg-proxmox-card border border-orange-500/30 rounded-2xl shadow-2xl animate-scale-in overflow-hidden"}
                        onClick={e => e.stopPropagation()}
                    >
                        <div className={isCorporate ? 'corp-card-header' : "p-6 border-b border-orange-500/30 bg-orange-500/10"}>
                            <div className="flex items-center gap-3">
                        ...
                        <div className="p-6">
                            ...
                            <div className="flex gap-3">
                                ...
                                >
                                    {actionLoading === 'reboot' ? <Icons.RotateCw className="w-4 h-4 animate-spin" /> : <Icons.RefreshCw />}
                                    {t('rebootNow')}
                                </button>
                            </div>
                        </div>
            ...
            {showShutdownConfirm && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 modal-backdrop bg-black/60" onClick={() => setShowShutdownConfirm(false)}>
                    <div
                        className={isCorporate ? 'w-full max-w-md overflow-hidden corp-settings-card' : "w-full max-w-md bg-proxmox-card border border-red-500/30 rounded-2xl shadow-2xl animate-scale-in overflow-hidden"}
                        onClick={e => e.stopPropagation()}
                    >
                        ...
                        <div className="p-6">
                            ...
                            <div className="flex gap-3">
                                ...
                                >
                                    {actionLoading === 'shutdown' ? <Icons.RotateCw className="w-4 h-4 animate-spin" /> : <Icons.Power />}
                                    {t('shutdownNow')}
                                </button>
                            </div>
                        </div>
...
// Compact node row for corporate overview
// Added sparkline history + detail cards
        ...
        <div className={`corp-node-row ${nodeStatusCls}`}>
                    ...
                    <>
                        ...
                        {isUpdating && updateTask && !isInMaintenance && (
                            <span className="corp-badge" style={{
                                ...
                                {updateTask.status === 'completed' ? '✓ ' : updateTask.status === 'failed' ? '✕ ' : '⟳ '}
                                {updateTask.status === 'failed' ? t('updateFailed') : updateTask.status === 'completed' ? t('updateCompleted') : `${t('updating')}: ${updateTask.phase || '...'}`}
                            </span>
                        )}
                ...
                <div className="px-3 pb-3 pt-1 ml-5" style={{ borderTop: '1px solid var(--corp-divider)' }}>
                    ...
                    {isInMaintenance && maintenanceTask && (
                        <div className="mb-2 p-2 text-[12px]" style={{ background: 'rgba(239, 192, 6, 0.08)', border: '1px solid rgba(239, 192, 6, 0.2)' }}>
                            ...
                            {/* Force/exit buttons - only when NOT yet acknowledged */}
                            {!metrics.maintenance_acknowledged && (maintenanceTask.status === 'completed_with_errors' || (maintenanceTask.failed_vms && maintenanceTask.failed_vms.length > 0)) && (
                                <div className="mt-1.5 flex items-center gap-2">
                                    <button onClick={(e) => {
                                        e.stopPropagation();
                                        if (confirm(t('forceMaintenanceWarning3'))) {
                    ...
                    {/* Property cards instead of flat text grid */}
                    <div className="corp-node-detail-grid">
                        ...
                        <div className="corp-node-detail-card">
                            <div className="corp-node-detail-label">PVE</div>
            ...
            {showMaintenanceConfirm && (
                <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60">
                    <div className={isCorporate ? 'w-full max-w-sm corp-settings-card' : "w-full max-w-sm bg-proxmox-card border border-proxmox-border p-5"}>
                        <h3 className={isCorporate ? 'corp-label' : "text-[14px] font-semibold mb-2"} style={{ color: '#e9ecef' }}>{t('enterMaintenance')}</h3>
            ...
            {showUpdateConfirm && (
                <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60">
                    <div className={isCorporate ? 'w-full max-w-sm corp-settings-card' : "w-full max-w-sm bg-proxmox-card border border-proxmox-border p-5"}>
                        <h3 className={isCorporate ? 'corp-label' : "text-[14px] font-semibold mb-2"} style={{ color: '#e9ecef' }}>{t('startUpdate')}</h3>
                        ...
                        <div className="flex justify-end gap-2">
                            ...
                            <button onClick={() => { onStartUpdate && onStartUpdate(name, updateWithReboot); setShowUpdateConfirm(false); }} className="px-3 py-1.5 text-[13px] text-white" style={{ background: '#49afd9', border: '1px solid #3d9bc2' }}>{t('startUpdate2')}</button>
                        </div>
                    </div>
                </div>
            )}
            {showRebootConfirm && (
                <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60">
                    <div className={isCorporate ? 'w-full max-w-sm corp-settings-card' : "w-full max-w-sm bg-proxmox-card border border-proxmox-border p-5"}>
                        <h3 className={isCorporate ? 'corp-label' : "text-[14px] font-semibold mb-2"} style={{ color: '#e9ecef' }}>{t('rebootNode') || `Reboot ${name}`}</h3>
            ...
            {showShutdownConfirm && (
                <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60">
                    <div className={isCorporate ? 'w-full max-w-sm corp-settings-card' : "w-full max-w-sm bg-proxmox-card border border-proxmox-border p-5"}>
                        <h3 className={isCorporate ? 'corp-label' : "text-[14px] font-semibold mb-2"} style={{ color: '#e9ecef' }}>{t('shutdownNode') || `Shutdown ${name}`}</h3>
...
// The main VM/CT list - supports cards, table, and detail view
// Added bulk select for mass operations (migration, etc.)
// This component does a lot... might need to split it up eventually
// Filtering + sorting uses useMemo below (lines 1320+)
function ResourceTable({ resources, clusterId, clusters, sourceCluster, onVmAction, onBulkAction, onOpenConsole, onOpenConfig, onMigrate, onBulkMigrate, onDelete, onClone, onForceStop, onCrossClusterMigrate, nodes, onOpenTags, highlightedVm, addToast, pendingVmAction, onPendingActionConsumed, onVmNavigate, backupStatus }) {
    const { t } = useTranslation();
    const { getAuthHeaders, user } = useAuth();
    const { isCorporate } = useLayout(); // Corporate defaults to table view
    // Per-VM sparkline history for table view
    ...
    // #T018: remember table filter set per user.
    const _filtersKey = `ProxmoxVEx-vmfilters-${user?.username || '_'}`;
    const _savedFilters = (() => { try { return JSON.parse(localStorage.getItem(_filtersKey) || '{}'); } catch (e) { return {}; } })();
    const [filter, setFilter] = useState(_savedFilters.filter || 'all');
    const [nodeFilter, setNodeFilter] = useState(_savedFilters.nodeFilter || 'all');
    const [tagFilter, setTagFilter] = useState(_savedFilters.tagFilter || 'all');
    const [itemsPerPage, setItemsPerPage] = useState(_savedFilters.itemsPerPage || 50);
    // #431: remember the table sort per user (localStorage, keyed by
    // username so a shared browser keeps each operator's choice).
    const _sortKey = `ProxmoxVEx-vmsort-${user?.username || '_'}`;
    ...
    // #T018: persist the VM filter set whenever it changes
    useEffect(() => {
        try {
            localStorage.setItem(_filtersKey, JSON.stringify({
                filter,
                nodeFilter,
                tagFilter,
                itemsPerPage
    ...
    }, [filter, nodeFilter, tagFilter, itemsPerPage, _filtersKey]);

    const [viewMode, setViewMode] = useState(isCorporate ? 'table' : 'cards'); // Corporate defaults to table view
    const [actionLoading, setActionLoading] = useState({});
    const [selectedVms, setSelectedVms] = useState([]);
    const [bulkAction, setBulkAction] = useState('start');
    ...
    // Consume pending action from context menu
    useEffect(() => {
    ...
    // Pagination states
    // Moved up 25.01.2026 - must be declared before useEffect that uses them
    ...
    // itemsPerPage is initialized earlier from persisted filters

    // Scroll to highlighted VM from global search
    useEffect(() => {
        if (highlightedVm) {
            // clear filters first
            setSearch('');
    ...
    // Update selectedDetailVm when resources change (via SSE)
    ...
    // #127 - lazy-load IPs from guest agent for running qemu VMs
    ...
    // #431: IP sorts by octet value, not as a string. Pull the IP from
    ...
    // Filter + sort in one useMemo for perf
    // Added tag/node/ip search, people kept complaining they couldnt find stuff
    const filteredResources = useMemo(() => {
        ...
        filtered.sort((a, b) => {
            ...
            if (sortBy === 'ip') {  // #431: per-octet numeric, not lexical
    ...
    // Also reset when cluster changes (via clusterId) to avoid showing empty page
    useEffect(() => {
        setCurrentPage(1);
    ...
    // Clamp currentPage to valid bounds - direct calculation, no effect loop
    const effectivePage = Math.max(1, Math.min(currentPage, totalPages));

    // Auto-correct if currentPage is out of bounds (e.g., after cluster switch)
    useEffect(() => {
    ...
    const runBulkAction = async () => {
        if (!onBulkAction || !bulkAction || selectedVms.length === 0) return;
        const actionLabel = t(`bulkActions.${bulkAction}`, { defaultValue: bulkAction });
        const nameList = selectedVms.length === 1
            ? selectedVms[0].name || `VM ${selectedVms[0].vmid}`
            : `${selectedVms.length} ${t('items')}`;
        if (!window.confirm(`${t('bulkActions.confirm')} ${actionLabel} ${t('bulkActions.on')} ${nameList}?`)) return;

        setBulkWorking(true);
        try {
            await onBulkAction({ action: bulkAction, guests: selectedVms });
            setSelectedVms([]);
        } finally {
    ...
    return (
        <div className={isCorporate ? 'space-y-0' : 'space-y-4'}>
            {/* Corporate flat toolbar vs modern rounded pills */}
            {isCorporate ? (
                ...
                <div className="flex flex-wrap items-center gap-3">
                    <div className="relative flex-1 min-w-[200px]">
                        ...
                        <input
                            ...
                            value={search}
                            onChange={(e) => setSearch(e.target.value)}
                            className={isCorporate ? 'pl-10 corp-input' : "w-full pl-10 pr-4 py-2 bg-proxmox-dark border border-proxmox-border rounded-lg text-sm text-white placeholder-gray-500 focus:outline-none focus:border-proxmox-orange transition-colors"}
                        />
                        ...
                        {['all', 'running', 'stopped', 'vm', 'lxc'].map(f => (
                            <button
                                ...
                                className={isCorporate
                                    ...
                                    : `px-3 py-1.5 text-xs font-medium rounded-lg transition-all ${filter === f
                        ...
                        {availableNodes.length > 1 && (
                            <select
                                ...
                                className={isCorporate
                                    ...
                                    : `px-3 py-1.5 text-xs font-medium rounded-lg transition-all cursor-pointer ${nodeFilter !== 'all'
                                        ? 'bg-proxmox-orange text-white'
                                        : 'bg-proxmox-dark text-gray-400 border border-proxmox-border'
                                    }`}
                            >
                                <option value="all">{t('allNodes2')}</option>
                        ...
                        {availableTags.length > 0 && (
                            <select
                                ...
                                className={isCorporate
                                    ...
                                    : `px-3 py-1.5 text-xs font-medium rounded-lg transition-all cursor-pointer ${tagFilter !== 'all'
                                        ? 'bg-proxmox-orange text-white'
                                        : 'bg-proxmox-dark text-gray-400 border border-proxmox-border'
                                    }`}
                            >
            ...
            {!isCorporate && selectedVms.length > 0 && (
                <div className="flex flex-wrap items-center gap-3 p-3 bg-proxmox-orange/10 border border-proxmox-orange/30 rounded-lg">
                    ...
                    <select
                        ...
                        className="px-2 py-1.5 text-sm bg-proxmox-dark border border-proxmox-border rounded-lg text-white focus:outline-none focus:border-proxmox-orange"
                    >
                        <option value="start">{t('bulkActions.start')}</option>
                        <option value="stop">{t('bulkActions.stop')}</option>
                        <option value="shutdown">{t('bulkActions.shutdown')}</option>
                        <option value="reboot">{t('bulkActions.reboot')}</option>
                        <option value="reset">{t('bulkActions.reset')}</option>
                        <option value="suspend">{t('bulkActions.suspend')}</option>
                        <option value="resume">{t('bulkActions.resume')}</option>
                    </select>
                    <button
                        onClick={runBulkAction}
                        disabled={bulkWorking}
                        className="flex items-center gap-2 px-3 py-1.5 bg-proxmox-orange rounded-lg text-white text-sm hover:bg-orange-600 disabled:opacity-50"
                    >
                        {bulkWorking && <Icons.RotateCw className="w-4 h-4 animate-spin" />}
                        {t('bulkActions.run')}
                    </button>
                    <button
                    ...
                    {paginatedResources.length === 0 ? (
                        ...
                        paginatedResources.map((resource, idx) => (
                            <div
                                ...
                                className={`bg-proxmox-card border rounded-xl overflow-hidden transition-all hover:border-proxmox-orange/50 animate-fade-in ${selectedVms.find(v => v.vmid === resource.vmid)
                            ...
                            >
                                {/* Card Header */}
                                <div className={isCorporate ? 'flex items-center justify-between corp-card-header' : "flex items-center justify-between p-4 border-b border-proxmox-border bg-proxmox-dark/50"}>
                                    <div className="flex items-center gap-3">
                                        <input
                                            type="checkbox"
                                    ...
                                    <span className={`w-2.5 h-2.5 rounded-full ${resource.status === 'running' ? 'bg-green-500 animate-pulse' : 'bg-red-500'
                                        }`} />
                                </div>
                                ...
                                <div className="p-4 space-y-3">
                                    ...
                                    <div className="flex items-center justify-between text-sm">
                                        ...
                                        <span className={`px-2 py-0.5 rounded text-xs font-medium ${resource.status === 'running'
                                    ...
                                    {/* Backup status pill */}
                                    {backupStatus && backupStatus[resource.vmid] && window.ProxmoxVExBackupStatusPill && (
                                        <div className="flex items-center justify-between text-sm">
                                            <span className="text-gray-500 text-xs">Backup</span>
                                            {React.createElement(window.ProxmoxVExBackupStatusPill, {
            ...
            {/* Table view, corporate data-grid */}
            {viewMode === 'table' && (
                            ...
                            {paginatedResources.length === 0 ? (
                                ...
                                paginatedResources.map((resource, idx) => (
                                    ...
                                    >
                                        ...
                                        <td className="px-4 py-3">
                                            <div>
                                                <div className="flex items-center gap-1.5">
                                                    ...
                                                    {/* Backup status pill (table-row layout) */}
                                                    {backupStatus && backupStatus[resource.vmid] && window.ProxmoxVExBackupStatusPill && React.createElement(window.ProxmoxVExBackupStatusPill, {
                                                        status: backupStatus[resource.vmid].status,
                                                        lastAgeHours: backupStatus[resource.vmid].last_backup_age_hours,
                                                        encrypted: backupStatus[resource.vmid].encrypted,
                                                        verifyAgeHours: backupStatus[resource.vmid].last_verify_age_hours,
                                        ...
                                        <td className="px-4 py-3">
                                            <span className={`inline-flex items-center gap-1.5 px-2 py-1 rounded-md text-xs font-medium ${isGuestTemplate(resource)
                                        ...
                                        <td className="px-4 py-3">
                                            <span className={`inline-flex items-center gap-1.5 px-2 py-1 rounded-md text-xs font-medium ${resource.status === 'running'
                                                ? 'bg-green-500/10 text-green-400 border border-green-500/20'
                                                : resource.status === 'stopped'
                                                    ? 'bg-red-500/10 text-red-400 border border-red-500/20'
                                                    : 'bg-gray-500/10 text-gray-400 border border-gray-500/20'
                                                }`}>
                                                <span className={`w-1.5 h-1.5 rounded-full ${resource.status === 'running' ? 'bg-green-400' : 'bg-red-400'
                                                    }`} />
                                                {resource.status}
                                            </span>
                                        </td>
                                                ...
                                                <div className="flex items-center gap-1">
                                                    ...
                                                    {resource.status === 'stopped' ? (
                                                        <button
                                                            ...
                                                            title={t('start')}
                                                        >
                                                            {actionLoading[`${resource.vmid}-start`] ? <Icons.RotateCw className="w-4 h-4 animate-spin" /> : <Icons.PlayCircle />}
                                                        </button>
                                                    ) : (
                                                        <>
                                                            <button
                                                                ...
                                                                title={t('shutdown')}
                                                            >
                                                                {actionLoading[`${resource.vmid}-shutdown`] ? <Icons.RotateCw className="w-4 h-4 animate-spin" /> : <Icons.Power />}
                                                            </button>
                                                            <button
                                                                ...
                                                                title={t('forceStop')}
                                                            >
                                                                {actionLoading[`${resource.vmid}-stop`] ? <Icons.RotateCw className="w-4 h-4 animate-spin" /> : <Icons.XCircle />}
                                                            </button>
                                                            <button
                                                                onClick={() => handleAction(resource, 'reboot')}
                                                                disabled={actionLoading[`${resource.vmid}-reboot`]}
                                                                ...
                                                                title={t('reboot')}
                                                            >
                                                                {actionLoading[`${resource.vmid}-reboot`] ? <Icons.RotateCw className="w-4 h-4 animate-spin" /> : <Icons.RefreshCw />}
                                                            </button>
                                                            {resource.type === 'qemu' && (
                                                                <button
                                                                    ...
                                                                    title={t('forceReset')}
                                                                >
                                                                    {actionLoading[`${resource.vmid}-reset`] ? <Icons.RotateCw className="w-4 h-4 animate-spin" /> : <Icons.Zap />}
                                                                </button>
                                                            )}
                                                        </>
                                                    )}
            ...
            {viewMode === 'detail' && (
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                    {/* VM List — 2026-04-24: viewport-proportional height so it
                    ...
                    <div className="lg:col-span-1 bg-proxmox-card border border-proxmox-border rounded-xl overflow-hidden flex flex-col"
                        ...
                        <div className={isCorporate ? 'corp-card-header flex-shrink-0' : "p-3 border-b border-proxmox-border bg-proxmox-dark/50 flex-shrink-0"}>
                        ...
                        <div className="flex-1 overflow-y-auto" style={{ scrollbarWidth: 'thin' }}>
                            {paginatedResources.map(resource => (
                                <div
                                    ...
                                    className={`flex items-center gap-3 p-3 cursor-pointer transition-all border-b border-gray-700/50 ${selectedDetailVm?.vmid === resource.vmid
                                ...
                                >
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <div className="flex flex-wrap gap-1 mt-1 items-center">
                                            {/* Backup status pill (mobile stack layout) */}
                                            {backupStatus && backupStatus[resource.vmid] && window.ProxmoxVExBackupStatusPill && React.createElement(window.ProxmoxVExBackupStatusPill, {
                                                status: backupStatus[resource.vmid].status,
                                                lastAgeHours: backupStatus[resource.vmid].last_backup_age_hours,
                                                encrypted: backupStatus[resource.vmid].encrypted,
                                    ...
                                    <span className={`w-2 h-2 rounded-full ${resource.status === 'running' ? 'bg-green-500' : 'bg-red-500'
                                        }`} />
                                </div>
                            ))}
                        </div>
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <select
                            ...
                            className={isCorporate ? 'corp-input' : "bg-proxmox-dark border border-proxmox-border rounded px-2 py-1 text-sm"}
                        >
                        ...
                        {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`px-3 py-1 rounded border ${effectivePage === pageNum
...
// 014-color-coded-status-for-dashboard: shared status badge color helper
...
// TaskLogColorTheme: dark-mode wrapper for the Task Log page.
function TaskLogColorTheme({ children, dark }) {
    return (
...
// TaskLogCompactGrid: compact density table wrapper for the Task Log page.
function TaskLogCompactGrid({ children, compact }) {
    return (
...
// TaskLogResizablePanels: resizable layout wrapper for the Task Log page.
function TaskLogResizablePanels({ children, resizable }) {
    return (
...
// TaskLogColorCodedStatus: status badge wrapper for the Task Log page.
function TaskLogColorCodedStatus({ children, status }) {
...
// TaskLogHoverCards: hover card wrapper for the Task Log page.
function TaskLogHoverCards({ children, hover }) {
    return (
...
// TaskLogBreadcrumbBar: breadcrumb navigation for the Task Log page.
function TaskLogBreadcrumbBar({ items }) {
    return (
...
// TaskLogCustomizableColumns: column picker for the Task Log page.
function TaskLogCustomizableColumns({ columns, visible, onToggle }) {
    return (
...
// TaskLogDragAndDrop: drag handle for reordering the Task Log page.
function TaskLogDragAndDrop({ children, onDragEnd }) {
    return (
...
// TaskLogFilterSidebar: filter sidebar for the Task Log page.
function TaskLogFilterSidebar({ filters, onChange }) {
    return (
...
// TaskLogKeyboardShortcuts: keyboard shortcut hints for the Task Log page.
function TaskLogKeyboardShortcuts({ shortcuts }) {
    return (
...
// BackupJobsStatusBadge: color-coded status badge for backup job rows.
// 923-color-coded-status-for-backup-jobs: maps a status value to a semantic Tailwind badge.
function BackupJobsStatusBadge({ status }) {
    const { t } = useTranslation();
    ...
    const cls = variants[s] || variants.idle;
    const label = t(s, { defaultValue: s });
    return (
        <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium border ${cls}`} data-testid="backup-jobs-status-badge">
            {label}
        </span>
    );
}

// BackupJobsCompactGrid: shared compact/expanded table-density wrapper for the Backup Jobs page.
// 921-compact-grid-for-backup-jobs: toggles the `.compact` class on child tables to reduce cell padding and font size.
function BackupJobsCompactGrid({ compact, onToggle, children }) {
    const { t } = useTranslation();
    return (
        <div className={`backup-jobs-grid ${compact ? 'compact' : ''}`} data-testid="backup-jobs-compact-grid">
            {children}
        </div>
    );
}

// BackupJobsColumnPicker: column visibility picker for the Backup Jobs page.
// 926-customizable-columns-for-backup-jobs: lets operators show/hide backup job table columns.
function BackupJobsColumnPicker({ columns = [], visible = [], onChange }) {
    const { t } = useTranslation();
    const toggle = (key) => {
    ...
    return (
        <div className="p-2 border border-proxmox-border rounded bg-proxmox-card mb-4" data-testid="backup-jobs-column-picker">
            <div className="text-sm font-medium mb-2 text-white">{t('columns')}</div>
...
// BackupJobsDragHandle: visual drag handle for reordering backup job rows.
// 927-drag-and-drop-reordering-for-backup-jobs: lets operators reorder backup jobs.
function BackupJobsDragHandle() {
    return (
        <span className="backup-jobs-drag-handle" data-testid="backup-jobs-drag-handle" aria-hidden="true">
            ⋮⋮
        </span>
    );
}
